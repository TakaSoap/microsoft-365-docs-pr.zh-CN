---
title: 作为来宾加入团队
f1.keywords:
- NOCSH
ms.author: sirkkuw
author: Sirkkuw
manager: scotv
audience: Admin
ms.topic: article
ms.service: o365-administration
localization_priority: Normal
ms.collection:
- M365-subscription-management
- Adm_O365
ms.custom:
- AdminSurgePortfolio
- adminvideo
search.appverid:
- BCS160
- MET150
- MOE150
description: 了解如何在 Microsoft Teams 中以来宾加入团队。
ms.openlocfilehash: 51f287361877cb0b427a38c48683c064bda99126
ms.sourcegitcommit: 9adb89206daa075af34a73bcb7e8fb86d7c2919a
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/10/2021
ms.locfileid: "50603956"
---
# <a name="join-a-team-as-a-guest"></a>作为来宾加入团队

> [!VIDEO https://www.microsoft.com/videoplayer/embed/RE4tyys?autoplay=false]

## <a name="try-it"></a>试一试！

如果你收到 Microsoft Teams 加入团队的邀请，则有两个步骤可以接受邀请。

1. 打开电子邮件邀请，然后选择"**打开 Microsoft Teams"。** 如果看不到邀请，请检查垃圾邮件文件夹。
  1. 如果你的电子邮件地址之前未用于创建 Microsoft 帐户，则需要创建一个。

     在"**创建帐户"** 对话框中，选择 **"下** 一步"，创建密码，然后选择"下 **一步"。**
  1. 若要验证您的电子邮件地址，请打开验证电子邮件，然后复制安全代码。
  1. 在"验证电子邮件"对话框中输入安全代码，选择 **"下** 一步"，在"输入看到的字符行"中输入 CAPTCHA，**选择"下** 一步"，然后 **接受**。
1. 如果不想下载 Microsoft Teams 应用，请选择"改为 **使用 Web 应用"。**
1. 在 Teams Web 应用中，可以查看或回复帖子，也可以创建自己的帖子，并选择"文件"选项卡查看或编辑团队文档。

    如果你已在组织中使用 Teams 应用以来宾加入团队，则要简单很多。

1. 只需打开 Microsoft Teams 应用，然后选择"  **是**  "以切换到受邀加入的团队。  现在，你可以协作处理 Teams 中的项目。
2. 若要切换回公司的团队，请从"组织  **"下拉列表中选择**  它。
