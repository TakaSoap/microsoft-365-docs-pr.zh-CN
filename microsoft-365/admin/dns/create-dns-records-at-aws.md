...
如果 AWS 是 DNS 托管提供商，请按照本文中的步骤验证域，并设置电子邮件、Skype Online for Business 等的 DNS 记录。
...
## <a name="add-an-mx-record-so-email-for-your-domain-will-come-to-microsoft-365"></a>添加 MX 记录，以便你的域的电子邮件发送到 Microsoft 365
...
7. 如果有任何其他 MX 记录，请删除它们。
    ...
    > [!IMPORTANT]
    > AWS 将 MX 记录存储为可能包含多个记录的集合。 **请勿选择****"删除记录** 集"，因为此操作将删除所有 MX 记录，包括刚添加的记录。 请使用下面的说明。 
...
## <a name="add-the-five-cname-records-that-are-required-for-microsoft-365"></a>添加 Microsoft 365 所需的五条 CNAME 记录
...
5. 添加第一条 SRV 记录：
    ...
    |**名称**|**类型**|**别名**|**TTL（秒）**|**值**|**路由策略**|
    |:-----|:-----|:-----|:-----|:-----|:-----|
    |_sip._tls|SRV - 服务定位器|否|300|100 1 443 sipdir.online.lync.com. **此值必须以 (.)**><br> **注意：** 建议复制粘贴此条目，以保证正确保留所有空格。           |简单|
