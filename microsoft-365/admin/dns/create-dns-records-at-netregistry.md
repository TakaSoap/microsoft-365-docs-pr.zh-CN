...
---
# <a name="create-dns-records-at-netregistry-for-microsoft"></a>在 Netregistry 为 Microsoft 创建 DNS 记录

如果找不到要查找的内容，请[查看域常见问题解答](../setup/domains-faq.yml)。 
  
如果 Netregistry 是 DNS 托管提供商，请按照本文中的步骤验证域并设置电子邮件、Skype for Business Online 等的 DNS 记录。
...
在 Netregistry 添加这些记录后，域将设置为使用 Microsoft 服务。
...
4. 在 **"添加区域记录"下**，从列表中选择 **"TXT** 记录"，然后选择"**创建新记录"。**
    ...
    > [!NOTE]
    > 必须在 TXT 框中的条目之前和之后使用引号。 
  
    在 **"新建 TXT 记录** "表单中，键入或复制并粘贴下表中的值。 
    
    |**名称**|**TTL (SEC)**|**TXT (指向地址或值)**|
    |:-----|:-----|:-----|
...
4. 在当前 **区域记录下**，通过选择列表中每个 MX记录旁边的"删除"来删除默认 MX 记录。 
...
6. 在 **"新建 MX 记录** "表单中，键入或复制并粘贴下表中的值。 
    ...
    |**名称**|**TTL (SEC)**|**Exchange (指向地址或值)**|**主机是否完全限定？**|**首选项 (优先级)**|
    |:-----|:-----|:-----|:-----|:-----|
    |（保留为空白）  <br/> |3600 (秒)   <br/> | *\<domain-key\>*  .mail.protection.outlook.com  <br/> **注意：** 从  *\<domain-key\>*  Microsoft 帐户获取你的帐户。  [如何查找此项？](../get-help-with-domains/information-for-dns-records.md)      | (复选框)   <br/> |10   <br/> For more information about priority, see What is MX priority?  <br/> |
...
5. 在新记录的框中，键入或复制并粘贴下表中的值。
    ...
    > [!NOTE]
    > "名称"字段是服务实例的组合， (例如，_sip) 和协议 (例如_tls) 。 
  
    |**类型**|**名称**|**TTL (SEC)**|**优先级**|**权重**|**端口**|**目标**|
    |:-----|:-----|:-----|:-----|:-----|:-----|:-----|
