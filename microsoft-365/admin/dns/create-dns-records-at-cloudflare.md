...
1. 在 Cloudflare 登录页面上，登录到你的帐户， **然后选择授权**。
...
1. 在"DNS 管理"页上，选择 **"+添加** 记录"，然后键入或复制并粘贴表中的值。 
...
4. 在"DNS 管理"页上，选择 **"+添加** 记录"，然后键入或复制并粘贴表中的值。 
...
6. 如果"MX 记录"部分列出了任何其他 **MX** 记录，则通过选择"编辑"将其删除，然后选择"删除 **"。** 
  
7. 在确认对话框中，选择 **"删除** "以确认所做的更改。 
...
4. 添加 CNAME 记录。
    ...
    在 **"DNS 管理"** 页上，选择 **"+添加** 记录"，然后键入或复制并粘贴表中的值。
...
4. 在"DNS 管理"页上，选择 **"+添加** 记录"，然后从下表中选择值。  
...
4. 添加两条 SRV 记录中的第一条记录。
    ...
    在"DNS 管理"页上，选择 **"+添加** 记录"，然后选择或复制并粘贴表中第一行的值。
        
    | **类型** | **名称** | **服务** | **协议** |  **TTL** | **优先级** | **权重** | **端口** | **目标** |
    |:-----|:-----|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
    |SRV|_sip | 使用你的 *domain_name*;例如，contoso.com  ||TLS |30 分钟 | 100|1 |443 |sipfed.online.lync.com  |
...
4. 添加第一条 CNAME 记录。
    ...
    在"DNS 管理"页上，选择 **"+添加** 记录"，然后键入或复制并粘贴下表中第一行的值。
...
4. 添加第一条 CNAME 记录。
    ...
    在"DNS 管理"页上，选择 **"+添加** 记录"，然后键入或复制并粘贴表中第一行的值。
