...
1. 在"Microsoft 365 管理中心"中，**设置**\>"域"，然后选择要设置的域。<a href="https://go.microsoft.com/fwlink/p/?linkid=834818" target="_blank"></a>
...
2. 使用下表中的值创建两个名称机记录，或编辑现有名称机记录，以便它们与这些值匹配。
    ...
    |类型|值|
    |---|---|
    |首选名称服务器|使用 Cloudflare 提供的名称器值。|
    |次要名称服务器|使用 Cloudflare 提供的名称器值。|
...
1. 从下拉列表中选择 MX 类型，然后键入或复制并粘贴此表中的值。
   ...
   |类型|名称|邮件服务器|TTL|Priority|
   |---|---|---|---|---|
   |MX|@|*\<domain-key\>*.mail.protection.outlook.com <br/> **注意：** 从你的 *\<domain-key\>* 帐户获取Microsoft 365帐户。 [如何查找此项？](../get-help-with-domains/information-for-dns-records.md)|30 分钟|1 <br/> 有关优先级的详细信息，请参阅[什么是 MX 优先级？](../setup/domains-faq.yml) <br/>|
...
> [!IMPORTANT]
> 一个域所拥有的 SPF 的 TXT 记录不能超过一个。 如果域具有多个 SPF 记录，你将收到电子邮件错误，其中随附发送和垃圾邮件分类问题。 如果你的域已有 SPF 记录，请不要为 Microsoft 365 创建新记录。 相反，将Microsoft 365值添加到当前记录，以便具有一个同时包含这两组 *值的 SPF* 记录。
