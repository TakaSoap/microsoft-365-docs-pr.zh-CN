...
---
# <a name="create-dns-records-at-webcom-for-microsoft"></a>在 Microsoft web.com创建 DNS 记录

 如果找不到要查找的内容，请 **[查看域常见问题解答](../setup/domains-faq.yml)**。 
  
如果web.com DNS 托管提供商，请按照本文中的步骤验证域并设置电子邮件、Skype for Business Online 等的 DNS 记录。
  
在网站中添加这些web.com，域将设置为使用 Microsoft 服务。
...
注册域web.com，即使用"设置"过程web.com **域** 。 
  
若要在 Microsoft 中验证和创建域的 DNS 记录，首先需要更改域注册机构的名称服务器，以便它们使用 web.com 的名称服务器。
...
2. 使用下表中的值创建两个名称机记录，或编辑现有名称机记录，以便它们与这些值匹配。
    ...
    |||
    |:-----|:-----|
    |首选名称服务器  <br/> |使用由 web.com 提供的名称web.com。  <br/> |
    |次要名称服务器  <br/> |使用由 web.com 提供的名称web.com。  <br/> |
...
4. 在" **域名** "页上的 **"TXT** 记录 (下) ，单击"编辑 **TXT** 记录"，然后从下表中选择值。 
...
4. 在 **"邮件 (MX** 记录) 下，单击"编辑 **MX** 记录"，然后从下表中选择值。 
    
    |**优先级**|**TTL**|**邮件服务器**|
    |:-----|:-----|:-----|
    |1   <br/> 有关优先级的详细信息，请参阅[什么是 MX 优先级？](https://docs.microsoft.com/microsoft-365/admin/setup/domains-faq) <br/> |3600  <br/> |*\<domain-key\>*  .mail.protection.outlook.com  <br/> **注意：** 从  *\<domain-key\>*  Microsoft 帐户获取你的帐户。   [如何查找此项？](../get-help-with-domains/information-for-dns-records.md) |
   

5. 选择“**保存**”。
  
6. 如果 **"MX** 记录"部分列出了任何其他 MX 记录，请选中"删除"下记录旁边的复选框，然后选择"保存 **"。** 
...
1. To get started， go to your domains page at web.com by using [this link.](https://checkout.web.com/manage-it/index.jsp) 系统将会提示您先登录。
...
4. 添加第一条 CNAME 记录（共 6 条）。
    ...
    在 **"主机别名 (CNAME** 记录) ，单击"编辑 **CNAME** 记录"，然后从下表中选择值。
...
> [!IMPORTANT]
> 请记住，web.com负责提供此功能。 如果您看到以下步骤与图形用户界面用户界面web.com当前 web.com GUI (存在) ，请利用 web.com [社区](https://community.web.com.com/)。 
...
4. 添加两条 SRV 记录中的第一条记录。
    ...
    在 **" (SRV 记录**) ，单击"编辑 **SRV 记录**"，然后从下表中选择值。 
