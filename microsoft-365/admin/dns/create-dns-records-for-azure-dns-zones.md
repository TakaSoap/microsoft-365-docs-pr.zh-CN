...
注册 Azure 后，你在 DNS 区域中创建了一个资源组，然后将你的域名分配给该资源组。 该域名注册到外部域注册机构;Azure 不提供域注册服务。
  
若要在 Microsoft 中验证和创建域的 DNS 记录，首先需要更改域注册机构中的名称服务器，以便它们使用分配给你的资源组的 Azure 名称服务器。
...
**第二个名称机：** 使用 Azure 分配的名称服务器值。  

![Azure-BP-Redelegate-1-1](../../media/3e4805ea-608a-4df9-8f68-1fbf70d13d08.png)
  
> [!TIP]
> You should use at least two name server records. 如果域注册机构的网站中列出了任何其他名称服务器，您应该将其删除。 
...
2. 使用 **仪表板页面上** 的搜索 **栏** ，键入 DNS **区域**。 在结果显示中，选择 **"服务"部分下的"DNS** **区域** "。 重定向后，选择要更新的域。
...
4. 在 **"添加记录集** "区域中新记录集的框内，从下表中选择值。 
    
     (从下拉列表 **中选择 Type** 和 **TTL** 单位值。)  
...
4. 在 **"添加记录集** "区域中新记录集的框内，从下表中选择值。 
    ...
    |:-----|:-----|:-----|:-----|:-----|:-----|
    |@  <br/> |MX  <br/> |1  <br/> |工作时间  <br/> |10    <br/> 有关优先级的详细信息，请参阅[什么是 MX 优先级？](../setup/domains-faq.yml) <br/> | *\<domain-key\>*  .mail.protection.outlook.com  <br/> **注意：** 从  *\<domain-key\>*  Microsoft 帐户获取你的信息。   [如何查找此项？](../get-help-with-domains/information-for-dns-records.md)  
...
4. 添加四条 CNAME 记录中的第一条。
    ...
    在 **"添加记录集** "区域中新记录集的框内，键入或复制并粘贴下表中第一行的值。 
