---
title: 连接 IONOS 的 DNS 记录从 1&1 到 Microsoft 365
f1.keywords:
- CSH
ms.author: pebaum
author: pebaum
...
ms.locfileid: "60556462"
---
# <a name="connect-your-dns-records-at-ionos-by-11-to-microsoft-365"></a>连接 IONOS 的 DNS 记录从 1&1 到 Microsoft 365
...
> [!CAUTION]
> 请注意，IONOS x 1&1 不允许域同时具有 MX 记录和顶级自动发现 CNAME 记录。 这将限制为 Microsoft 配置 Exchange Online 的方式。 有一个解决方法，但我们建议仅在你已拥有在 IONOS 上按 1 或 1 创建子域的经验&它。 尽管存在此服务[](../setup/domains-faq.yml)限制，但您仍选择在 IONOS 1&1 之前管理自己的 Microsoft DNS 记录，请按照本文中的步骤验证域并设置电子邮件、Skype for Business Online 等的 DNS 记录。 
...
1. 在"添加 DNS 记录"页上新记录的框中，键入或复制并粘贴下表中的值。 
    ...
    |:-----|:-----|:-----|
  |**主机名** <br/> |**值** <br/> |
| (将此字段留空)   <br/> |MS=ms *XXXXXXXX*  <br/> 注意：这是一个示例。 在这里使用表中的特定“**目标地址或指向的地址**”值。 [如何查找此项？](../get-help-with-domains/information-for-dns-records.md)          |
...
8. 选择“**保存**”。<br/>（您可能需要向下滚动。）<br/>

6. 如果已列出任何 MX 记录，则通过选择"删除记录" **删除每个记录**。
...
1. 使用上述第一 (CNAME 记录的说明) Lyncdiscover 应用程序创建另一个子域。
