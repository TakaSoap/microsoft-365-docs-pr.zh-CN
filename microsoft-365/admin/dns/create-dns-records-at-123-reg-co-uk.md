...
5. In the **Type** box for the new record choose **TXT/SPF** from the drop-down list， and then type or copy and paste the other values from the following table.
    ...
    |---|---|---|
    |@|TXT/SPF|MS=*msXXXXXXXX* <br/> **注意：** 这是一个示例。 在这里使用表中的特定“**目标地址或指向的地址**”值。 [如何查找此项？](../get-help-with-domains/information-for-dns-records.md)|
...
5. In the **Type** box for the new record choose **MX** from the drop-down list， and then type or copy and paste the other values from the following table.
    ...
    |主机名|类型|优先级|目标 MX|
    |---|---|---|---|
    |@|MX|1 <br/> 有关优先级的详细信息，请参阅[什么是 MX 优先级？](../setup/domains-faq.yml)|*\<domain-key\>*.mail.protection.outlook.com。 <br/> **此值必须以句点 (.) 结尾。** <br/> **注意：** 从 Microsoft 帐户获取 \<domain-key\>。 [如何查找此项？](../get-help-with-domains/information-for-dns-records.md)|
...
5. 添加 CNAME 记录。
    ...
    |主机名|类型|目标 CNAME|
    |---|---|---|
    |自动发现|CNAME|autodiscover.outlook.com. <br/> **此值必须以句点 (.) 结尾。**|
...
5. In the **Type** box for the new record choose **TXT/SPF** from the drop-down list， and then type or copy and paste the other values from the following table.
    ...
    |主机名|类型|Destination TXT/SPF|
    |---|---|---|
    |@|TXT/SPF|v=spf1 include:spf.protection.outlook.com -all <br/> **注意：** 建议复制粘贴此条目，以保证正确保留所有空格。|
...
### <a name="add-the-two-required-cname-records-for-skype-for-business"></a>为记录添加两个必需的 CNAME Skype for Business
...
1. 添加第一条 CNAME 记录。
    ...
    |主机名|类型|目标 CNAME|
    |---|---|---|
    |sip|CNAME|sipdir.online.lync.com. <br/> **此值必须以句点 (.) 结尾。**|
    |lyncdiscover|CNAME|webdir.online.lync.com. <br/> **此值必须以句点 (.) 结尾。**|
...
1. 添加第一条 CNAME 记录。
   ...
   |主机名|类型|目标 CNAME|
   |---|---|---|
   |enterpriseregistration|CNAME|enterpriseregistration.windows.net. <br/> **此值必须以句点 (.) 结尾。**|
   |enterpriseenrollment|CNAME|enterpriseenrollment.manage.microsoft.com。 <br/> **此值必须以句点 (.) 结尾。**|
