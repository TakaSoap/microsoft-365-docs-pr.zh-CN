...
2. 在仪表板主页上，单击活动用户- Microsoft 365 应用版或活动用户 - Microsoft 365 服务卡上的查看更多按钮，以访问 Office 365 报告页面。
  
## <a name="interpret-the-groups-report"></a>解释组报告

You can view the activations in the Office 365 report by choosing the **Groups activity** tab.<br/>![Microsoft 365 报表 - Microsoft Office 365 组活动。](../../media/ab90e30b-8938-4110-ab3d-ee472a4cfe21.png)

选择 **"选择要在** 报表中添加或删除列的列"。  <br/> ![Office 365 组活动报表 - 选择列](../../media/1600556a-f5f1-47d9-b325-cd77c78f4004.png)
...
|**跃点数**|**定义**|
|组名称  <br/> |组的名称。  <br/> |
|Deleted  <br/> |已删除的组数。 如果该组被删除，但在报告时间段中有活动，它将显示在网格中，并显示设置为 true 的标记。  <br/> |
|组所有者  <br/> |组所有者的名称。  <br/> |
|上次活动日期 (UTC)   <br/> |组收到邮件的最近日期。 - 这是电子邮件对话、Yammer 或网站中活动发生的最新日期。  <br/> |
|类型  <br/> |组的类型。 可以是私有组，也可以是公用组。  <br/> |
|在 Exchange 中接收的电子邮件  <br/> |组接收的邮件数。|
|Exchange 中的电子邮件 (总数)   <br/> |组邮箱中的项目总数。  <br/> |
|用于 Exchange 邮箱存储 (MB)   <br/> |组邮箱使用的存储。 <br/>|
|SharePoint 文件 (总数)   <br/> |存储在 SharePoint 组网站中的文件数。  <br/> |
|SharePoint 文件 (活动)   <br/> |在报告期间对 sharePoint 组网站中 (查看或修改、同步、在内部或外部共享) 文件数。  <br/> |
|用于 SharePoint 的网站总存储 (MB)   <br/> |报告期间使用的存储量（以 MB 为单位）。  <br/> |
|Yammer 消息 (发布)   <br/> |报告期间 Yammer 组中发布的消息数。  <br/> |
|Yammer 中的消息 (阅读)   <br/> |报告期间 Yammer 组中读取的对话数。  <br/> |
|Yammer 中 (喜欢)   <br/> |报告期间 Yammer 组中喜欢的消息数。  <br/> |
