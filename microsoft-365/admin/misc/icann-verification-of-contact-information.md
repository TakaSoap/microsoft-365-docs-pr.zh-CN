---
title: 联系人信息的 ICANN 验证
f1.keywords:
- NOCSH
ms.author: pebaum
author: pebaum
manager: mnirkhe
audience: Admin
ms.topic: get-started-article
ms.service: o365-administration
localization_priority: Normal
ms.collection:
- M365-subscription-management
- Adm_O365
- Adm_NonTOC
search.appverid:
- MET150
- MOE150
ms.assetid: 0413dcce-26be-4b8c-b3fa-73ac1ac4fb35
ROBOTS: NOINDEX
description: 了解如何验证您的联系信息，以符合 ICANN 法规。
ms.openlocfilehash: 1173806aee452f8ca698702a67d3c89b575e6418
ms.sourcegitcommit: 812aab5f58eed4bf359faf0e99f7f876af5b1023
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/02/2020
ms.locfileid: "42361363"
---
# <a name="icann-verification-of-contact-information"></a>联系人信息的 ICANN 验证

在购买域或对域的联系信息进行更改时，需要注册器验证您的联系信息，以符合 ICANN 法规。

若要进行验证，请将电子邮件发送到注册人的地址。

 **若要验证您的联系信息，请选择验证电子邮件中的链接。** 如果域所有者未验证电子邮件地址，则该域将被挂起。 然后，转到 "管理中心" 中的 "[域" 页面](https://admin.microsoft.com/adminportal/home?ref=Domains)并检查您的域状态。

电子邮件将如下所示：

![电子邮件示例](../../media/8bf27c08-510c-4d49-b152-8d047d038f1f.jpg)


