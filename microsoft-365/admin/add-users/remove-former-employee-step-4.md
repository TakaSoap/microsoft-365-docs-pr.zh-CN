---
title: 步骤 4 - 向另一名员工授予OneDrive和Outlook数据的访问权限
f1.keywords:
- NOCSH
ms.author: kwekua
author: kwekua
...
---
# <a name="step-4---give-another-employee-access-to-onedrive-and-outlook-data"></a>步骤 4 - 向另一名员工授予OneDrive和Outlook数据的访问权限
...
如果您删除用户的许可证但不删除帐户，您可以为自己授予访问用户许可证中的内容OneDrive。 如果删除用户帐户，则默认情况下你有 30 天的时间访问前用户OneDrive数据。 [了解如何设置已删除OneDrive的用户的保留时间](/onedrive/set-retention)。 如果您在此时间内[没有还原用户帐户，则](/office365/admin/add-users/restore-user)其OneDrive内容将被删除。
...
1. 以全局<a href="https://go.microsoft.com/fwlink/p/?linkid=2024339" target="_blank">管理员或</a>管理员管理员SharePoint管理中心。
...
10. 用户现在将能够使用 OneDrive URL 访问前OneDrive服务器。 
...
1. [将前员工的电子邮件添加到](https://support.microsoft.com/office/6e27792a-9267-4aa4-8bb6-c84ef146101b)你的Outlook (如果你重置用户密码，你可以将其[](reset-passwords.md)设置为仅你知道) 

2. 在Outlook中，选择"文件 **"。**

    ![这是功能区在功能区Outlook 2016。](../../media/d7f66ed3-9861-4521-b410-e86a58ab15a7.png)
...
Outlook，除非已创建一个新的 Outlook.pst (.pst) 或使用了密码保护的文件，否则立即开始导出。
  
- 如果要创建一个Outlook.pst (.pst) ，则可选密码可以帮助保护文件。 当出现 **"Outlook数据文件**"对话框时，在"密码"和"验证密码"框中键入密码，然后选择"确定 **"。** 在 **"Outlook数据文件密码**"对话框中，键入 *密码*，然后选择"确定 **"。**

- 如果要导出到受密码保护的现有 Outlook 数据文件 (.pst) ，请在 **"Outlook 数据文件** 密码"对话框中，键入密码，然后选择"确定 **"。**
...
> [!TIP]
> 如果希望仅从 Outlook 数据文件 (.pst) 导入或还原几个项目，可以打开 Outlook 数据文件。 然后，在导航窗格中，将"数据文件"文件夹中Outlook项目拖动到现有Outlook文件夹中。

## <a name="related-content"></a>相关内容

[在管理员帐户上添加OneDrive (管理员) ](/sharepoint/manage-user-profiles#add-and-remove-admins-for-a-users-onedrive)
