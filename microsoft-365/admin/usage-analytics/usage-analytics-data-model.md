...
## <a name="data-for-the-microsoft-365-usage-analytics-tables"></a>Microsoft 365 使用情况分析表的数据
...
|租户 Microsoft 365 组使用情况  <br/> |包含有关 Microsoft 365 组使用情况的数据，包括邮箱、SharePoint 和 Yammer。  <br/> |包含滚动的 12 个月（包括当前部分月份）的月份结束状态数据。  <br/> |
|租户 Office 激活  <br/> |包含有关 Office 订阅激活数、每个设备 (Android/iOS/Mac/PC) 激活数、按服务计划（例如，Microsoft 365 企业应用版、Visio、Project）进行激活的数据。  <br/> |包含滚动的 12 个月（包括当前部分月份）的月份结束状态数据。  <br/> |
...
|IDType  <br/> |如果用户是使用 Yammer ID 进行连接的 Yammer 用户，则 ID 类型设置为 1;如果用户使用 Microsoft 365 ID 连接到 Yammer，则设置为 0。  <br/> 值为 1 表示此用户使用其 Yammer ID 而非 Microsoft 365 ID 连接到 Yammer  <br/> |
...
|HasLicenseTeams  <br/> |如果为用户分配了许可证并启用了使用 Microsoft Teams，则设置为 true。  <br/> |
...
|Deleted  <br/> |如此 如果该用户已在上一个完整的月份从 Microsoft 365 中删除。  <br/> |
|DeletedDate  <br/> |从 Microsoft 365 中删除用户的日期。  <br/> |
|YAM_State  <br/> |Yammer 系统中用户状态可以是活动状态、已删除状态或已挂起状态。  <br/> |
...
|ODB_FileSharedInternally  <br/> |此用户从任何 OneDrive for Business 内部共享的文件数，或与组中可能包含外部用户 (组) 。  <br/> |
...
|Teams_HasOtherAction  <br/> |如果用户在 Microsoft Teams 中执行了其他操作，则布尔值。  <br/> |
|YAM_MessagePost  <br/> |此用户发布 Yammer 消息的数量。  <br/> |
|YAM_MessageLiked  <br/> |此用户喜欢的 Yammer 消息数。  <br/> |
|YAM_MessageRead  <br/> |此用户读取的 Yammer 消息数。  <br/> |
...
此表提供有关 Microsoft 365 中每种产品的启用、活跃、返回和首次使用用户的每月采用数据。 Microsoft 365 值表示任一产品中的活动使用情况。
  
|**列名称**|**列说明**|
|:-----|:-----|
|Product  <br/> |产品名称，已汇总该产品的使用情况信息。 产品列中的 Microsoft 365 值表示跨任何产品的活动  <br/> |
...
|FirstTimeUsers  <br/> |自在新使用情况系统中启用数据收集起在时间范围内首次处于活动状态的用户数。  <br/> 如果自在此新报告系统中启用数据收集起，首次检测到某一用户的活动，该用户将被视为某一特定月份中首次使用的用户。 一旦计为第一次用户，即使此用户的活动存在较大的间隙，他们绝不会再次被计为首次使用的用户  <br/> |
...
|:-----|:-----|
|Timeframe  <br/> |月份值。对于过去 12 个月（包括当月），每个产品每月对应一行。  <br/> |
|Product  <br/> |Microsoft 365 中可用使用情况数据的产品的名称。  <br/> |
...
此表包含所有拥有用户邮箱的许可 Exchange Online 用户的摘要数据。 它包含跨所有用户邮箱的月末状态。 此表中的数据不能跨多个月份进行累加。 此表中的最新月份的数据表示最新状态。
...
此表提供有关用户用于连接到 Exchange Online、Skype for Business 和 Yammer 的客户端的每月汇总数据。 此表中的客户端尚未使用 SharePoint Online 和 OneDrive for Business 数据。
  
|**列名称**|**列说明**|
|:-----|:-----|
|Product  <br/> |提供客户端使用情况数据的 Microsoft 365 中的产品名称。  <br/> |
...
此表包含有关 SharePoint Online 网站的使用情况或活动的逐月汇总数据。 这仅包含团队网站和组网站。 SharePoint Online 网站的月末状态在此列中表示，例如，如果用户创建了五个文档，并使用了 10 MB 的总存储，然后删除了一些文件，并添加了更多文件，以便文件在当月结束时总共使用 5 MB 存储的七个总数，则此表中表示的值是月份结束状态。 此表处于隐藏状态，以避免汇总时重复计数，并用作创建两个引用表的源。
...
|Diplansed  <br/> |在时间范围结束时，所有 OneDrive 帐户使用的存储总量总计。  <br/> |
...
|SPO_TotalGroups  <br/> |SharePoint 组数。  <br/> |
|SPO_ActiveGroups  <br/> |活动 SharePoint 组的数量。  <br/> |
|SPO_FileAccessedActiveGroups  <br/> |具有文件访问活动的 SharePoint 组数。  <br/> |
|SPO_FileSyncedActiveGroups  <br/> |具有文件同步活动的 SharePoint 组数。  <br/> |
|SPO_FileSharedInternallyActiveGroups  <br/> |在内部拥有共享活动的 SharePoint 组数，或具有可能包含外部用户 (的组) 。  <br/> |
|SPO_FileSharedExternallyActiveGroups  <br/> |在外部共享活动的 SharePoint 组数。  <br/> |
|SPO_TotalActivities  <br/> |SharePoint 活动数。  <br/> |
|SPO_FileAccessedActivities  <br/> |SharePoint 文件访问的活动数。  <br/> |
|SPO_FileSyncedActivities  <br/> |SharePoint 文件同步活动的数量。  <br/> |
|SPO_FileSharedInternallyActivities  <br/> |内部 SharePoint 文件共享活动数，或与可能包含外部成员 (组共享) 。  <br/> |
|SPO_FileSharedExternallyActivities  <br/> |在外部共享 SharePoint 文件的活动数。  <br/> |
|SPO_TotalFiles  <br/> |SharePoint 文件数。  <br/> |
|SPO_ActiveFiles  <br/> |活动 SharePoint 文件的数量。  <br/> |
|SPO_StorageUsed  <br/> |使用的 SharePoint 存储数量。  <br/> |
|YAM_TotalGroups  <br/> |Yammer 组数。  <br/> |
|YAM_ActiveGroups  <br/> |活动 Yammer 组的数量。  <br/> |
|YAM_LikedActiveGroups  <br/> |具有类似活动的 Yammer 组数。  <br/> |
|YAM_PostedActiveGroups  <br/> |具有帖子活动的 Yammer 组数。  <br/> |
|YAM_ReadActiveGroups  <br/> |具有读取活动的 Yammer 组数。  <br/> |
|YAM_TotalActivities  <br/> |Yammer 活动数。  <br/> |
|YAM_LikedActivities  <br/> |Yammer 喜欢的活动数。  <br/> |
|YAM_PostedActivties  <br/> |Yammer 帖子活动的数量。  <br/> |
|YAM_ReadActivites  <br/> |Yammer 读取活动的数量。  <br/> |
   
### <a name="data-table---tenant-office-activation"></a>数据表 - 租户 Office 激活

该表提供有关跨服务计划（例如，Microsoft 365 企业应用版、Visio、Project）激活 Office 订阅数的数据。 此外，它还提供了有关每个设备 (Android/iOS/Mac/PC) 的激活次数的数据。
