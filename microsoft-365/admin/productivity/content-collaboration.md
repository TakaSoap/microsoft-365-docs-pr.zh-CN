...
数字转型的一个关键方面是人们如何协作处理文件。 使用内容Microsoft 365，人员可以随时随地访问、创建、修改和协作处理内容。 研究表明，当用户使用联机文件进行协作时，每个人每周平均节省 100 分钟。
...
> [!NOTE]
> 2021 年 4 月 22 日，我们更改了协作者指标的计算方式。 这会影响主要[见解、](#primary-insight)[文件协作见解](#number-of-files-collaborated-on)以及内容协作分数的度量方式。 此更改有助于减少来自来自 Microsoft 和其他第三 (应用程序的非人工代理) 或机器人）的数据的干扰，从而获得更准确且可操作的分数。
...
1. **标头：** 显示组织中的有权访问协作内容OneDrive SharePoint用户百分比。
...
3. **可视化 (当前状态) ：**
    - 蓝色部分表示通过 OneDrive 或 SharePoint 启用文件协作的百分比的水平栏，这些人员是过去 28 天内在线文件的读者、创建者或协作者。
        ...
        **协作者：** 使用联机文件或联机文件进行OneDrive SharePoint。 如果其中一个人在 28 天内创建或修改联机 Office 应用 或 PDF 后读取或编辑联机文档或 PDF，则两个人是协作者。
    ...
    - 突出显示 (分数的) /分母值用于计算每个水平条中表示的百分比。
      ...
      - **创建者：**</br>
        ...
        - 分母：过去 28 天内OneDrive或SharePoint至少 1 次访问次数。 </br> 
      - **协作者：**</br>
        - 数字：过去 28 天内在 OneDrive 或 SharePoint 中协作处理联机文件的人数</br>
        - 分母：过去 28 天内OneDrive或SharePoint至少 1 次访问次数

    - 每个读者、创建者和协作者的对等基准值也显示为百分比。 换句话说，创建者数量的值以有权访问创作者或创作者OneDrive百分比SharePoint。
...
组织的内容协作分数以 (组织) 聚合级别衡量，无论用户是否一致地阅读、创建或协作联机 Office 文件（如 Word、Excel、PowerPoint、OneNote 或 PDF，或者 OneDrive 或 SharePoint 中）。
...
:::image type="content" source="../../media/sharepointonedrivefiles.jpg" alt-text="显示以用户或用户OneDrive文件SharePoint。":::

1. **标头：** 突出显示活动在 Microsoft 365 Office 应用程序上或在 OneDrive 或 SharePoint 上创建文件SharePoint。
...
3. **可视化：** 可视化中的细分表示使用 Microsoft Office 应用在 OneDrive 和 SharePoint 创建文件的程度，如下所示：
      - **OneDrive：** 蓝色 (条) 部分，而条形上的分数表示活动在 Office 应用程序上创建内容OneDrive的百分比，如下所示：
        - Numerator：过去 28 天内在 OneDrive 内创建、修改、上载、同步、签入、复制或移动联机 Office 文件的人数。</br>
        - 分母：过去 28 天内OneDrive或SharePoint访问办公室文件的人数。
      - **SharePoint：** 蓝色 (条的) 部分，而条形上的分数表示活跃在 Office 应用程序上并按以下SharePoint内容的百分比：</br>
         - 数字：创建、修改、上载、同步、签入、复制或移动联机 Office 文件 (Microsoft Word、Excel、PowerPoint 或 OneNote 文件) 最近 28 天内在 SharePoint 上移动的数量。</br>
        - Denominator：过去 28 天内OneDrive或SharePoint访问Office文件的数量。
...
3. **可视化：** 表示有权访问内部或外部共享OneDrive SharePoint共享文件的程度：
      - **外部：** 蓝色 (条的) 部分以及条形上的分数 (numerator/分母) 表示有权访问 OneDrive 或 SharePoint 且正在外部共享文件的人的百分比。
        -  数字：过去 28 天内与外部共享文件的人数
        - 分母：过去 28 天内OneDrive或SharePoint至少 1 个用户。
      - **仅内部：** 蓝色 (条的) 部分和条形上的分数 (numerator/分母) 表示有权访问 OneDrive 或 SharePoint 且仅在内部共享文件的人的百分比。
...
3. **可视化：** 显示有权访问 OneDrive 或 SharePoint 的SharePoint，基于他们协作处理的文件数。 这通过以下 4 个类别 (显示，每个类别的蓝色部分和分数表示有权访问属于该类别的 OneDrive 或 SharePoint 的) ：
      - **无协作：**
...
1. **标头：** 突出显示所有经过测试的设备中与 OneDrive 和 SharePoint。 
...
3. **可视化：** 显示具有与移动设备和移动设备相关的不同网络连接性能级别的OneDrive SharePoint：
      ...
      - **41-60： (** 的橙色颜色) 表示网络性能分数介于 40 到 60 之间的设备的百分比。 
      - **21-40： (** 的红色) 表示网络性能分数在 20-40 之间的设备的百分比。 
