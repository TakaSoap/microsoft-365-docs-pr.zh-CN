...
> [!NOTE]
> 2021 年 4 月 22 日，我们更改了协作者指标的计算方式。 这会影响主要[见解、](#primary-insight)[文件协作见解](#number-of-files-collaborated-on)以及内容协作分数的度量方式。 此更改有助于减少来自非人工代理 (Microsoft) 其他第三方应用程序的机器人）的数据的干扰，从而获得更准确且可操作的分数。
...
1. **标头：** 显示组织中的有权访问协作内容或内容OneDrive SharePoint用户百分比。
...
3. **可视化 (当前状态) ：**
    ...
    - 突出显示 (分数的) /分母值用于计算以每个水平线表示的百分比。
      ...
      - **协作者：**</br>
        ...
        - 分母：过去 28 天内OneDrive或SharePoint访问次数至少为 1 人
...
1. **标头：** 突出显示在应用程序上处于活动状态Microsoft 365 Office或创建文件的应用程序OneDrive SharePoint。
...
3. **可视化：** 可视化中的细分表示使用 Microsoft Office 应用在 OneDrive 和 SharePoint 创建文件的程度，如下所示：
      - **OneDrive：** 蓝色 (条) 部分，而条形上的分数表示活动在 Office 应用程序上创建内容OneDrive的百分比，如下所示：
        ...
        - Denominator：过去 28 天内OneDrive或SharePoint访问办公室文件的数量。
      - **SharePoint：** 蓝色 (条) 部分，而条形上的分数表示活跃在 Office 应用程序上并按如下SharePoint内容的百分比：</br>
...
3. **可视化：** 可视化的细分旨在表示电子邮件中附加内容的人使用不同模式（未保存到联机文件的 (文件、指向联机文件的链接）) ：
      - **附加文件：** 蓝色 (条) 部分的颜色，而条形上的分数 (numerator/denominator) 表示电子邮件中使用附件的百分比。
      ...
      - **指向联机文件的链接：** 蓝色 (条的) 部分，而条形上的分数 (numerator/denominator) 表示使用附件并将链接附加到电子邮件中的文件的百分比。
        ...
        - 分母：过去 28 天内有权访问Exchange和OneDrive、SharePoint或两者的人的数量。
4. **指向资源的链接：** 选择此链接可查看帮助内容。
...
1. **标头：** 突出显示对 4 个或多个文件进行OneDrive SharePoint用户的访问权限的百分比。
2. **正文：** 提供有关用户如何利用联机文件进行更好的协作的信息。
3. **可视化：** 显示有权访问 OneDrive 或 SharePoint 的SharePoint，基于他们协作处理的文件数。 这通过以下 4 个类别 (，栏的蓝色部分和分数表示有权访问属于该类别的 OneDrive 或 SharePoint 用户百分比) ：
      - **无协作：**
