...
企业工作效率的基本支柱是人们在文件中进行协作的方式。 通过 Microsoft 365 上的内容，用户访问、创建、修改和协作处理来自任何位置的其他人的内容。 研究表明，当人们使用联机文件进行协作时，每个人平均每周保存100分钟。 请参阅 [证据](https://vc2prod.blob.core.windows.net/vc-resources/TEIStudies/TEI%20of%20Microsoft%20365%20E5%20-%20Oct%202018.pdf)。
...
3. **可视化 (当前状态) ：**
    - 具有蓝色颜色的部分的水平条形图表示通过 OneDrive 或 SharePoint 启用了文件协作的人员所占的百分比，即最近28天内联机文件中的 **读者、创建者** 或 **协作** 者。
        ...
        **创建者：** 创建、修改、上传、同步、签入、复制或移动联机 OneDrive 或 SharePoint 文件的人员。</br>
        **协作者：** 使用 OneDrive 或 SharePoint 与联机文件进行协作的人员。 如果用户在创建或修改了一个或多个联机 Office 应用程序或 PDF 之后，他们在28天的时段内阅读或编辑了这些人，则这两个人就是协作者。
    ...
    - 突出显示用于计算每个水平栏中表示的百分比的分数的 (分子/分母) 。
      ...
      - **协作者**</br>
        - 分子：最近28天内在 OneDrive 或 SharePoint 中共同使用联机文件的人数</br>
        - 分母：在最近28天内至少有一天访问 OneDrive 或 SharePoint 的人员人数

    - 每个读者、创建者和协作者的对等基准值也显示为百分比。 换言之，创建者数的值显示为有权访问 OneDrive 或 SharePoint 的人员数量的百分比。
    
1. **链接到资源：** 选择此链接可查看逐份打印的视频和其他相关帮助内容。


#### <a name="trend-visualization-of-primary-insight"></a>主要洞察力的趋势可视化

趋势可视化图表显示了最近180天内读者、创建者和协作者的主要洞察力关键指标的趋势线。 图表上的每个数据点都是最近28天的活动合计。 每个创建者数据点都提供在 x 轴上的每个日期的最后28天内被标记为创建者的所有人员的计数。

:::image type="content" source="../../media/trendvisualization.jpg" alt-text="来自通信协作分数的主要见解。":::
...
:::image type="content" source="../../media/sharepointonedrivefiles.jpg" alt-text="来自通信协作分数的主要见解。":::

1. **标头：** 突出显示在 OneDrive 或 SharePoint 上创建文件的 Microsoft 365 Office 应用程序上活动活动的用户所占的百分比。
2. **正文：** 提供有关 OneDrive 和 SharePoint 中的内容创建值的信息。
3. **可视化：** 可视化中的细目表示使用 Microsoft Office 应用程序在 OneDrive 和 SharePoint 中创建文件的用户的程度，如下所示：
      - **OneDrive：** 蓝色 (条) 部分的颜色，条形图上的分数表示在 Office 应用程序上按如下所示创建内容的活动用户所占的百分比：
        - 分子：最近28天内创建、修改、上传、同步、签入、复制或移动 OneDrive 中的 online Office 文件的人数。</br>
        - 分母：有权访问 OneDrive 或 SharePoint 并在最近28天内访问 office 文件的人员的数量。
      - **SharePoint：** 蓝色 (条) 部分的颜色，条形图上的分数表示在 Office 应用程序上活动的用户数和在 SharePoint 上创建内容的用户的百分比，如下所示：</br>
         - 分子：在最近28天内，在 SharePoint 上创建、修改、上传、同步、签入、复制或移动联机 Office 文件 (Microsoft Word、Excel、PowerPoint 或 OneNote 文件) 的人员数量。</br>
        - 分母：有权访问 OneDrive 或 SharePoint 并在最近28天内访问 Office 文件的人员的数量。

4. **链接到资源：** 选择此链接可查看帮助内容。

### <a name="use-of-attachments-in-email"></a>在电子邮件中使用附件

:::image type="content" source="../../media/emailattachments.png" alt-text="来自通信协作分数的主要见解。" (条形图的颜色) 部分，条形图上的分数表示在电子邮件正文中嵌入链接的人员所占的百分比。
...
:::image type="content" source="../../media/sharingonlinefiles.png" alt-text="来自通信协作分数的主要见解。":::
...
:::image type="content" source="../../media/intensityofcollab.png" alt-text="来自通信协作分数的主要见解。":::
...
:::image type="content" source="../../media/networkperfstrength.png" alt-text="来自通信协作分数的主要见解。":::
