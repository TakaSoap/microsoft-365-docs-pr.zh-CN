...
组织的通信分数以 (组织) 衡量人们是否在 28 天时段内在电子邮件、聊天和社区帖子之间持续使用多种模式进行通信。
...
3. **可视化：** 细分表示每种模式的使用。 每一栏上的彩色部分和分数表示发送电子邮件、邮件或社区帖子的人数，以启用该模式人数的百分比表示：
   ...
   - **在社区中发布人员：** 颜色部分和分数表示为在社区中发布Yammer用户百分比。 此分数构建于：
...
3. **可视化：** 根据新电子邮件线程是否包含以下主题，@mentions速率：
   ...
   - **无提及电子邮件响应 \@ ：** 颜色部分和分数表示包含电子邮件主题的新电子邮件@mentions。 此分数构建于：
     ...
     - **分子：** 不包含电子邮件的新@mentions，它们已启动，并且最近 28 天内也收到响应。
...
1. **标头：** 以发送任何类型的消息（包括聊天和频道消息 (过去 28 天内发送) 消息的百分比突出显示发送频道消息的人。
...
2. **正文：** 提供有关在用户中使用问题和答案来共享Yammer的价值的信息。
