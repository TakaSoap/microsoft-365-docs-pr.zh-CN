...
---
# <a name="get-your-environment-ready-for-microsoft-viva-topics"></a>准备好环境以使用 Microsoft Viva 主题

为了充分利用 Viva 主题，您希望包含尽可能多的内容用于主题发现，以便为用户提供一组丰富的主题。 但是，哪些内容应该用于主题发现？ 如何在保持控制的同时最大化已编制索引的内容？ 范围中的内容越多，人工智能可以发现的见解就越好。 本文将引导您完成规划步骤，以确保您包含适当的内容，并且您具有合适的人员和资源来为用户提供良好的体验。
...
4. [适当地保护内容](#4-secure-content-appropriately)
    ...
    - 查看有关用户和内容的 Microsoft Graph 配置：
      - 主题索引会考虑不包括搜索或 Delve (内容的配置，例如，NOINDEX) 。 查看这些配置是否仍然相关。
...
由于主题卡片和突出显示内容只能在新式页面上显示，因此请更新您想要在 Viva 主题中包括的任何页面，从经典页面更新到新式主题。 请参阅["现代化经典 SharePoint 网站"。](https://docs.microsoft.com/sharepoint/dev/transform/modernize-classic-sites) 可以使用 [SharePoint 现代化扫描程序](https://docs.microsoft.com/sharepoint/dev/transform/modernize-scanner) 准备经典网站进行现代化。

如果有很多经典网站，请确定要转换为新式页面的高配置文件页面的优先级。

## <a name="4-secure-content-appropriately"></a>4. 适当地保护内容

当用户与主题卡片或主题页面交互时，他们可能会看到不同的资源。 这是因为他们有权访问与主题关联的不同文件。 如果基础权限过于严格，则信息通过主题发现的潜在方面可能会减少。 另一方面，如果内容过于广泛，则主题可能会向用户显示您不希望他们看到的内容。
...
- 审核 Microsoft 365 组和 Teams，确保它们已正确设置为公共或专用组或团队。 默认情况下，新 Teams 和 Microsoft 365 组设置为专用组，但在首次发布时默认为公开发布。 如果你是这些技术的先前采用者，你可能想要查看。 此外，团队的功能通常会在其生命周期中不断发展，并且可能需要更新设置以反映团队的当前使用。
...
- 允许跨所有 SharePoint 网站进行发现，或指定要作为主题源包含或排除的网站。
...
- KA (管理员) 是一个技术角色，通常在 IT 中。 此角色允许在 M365 管理中心中设置 Viva 主题，并配置主题发现和可见性。
...
| 设置 Viva 主题 | 拥有主题的管理和质量 | 根据主题的专业知识参与主题。 |
...
- 谁可以看到主题？ 主题可见性在 Microsoft 365 管理中心中配置。 选择要允许查看主题的组：
  ...
  - 仅选定人员或安全组 (在仍推出 Viva 主题时使用此选项是不错的，因此可以使用部分用户进行测试) 。 如果希望来宾查看主题，则需要使用"选定的人员或安全组"选项，并授予他们许可证。
  - 没有人。
...
- 使用现有分类，从 SharePoint 或其他位置帮助规划主题。 现有分类通常包括组织术语、产品、主题区域等。 主题源还可以来自项目列表、现有搜索书签等。
