...
4. [适当地保护内容](#4-secure-content-appropriately)
    ...
    - 查看你的 Microsoft Graph用户和内容配置：
      - 主题索引使用不包括搜索或搜索内容Delve (例如，NOINDEX) 。 检查这些配置是否仍然相关。
...
可以使用多种工具和服务来帮助进行迁移-可以在将内容迁移到迁移中获取有关如何迁移[的Microsoft 365。](/sharepointmigration/migrate-to-sharepoint-online) 迁移工具包括：
...
## <a name="2-connect-information-to-microsoft-graph"></a>2. 连接 Microsoft Graph
...
- 鼓励网站所有者查看共享和权限。
  ...
  SharePoint查看其网站的共享报告，该报告显示网站上配置的所有权限和共享链接的完整详细信息，请参阅[共享报告](/sharepoint/sharing-reports)。 这将列出来宾用户的 (和) 用户。

  网站所有者还可以访问"网站权限"和"高级权限"页面，查看哪些人具有 **设置** 权限。
...
- 审核 Microsoft 365 组和 Teams，确保适当地设置它们为公共或专用组或团队。 默认情况下Teams组Microsoft 365组设置为专用组，但在首次发布时，默认情况下是公开的。 如果您之前是这些技术的采用者，您可能需要查看。 此外，团队的功能往往在其生命周期中不断演变，设置可能需要更新以反映该团队的当前用途。
...
- KA 管理员 (KA) 是一个技术角色，通常是 IT 人员。 此角色允许在 M365 管理中心中设置 Viva 主题，并配置主题发现和可见性。
...
- 最佳做法是，将主题设定为主题列表的种子。 主题的质量和数量基于您的内容 -只有主题包含在范围内的内容中时，才能作为主题创建。 如果主题有足够的信息和证据，它将由 AI 创建。 种子设定主题是知识管理器和主题专家可以提供帮助的地方。 将人类知识与 AI 合并是提供高质量主题的最佳方法。 因此，如果预计有主题，可以在主题中心手动创建这些主题。 这样做将为 AI 提供该主题的相关性的强信号，并标识要与该主题关联的资源和人员。
