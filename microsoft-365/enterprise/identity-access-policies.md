...
建议的做法是为条件访问排除创建 Azure AD 组。 在 "**工作分配**" 部分的 "**用户和组**的**排除**值" 设置中，将此组添加到所有条件访问规则。 这为您提供了在对访问问题进行故障排除时提供对用户的访问权限的方法。 建议仅将其作为临时解决方案。 监视此组的更改，并确保仅按预期使用排除组。 
...
|云应用或操作| **包含的云应用 >** | **选择 "应用**程序"：选择要应用此规则的应用程序。 例如，选择 "Exchange Online"。||
|条件| | |配置特定于您的环境和需求的条件。|
||登录风险||请参阅下表中的指南。|
|||||
