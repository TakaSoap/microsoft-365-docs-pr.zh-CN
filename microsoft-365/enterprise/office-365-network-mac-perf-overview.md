...
ms.locfileid: "62187433"
---
# <a name="network-connectivity-in-the-microsoft-365-admin-center"></a>Microsoft 365 管理中心中的网络连接

当前Microsoft 365 管理中心包括从 Microsoft 365 租户收集的聚合网络连接指标，并且仅可供租户中的管理用户查看。
...
首次导航到网络性能页面时，必须配置位置，才能查看全局网络性能地图、范围为整个租户的网络评估、远程工作与现场工作的用户百分比，以及要针对和/或进一步调查的当前问题列表。 从概述窗格中，可以向下钻取以查看特定网络性能指标和按位置的问题。 有关详细信息，请参阅网络性能[概述Microsoft 365 管理中心](#network-connectivity-overview-in-the-microsoft-365-admin-center)。

系统可能会要求你代表你的组织加入此功能的公共预览版。 接受通常会立即发生，之后你将看到网络连接页面。

若要访问"网络连接"页，您必须是组织内部Microsoft 365。 报告读者管理角色将拥有对此信息的读取访问权限。 若要配置网络连接的位置和其他元素，管理员必须具有服务支持管理员角色。
...
Windows位置服务必须在计算机上获得同意。 可以通过运行应用并找到自己 **地图** 测试这一点。 可以在具有安全选项的单个 **计算机上设置 |隐私|必须** 启用"_允许应用访问你_ 的位置"设置的位置。 Windows使用 MDM 或组策略以及设置 _LetAppsAccessLocation_ 将位置服务许可部署到电脑。
...
> [!NOTE]
> 目前，当将办公位置添加到 Microsoft 365 网络连接中时Microsoft 365 管理中心只能为 LAN 子网提供 IPv4 地址。 Egress IP 地址必须使用 IPv4。
...
**网络评估** 将许多网络性能指标聚合到企业网络运行状况的快照中，以 0 到 100 的点值表示。 网络评估的范围既包括整个租户，又针对用户连接到租户的每个地理位置，为 Microsoft 365 管理员提供了一种简单的方式，以便立即掌握企业网络运行状况的全局信息，并快速深入到任何全局办公地点的详细报告。
...
企业可以通过遵循连接原则，以及使用 Office 365[中心](./microsoft-365-network-connectivity-principles.md)网络连接功能，改进常规用户体验并Microsoft 365 管理环境。 在大多数情况下，遵循这些一般原则将对最终用户延迟、服务可靠性和服务整体性能产生显著Microsoft 365。
...
位置自动分类为现场或远程，但是，您可以选择手动输入所有现场出口 IP 地址，以确保 100% 分类。 如果决定选择此路由，在添加所有出口 IP 地址后，必须选中"位置"设置"飞出"中的"手动输入所有现场出口 **IP** 地址"复选框。 完成此操作后，已标记为现场出口 IP 地址的所有网络流量日志将始终分类为办公室，其他每个出口 IP 地址将归类为远程地址。
...
- **Exchange最佳服务前端**- 此Exchange位置的用户应连接到的推荐服务前端门之一
...
1. 添加办公室位置并保存文件后，单击"浏览"按钮Upload **字段** 旁边的"浏览"按钮，然后选择保存的 CSV 文件。 
...
Microsoft 365服务前端是 Microsoft 全局网络的入口点，其中Office服务终止其网络连接。 为了获得最佳网络连接Microsoft 365，建议将网络连接终止到最靠近Microsoft 365门。
...
最佳的Microsoft 365服务前端是离网络出口最近的入口，通常位于城市或都市区域。 使用[Microsoft 365连接测试 (预览) ](office-365-network-mac-perf-onboarding-tool.md)确定使用中的 Microsoft 365 服务前端和最优服务前端的位置。 如果该工具确定使用中的前端是最佳的，则最好连接到 Microsoft 的全局网络。

### <a name="what-is-an-internet-egress-location"></a>什么是 Internet 出口位置？

Internet 出口位置是网络流量退出企业网络并连接到 Internet 的位置。 这还标识为具有网络地址转换 (NAT) 设备的位置，并且通常是与 Internet 服务提供商 (ISP) 。 如果您看到您的位置和 Internet 出口位置之间的距离很长，这可能表示 WAN 回程很大。
