...
Windows必须在计算机上同意位置服务。 可以通过运行应用和定位 **地图测试这** 一点。 可以在具有自动安装功能的单个 **计算机上设置 |隐私|必须** 启用"_允许应用访问你_ 的位置"设置的位置。 Windows可以使用 MDM 或组策略（设置为 _LetAppsAccessLocation）_ 将位置服务许可部署到电脑。
...
如果添加公共出口 IP 地址，这些地址将用作辅助区分符，并且适用于使用同一 LAN 子网 IP 地址范围的多个站点时。 若要确保显示测试结果，应首先将公共出口 IP 地址范围留空。 如果包含它们，则测试结果必须匹配 LAN 子网 IP 地址范围之一和公用出口 IP 地址范围之一。
...
对于此选项，你需要在每个位置标识一个人。 要求他们浏览Microsoft 365[具有](https://connectivity.office.com)管理权限Windows计算机进行网络连接测试。 在网站中，他们需要登录到Office 365查看结果的同一组织的帐户。 然后，他们应单击"**运行测试"。** 在测试期间，有下载的连接测试 EXE。 他们需要打开并执行它。 测试完成后，测试结果将上载到管理中心。
...
**网络评估** 将许多网络性能指标聚合到企业网络运行状况的快照中，以 0 到 100 的点值表示。 网络评估的范围既包括整个租户，也适用于用户连接到租户的每个地理位置，为 Microsoft 365 管理员提供了一种简单方法，以便立即掌握企业网络运行状况的全局信息，并快速深入到任何全局办公地点的详细报告。
...
位置自动分类为现场或远程，但是，您可以选择手动输入所有现场出口 IP 地址，以确保 100% 分类。 如果决定选择此路由，在添加所有出口 IP 地址后，必须选中"位置"设置"飞出"中的"手动输入所有现场出口 **IP** 地址"复选框。 完成此操作后，已标记为现场出口 IP 地址的所有网络流量日志将始终分类为办公室，其他每个出口 IP 地址将被归类为远程。
...
1. 添加办公地点并保存文件后，单击"浏览"按钮Upload **字段旁边的**"浏览"按钮，然后选择保存的 CSV 文件。 
...
最佳的Microsoft 365入口是离网络出口最近的入口，通常位于城市或都市区域。 使用[Microsoft 365连接](office-365-network-mac-perf-onboarding-tool.md)测试 (预览) 确定使用中的 Microsoft 365 服务前端和最优服务前端的位置。 如果该工具确定使用中的前端是最佳的，则最好连接到 Microsoft 的全局网络。
...
[Microsoft 365预览版 (连接) ](office-365-network-mac-perf-onboarding-tool.md)
