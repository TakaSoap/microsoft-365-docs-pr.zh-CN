...
对于此选项，在每个支持先决条件的办公地点必须至少有两台计算机运行。 OneDrive更新Windows版本必须更新并安装在每台计算机中。 有关新版本OneDrive，请参阅OneDrive[发行说明](https://support.office.com/article/onedrive-release-notes-845dcf18-f921-435e-bf28-4e24b95e5fc0)。 计划将网络度量添加到其他 Office 365客户端应用程序中。

Windows必须在计算机上同意位置服务。 可以通过运行应用并 **找到自己地图** 测试这一点。 可以在具有安全选项的单个计算机上 **设置 |隐私|必须** 启用"_允许应用访问你_ 的位置"设置的位置。 Windows可以使用 MDM 或组策略（设置为 _LetAppsAccessLocation）_ 将位置服务许可部署到电脑。
...
对于此选项，既不Windows定位服务，也不Wi-Fi位置服务。 OneDrive版本Windows必须处于最新状态，并且必须至少安装在该位置的一台计算机中。
...
对于此选项，你需要在每个位置标识一个人。 要求他们浏览Microsoft 365[具有](https://connectivity.office.com)管理权限Windows计算机上执行网络连接测试。 在网站中，他们需要登录到要Office 365查看结果的同一组织的帐户。 然后，他们应单击"**运行测试"。** 在测试期间，有下载的连接测试 EXE。 他们需要打开并执行它。 测试完成后，测试结果将上载到管理中心。
...
> [!NOTE]
> 目前，当将办公位置Microsoft 365网络连接时Microsoft 365 管理中心，只能为 LAN 子网提供 IPv4 地址。 EgressIP 地址必须使用 IPv4。

## <a name="how-do-i-use-this-information"></a>如何使用此信息？

**网络见解**、其相关的性能建议和网络评估旨在帮助您设计办公地点的网络外围。 每个见解提供有关用户访问租户的每个地理位置的特定常见网络问题的性能特征的详细信息。 **每个网络** 见解的性能建议都提供了特定的网络体系结构设计更改，可改善与网络连接相关的Microsoft 365体验。 网络评估显示了网络连接如何影响用户体验，从而可以比较不同的用户位置网络连接。

**网络评估** 将许多网络性能指标聚合到企业网络运行状况的快照中，以 0 到 100 的点值表示。 网络评估的范围既包括整个租户，也适用于用户连接到租户的每个地理位置，为 Microsoft 365 管理员提供了一种简单的方式，以便立即掌握企业网络运行状况的全局信息，并快速深入到任何全局办事处位置的详细报告。
...
许多企业具有网络外围配置，这些配置已随着时间的推移而增长，主要设计用于容纳员工 Internet 网站访问，其中大多数网站都未事先知道且不受信任。 当前和必要的焦点是避免来自这些未知网站的恶意软件和网络钓鱼攻击。 此网络配置策略虽然有助于实现安全目的，但可能会导致Microsoft 365性能和用户体验下降。

## <a name="how-we-can-solve-these-challenges"></a>如何解决这些问题

企业可以通过遵循连接原则Office 365使用 Microsoft 365 管理 中心网络连接[](./microsoft-365-network-connectivity-principles.md)功能来改进一般用户体验并保护其环境。 在大多数情况下，遵循这些一般原则将对最终用户延迟、服务可靠性和服务整体性能产生显著Microsoft 365。
...
Microsoft 具有来自多个桌面Office Web 客户端的现有网络度量，这些客户端支持Microsoft 365。 这些测量现在用于提供网络体系结构设计见解和网络评估，这些见解显示在 Microsoft 365 管理 中心的网络连接页面中。 
...
- **Exchange次最佳** 前端 - Exchange连接到但不推荐使用的服务前端
- **SharePoint最佳服务** 前端 - 此SharePoint位置的用户应连接到的推荐服务前端门之一
- **SharePoint次最优服务** 前端 - SharePoint连接到但不推荐使用的服务前端
...
1. 在要添加的每个位置的新行中填写以下字段，打开 CSV 并添加位置。 将所有其他字段留空;在其他字段中输入的值将被忽略。
   ...
   1. **LanIps** (要求) ：列出此办公地点使用的 LAN 子网范围。 LAN 子网 ID 需要包含 CIDR 网络大小，其中网络大小可以介于 /8 和 /29 之间。 可以使用逗号或分号分隔多个 LAN 子网范围。
...
[Microsoft 365预览 (连接测试) ](office-365-network-mac-perf-onboarding-tool.md)
