...
若要访问"网络连接"页，您必须是组织内部Microsoft 365。 报告读者管理角色将具有此信息的读取权限。 若要配置网络连接的位置和其他元素，管理员必须是服务器管理员角色（如服务支持管理员角色）的一部分。
...
Windows必须在计算机上同意位置服务。 可以通过运行应用并 **找到自己地图** 测试这一点。 可以在具有安全选项的单个 **计算机上设置 |隐私|必须** 启用"_允许应用访问你_ 的位置"设置的位置。 Windows可以使用 MDM 或组策略（设置为 _LetAppsAccessLocation）_ 将位置服务许可部署到电脑。
...
对于此选项，既不Windows定位服务，也不Wi-Fi位置服务。 OneDrive版本Windows版本必须处于最新状态，并且必须至少安装在该位置的一台计算机中。
...
对于此选项，你需要在每个位置标识一个人。 要求他们浏览Microsoft 365[具有](https://connectivity.office.com)管理权限Windows计算机上执行网络连接测试。 在网站中，他们需要登录到Office 365查看结果的同一组织的帐户。 然后，他们应单击"**运行测试"。** 在测试期间，有下载的连接测试 EXE。 他们需要打开并执行它。 测试完成后，测试结果将上载到管理中心。
...
> [!NOTE]
> 目前，当向 Microsoft 365 中的网络连接添加办公Microsoft 365 管理中心时，只能为 LAN 子网提供 IPv4 地址。 EgressIP 地址必须使用 IPv4。
...
**网络评估** 将许多网络性能指标聚合到企业网络运行状况的快照中，以 0 到 100 的点值表示。 网络评估的范围既包括整个租户，也针对用户连接到租户的每个地理位置，为 Microsoft 365 管理员提供了一种简单方法，以便立即掌握企业网络运行状况的全局信息，并快速深入到任何全局办公地点的详细报告中。

具有多个办公地点和普通网络外围体系结构的复杂企业可以在初次载入 Microsoft 365 或修正使用增长时发现的网络性能问题时，从此信息中获益。 通常，对于使用 Microsoft 365 的小型企业，或已具有简单直接网络连接的企业来说，这通常不是必需的。 拥有 500 多个用户和多个办公地点的企业预期受益最大。
...
Microsoft 具有来自多个桌面Office Web 客户端的现有网络度量，这些客户端支持 Microsoft 365。 这些测量现在用于提供网络体系结构设计见解和网络评估，这些见解显示在 Microsoft 365 管理 中心的"网络连接"页中。
...
- **代理服务器** - 检测到的代理服务器的地理位置 IP 数据库中 (（如果) 
...
1. 在要添加的每个位置的新行中填写以下字段，打开 CSV 并添加位置。 将所有其他字段留空;在其他字段中输入的值将被忽略。
   ...
   1. **姓名** (必) ：办公室位置的名称
   1. **地址** (要求) ：办公室的物理地址
   1. **Latitude** (可选) ：从必应地图中填充的地址（如果为空）
