...
5. 在"注册应用程序"页上的"支持的帐户类型"下，选择"任何组织目录中的帐户 ("Azure AD - 多租户) 。 然后，在重定向 URI (可选) ，选择 Web 并输入 <https://office.com> 。 最后，选择"注册"。
...
19. 现在，你已成功创建迁移应用程序和密码，你将需要同意该应用程序。 若要同意应用程序，请返回到 Azure Active Directory 登陆页面，单击左侧导航栏中的 Enterprise 应用程序，找到你创建的迁移应用，选择它，然后选择左侧导航上的权限。
...
23. 制定要发送到源租户管理员帐户 (合作伙伴的 URL) 以便他们还可以接受应用程序以启用邮箱迁移。 下面是要提供的 URL 示例，将需要你创建的应用的应用程序 ID：
...
2. 为跨租户邮箱移动创建新的迁移终结点
   ...
   > [!NOTE]
   > 将需要刚创建的邮箱迁移应用程序的应用程序 ID，以及 (过程中) 的密码。 此外，根据Microsoft 365的云实例可能有所不同。 请参阅"Microsoft 365[终结点](/microsoft-365/enterprise/microsoft-365-endpoints)"页面，为租户选择正确的实例，并查看"Exchange Online"所需地址并根据需要替换。
...
### <a name="prepare-the-source-current-mailbox-location-tenant-by-accepting-the-migration-application-and-configuring-the-organization-relationship"></a>通过接受 (并配置) 关系，为租户当前邮箱位置准备源
...
迁移的用户必须存在于目标租户和Exchange Online系统中 (MailUsers) 使用特定属性进行标记，以启用跨租户移动。 对于未在目标租户中正确设置的用户，系统移动将失败。 以下部分详细介绍了目标租户的 MailUser 对象要求。
...
1. 对于从源组织移动的任何邮箱，必须在目标组织中设置 MailUser 对象：
   ...
   - 目标 MailUser 必须具有源邮箱中的这些属性或分配有新的 User 对象：
      ...
      - 不能将源邮箱中的旧版 smtp 代理地址添加到目标 MailUser。 例如，不能维护 contoso.com 租户对象中 MEU fabrikam.onmicrosoft.com MEU) 。 域仅与一个Azure AD或Exchange Online关联。
...
2. 如果源邮箱位于 LitigationHold 中，且源邮箱"可恢复的项目"大小大于数据库默认 (30 GB) ，将不会继续移动，因为目标配额小于源邮箱大小。 您可以更新目标 MailUser 对象以将 ELC 邮箱标志从源环境转换到目标，这将触发目标系统将 MailUser 的配额扩展到 100 GB，从而允许移动到目标。 这些说明仅适用于运行 Azure AD 连接 的混合标识，因为标记 ELC 标志的命令不会向租户管理员公开。
    ...
    > 示例 - 就像现在一样，无担保
    >
    > 此脚本假定连接到源邮箱 (，以获取源值) 和目标本地 Active Directory (标记 ADUser) 。 如果源已启用诉讼或单个项目恢复，则对目标帐户设置此项。  这会将目标帐户的垃圾站大小增加至 100 GB。
...
选择跨租户选项时，还支持从<a href="https://go.microsoft.com/fwlink/p/?linkid=2059104" target="_blank"></a>新Exchange管理中心提交迁移批处理。
...
> [!NOTE]
> 在批处理用户以完成时进行相应规划。 创建客户端配置文件和将后续 OST 和 OAB 文件下载到Outlook时，您需要考虑网络利用率和容量。

**我需要Exchange哪些 RBAC 角色来设置或完成跨租户移动？**

根据执行邮箱移动时委派的职责假设，存在一个角色矩阵。 目前，需要两个角色：

- 第一个角色用于建立将内容移入或移出租户/组织边界的授权的一次设置任务。 由于将数据从组织控制中迁出是所有公司的重要问题，因此，我们选择了组织管理员分配的最高角色 (OrgAdmin) 。 此角色必须更改或设置一个新的 OrganizationRelationship，该关系定义远程组织的 -MailboxMoveCapability。 只有 OrgAdmin 可以更改 MailboxMoveCapability 设置，而 OrganizationRelationship 上的其他属性也可由联合共享管理员管理。

- 执行实际移动命令的角色可以委派给较低级别的函数。 将"移动邮箱"角色分配给将邮箱移进组织或将邮箱移出组织的功能。

**我们如何针对在转换为 MailUs) er 转换邮箱的转换邮箱 (targetDeliveryDomain) 选择哪个 SMTP 地址 (目标？**
...
- 将主体和代理移动到目标系统时，邮箱中存储的邮箱权限将随邮箱一起移动。 例如，向用户TestUser_7租户中的邮箱TestUser_8 FullAccess SourceCompany.onmicrosoft.com。 邮箱移动完成后，TargetCompany.onmicrosoft.com 目标目录中设置相同的权限。 对源 *租户和目标TestUser_7 Get-MailboxPermission* 的示例如下所示。 Exchange cmdlet 的前缀为 source，并相应地确定目标。
...
- **问题：Teams租户中的迁移后功能将受到限制。** 将邮箱迁移到目标租户Teams，源租户中的邮箱将无法再访问用户的邮箱。 因此，如果用户使用源租户凭据登录到 Teams，将会丢失功能，例如无法更新个人资料图片、没有日历应用程序，以及无法搜索和加入公共团队。
...
- **问题：具有"外部"主 SMTP 地址的 MailUser 对象被修改/重置为"内部"公司声明的域**
  ...
  MailUser 对象是指向非本地邮箱的指针。 对于跨租户邮箱迁移，从目标组织的角度看，我们使用 MailUser 对象表示源邮箱 () 或目标邮箱 (（从源组织的角度来看) ）。 MailUsers 将具有一个 ExternalEmailAddress (targetAddress) ，该地址指向实际邮箱 (ProxyTest@fabrikam.onmicrosoft.com) 的 smtp 地址和表示目录中邮箱用户的显示 SMTP 地址的 primarySMTP 地址。 一些组织选择将主 SMTP 地址显示为外部 SMTP 地址，而不是本地租户拥有/验证的地址 (如 fabrikam.com 而不是 contoso.com) 。  但是，一旦Exchange许可操作将服务计划对象应用于 MailUser，主 SMTP 地址将修改为本地组织验证的域 (contoso.com) 。 有两个可能的原因：
  ...
  - 删除租户中 MailUser 上的服务计划的示例 contoso.onmicrosoft.com 如下所示。
