...
1. 由于 **许多先决条件** 对于 Skype for Business 和 Exchange都很常见，请参阅有关预重新q 清单 [的概述文章](hybrid-modern-auth-overview.md)。 在开始  *执行本文*  中的任一步骤之前，应先执行这些步骤。
...
请参阅可[支持性主题，了解Skype for Business拓扑的 MA](/skypeforbusiness/plan-your-deployment/modern-authentication/topologies-supported)支持性。
...
在仔细检查是否满足使用新式验证的先决条件 (请参阅上述[](hybrid-modern-auth-overview.md)) 注释后，应创建一个文件来保存在前一步骤中配置 HMA 时需要的信息。 本文中使用的示例：

- **SIP/SMTP 域**

  - 例如， contoso.com (与用户Office 365) 

- **租户 ID**

  - 表示你的租户租户Office 365 GUID (登录时 contoso.onmicrosoft.com) 。
...
## <a name="turn-on-hybrid-modern-authentication-for-skype-for-business-on-premises"></a>为本地部署Skype for Business混合新式验证
...
现在，你需要运行命令，将之前收集的 URL (SFBO) 服务主体。
...
3. 例如， **如果缺少内部** 部署中的内部或外部 SFB URL (，我们将需要向此列表添加这些 https://lyncwebint01.contoso.com https://lyncwebext01.contoso.com) 特定记录。
...
还应检查"OAuth 颁发机构"Skype for Business客户端的"配置信息"。 若要在客户端计算机上执行此操作，请按住 Ctrl 键，同时右键单击"通知Skype for Business栏中的Windows图标。 在 **出现的菜单中** 单击"配置信息"。 在桌面Skype for Business"配置信息"窗口中，查找以下内容：
...
是否需要了解如何将新式验证用于 (ADAL) 客户端Skype for Business？ 我们在此处有 [步骤](./hybrid-modern-auth-overview.md)。