...
- **将内部部署Exchange组织添加为组织接受Microsoft 365域。** 迁移服务使用内部部署邮箱的 SMTP 地址为新的Microsoft Online Services邮箱创建用户 ID 和Microsoft 365地址。 如果你的域不是接受Exchange组织的主要域，则迁移Microsoft 365失败。 有关详细信息，请参阅验证 [域](../admin/setup/add-domain.md)。
...
- **为内部部署用户帐户分配访问 Exchange 组织中的邮箱所需的权限。** 用于连接到内部部署 Exchange 组织 (（也称为迁移管理员) ）的本地用户帐户必须具有访问要迁移到 Microsoft 365 内部部署邮箱的必要权限。 该用户帐户用于创建内部部署组织的迁移终结点。
