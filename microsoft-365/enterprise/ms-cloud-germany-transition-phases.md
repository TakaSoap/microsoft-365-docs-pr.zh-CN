...
阶段及其操作可确保将关键数据和体验迁移到 Office 365 服务。 将租户添加到迁移队列后，每个工作负载都将作为在后端服务上执行的一组步骤完成。 某些工作负载可能需要管理员或 (用户) 操作，或者迁移可能会影响执行并讨论了如何组织迁移 [的阶段的使用情况？](ms-cloud-germany-transition.md#how-is-the-migration-organized)
