...
CDN 有助于使最终用户Office 365且可靠。 云服务（Office 365 CDN）使用与请求它们加快下载速度并减少感知最终用户延迟的浏览器较近的静态资产进行缓存。 本主题中的信息将帮助你了解内容交付网络 (CDN) 以及这些网络Office 365。
...
您可以使用 **Azure CDN** 来部署自己的 CDN 实例来承载自定义 Web 部件、库和其他资源资产，从而允许您将访问键应用到 CDN 存储，并更好地控制 CDN 配置。 使用 Azure CDN不是免费的，并且需要 Azure 订阅。
...
2. Office 365将数据直接返回到客户端，或者，如果数据是 CDN 托管的一组内容的一部分，则将您的客户端重定向到 CDN URL。
    ...
    b. 如果数据已缓存在专用源中，CDN服务将Office 365用户帐户对源的权限。  如果您具有权限，SharePoint Online 将动态生成一个自定义 URL，该 URL 由 CDN 中的资产路径和两个访问令牌组成，并返回您的客户端的自定义 URL。 然后，客户端使用自定义 URL 将数据直接从最近的CDN位置下载到客户端。
...
最大程度地减少网络上客户端与CDN之间的延迟是确保最佳性能的关键考虑因素。 您可以使用管理[Office 365](managing-office-365-endpoints.md)终结点中概述的最佳实践，以确保网络配置允许客户端浏览器直接访问 CDN，而不是通过中央代理路由 CDN 流量，以避免引入不必要的延迟。
...
我们非常小心保护运行你的业务的数据。 存储在 Office 365 CDN 的数据在传输中和处于其余位置都进行加密，并且对 Office 365 SharePoint CDN 中数据的访问受 Office 365 用户权限和令牌授权保护。 必须提交对 Office 365 SharePoint CDN 数据的请求 (从) 租户Office 365重定向，否则不会生成授权令牌。
...
对于希望将发往 Microsoft 或 Office 365 数据中心的请求与发往第三方的请求隔离的客户，我们已编写有关管理[Office 365](https://support.office.com/article/99cab9d4-ef59-4207-9f2b-3728eb46bf9a)终结点的指导。
