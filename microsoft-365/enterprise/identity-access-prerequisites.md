...
| :------------- | :-----------: | :--------------: | :------------: | :------------: |
|  [配置密码哈希同步](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnectsync-implement-password-synchronization)。必须启用此功能，以检测泄露的凭据并对其执行基于风险的条件访问。 **注意：** 无论您的组织是使用托管身份验证（如传递身份验证 (PTA) 还是联合身份验证），都是必需的。 |    | 是 | 是 | 是 |
...
此外，我们还建议对公司拥有的设备 (货到付款) 在支持使用 Android for Work 或 Samsung Knox 的 Oem 和版本上标准化，以允许通过 Intune MDM 策略管理和保护邮件帐户。
...
Azure AD 为你提供一种简单的方法，以使用预配置的条件访问策略开始保护管理员访问。 在 Azure AD 中，转到 " **条件访问** 并查找此策略- **基准策略：要求对管理员的 MFA (preview) **。 选择 "此策略"，然后选择 " **立即使用策略**"。 
