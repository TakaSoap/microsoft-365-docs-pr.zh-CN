...
在将敏感的数字资产迁移到云之前，Contoso 确保其本地信息分类和保护要求受 Microsoft 365 企业版基于云的服务支持。
...
## <a name="the-contoso-path-to-information-protection-with-microsoft-365-for-enterprise"></a>Microsoft 365 企业版信息保护的 Contoso 路径

Contoso 按照以下步骤为 Microsoft 365 企业版准备其信息保护要求：

1. 确定要保护的信息

   Contoso 对位于本地 SharePoint 网站和文件共享中的现有数字资产进行了广泛审查，并分类了每项资产。
...
4.  将数据从本地 SharePoint 网站和文件共享移动到其新 SharePoint 网站

    迁移到新的 SharePoint 网站的文件继承了分配给该网站的默认保留标签。

5.  培训员工如何对新文档使用敏感度标签、如何在创建新 SharePoint 网站时与 Contoso IT 进行交互，以及如何始终在 SharePoint 网站上存储数字资产
...
- 电脑和移动设备必须兼容。
- Exchange Online 使用适用于 Exchange Online 的 OME (OME) Office 365 邮件加密。
- SharePoint 使用应用实施的限制。
...
| 3 级：高度管控 | 2 级再加上： <BR><BR> 将你自己的密钥 (BYOK) 加密和保护商业机密信息 <BR> 用于与 Microsoft 365 服务交互的业务线应用程序的 Azure Key Vault | 2 级 | 1 级 |
|||||
...
了解 Contoso 如何使用 [Microsoft 365](contoso-security-summary.md) 企业版中的安全功能进行标识和访问管理、威胁防护、信息保护和安全管理。
