...
- 如果你使用的是 Exchange Online 或 Exchange 混合，请 [执行此步骤](#exchange-online)。
...
- 如果你还使用除 Office 365 订阅中包含的服务之外的 Azure 服务，请 [执行此步骤](#microsoft-azure)。
...
**适用于：** 使用本地 AD FS 对连接到 Microsoft Office 365 的用户进行身份验证的客户

| 步骤 (步骤)  | 说明 | 影响 |
|:-------|:-------|:-------|
| [针对灾难恢复方案的 Active Directory 联合身份验证服务 (AD FS) ](ms-cloud-germany-transition-add-adfs.md#backup) 服务器场的备份。 | 客户需要适当备份 AD FS 场，以确保可以在不接触域的颁发者 URI 的情况下还原对全局 & Germany 终结点的信赖方信任。 如有必要，Microsoft 建议使用 AD FS 快速还原来备份服务器场和相应的还原。 | 必需操作。 如果客户的 AD FS 服务器场失败，则不操作将在迁移期间产生服务影响。 有关详细信息，请参阅 [ADFS 迁移步骤](https://docs.microsoft.com/microsoft-365/enterprise/ms-cloud-germany-transition-add-adfs) |
||||
...
| 查看并准备迁移相关的 DNS 更改。 | Skype for Business Online 的客户拥有的 DNS 区域更改。 |<ul><li>建议将任何客户拥有的域 DNS 记录的"生存时间 (TTL) 更新为 5 分钟，以加快 DNS 记录的刷新。 但是，与此 DNS 更改关联的 Microsoft 托管的转换可能会随时在提供的 24 小时更改时段内发生。 </li><li>将来可能会中断服务。 用户将无法登录 Skype for Business，并且将被重定向到 Office 365 服务中迁移的 Teams 体验。 </li></ul>|
| 准备最终用户和管理培训和准备过渡到 Microsoft Teams。 | 通过规划用户通信和准备情况，成功从 Skype 过渡到 Teams。 | <ul><li>客户端需要了解新服务，以及一旦其服务转换到 Office 365 服务后如何使用。 </li><li>为客户虚域和初始域都进行了 DNS 更改后，用户将登录到 Skype for Business，并查看他们现在已迁移到 Teams。 这还会在后台下载 Teams 的桌面客户端。 </li></ul>|
||||

## <a name="mobile"></a>移动版

如果你使用 MDM 解决方案的第三方移动设备 () ：
...
| 确定迁移后是否要求任何重新配置。 | 移动设备管理 (MDM) 解决方案可能面向 `outlook.de` 终结点。 在此过渡到 Office 365 服务时，客户端配置文件应更新为 Office 365 服务 `outlook.office365.com` URL。 | Exchange Online 和 MDM 客户 | 客户端可能在终结点可访问时继续运行，但如果 Microsoft 云德国终结点不再可用，客户端将 `outlook.de` 失败。 |
|||||

## <a name="line-of-business-apps"></a>业务线应用

如果你使用的是第三方服务或业务线 (LOB) Office 365 集成的应用： 

| 步骤 (步骤)  | 说明 | 影响 |
|:-------|:-------|:-------|
| 确定迁移后是否要求任何重新配置。 | 可以编码与 Office 365 集成的第三方服务和应用程序，以期望 Microsoft 云德国 IP 地址和 URL。 | 必需操作。 不操作可能会导致服务或客户端软件失败。 |
||||
...
| 步骤 (步骤)  | 说明 | 影响 |
|:-------|:-------|:-------|
| 对于 Dynamics 365 沙盒订阅，请务必从 Microsoft 云德国的 Dynamics 365 订阅下载 Dynamics SQL 实例的生产环境。 在沙盒迁移之前，应该将最新的生产备份还原到沙盒。 | Dynamics 365 的迁移要求客户确保使用最新的生产数据库刷新沙盒环境。 | FastTrack 团队将帮助客户执行干运行，以验证版本从 8.x 升级到 9.1.x。 |
||||
...
| 步骤 (步骤)  | 说明 | 影响 |
|:-------|:-------|:-------|
| 从 Power BI 订阅删除不会从 Power BI Microsoft 云德国迁移到 Office 365 服务的对象。 | 迁移 Power BI 服务需要客户操作来删除某些项目，如数据集和仪表板。 | <ul><li>管理员可能必须删除其订阅中的以下项目： </li><li>Real-Time数据集 (例如流式处理或推送数据集)  </li><li>Power BI 本地数据网关配置和数据源 </li></ul>|
||||

## <a name="dns"></a>DNS

**适用于：** 使用 Office 桌面客户端 (Microsoft 365 应用版、Office 365 专业增强版、Office 2019、2016、...) <br>
如果 MSOID、CName 在 Azure Active Directory (Azure AD) 之前存在，请将其从客户拥有的 DNS 中删除。 可以设置 5 分钟的 TTL，以便更改可以快速生效。
