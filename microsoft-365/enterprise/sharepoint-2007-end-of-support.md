...
> [!TIP]
> 如果尚未规划升级或迁移，请参阅[：SharePoint 2007](sharepoint-2007-migration-options.md)迁移选项，以查看一些开始位置的示例。 还可以搜索可帮助升级[或](https://go.microsoft.com/fwlink/?linkid=841249)迁移迁移或迁移Microsoft 365或 (两) 。
  
有关 Office 2007 服务器和停止提供支持的信息，请参阅帮助您从[Office 2007](upgrade-from-office-2007-servers-and-products.md)服务器和客户端升级的资源。
...
> [!IMPORTANT]
> 如果很少使用 SharePoint 2007 中的内容，则还有另一个选项。 某些 SharePoint 管理员选择创建 Microsoft 365 订阅，设置一个新的 SharePoint Online 网站，然后完全离开 SharePoint 2007，仅将基本文档提交到全新的 SharePoint Online 网站。 然后，可以将数据从 SharePoint 2007 网站排入存档。 考虑用户如何使用 2007 SharePoint数据。 可能有一些用于管理需求的创意方法。
...
不能直接从 2007 SharePoint迁移到 SharePoint Online。 移动到 SharePoint Online 将手动完成。 如果升级到 SharePoint Server 2013 或 SharePoint Server 2016，可以使用 SharePoint 迁移 API (将信息迁移到 OneDrive for Business 中，例如) 。
  
|**在线专业人员**|**Online con**|
|:-----|:-----|
|Microsoft 提供 SPO 硬件以及所有硬件管理。  <br/> |可用功能在 SharePoint Server 本地和 SPO 之间可能有所不同。  <br/> |
...
|合作伙伴可以协助完成将数据迁移到 SharePoint Online 的一SharePoint作业。  <br/> ||
...
> [!NOTE]
> 可以维护生命周期结束的 SharePoint 2007 场，在新的硬件 (上安装 SharePoint Server 2016 场，以便单独的服务器场并行运行) ，然后计划并执行内容 (的手动迁移，以便下载和重新上载内容，例如) 。 但请注意手动移动的一些错误，例如将最后修改的帐户替换为执行手动移动的帐户别名的文档移动。 还要考虑必须提前完成的工作，例如重新创建网站、子网站、权限和列表结构。 提前考虑可以移入存储或删除哪些数据，以减少迁移的影响。
...
|**本地专业人员**|**本地 con**|
|:-----|:-----|
|从服务器硬件SharePoint对服务器场的所有方面进行完全控制。  <br/> |所有中断和修复都由你的公司负责 (如果你的产品未停止停止提供支持，你可以联系付费的 Microsoft) 。  <br/> |
|本地 SharePoint Server 的完整功能集，通过混合将本地服务器场连接到 SharePoint Online 订阅的选项。  <br/> |升级、修补程序、安全修补程序以及本地SharePoint服务器的所有维护。  <br/> |
...
如果你的迁移需求答案介于本地提供的自控制与 SharePoint Online 提供的所有权的较低成本之间，则可以通过混合将 SharePoint Server 2013 或 2016 场连接到 SharePoint Online。 [了解混合SharePoint解决方案](https://support.office.com/article/4c89a95a-a58c-4fc1-974a-389d4f195383.aspx)。
...
> [!NOTE]
> 可以维护生命周期结束的 SharePoint 2007 场，在新的硬件 (上安装 SharePoint Server 2016 场，以便单独的服务器场并行运行) ，然后计划并执行内容 (的手动迁移，以便下载和重新上载内容，例如) 。 但请注意手动移动的潜在错误，例如将上次修改的帐户替换为执行手动移动的帐户别名的文档移动，以及必须提前完成的工作，如重新创建网站、子网站、权限和列表结构。 请考虑可以移入存储或删除哪些数据，以减少迁移的影响。
...
|**本地Pro**|**本地 Con**|
|:-----|:-----|
|从服务器硬件SharePoint对服务器场的所有方面进行完全控制。  <br/> |所有中断和修复都由你的公司负责。  (如果你的产品未停止提供支持，你可以参与付费 Microsoft 支持。)   <br/> |
|本地 SharePoint Server 的完整功能集，通过混合将本地服务器场连接到 SharePoint Online 订阅的选项。  <br/> |升级、修补程序、安全修补程序以及本地SharePoint服务器的所有维护。  <br/> |
|可进行更大自定义的完全访问权限。  <br/> |[必须在本地](/compliance/regulatory/offering-home) 手动配置 Microsoft 合规性产品/服务。  <br/> |
...
如果你的迁移需求答案介于本地提供的自控制与 SharePoint Online 提供的所有权的较低成本之间，则可以通过混合将 SharePoint Server 2013 或 2016 场连接到 SharePoint Online。 [了解SharePoint解决方案](https://support.office.com/article/4c89a95a-a58c-4fc1-974a-389d4f195383.aspx)
