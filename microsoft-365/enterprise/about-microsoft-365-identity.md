...
Azure AD 连接提供持续的帐户同步。 它在本地服务器上运行，检查 AD DS 中的更改，将这些更改转发到 Azure AD。 Azure AD 连接 提供了筛选哪些帐户已同步以及是否同步哈希版本的用户密码（称为密码哈希同步 (PHS) ）。
...
Azure AD 租户具有 AD DS 帐户的副本。 在此配置中，本地用户和访问云服务的远程用户Microsoft 365 Azure AD 进行身份验证。
