...
> [!TIP]
> 还有一些 Microsoft 托管的 CDN 可以与 Office 365 一起用于专用使用方案，但是本主题中未讨论这些 CDN，因为它们不在 Office 365 CDN。 有关详细信息，请参阅其他[Microsoft CDN。](content-delivery-networks.md#other-microsoft-cdns)
...
+ 使用 PowerShell 或 CDN ONLINE CLI 设置和SharePoint客户端

  + [使用 SharePoint Online 命令行管理程序设置和CDN配置命令行管理程序](use-microsoft-365-cdn-with-spo.md#CDNSetupinPShell)
...
完成设置后，可以按[以下方式Office 365 CDN](use-microsoft-365-cdn-with-spo.md#CDNManage)管理安装程序：
...
例如，在将 SharePoint Online 网站添加到 CDN 源时，重复请求的小文件（如网站图像和脚本）可以显著提高网站呈现性能并逐步减少 CDN 负载。 可以从 CDN 下载安装可执行文件等较大的文件，从而对性能产生积极的影响，并随后减少 SharePoint Online 网站的负载，即使它们未经常访问。

基于每个文件的性能改进取决于许多因素，包括客户端与最近的 CDN 终结点的邻近度、本地网络上暂时的情况等等。 许多静态文件非常小，可以在不到一Office 365从文件下载。 但是，网页可能包含许多嵌入式文件，其累积下载时间为几秒。 从网站提供这些文件CDN可以显著缩短整个页面加载时间。 有关[示例，请参阅CDN性能](content-delivery-networks.md#what-performance-gains-does-a-cdn-provide)提升？
...
你可以定义 _CDN_ 策略以从源中排除CDN。 CDN策略按文件类型和网站分类等属性排除公用源或专用源中的资产，并应用于在策略中指定的 CdnType (private 或 public) 的所有源。 例如，如果添加包含多个子网站的网站的专用源，您可以定义一个策略来排除标记为"机密"的网站，以便不会从 CDN提供应用了该分类的网站中的内容。 该策略将应用于已 _添加到_ 策略的所有私有源CDN。
...
标识源时，指定应公开 _还是私有。_  对CDN源中资产的访问权限是匿名的，并且CDN源中的内容受动态生成的令牌保护，从而获得更高的安全性。 无论你选择哪个选项，Microsoft 都会在管理用户本身方面CDN工作。 此外，在设置帐户并识别CDN之后，你可以改变主意。
...
**用户** 网站中的专用源Office 365 CDN用户内容（如 SharePoint Online 文档库、网站和专有图像）的专用访问权限。 对专用源中内容的访问受动态生成的令牌保护，因此只有对原始文档库或存储位置具有权限的用户才能访问它。 Office 365 CDN中的专用源只能用于 SharePoint Online 内容，并且你仅可以通过从 SharePoint Online 租户重定向访问专用源中的资产。
...
## <a name="set-up-and-configure-the-office-365-cdn-by-using-the-sharepoint-online-management-shell"></a>使用 Office 365 CDN Online 命令行管理程序设置SharePoint配置命令行管理程序
...
在更改租户设置CDN，应先检索租户中专用CDN配置Office 365状态。 连接命令行管理程序SharePoint租户：
...
_ExcludeRestrictedSiteClassifications_ 属性包含网站分类，您希望从网站CDN。 例如，您可以排除标记为"机密"的网站，以便网站中应用了该分类的内容不会从网站CDN。
...
无法修改已创建的源。 请删除原点，然后添加新源。 有关详细信息，请参阅从资源[中删除Office 365 CDN](use-microsoft-365-cdn-with-spo.md#Office365CDNforSPORemoveOriginPosh)[和为资产添加源](use-microsoft-365-cdn-with-spo.md#Office365CDNforSPOOriginPosh)。
...
在更改租户设置CDN，应先检索租户中专用CDN配置Office 365状态。 连接 PnP PowerShell 访问租户：
...
请参阅[默认](use-microsoft-365-cdn-with-spo.md#default-cdn-origins)CDN源，了解在启用 Office 365 CDN 时默认预配的源，以及跳过默认源设置的潜在影响。
...
### <a name="change-the-list-of-site-classifications-you-want-to-exclude-from-the-office-365-cdn-optional"></a>更改要从"可选"选项中排除Office 365 CDN () 
...
#### <a name="modify-an-origin-in-the-office-365-cdn"></a>修改源Office 365 CDN

无法修改已创建的源。 请删除原点，然后添加新源。 有关详细信息，请参阅从资源[中删除Office 365 CDN](use-microsoft-365-cdn-with-spo.md#Office365CDNforSPORemoveOriginPnPPosh)[和添加资产的来源](use-microsoft-365-cdn-with-spo.md#Office365CDNforSPOOriginPnPPosh)。
...
本节中的过程要求你已安装 Office 365 [CLI](https://aka.ms/o365cli)。 接下来，使用登录Office 365连接到[你的租户](https://pnp.github.io/office365-cli/cmd/login/)。
...
请参阅[CDN](use-microsoft-365-cdn-with-spo.md#default-cdn-origins)源，了解在启用默认设置时预配Office 365 CDN。
...
例如，考虑已复制到文档库文件夹 _/site/CDN_origins/public/ 的图像 /site/SiteAssets/images/image.png_ 。 若要使用CDN资源，请将图像文件位置的原始路径替换为原点的路径，使新的 URL _/site/CDN_origins/public/image.png_。
...
> [!NOTE]
> 一般情况下，不应将 URL 直接硬编码到资源CDN。 但是，如果需要，你可以手动为公用源中的资产构建 URL。 有关详细信息，请参阅[Hardcoding CDN URL for public assets](use-microsoft-365-cdn-with-spo.md#constructing-cdn-urls-for-public-assets)。
...
如果您的源位于启用了"发布"功能的网站中，并且要卸载到 CDN 的资产位于以下类别之一，SharePoint 将自动重写源中资产的 URL，只要 CDN 策略未排除该资产。
...
> [!NOTE]
> 无法硬编码或构造CDN源中资产的 URL，因为生成形成 URL 的最后一节所需的访问令牌是在请求资源时生成的。 您可以构建公用网站的 URL CDN URL 不应硬编码，因为它可能会更改。
...
下图说明了当用户收到SharePoint来自专用源的资产的页面的请求时，工作流。
...
生成访问令牌后，SharePoint Online 会向客户端返回一个自定义 URI，该客户端包含两个授权参数， (边缘授权令牌 _)_ 和 _oat_ (源授权令牌) 。 每个令牌的结构都<纪元时间格式的过期时间>__<_安全签名>。_ 例如：
...
请注意，SharePoint Online 不支持私有源中的资产的项目级权限。 例如，对于位于 的文件，在 满足以下条件的情况下，用户 `https://contoso.sharepoint.com/sites/site1/library1/folder1/image1.jpg` 具有对文件的有效访问权限：
...
|用户 4     |具有对 folder1 的访问权限，但已明确拒绝image1.jpg SharePoint Online         |无法从 SharePoint Online 访问资产，但可以从 CDN 即使被拒绝访问 SharePoint Online 中的文件         |
...
向页面添加指向 CDN 资产的链接后，您可以浏览到该页面，在图像呈现并查看图像 URL 后右键单击该图像，以确认该资产是否从 CDN 提供。
