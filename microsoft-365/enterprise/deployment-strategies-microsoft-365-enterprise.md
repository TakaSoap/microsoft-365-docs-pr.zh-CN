...
- 10 月，执行设备注册并实施应用和条件访问策略。（移动设备管理）
...
- 在当前阶段的用户推出期间，你需要开始下一阶段的用户推出，确保用户不会同时处理多个阶段的推出。这里假设你通过区域、部门或其他单位以相同的方式推出底层基础结构的每个阶段。
...
- 4 月，执行设备注册并实施应用和条件访问策略。（移动设备管理）
- 5 月，安装 Azure 信息保护客户端，并接受关于如何将标签应用于文档的培训。（信息保护）

结果是在连续的用户推出之间保持 30 天节奏。

此部署策略的优势在于，可以花费更少的时间来完全部署 Microsoft 365 企业版底层基础结构，仍然无需各个用户同时处理多个推出。但是，用户不会在连续阶段之间休息。
