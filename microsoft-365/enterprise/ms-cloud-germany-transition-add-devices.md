...
对于已加入 Azure AD 的混合设备以及向 Azure AD 注册的公司拥有 Windows 设备，管理员将能够通过远程触发的工作流管理这些设备的迁移，这些工作流将取消注册旧设备状态。
  
对于所有其他设备，包括在 Azure AD 中注册的个人 Windows 设备，最终用户必须手动执行这些步骤。 对于加入 Azure AD 的设备，用户需要具有本地管理员帐户才能注销然后重新注册其设备。

Microsoft 将发布有关如何成功还原设备状态的说明。 
 
**我如何知道我的所有设备都注册到公共云中？**

若要检查你的设备是否在公共云中注册，你应该将设备列表从 Azure AD 门户导出并下载到 Excel 电子表格。 然后，使用 _registeredTime_ 列 (从 [德国 Microsoft](ms-cloud-germany-transition.md#how-is-the-migration-organized) 云迁移) 注册的设备。
...
_Windows 下_ 层设备是当前运行早期版本的 Windows (如 Windows 8.1 或 Windows 7) 或运行早于 2019 和 2016 的 Windows Server 版本的 Windows 设备。 如果之前已注册此类设备，则需要注销并重新注册这些设备。 
...
每个在 Windows 低级别设备上登录的域用户只需运行一次上述命令。 此命令应在域用户登录的上下文中运行。 
...
**重要提示：** 商业迁移后将启用 Intune 服务主体，这意味着激活 Azure AD 设备注册。 如果在迁移之前阻止了 Azure AD 设备注册，则必须使用 PowerShell 禁用 Intune 服务主体，以再次禁用 Azure AD 门户的 Azure AD 设备注册。 可以使用此命令在 Azure Active Directory PowerShell graph 模块中禁用 Intune 服务主体。
...
用户可以从 Windows 设置将设备加入 Azure AD：设置> **帐户>访问工作或>连接**。
...
- 提取 ZIP 文件并运行 **WPJCleanup.cmd**。 此工具将基于设备上 Windows 版本启动正确的可执行文件。
...
对于 Android，用户需要注销并重新注册其设备。 这可以通过 Microsoft Authenticator 应用或公司门户应用完成。 

- 从 Microsoft Authenticator 应用中，用户可以转到"设备注册>**设置"。** 在这里，用户可以注销和重新注册其设备。
 
- 从公司门户，用户可以转到" **设备** "选项卡并删除设备。 此后，使用公司门户重新注册设备。
 
- 用户还可以从帐户设置页中删除帐户，然后重新添加工作帐户，从而注销和重新注册。

若要使用 Microsoft Authenticator 应用在 Android 上注销和重新注册设备，请运行以下操作：

1.  打开 Microsoft Authenticator 应用， **然后转到设置**。
2.  选择 **"设备注册"。**
3.  通过选择"注销"取消 **注册设备**。
4.  对于 **设备注册**，请通过键入你的电子邮件地址重新注册设备，**然后选择注册。**

若要使用"Android 设置"页注销和重新注册 Android 设备：
...
在 iOS 设备上，用户需要从 Microsoft Authenticator 手动删除所有缓存的帐户、注销设备以及从设备上的任何本机应用注销。
