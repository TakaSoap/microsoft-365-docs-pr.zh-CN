...
转到第 3 阶段之前，请确保你的身份基础结构满足这些条件。另请参阅[先决条件](https://docs.microsoft.com/microsoft-365-enterprise/identity-access-policies#prerequisites)，以了解有关身份基础结构的其他建议。
...
- 组织内的员工和供应商、承包商以及与组织协同工作的合作伙伴在 Azure Active Directory (AD) 中具有相应的用户帐户。
...
如果你有现有的本地标识提供者，如 Windows Server Active Directory (AD)，则已使用 Azure AD Connect 将用户帐户和组从本地标识提供者同步到 Azure AD 租户。

通过目录同步，你的用户可以使用与登录到其计算机相同的凭据登录到 Office 365 和其他 Microsoft 云服务，并访问本地资源。

如果需要，请执行[步骤 7](identity-azure-ad-connect.md)，这样做有助于满足此要求。
...
要验证本地凭据身份验证是否正常运行，请使用本地凭据登录到 Office 365 门户。

要验证目录同步正常运行，请执行以下操作：

1.  在 Windows Server AD 中创建新的测试组。
2.  等待同步时间。
...
已[保护 Office 365 全局管理员帐户](https://support.office.com/article/Protect-your-Office-365-global-administrator-accounts-6b4ded77-ac8d-42ed-8606-c014fd947560)，以避免凭据泄露，进而可能导致 Office 365 订阅漏洞。

如果忽略此要求，全局管理员帐户很容易受到攻击和入侵，攻击者可以获取系统范围内的数据访问权限并加以收集、销毁或勒索。

如果需要，请执行[步骤 2](identity-designate-protect-admin-accounts.md)，这样做有助于满足此要求。
...
> [!Note]
> 有关安装 Azure AD V2 PowerShell 模块和登录到 Office 365 的说明，请参阅[连接到 Office 365 PowerShell](https://docs.microsoft.com/office365/enterprise/powershell/connect-to-office-365-powershell)。
...
如果需要，可在[步骤 11](identity-customize-office-365-sign-in.md) 中设置此选项。
...
如果需要，可在[步骤 8](identity-azure-ad-connect-health.md) 中设置此选项。
...
如果需要，可在[步骤 4](identity-password-reset.md) 中设置此选项。

### <a name="how-to-test"></a>如何测试

1. 创建包含初始密码的测试用户帐户。
2. 使用[让用户在 Office 365 中重置自己的密码](https://support.office.com/article/Let-users-reset-their-own-passwords-in-Office-365-5bc3f460-13cc-48c0-abd6-b80bae72d04a)中的步骤来重置测试用户帐户的密码。
...
如果忽略此选项，未连接到本地网络的用户必须通过 IT 管理员重置或解除锁定其 Windows Server AD 密码。

如果需要，可在[步骤 9](identity-password-writeback.md) 中设置此选项。
...
通过在 Office 365 中更改密码来测试密码写回。你应能够使用帐户和新的密码来访问本地 Windows Server AD 资源。

1. 在本地 Windows Server AD 中创建测试用户帐户，允许进行目录同步，然后在 Office 365 管理门户中授予 Office 365 许可证。
2. 从加入到本地 Windows Server AD 域的远程计算机，使用测试用户帐户的凭据登录到计算机和 Office 365 门户。
3. 选择“设置 > Office 365 设置 > 密码 > 更改密码”****。
4. 键入当前密码、键入新密码，然后确认。
5. 注销 Office 365 门户和远程计算机，然后使用测试用户帐户及其新密码登录到计算机。这可证明你是否能够使用 Azure AD 租户更改本地 Windows Server AD 用户帐户的密码。

<a name="crit-identity-sso"></a>
## <a name="optional-users-can-sign-in-using-single-sign-on"></a>可选：用户可以使用单一登录进行登录
...
如果需要，可在[步骤 10](identity-single-sign-on.md) 中设置此选项。
...
如果需要，可在[步骤 12](identity-group-based-licensing.md) 中设置此选项。

### <a name="how-to-test"></a>如何测试

1. 在具有 Azure 门户的 Azure AD 中创建测试安全组，并配置基于组的许可来分配 Office 365 和 EMS 许可证。
2. 在 Azure AD 中创建测试用户帐户并将其添加到测试安全组。
3. 在 Office 365 管理门户中检查用户帐户的属性，确保其分配了 Office 365 和 EMS 许可证。
4. 从测试安全组删除测试用户帐户。
...
如果需要，可在[步骤 15](identity-automatic-group-membership.md) 中设置此选项。
...
如果需要，可在[步骤 14](identity-self-service-group-management.md) 中设置此选项。
...
如果需要，可在[步骤 6](identity-azure-ad-identity-protection.md) 中设置此选项。
...
如果需要，可在[步骤 3](identity-azure-ad-identity-protection.md) 中设置此选项。
...
|||
|:-------|:-----|
|![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)| Microsoft 365 企业版端到端部署过程中的下一个阶段是 [Windows 10 企业版](windows10-infrastructure.md)。 |
