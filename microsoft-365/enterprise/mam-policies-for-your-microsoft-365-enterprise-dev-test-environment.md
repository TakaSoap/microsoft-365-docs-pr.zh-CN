...
在此阶段，你将为设备创建Windows 10策略。 此阶段Microsoft Intune和Microsoft Endpoint Manager管理中心添加组[](https://go.microsoft.com/fwlink/?linkid=2109431)，并创建合规性策略。
...
2. 在 **"组**"中，Microsoft 365"已分配成员身份"类型的名为"托管Windows 10 **组或**"安全"组。  在以下步骤中，将合规性策略分配给该组。 
    ...
    有关特定步骤以及有关Microsoft 365 **组** 的信息，请参阅添加 [组以组织用户和设备](/mem/intune/fundamentals/groups-add)。

3. 在 **"设备"** 中，创建Windows 10合规性策略。 将此策略分配给你 **Windows 10托管设备** 用户组。
