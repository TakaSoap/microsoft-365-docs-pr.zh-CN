...
需要为要所提取的文档中的每个实体创建一个提取程序。 在示例中，我们希望提取模型标识的每个 **合同续订****文档的服务开始日期**。 我们希望能够在所有 **合同续订** 文档的文档库中看到一个视图，其中包含显示每个文档 **的服务开始** 日期值的列。
...
如果你在标记的示例文件上收到匹配，则现在可以在其余未标记的示例文件中测试模型。 这是可选的，但一个有用的步骤，通过在模型之前未看到的文件上对其进行测试来评估模型的“适合性”或就绪性。
