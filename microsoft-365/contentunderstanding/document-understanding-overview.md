...
将 *分类器* 和 *提取器* 添加到文档理解模型，以执行以下操作：

- 分类器用于识别上载到文档库的文档并对其进行分类。 例如，一个分类器可通过“训练有素”来标识上载到库的所有 *合同续订* 文档。 当你创建分类器时，合同续订内容类型由你定义。

- 提取器从这些文档提取信息。 例如，对于文档库中标识的每个合同续订文档，将显示显示每个文档 *的服务开始日期* 和 *客户* 端的列。 
...
文档理解模型使用光学字符识别 (OCR) 技术来扫描 PDF、图像和 TIFF 文件。 使用示例文件训练模型时，以及针对文档库中的文件运行模型时，会扫描文件。

请注意Microsoft Office PDF、图像或 TIFF)  (基于文本的文件和 OCR 扫描文件的以下差异：
...
- 对于图像，维度必须介于 50 x 50 和 10，000 x 10，000 像素之间。
   > [!NOTE]
