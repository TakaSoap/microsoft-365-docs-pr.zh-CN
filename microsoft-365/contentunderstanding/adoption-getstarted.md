...
借助新的 AI 服务和功能，可以使用 SharePoint Syntex 将内容理解和分类应用程序直接构建到内容管理流中：

|手动输入| 表单处理 | 文档理解 |
|:-------|:--------|:--------|
| 任何内容的数据输入和大量工作。 | 标识文件或从结构化或半结构化文档（如表单或发票）中提取数据。 |  标识和提取非结构化文档（如信函或合同）的数据，其中要提取的文本实体位于文档的句子或特定区域。 |
| 交互式。   |  自定义，协助。  | 预建、自动化。 |
| 工作人员。 | 由行业专家与中小型企业 (专家) 。 | SMES 参与较少。 |
...
有关 AI Builder 信用和单位详细信息，请参阅 [AI Builder 许可](https://docs.microsoft.com/ai-builder/administer-licensing)。

有两种不同的内容理解方式。 使用的模型类型基于文件格式和用例：
...
| 仅限于单个库。 | 可应用于多个库。 |
| 培训 PDF、JPG、PNG 格式，总计 50 MB/500 pp。 | 培训 5-10 个 PDF、Office 或电子邮件文件，包括负数示例。 |
...
若要准备在组织中使用 SharePoint 合成，首先需要了解它很有用的方案。 原因有助于确定需要哪种模型，以及如何根据应用模型的地方构建组织。 下面是一些文档理解方案，可帮助您的组织：
...
例如，您可以使用 SharePoint Syntex 和 Power Automate 功能设置流程来跟踪和监视发票。
...
### <a name="document-understanding-scenario-example"></a>文档了解方案示例

另一个示例是，您可以设置一个流程来识别贵公司与其他公司或个人的合同。 您可以设置一个模型以从这些合同中提取关键信息，如客户端名称、费用、日期或其他重要信息，并将其添加到库中作为字段，以便您可以快速查看。 您可以对文档库应用保留标签，以确保在一段特定时间之前无法删除合同，以适当遵守业务法规。
...
知识管理器、业务流程所有者和内容模型所有者在组织中创建示例模型和推广采用。
