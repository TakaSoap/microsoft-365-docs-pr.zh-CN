...
借助新的 AI 服务和功能，您可以使用 SharePoint Syntex 将内容理解和分类应用程序直接构建到内容管理流中：

|手动输入| 表单处理 | 文档理解 |
|:-------|:--------|:--------|
| 任何内容的数据输入和大量工作 | 处理数字内容 - 照片、扫描、收据、名片、包含 OCR &文本的视频 |  从合同、简历和其他结构化文档中捕获内容类型和元数据 |
| Interactive   | 预建、自动化   | 自定义、协助  |
| 工作人员 | 由行业专家与中小型企业 (专家) 。 捕获合同、简历和其他结构化文档中的内容类型和元数据。 | SMES 参与较少。 来自采购订单、应用程序、其他半结构化和结构化文档 |

下表介绍了使用 SharePoint 合成时得到什么：

| 表单处理 | 文档理解 |
|:-------|:-------|
| 在亚太地区、澳大利亚、加拿大、欧盟、JP、LATAM、英国、美国提供 | 可在所有区域使用 |
| 使用 AI Builder 信用 - 1M 信用 = 2000 页;消耗大约 2000 张发票=2 个单位。 需要 Power Automate - 如果需要更多功能，可以添加它。 为购买的 300 多个许可证分配的 1M 信用额度。 还可以单独购买信用额度。 | 模型在所有拉丁字母语言中均可用。 除了英语之外，还有德语、瑞典语、法语、西班牙语、意大利语和葡萄牙语。 |
| 针对默认公共数据服务环境进行预配| 没有容量限制。 |
...
| 可设置分类器 | 带可选提取程序的可训练分类器 |
| 仅限于单个库 | 可应用于多个库 |
| 培训 PDF、JPG、PNG 格式，总计 50 MB/500 pp | 培训 5-10 PDF、Office 或电子邮件文件，包括负面示例 |
