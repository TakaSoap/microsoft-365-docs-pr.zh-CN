...
## <a name="considerations-when-configuring-explanations"></a>配置说明时的注意事项

训练分类器时，需要记住一些可以产生更可预测结果的事项：

- 训练的文档越多，分类器就越准确。  如果可能，请使用 5 个以上的好文档，并使用 1 个以上的错误文档。  如果正在使用的库中包含多个不同的文档类型，则每种类型中有几个会产生更可预测的结果。
- 标记文档在训练过程中起着重要作用。  它们与说明一起用于训练模型。  在使用不包含大量内容的文档训练分类器时，你可能会看到一些异常。  该说明可能与文档中的任何内容都不匹配，但由于它被标记为“好”文档，因此你可能会在训练期间看到它匹配。
- 创建解释时，它将 OR 逻辑与标签结合使用，以确定它是否是匹配项。  使用 AND 逻辑的正则表达式可能更可预测。  下面是用于实际文档的示例正则表达式，用于训练它们。  请注意，以红色突出显示的文本是要查找的短语 () 。

    <pre>(?=.*network provider)(?=.*participating providers).*</pre>

- 标签和说明协同工作，用于训练模型。  它不是一系列规则，可以取消耦合和精确的权重或预测应用于已配置的每个变量。  训练中使用的文档变体越大，模型中的准确性就越高。
