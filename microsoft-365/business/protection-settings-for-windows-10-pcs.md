...
在"设置"页上设置默认 Windows 保护设置后，可以添加适用于所有用户或一组用户的新设置。 还可以编辑已创建的任何文件。
...
有关详细信息，请参阅 [Microsoft 365](map-protection-features-to-intune-settings.md)高级版中的保护功能如何映射到 Intune 设置。 
...
|防止网络访问 Internet 上的潜在恶意内容  <br/> |使用此设置可阻止出站用户与可能承载网络钓鱼欺诈、攻击或其他恶意内容的低信誉 Internet 位置的连接。 只有在将防病毒设置为"Windows Defender时，此设置才 **可用**。 有关详细信息，请参阅 [保护你的网络](/windows/security/threat-protection/windows-defender-antivirus/configure-real-time-protection-windows-defender-antivirus)。  <br/> |
...
|允许用户访问 Cortana  <br/> |Cortana 非常有用！ Cortana 可以打开或关闭设置、提供方向，并确保你及时进行约会，因此默认情况下，我们将 **此设置保持打开** 状态。  <br/> |
