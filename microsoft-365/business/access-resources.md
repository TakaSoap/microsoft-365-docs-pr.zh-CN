...
1. 若要将您的用户、组和联系人从本地 Active Directory 同步到 Azure Active Directory，请运行目录同步向导和 Azure AD Connect，如 [设置 Office 365 的目录同步](https://docs.microsoft.com/microsoft-365/enterprise/set-up-directory-synchronization)中所述。
...
如果您计划登录到 AADJ 设备而不是 password 方法（如通过 WHFB 凭据登录的 PIN/Bio 指标），然后访问本地资源 (共享、打印机）。etc) ，请按照 https://docs.microsoft.com/windows/security/identity-protection/hello-for-business/hello-hybrid-aadj-sso-base
...
- 如果设备使用组策略对象 (GPO) ，则在 Intune 中，某些 Gpo 的 [配置服务提供程序](https://docs.microsoft.com/windows/configuration/provisioning-packages/how-it-pros-can-use-configuration-service-providers) (CSP) 可能没有。 运行 [MMAT 工具](https://www.microsoft.com/download/details.aspx?id=45520) 以查找现有 gpo 的可比较 csp。
