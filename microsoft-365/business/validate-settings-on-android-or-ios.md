...
3. 按" **重置 PIN**"。 系统将提示你使用用户的 Microsoft 365 业务凭据登录，然后需要设置新 PIN。
...
3. 按" **确定**"。 系统将提示你使用用户的 Microsoft 365 业务凭据登录，然后需要设置新 PIN。
...
3. 应只会看到用于 OneDrive for Business 的选项。 如果不是，请点击 "**添加帐户**"，然后从 "**添加存储帐户**" 屏幕中选择 " **OneDrive for business** "。 系统提示时，提供最终用户的 Microsoft 365 商业版 进行登录。 
