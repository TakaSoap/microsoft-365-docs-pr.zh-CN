...
您必须为浏览器起始页提供 intranet 或 Internet 站点的完全限定域名 (FQDN) 。 如果配置了内部网站，请让用户知道，仅当在 office 中连接到内部网络或使用 VPN 连接进行连接时，才允许访问这些网站。 
...
为每个受信任站点提供 intranet 或 Internet 站点的完全限定的域名 (FQDN) 。 
...
1. 登录到 [Microsoft 终结点管理器](https://endpoint.microsoft.com/) 并导航到 " **设备** " 菜单
2. 查找 "Microsoft 托管桌面" 部分，选择 " **设置**"。
3. 在 " **设置** " 工作区中，选择 " **受信任的站点**"，然后选择 " **添加可信站点**" 
