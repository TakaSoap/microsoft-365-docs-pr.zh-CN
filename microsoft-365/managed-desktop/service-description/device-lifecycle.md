...
Microsoft 托管桌面优势用户确保他们始终使用可提供最佳性能、可靠性、设计和安全功能的设备 (例如 Windows Hello) 等功能的支持。 为实现此目的，Microsoft 托管桌面维护了连续更新的 [经过审批的设备](device-list.md)的简短目录。 
...
在此期间内注册的任何设备都将从 Microsoft 托管桌面 (获得完整的三年支持，如蓝色日程表) 所示。 此时段一直持续到结束日期设置为从一般可用性日期起的24个月。

你可以将此期间视为有效的 "打开注册"，因此，要最大限度地提高 Microsoft 托管桌面的价值，应瞄准你的采购模型和所选的产品在此时间段内。 作为一个小示例，客户应避免在两年三年的滚动期内使用主可用性最后一个月的产品进行结算–大多数设备将不会收到全部三年的 Microsoft 托管桌面管理 (请参阅 [宽限期](#product-grace-period) ，了解详细信息) 。  
