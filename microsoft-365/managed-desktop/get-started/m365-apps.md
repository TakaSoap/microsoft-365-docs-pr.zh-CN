...
ms.locfileid: "63494981"
---
# <a name="microsoft-365-apps-for-enterprise"></a>Microsoft 365 企业应用版

## <a name="initial-deployment"></a>初始部署

Microsoft 托管桌面确保Microsoft 365 企业应用版 (64) 作为映像的一部分安装在所有[程序设备上](../service-description/device-list.md)。 以下所有应用程序应在交付时存在于设备上：
...
如果用户出于任何原因Microsoft 365 应用版设备，可以使用程序包将设备返回到其预期状态。 将用户添加到"新式 **工作区Office Office365_Install组**，应用将在"新式工作区"公司门户。
...
Microsoft 托管桌面每个版本错开，以确定环境中的任何潜在问题。 我们已完成从应用产品Microsoft 365推出。 Microsoft 托管桌面计划向不同组发布更新，以留出验证和测试时间，如下所示：
...
在发布期间，Microsoft 托管桌面监视所有Microsoft 365 应用版。 如果我们发现新版本和上一版本的质量显著差异，我们可能会通过管理门户Microsoft 托管桌面联系。
...
Microsoft 将某些设置作为服务的一部分管理。 Microsoft 托管桌面管理安全Office比较基准。 但是，您可以按照管理部分中的指导操作，[自己设置设置一个](#settings-you-manage)。
...
| Setting | 默认值 | 描述 |
| ------ | ------ | ------ |
| 设置自动进行的更新 | 已启用 | 配置此策略是为了确保Office设备在云中保持最新状态。 |
...
| 更新通知设置 | False | 在设备上，"隐藏更新通知"设置Microsoft 托管桌面设置为 **False**，以在需要更新时通知用户，以提供最佳更新体验。[](/deployoffice/end-user-update-notifications-microsoft-365-apps#notifications-your-users-see-when-you-set-an-update-deadline-for-microsoft-365-apps)|
| 指定用于查找更新的位置 | 每月企业频道 | 根据需要使用 **UpdatePath** 和 **UpdateChannel** 策略的组合来实现更新计划。 设置这些策略是为了确保所有Office设备直接从每月频道频道CDN更新Enterprise更新。|
| 指定目标版本Microsoft 365 应用版 | 请参阅说明 | 目标版本策略有时由用户Microsoft 托管桌面，以便回滚或固定特定版本的Office。|
| 隐藏启用或禁用自动更新Office选项 | 已启用 | 若要使用户满足Microsoft 托管桌面应用程序的更新目标，Microsoft 365此设置。 |
| 首次运行设置 | 请参阅说明 | 首次运行时，有几个设置Office行为。 |
| 代表最终用户接受许可条款 | Disabled | 用户首次打开 Microsoft 365 应用时，系统会提示他们接受许可条款。 如果要代表用户接受许可条款，请向 Microsoft 托管桌面 Operations 团队提出支持请求，并要求启用此设置。 |
| 禁止Outlook移动复选框 | Disabled | 用户首次打开Outlook时，系统会提示他们安装Outlook移动版。 如果你不希望用户看到该复选框，请向 Microsoft 托管桌面 Operations 团队提出支持请求，并要求为设备启用此设置。 |
...
| Setting | 默认值 | 描述 |
| ------ | ------ | ------ |
| 禁用个人OneDrive | Disabled | 一些组织关注用户可以访问其设备上公司和个人文件。 你可以向运营团队提出Microsoft 托管桌面请求，并要求启用此设置。 |
...
1. 在 **"创建策略** 配置"页中，执行以下操作：
    ...
    - 选择 **AAD** 策略配置的基于安全组的安全组。 每个策略配置只能分配给一个组。 只能为每个组分配一个策略配置。
