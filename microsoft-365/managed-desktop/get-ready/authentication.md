...
有关加入 Azure Active Directory 的信息，请参阅 how [to： Plan a AZURE AD join 实现](https://docs.microsoft.com/azure/active-directory/devices/azureadjoin-plan)。 有关连接到 Azure AD 的设备上的单一登录 (SSO) 的背景信息，请参阅 [内部部署资源的 sso 在 AZURE ad 联接设备上的工作原理](https://docs.microsoft.com/azure/active-directory/devices/azuread-join-sso#how-it-works)。
...
单一登录 (使用 UPN) 的 SSO 和密码在 Microsoft 托管桌面设备上默认启用。 但您的用户也可以使用 Windows Hello 企业版，这需要执行一些额外的设置步骤。 
...
- 如果您使用 **基于云的应用程序**（例如，添加到 Azure AD 应用程序库中的应用程序），则大多数不需要进一步准备使用 Microsoft 托管桌面。 但是，任何不使用 Web 帐户管理器 (WAM) 的 Win32 应用仍可能会提示用户进行身份验证。
