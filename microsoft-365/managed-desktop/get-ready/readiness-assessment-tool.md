...
联机[工具](https://aka.ms/mmdart)会检查Microsoft Endpoint Manager (中的设置，具体Microsoft Intune) 、Azure Active Directory (Azure AD) 和Microsoft 365，以确保这些设置能够使用Microsoft 托管桌面。 Microsoft 托管桌面自上次在 Azure AD 组织或租户管理租户中运行检查后，与这些检查 (数据保留 12) 。 12 个月后，我们会以已取消标识的形式保留它。 你可以选择删除我们收集的数据。
...
|设备合规性策略     | 检查未将 Intune 合规性策略分配给所有用户 (这些策略不应分配给任何Microsoft 托管桌面设备。)     |
|设备配置文件     | 确认配置文件未分配给所有用户，或不应将 (配置文件的所有设备分配给任何Microsoft 托管桌面设备。)      |
|设备类型限制     | 检查Windows 10组织中允许的设备在 Intune 中注册        |
|注册状态页     | 确认未启用注册状态页      |
...
完成注册后，请Microsoft 托管桌面并调整某些 Intune 和Azure AD设置。 有关详细信息，请参阅 [在注册后调整设置](../get-started/conditional-access.md)。
