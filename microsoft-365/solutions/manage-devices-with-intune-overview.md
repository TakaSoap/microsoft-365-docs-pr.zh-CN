...
- 计划手动将设备注册到 Intune。
- 允许使用计划实现应用和数据保护的 BYOD 设备，并/或注册这些设备以Intune。
...
保护设备、设备上的应用和数据、设备本身是一个多层的过程。 可以在非托管设备上获得一些保护。 将设备注册到Intune后，可以实现更复杂的控件。 在终结点中部署威胁防护时，你将获得更多见解，并能够自动修正某些攻击。 最后，如果你的组织已投入工作来识别敏感数据、应用分类和标签以及配置数据丢失防护策略，则可以为终结点上的数据获取更精细的保护。
...
|2|将设备注册到Intune|此任务需要更多规划和时间来实现。 Microsoft 建议使用 Intune 注册设备，因为此工具提供最佳集成。 可通过多个选项注册设备，具体取决于平台。 例如，可以使用 Azure AD Join 或 Autopilot 注册 Windows 设备。 你需要查看每个平台的选项，并确定哪个注册选项最适合你的环境。 有关详细信息，请参阅[步骤 3 - 注册设备以Intune](manage-devices-with-intune-enroll.md)。|E3、E5、F1、F3、F5|
...
- 在步骤 2、3 和 4 中，使用Intune将设备注册到管理中，定义设备符合性策略，然后与标识团队协调，将相关的条件访问规则配置为仅允许访问合规设备。
...
如果遵循本指南，将使用Intune将设备注册到管理中，并且你将载入设备以获得以下Microsoft 365功能：

- Microsoft Defender for Endpoint
- Microsoft 365 DLP) ) 终结点数据丢失防护 (合规性 ( 
...
2. 使用Intune载入设备以Microsoft Defender for Endpoint。
3. 载入到 Defender for Endpoint 的设备也已加入Microsoft 365符合性功能，包括终结点 DLP。

请注意，只有 Intune 正在管理设备。 载入是指设备能够使用特定的服务功能共享信息。 下表汇总了将设备注册到管理和载入特定功能设备之间的差异。
...
|范围|这些设备管理工具管理整个设备，包括将设备配置为满足特定目标，例如安全性。|载入仅影响应用的功能。|
