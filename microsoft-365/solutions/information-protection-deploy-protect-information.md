...
为了帮助你开始使用 Microsoft 365 中的信息保护方案，以下部分包含适用于 Microsoft 365 的相关功能和改进操作的简短列表。 此列表包含适用于数据隐私法规的功能和改进操作。 但是，如果存在主要取代旧的功能的较新功能，则该列表不包含较旧的技术。 例如，列表中不包含 SharePoint 和 OneDrive 的信息权限管理 (IRM) ，但包含灵敏度标签。
...
- 数据丢失防护 (DLP)
- [Microsoft 365 终结点数据丢失防护 (预览) ](https://docs.microsoft.com/microsoft-365/compliance/endpoint-dlp-learn-about?view=o365-worldwide)
- [Office 365 邮件加密 (OME) ](../compliance/ome.md)和 OME[高级邮件加密](../compliance/ome-advanced-message-encryption.md)的新功能
...
1. 了解以下内容：
   ...
   - **需要记住的关键事项** 敏感度标签在 Microsoft 合规性管理中心中进行管理，但目标和应用程序选项明显不同。
      - 在容器级别有网站、组和团队的敏感度标签 (设置不适用于容器) 中的内容。 这些发布到在设置网站、组或团队时对其应用的用户和组。
...
3. 规划集成到一个更广泛的信息保护方案中。 在与 OME 共存的情况下，可以将当前敏感度标签与 Microsoft 365 数据丢失防护 (DLP) 和 Microsoft 云应用安全性等相关功能一起使用。 请参阅 [敏感度标签和 Microsoft 云应用安全](../compliance/sensitivity-labels.md#sensitivity-labels-and-microsoft-cloud-app-security) ，以实现与您的数据隐私相关的信息保护目标。
...
3. **其他建议** 在设计和实施策略、标签和条件时，请考虑遵循以下建议：
   ...
   - **如果任何) ，请使用现有分类架构 (。** 许多组织已使用某种形式的数据分类。 请仔细评估现有标签架构，如果可能，请按 "是"。 使用可识别给最终用户的熟悉标签将推动采用。
...
- Microsoft 团队网站
- Microsoft 365 组 (以前的 Office 365 组) 
- SharePoint 网站

使用以下标签设置来帮助保护这些容器中的内容：

- Microsoft 365 组连接的团队网站的隐私 (公共或专用) 
- 外部用户访问
- 非托管设备的访问
...
2. **定义每个标签的用途。** 配置要与每个标签关联的保护设置。 例如，您可能希望使用较低敏感度的内容 (例如 "常规" 标签) 仅应用页眉或页脚，而较高灵敏度内容 (如 "保密" 标签) 应具有水印并启用加密。
...
此方法称为使用敏感度标签自动分类。 此外，您还可能会听到，它称为 "自动标记 SharePoint 和 OneDrive 中的数据 () 中的数据，以及 Exchange) 发送或接收的电子邮件的传输 (中的数据。 对于 Exchange，它不会在静止邮箱中包含电子邮件。
...
对于数据隐私，您可以使用 OME 来保护包含敏感项目的内部邮件。 Office 365 邮件加密是基于 Microsoft Azure 权限管理 (Azure RMS) 的一种在线服务，它是 Azure 信息保护的一部分。 这包括加密、标识和授权策略，以帮助保护您的电子邮件。 您可以使用权限管理模板、"不转发" 和 "仅加密" 选项对邮件进行加密。
...
| 具有过期/撤消的外部方 <br> 使用过期和吊销控制对具有外部/消费者用户的邮件和内容的访问 | 是–用户可以手动跟踪文件 | 建议 (E5) –管理员可以从安全 & 合规中心撤销邮件 |
