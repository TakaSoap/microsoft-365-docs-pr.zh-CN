...
服务（Microsoft 365组、Teams和 Yammer）为用户提供了能力，通过提供协作所需的工具降低了影子 IT 的风险。 Microsoft 365提供了一组丰富的工具来实施组织可能需要的任何管理功能。 
...
有许多选项可以部署 Microsoft 365 组Teams，以确保组织的安全协作。 我们建议您在设置安全协作与[Microsoft 365及其关联](setup-secure-collaboration-with-teams.md)文章一起使用此管理内容，以为组织创建最佳协作解决方案。

### <a name="data-residency"></a>数据驻留

如果你的组织是一个多方组织，并且你对不同的地理位置有数据驻留要求，Microsoft 365[多](/microsoft-365/enterprise/microsoft-365-multi-geo)地理位置作为协作管理计划的一部分。

## <a name="why-microsoft-365-groups-are-important"></a>为什么Microsoft 365组很重要

Microsoft 365组允许你选择要与之协作的一组人员，并轻松设置供这些人员共享的资源集合。 向组添加成员会自动向组提供的所有资产授予所需的权限。 用户Teams Yammer组Microsoft 365管理其成员身份。

Microsoft 365组包括一套链接资源，用户可使用这些资源进行通信和协作。 组始终包括SharePoint、Planner、Power BI工作区、邮箱和日历以及 Stream。 根据组的创建方式，可以选择添加其他服务，如Teams、Yammer和Project。
...
当您确定要如何使用协作功能Microsoft 365，请考虑以下通信方法以及您的用户可能在不同的方案中使用的方法。

> [!NOTE]
> 通过 Office 365 或 Yammer Teams 新建组时，该组在 Outlook 或通讯簿中不可见，因为这些用户之间的主要通信发生在各自的客户端中。 Yammer组无法连接到Teams。
...
- **适应不同的组织和不同类型的** 内容和方案 - 考虑不同组或部门以及不同类型的内容（如 Intranet 内容）和用户的OneDrive需求。
...
2. 了解服务中的设置 -[组中设置SharePoint](groups-sharepoint-governance.md)交互，就像组、SharePoint 和 Teams [](groups-sharepoint-teams-governance.md) [服务中的设置一样](groups-services-interactions.md)。 在规划管理策略时，请务必了解这些交互。
3. 计划管理用户访问 - 规划要向组、用户和用户授予SharePoint[级别Teams。](groups-teams-access-governance.md)
4. 计划管理合规性设置 - 查看适用于组、Microsoft 365和Teams的SharePoint[选项](groups-teams-compliance-governance.md)。
