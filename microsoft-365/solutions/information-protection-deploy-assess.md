...
有关数据隐私管理法规的更广泛的规章框架的良好参考，请参阅 [Microsoft 服务信任门户](https://servicetrust.microsoft.com/) 和有关 [常规 data PROTECTION 规章 (GDPR) 法规的一系列文章](../compliance/gdpr.md)，以及您的行业或地区可能会受管理法规约束的其他资料。

### <a name="gdpr"></a>GDPR

GDPR （最著名和引用的数据隐私法规）规定了收集、存储、处理和共享与作为欧盟 (EU 的欧盟) 的自然个人相关的任何个人数据的收集、存储、处理和共享。 

根据 GDPR 文章4： 

- "个人数据" 是指与标识的或可识别的自然个人 ( "数据主体" ) 相关的任何信息。可识别的自然用户可以直接或间接地通过引用标识符（如名称、标识号码、位置数据、联机标识符或该自然人员的物理、physiological、遗传、精神、经济、文化或社会身份）的引用来进行识别。
...
在美国，其中包括加利福尼亚州消费者保护法 ([CCPA](../compliance/ccpa-faq.md)) 、HIPAA-高科技 (美国卫生保健隐私保护法案) 和 Graham 里奇比利雷 ACT (GLBA) 。 其他特定于状态的法规也就地或在开发中。 

在世界各地，其他示例包括德国的国家 GDPR 实施法案 (BDSG) 、巴西 Data Protection 法案 (LGPD) 以及其他许多其他。
...
|:-------|:-----|:-------|:-------|
| GDPR | 第5篇 (1) # B2 f)  | 应以确保个人数据的适当安全性的方式处理个人数据，包括防止未经授权或非法处理的保护，并在使用适当的技术或组织措施 ( "完整性和机密性" 时防止意外丢失、毁坏或损坏。  |   (所有)  <br> 标识 <br> 设备 <br> 威胁防护 <br> 保护信息 <br> 管理信息 <br> 发现和响应 |
|  | 文章 (32) # B2 1) # B4 a)  | 考虑图片的状态、实施的成本和性质、范围、上下文和处理目的，以及对自然人员的权利和自然人的可能性和严重性存在变化的风险，但控制器和处理器应实施适当的技术和组织措施，以确保适合风险的安全级别，包括 alia 的假名化和加密)  (的个人数据。 | 保护信息 |
|  | 文章 (13) # B2 2) # B4 a)  | "...在获取个人数据时，控制器将在获取个人数据时提供以下详细信息，以确保公平和透明处理： () 存储个人数据的时间段; 或者，如果不可能，则用于确定该时间段的条件。 | 管理信息 |
|  | 文章 (15) # B2 1) # B4 e)  | 数据主体应具有从控制器确认获取的权限，以确定是否正在处理与他/她有关的个人数据，在这种情况下，访问个人数据和以下信息的位置如下： (e) 从控制器纠正请求的权限，或擦除个人数据的权限，或将有关数据主体或对象的个人数据处理限制为此类处理。 | 发现和响应 |
...
| HIPPA-高科技 | 45 CFR 164.312 (e) # B2 1)  | 实施技术安全措施，以防止未经授权访问通过电子通信网络传输的电子保护健康信息。 | 保护信息 |
|  | 45 C.F.R。 164.312 (e) # B2 2) # B4 ii)  | 在认为合适时实现一种加密电子保护的健康信息的机制。 | 保护信息 |
|  | 45 CFR 164.312 (c) # B2 2)  | 实现电子机制，以 corroborate 未经授权的方式更改或销毁电子保护的运行状况信息。 | 管理信息 |
|  | 45 CFR 164.316 (b) # B2 1) # B4 i)  | 如果要记录此 subpart 所需的操作、活动或评估，请维护撰写 (，这可能是操作、活动或评估的电子) 记录。 | 管理信息 |
|  | 45 CFR 164.316 (b) # B2 1) # B4 ii)  | 将此部分的第 (b) # B2 1) 所需的文档保留一段时间，从创建之日起6年，或最后一次生效的日期（以稍后为准）。 | 管理信息 |
|  | 45 C.F.R。 164.308 () # B2 1) # B4 ii) # B6 D)  | 实施定期查看信息系统活动记录的过程，例如审核日志、访问报告和安全事件跟踪报告 | 发现和响应 |
|  | 45 C.F.R。 164.308 () # B2 6) # B4 ii)  | 确定和响应可疑或已知的安全事件;降低范围切实可行、对被覆盖的实体或业务关联的安全事件的有害影响;并记录安全事件及其结果。 | 发现和响应 |
|  | 45 C.F.R。 164.312 (b)  | 实施硬件、软件和程序机制，以记录和检查包含或使用电子保护运行状况信息的信息系统中的活动。 | 发现和响应 |
| CCPA | 1798.105 (c)  | 从使用者处收到可验证的请求以删除使用者的个人信息的业务，以在此部分的 () 中删除使用者的个人信息应从其记录中删除使用者的个人信息，并指示任何服务提供商从其记录中删除使用者的个人信息。 | 发现和响应 |
|  | 1798.105 (d)  |  (1798.105 (c 的例外)  <br> 如果业务或服务提供商需要维护使用者的个人信息，则业务或服务提供商不需要遵守使用者的个人信息，因为这样做的目的是为了： (引用当前规章以获取其他信息) 。 | 发现和响应 |
|||||

>[!Important]
>这不是一个详尽的列表。 请参阅 [合规性管理器](../compliance/compliance-manager.md) 或你的法律或合规性顾问，了解有关所引用的部分的适用性的详细信息，请参阅所列的技术控制类别。
>
...
- 组织有关向组织在企业到客户 (B2C) 方案中管理的联机服务提供信息的使用者的数据
...
尽管外部组织或标准控制集建议（例如 Microsoft 365 中的合规性管理器）可能有助于通知控制策略，但您选择的实施应由数据清单感知驱动，以量化实际风险暴露。
...
| 分区 | 说明 |
|:-------|:-----|
| 管控 | <ol><li>您的隐私策略是否明确声明要处理的数据信息？ </li><li>您是否定期运行隐私影响评估 (Pia) ？ </li><li> 您是否使用工具来管理个人信息 (PI) ？ </li><li> 您是否有合法的颁发机构在任何给定的人上使用 PI 数据开展业务？ 您是否跟踪数据的同意？ </li><li> 您是否跟踪、实现和管理审核控件？ 您是否监视数据泄露？ </li></ol>|
...
Microsoft 合规性管理中心预加载了100以上的敏感信息类型，与确定和查找个人数据相关的大部分信息类型都是如此。 这些内置的敏感信息类型可以根据正则表达式 (regex) 或函数定义的模式，帮助识别和保护信用卡号码、银行帐号、护照号码等。 若要了解详细信息，请参阅[敏感信息类型查找的内容](../compliance/what-the-sensitive-information-types-look-for.md)。
