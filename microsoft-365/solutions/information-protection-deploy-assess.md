...
在实施任何相关改进操作（包括那些可以通过 Microsoft 365 功能和服务实现的改进措施）之前，评估组织要承担的数据隐私法规和风险是一个关键的第一步。 

## <a name="potentially-applicable-data-privacy-regulations"></a>可能适用的数据隐私法规

若要很好地参考更广泛的数据隐私法规框架，请参阅 [Microsoft](https://servicetrust.microsoft.com/) 服务信任门户和有关一般数据保护条例 [ (GDPR) ](/compliance/regulatory/gdpr)法规的一系列文章，以及行业或地区可能受法规限制的其他材料。
...
## <a name="regulation-mapping-to-microsoft-365-technical-control-categories"></a>法规与 Microsoft 365 技术控制类别的映射
...
|:-------|:-----|:-------|:-------|
| GDPR | 第 5 (1)  (f)  | 个人数据的处理方式应该确保个人数据的适当安全性，包括防止未经授权的或非法处理，以及防止意外丢失、销毁或损坏，使用适当的技术或组织措施 (完整性和机密性"。  |   (所有)  <br> 标识 <br> Device <br> 威胁防护 <br> 保护信息 <br> 治理信息 <br> 发现和响应 |
|  | 文章 (32)  (1)  ()  | 考虑到技术状态、实现成本以及处理的性质、范围、上下文和目的，以及自然人的权利和自由存在各种可能性和严重性的风险，控制者与处理者应实施适当的技术和组织措施，以确保适当的安全级别，包括适当内部攻击： (a) 个人数据的假名和加密。 | 保护信息 |
|  | 文章 (13)  (2)  ()  | "...控制者应在获取个人数据时向数据主体提供确保公平、透明的处理所需的以下详细信息： () 存储个人数据的时间段，或者如果不可能，则提供用于确定该时间段的条件。 | 治理信息 |
|  | 第 (15)  (1)  ()  | 数据主体有权从控制者确认是否正在处理其相关的个人数据，以及是否正在访问个人数据和以下信息： (e) 是否存在从控制者纠正或清除个人数据或限制处理与数据主体相关的个人数据或对象处理此类数据的权利 | 发现和响应 |
| LGPD | 文章 46 | 处理代理应采取安全、技术和管理措施，以保护个人数据免受未经授权的访问以及意外或非法的销毁、丢失、更改、通信或任何类型的不正确或非法处理的情况。 | 保护信息 <br> 治理信息 <br> 发现和响应|
...
|  | 45 C.F.R. 164.312 (2)  (2)  (ii)  | 实施一种机制，在认为合适时对电子保护的运行状况信息进行加密。 | 保护信息 |
|  | 45 CFR 164.312 (c)  (2)  | 实施电子机制，以证实电子保护的运行状况信息未被未经授权更改或销毁。 | 治理信息 |
|  | 45 CFR 164.316 (b)  (1)  (i)  | 如果此子部分要求记录行动、活动或评估，请保留书面 (，该记录可能是) 、活动或评估的电子记录 | 治理信息 |
|  | 45 CFR 164.316 (b)  (1)  (ii)  | 将本节第 (b)  () 段所需的文档自创建日期或最后生效日期起保留 6 年，以较高者为准。 | 治理信息 |
|  | 45 C.F.R. 164.308 (1)  (1)  (ii)  (D)  | 实施定期查看信息系统活动记录的过程，如审核日志、访问报告和安全事件跟踪报告 | 发现和响应 |
|  | 45 C.F.R. 164.308 (6)  (6)  (ii)  | 识别并响应可疑或已知的安全事件;在可允许的范围内缓解覆盖的实体或业务关联已知的安全事件的负面影响;并记录安全事件及其结果。 | 发现和响应 |
|  | 45 C.F.R. 164.312 (b)  | 实施硬件、软件和过程机制，以记录和检查包含或使用电子受保护运行状况信息的信息系统中的活动。 | 发现和响应 |
| CCPA | 1798.105 (c)  | 如果企业收到来自消费者的可验证请求，以删除被细分为 (的消费者个人信息) 则本节的) 应删除其记录中的消费者个人信息，并指示任何服务提供商从记录中删除消费者的个人信息 | 发现和响应 |
...
要解决数据隐私法规，不能依赖于你认为个人数据可能存在于何处的一般概念，无论是现在还是将来。 数据隐私法规要求组织证明他们持续知道个人数据位于何处。 这使得获取所有数据源的初始快照以可能存储个人信息（包括 Microsoft 365 环境）以及建立持续监视和检测机制非常重要。
...
- 执行日常业务功能所需的员工数据
- 组织具有的有关其业务客户、合作伙伴和业务到企业或 B2B 业务方案中 (关系) 数据
- 组织具有的有关消费者 (的数据，这些客户向组织在企业到客户或 B2C) 方案中管理的在线服务提供相关信息

下面是组织典型部门不同类型的数据的示例。

![个人数据的类型](../media/information-protection-deploy-assess/information-protection-deploy-assess-data-types.png)

大部分受数据隐私条例保护的个人数据通常收集并存储在 Microsoft 365 外部。 来自面向消费者的 Web 或移动应用程序的任何个人数据都需要从此类应用程序导出到 Microsoft 365，以便受到 Microsoft 365 中数据隐私审查的约束。 
...
- **处理数据主体请求。** 需要哪些机制来处理 DSR (数据主体) 任何修正操作（如匿名化、修订和删除）？
- **持续监视和报告。** 哪些类型的日常监视、调查和报告技术可用于不同的数据类型和源？
- **数据处理的限制。** 对于通过这些方法收集或存储的信息，组织在隐私控制中必须反映的数据使用是否有限制？ 例如，销售人员不会使用个人数据的承诺可能要求贵组织建立机制，以防止在与销售组织关联的系统中传输或存储该信息。
...
>[!Note]
>有关 Microsoft 365 中个人数据的调查和修正技术，请参阅监视 [和回复文章](information-protection-deploy-monitor-respond.md)。 你可能还需要使用自动分类和保护方案，以确保个人数据在组织内部受到控制，以及防止它在恶意参与者情况下离开组织。 有关详细信息 [，请参阅保护](information-protection-deploy-protect-information.md) 信息文章。
>
...
如果在 Microsoft 365 环境中没有来自消费者的个人数据，或者它仅限于环境的某些部分，并且需要技术控制，前提是存在消费者类型的数据泄露，则技术控制可能只需要在环境的高风险部分使用，而不是在任何地方使用。
...
| 分区 | 说明 |
|:-------|:-----|
| 治理 | <ol><li>隐私策略是否明确声明正在处理哪些数据信息？ </li><li>您是否定期运行 PIA (隐私) ？ </li><li> 您是否使用工具来管理 PI (的个人信息) ？ </li><li> 你是否具有对任意给定个人使用 PI 数据开展业务的法律授权？ 你是否跟踪数据许可？ </li><li> 你是否跟踪、实施和管理审核控制？ 是否监视数据泄露？ </li></ol>|
...
## <a name="step-3-identify-sensitive-information-types-that-occur-in-your-microsoft-365-environment"></a>步骤 3：标识 Microsoft 365 环境中发生的敏感信息类型。 

此步骤涉及标识受特定法规控制的特定敏感信息类型，以及这些类型在 Microsoft 365 环境中出现的情况。 

在包含个人的环境中查找内容可能是一项艰巨的任务，以前涉及使用合规性搜索、电子数据展示、高级电子数据展示、DLP 和审核的组合。 
...
确定环境中出现敏感项目的重要工具是 Microsoft 365 合规性[](../compliance/data-classification-content-explorer.md)管理中心中的新内容资源管理器。 它是一个自动工具，用于对整个 Microsoft 365 订阅进行初始和持续扫描，以发现敏感信息类型出现并显示结果。
 
利用新的内容资源管理器工具，您可以使用内置敏感信息类型或自定义类型快速识别环境中敏感项目的位置。 这可能涉及到建立一个流程，并分配有定期调查敏感项目存在和位置的责任。

除了本文中重点介绍的其他步骤外，这还提供了一个起点，用于确定通过计划的 Microsoft 365 配置和监视进行保护的敏感项目的总体风险暴露、准备情况以及位置。 
...
本文中提供了关于使用内容搜索发现个人数据的详细 [指导](/compliance/regulatory/gdpr)。 内容搜索和其他发现技术也在 [GDPR 和 CCPA 的 DSR 中探索](/compliance/regulatory/gdpr-dsr-Office365#introduction-to-dsrs)。

监视器和回复文章中提供了有关 Microsoft 365 中个人数据调查和修正技术 [的其他见解](information-protection-deploy-monitor-respond.md)。
