...
若要很好地参考更广泛的数据隐私法规框架，请参阅 [Microsoft](https://servicetrust.microsoft.com/) 服务信任门户和一般数据保护条例 [ (GDPR) ](/compliance/regulatory/gdpr)法规的一系列文章，以及行业或地区可能受法规限制的其他材料。
...
- "个人数据"是指与已识别或可识别的自然人 ("数据主体"身份) ;可识别的自然人是可直接或间接识别的自然人，尤其是通过引用姓名、标识号、位置数据、在线标识等标识符，或特定于该自然人的性、基因、精神、经济、文化或社会标识的一个或多个因素进行识别。
...
在美国，其中包括加州消费者保护法案 ([CCPA](/compliance/regulatory/ccpa-faq)) 、HIPAA-HITECH (美国医疗保健隐私法案) 和 GlBA (《卡姆雷法案》) 。 其他特定于州的规定也就地或正在开发中。

世界各地的其他示例包括德国的国家 GDPR 实施法案 (BDSG) 、巴西数据保护法案 (LGPD) 等。

## <a name="regulation-mapping-to-microsoft-365-technical-control-categories"></a>与技术Microsoft 365类别的法规映射
...
|---|---|---|---|
|GDPR|第 5 (1)  (f) |个人数据的处理方式应保证个人数据的适当安全性，包括防止未经授权的或非法处理，以及防止意外丢失、销毁或损坏，并采用适当的技术或组织措施 (完整性和保密性"。| (所有)  <br> 标识 <br> 设备 <br> 威胁防护 <br> 保护信息 <br> 管理信息 <br> 发现和响应|
||文章 (32)  (1)  () |考虑到技术状态、实现成本以及处理的性质、范围、上下文和目的，以及自然人的权利和自由出现不同可能性和严重性的风险，控制者和处理者应实施适当的技术和组织措施，以确保适当的安全级别，包括适当的内部攻击： (a) 个人数据的假名和加密。|保护信息|
||文章 (13)  (2)  () |"...控制者应在获取个人数据时向数据主体提供确保公平、透明的处理所需的以下详细信息： () 存储个人数据的时间段，或者如果不可能，则提供用于确定该时间段的条件。|管理信息|
||第 (15)  (1)  () |数据主体有权从控制者确认是否正在处理有关其个人数据以及是否正在处理个人数据以及访问个人数据和以下信息的权利： (e) 是否存在从控制者纠正或清除个人数据或限制处理与数据主体或对象相关的个人数据的权利|发现和响应|
...
||45 C.F.R. 164.308 ()  (1)  (ii)  (D) |实施定期查看信息系统活动记录的过程，如审核日志、访问报告和安全事件跟踪报告|发现和响应|
...
|CCPA|1798.105 (c) |如果企业收到来自消费者的可验证请求，以删除消费者的个人信息，并规定 () 节的) 将从其记录中删除消费者的个人信息，并指示任何服务提供商从记录中删除消费者的个人信息|发现和响应|
||1798.105 (d) | (1798.105 (c)  <br> 如果企业或服务提供商需要维护消费者的个人信息以便维护消费者的个人信息，则不应要求企业或服务提供商遵守消费者删除其个人信息的请求，以便： (参考当前法规，获取其他信息) 。|发现和响应|
|||||
...
首先，必须清点环境中存在的个人数据Microsoft 365类型。 使用以下类别：

- 执行日常业务功能所需的员工数据
- 组织具有的有关其业务客户、合作伙伴以及企业到企业或 B2B 业务方案中 (关系) 数据
...
- **处理数据主体请求。** 需要哪些机制来处理 DSR (数据主体) 以及任何修正操作（如匿名、修订和删除）？
...
如果在 Microsoft 365 环境中没有来自使用者的个人数据，或者它仅限于环境的某些部分，并且需要技术控制，前提是存在消费者类型的数据泄露，则技术控制可能只需要在环境的高风险部分使用，而不是在任何地方使用。
...
受其他数据隐私法规（如美国 CCPA 或巴西 LGPD）限制的组织，也可能受益于此工具的就绪清单，因为与 GDPR 的重叠规定。

GDPR 评估包含以下部分：

|节|说明|
|:-------|:-----|
|治理|<ol><li>隐私策略是否明确声明正在处理哪些数据信息？ </li><li>你是否定期在 PIA (隐私) ？ </li><li> 您是否使用工具来管理 PI (的个人信息) ？ </li><li> 你是否具有对任意给定个人使用 PI 数据开展业务的法律授权？ 你是否跟踪数据许可？ </li><li> 你是否跟踪、实施和管理审核控制？ 是否监视数据泄露？ </li></ol>|
...
## <a name="step-3-identify-sensitive-information-types-that-occur-in-your-microsoft-365-environment"></a>步骤 3：确定在环境中发生的敏感信息Microsoft 365类型

此步骤涉及标识受特定法规控制的特定敏感信息类型，以及在你的环境中Microsoft 365类型。

在包含个人的环境中查找内容可能是一项强大的任务，以前涉及结合使用合规性搜索、电子数据展示、Advanced eDiscovery、DLP 和审核。
...
除了本文中重点介绍的其他步骤外，这还提供了一个起点，用于确定通过计划的部署和监视来保护的敏感项目Microsoft 365风险暴露、准备情况以及位置。
