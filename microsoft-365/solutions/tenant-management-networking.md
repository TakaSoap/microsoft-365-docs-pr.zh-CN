...
Microsoft 365企业版包括云生产力应用（如 Teams 和 Exchange Online）和 Microsoft Intune，以及 Microsoft Azure 的许多标识和安全服务。 所有这些基于云的服务都依赖于来自本地网络或 Internet 上任何位置的客户端设备连接的安全性、性能和可靠性。 
...
对于企业网络，应该通过启用客户端和最近的终结点之间性能最高的网络访问来优化最终用户Microsoft 365体验。 最终用户体验的质量与用户使用的应用程序的性能和响应能力直接相关。 例如，Microsoft Teams依赖于低延迟，以便用户电话呼叫、会议以及共享屏幕协作无故障。
...
- 对于受信任流量，将跳过执行重复数据包处理Microsoft 365网络发夹，从而减少延迟。
...
在此图中，VPN 客户端直接通过 Internet Microsoft 365到最近的 Microsoft 全球网络的前端，发送和接收关键的云服务流量。
...
基于云的服务器内容分发网络 (CDN) 减少加载时间、节省带宽并加快响应速度。 一CDN缓存静态资产（如图形或视频文件）以改进性能，这可帮助加快下载速度并减少延迟。 您可以使用 Microsoft 365 E3 和 E5 Office 365 内容分发网络 (CDN) 中内置的 SharePoint 来托管静态资产，以便为 SharePoint 页面提供更好的性能。
...
若要自动化客户端 PAC 文件和网络设备中 Microsoft 365 终结点的一览和更改管理，请使用[Office 365 IP 地址和基于 URL REST](../enterprise/microsoft-365-ip-web-service.md)的 Web 服务。 此服务可帮助您更好地识别和区分Microsoft 365网络流量，从而更轻松地评估、配置和了解最新更改。

可以使用 PowerShell、Python 或其他语言来确定终结点在一段时间的变化，并配置 PAC 文件和边缘网络设备。

基本过程为：

1. 使用 Office 365 IP 地址和 URL Web 服务以及您所选择的配置机制，使用当前的一组 MICROSOFT 365 终结点配置 PAC 文件和网络设备。
