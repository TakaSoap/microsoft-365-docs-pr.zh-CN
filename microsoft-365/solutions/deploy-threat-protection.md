...
此解决方案介绍了跨组织的强大威胁防护Microsoft 365 E5并解释了威胁防护为什么很重要。 阅读本文，大致了解组织中的威胁防护Microsoft 365 E5以及如何为组织进行设置和配置。
...
[Microsoft 365 E5](https://www.microsoft.com/microsoft-365/enterprise-e5-business-software?activetab=pivot%3aoverviewtab)使用自适应内置智能保护组织。 借助威胁防护功能Microsoft 365 E5，你可以检测并调查本地和云环境中的高级威胁、泄露的标识和恶意操作。

默认情况下Microsoft 365 E5集成威胁防护功能。 来自每个功能的信号为检测和响应威胁的整体能力增加强度。 与运行非 Microsoft 产品相比，组合的功能集为组织（尤其是跨国家/地区组织）提供最佳保护。 下图描述了本文中Microsoft 365 E5防护服务和功能。
...
|Microsoft Defender for Office 365     | 保护组织免受电子邮件、链接和 URL (和协作) 造成的恶意威胁。 恶意软件、网络钓鱼、欺骗和其他攻击类型的保护。 接下来，建议Office 365 Microsoft Defender for Office 365，因为更改控制、从系统迁移设置和其他注意事项可能需要更长时间。 <p>**注意**：请确保配置所有 Office 365 订阅中包含的威胁防护 (Exchange Online Protection) 。       |
|Microsoft Defender for Endpoint    | 有助于预防、检测、调查和响应高级威胁的终结点保护平台。  Defender for Endpoint 可能需要一些时间进行部署，但配置可以与其他功能并行完成。   |
|Microsoft Cloud App Security     |   用于发现、调查和治理的云访问安全代理。 你可以提前Microsoft Cloud App Security开始收集数据和见解。 在 SaaS 应用中实现信息和其他目标保护涉及规划并可能需要更多时间。       | 
