...
治理规划Microsoft 365确保保护关键信息资产，同时将风险最小化。 治理包括几个关键方面，需要规划、协调并调整每个方面，以与不断变化的组织需求和技术演变保持一致。 
...
许多管理决策可通过在管理中打开或关闭功能Microsoft 365。 这有助于强制执行管理标准，但可能帮助组织成员了解哪些条件可供他们使用，以及为什么 (或) 。 

通常，治理团队已创建长文档，概述了每个治理决策。 遗憾的是，这些长文档很少被阅读，这意味着依赖人员强制执行的治理要求很少有效。 在思考如何传达关键管理决策时，尝试避免创建面向多个访问群体的长期文档。 相反，考虑以下备选方法：

- **直接在所创建的解决方案中嵌入治理决策**。 若要确保敏感信息在整个过程中受到保护，Microsoft 365敏感度标签，以确保用户不会意外公开不应公开的信息。 阻止你无法提供适当的保护的应用程序。 [详细了解敏感度标签](../compliance/sensitivity-labels.md)。
...
- 尽量不要被"我们一直在做的工作"中捕获。 云和许多组织的全球化带来了新机遇。 如果假设与业务目标不一致，就具有挑战性。 能否对一些方案或某些用户放宽规则？ 能否使用组织功能获得相同的业务结果Microsoft 365而不是完全阻止活动？
...
除了治理团队外，还有一些其他关键角色或团队需要思考这些角色或团队，以成功Microsoft 365。 某些角色可能由同一个人组合或担任，而其他角色可能不适合所有组织。 大多数组织都有一个Microsoft 365执行发起人，一些组织还具有一个Microsoft 365或仅针对 Intranet 的委员会或团队。 [详细了解中提供的全面管理员](../admin/add-users/admin-roles-page.md)角色和Microsoft 365。
