...
description: 了解如何在 Microsoft Defender 防病毒 Server 2019 Windows Server 2016 Windows和配置配置配置。
...
在某些情况下，Microsoft Defender 防病毒称为 *"Endpoint Protection";* 但是，保护引擎是相同的。 尽管与 Microsoft Defender 防病毒 上的管理Windows 10基本相同，但 Windows [](microsoft-defender-antivirus-in-windows-10.md)Server 上存在一些关键差异：
...
|Setting|说明|
|---|---|
|**0**  - **始终提示**|the Microsoft Defender 防病毒 service prompts you to confirm submission of all required files. 这是 Microsoft Defender 防病毒 默认设置，但不建议在不带 GUI Windows Server 2016 2019 进行安装。|
...
为了帮助确保安全性和性能，根据在 Microsoft Defender 防病毒 或 2019 上使用 Microsoft Defender 防病毒 时安装的角色和功能，Windows Server 2016排除项。
...
|禁用Microsoft Defender 防病毒注册表项的注册表项|若要使用 [DisableAntiSpyware](/windows-hardware/customize/desktop/unattend/security-malware-windows-defender-disableantispyware) 注册表项，请导航到 `HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows Defender` ，并设置或创建名为 的 DWORD 项 `DisableAntiSpyware` 。 将它的值 `1` 设置为 (，这将将注册表项的值设置为 true *) 。*|
