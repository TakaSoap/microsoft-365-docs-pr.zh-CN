...
- 在 Windows Server [中，根据](configure-server-exclusions-microsoft-defender-antivirus.md) 定义的服务器角色应用自动排除项。
- 在 Windows Server 中，如果运行的是其他防病毒产品，Microsoft Defender 防病毒不会自动禁用自身。

## <a name="the-process-at-a-glance"></a>过程概览
...
默认情况下，Microsoft Defender 防病毒已安装且在 Windows Server 上正常运行。 默认情况下， (GUI) 用户界面安装在一些 SGUI 上，但不是必需的，因为您可以使用 PowerShell 或其他方法来管理 Microsoft Defender 防病毒。 如果服务器上未安装 GUI，可以使用"添加角色和功能"向导或 PowerShell cmdlet 进行添加。 
...
可以使用添加角色和功能 **向导** 或 PowerShell 安装 Microsoft Defender 防病毒。

### <a name="use-the-add-roles-and-features-wizard"></a>使用添加角色和功能向导
...
若要验证 Microsoft Defender 防病毒是否正在你的服务器上运行，请运行以下 PowerShell cmdlet：
...
示例提交允许 Microsoft 收集潜在恶意软件的示例。 为了帮助提供持续且最新的保护，Microsoft 研究人员使用这些示例来分析可疑活动并生成更新的反恶意软件安全智能。 我们收集程序可执行文件，如 .exe 文件和 .dll 文件。 我们不会收集包含个人数据的文件，如 Microsoft Word 文档和 PDF 文件。
...
如果你使用非 Microsoft 防病毒产品作为主要防病毒解决方案，请设置 Microsoft Defender 防病毒到被动模式。  
