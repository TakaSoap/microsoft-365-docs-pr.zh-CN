...
8. **仅适用于适用于终结点合作伙伴的 Microsoft Defender。** 同意后，将应用设置为 (租户中可用的多租户) 。 例如 **，** 如果你创建一个旨在在多个客户的租户 (运行的应用，则第三方应用需要) 。 如果创建的 **服务仅在** 租户中运行 (例如，如果创建一个应用程序供自己使用，而该应用程序仅与你自己的数据记录进行交互，则不需要) 。 若要将应用设置为多租户：
