...
本文介绍了如何创建 Azure AD 应用程序、获取 Microsoft Defender for Endpoint 的访问令牌并验证令牌。
...
8. **仅适用于适用于终结点合作伙伴的 Microsoft Defender。** 在获得同意后，将 (设置为可在所有租户中) 。 例如 **，** 如果你创建一个旨在在多个客户的租户 (运行的应用，则第三方应用需要) 。 如果创建的 **服务仅在** 租户中运行 (则不需要这样做，例如，如果创建一个应用程序供自己使用，而该应用程序仅与你自己的数据记录进行交互) 。 若要将应用设置为多租户：
