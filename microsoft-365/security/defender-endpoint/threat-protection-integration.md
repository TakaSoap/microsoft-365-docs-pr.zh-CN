...
我们最近弃用 Azure 信息保护集成，因为我们的 Endpoint DLP 功能包含针对存储在终结点设备上敏感数据的改进的发现和保护解决方案，以便提高解决方案的可见性和集成。 这一点在下面的博客中 [公布](https://techcommunity.microsoft.com/t5/microsoft-defender-for-endpoint/protecting-sensitive-information-on-devices/ba-p/2143555)。 我们建议客户开始使用 Endpoint DLP。
...
Microsoft Defender for Cloud Apps 利用 Microsoft Defender for Endpoint 信号直接查看云应用程序使用情况，包括从所有 Microsoft Defender for Endpoint 受监视设备使用不受支持的云服务 (卷影 IT) 。
...
> [!NOTE]
> Defender for Office 365最近 30 天内的事件显示数据。 对于警报，defender for Office 365数据根据第一次活动时间显示。 此后，数据将不再在 Defender for Office 365。
...
借助 Microsoft 365 Defender、Microsoft Defender for Endpoint 和各种 Microsoft 安全解决方案，形成一个统一的攻破前和入侵后企业防御套件，可跨终结点、标识、电子邮件和应用程序进行本机集成，以检测、阻止、调查和自动响应复杂的攻击。
...
- [Microsoft 365 Defender概述](/microsoft-365/security/defender/microsoft-365-defender)
- [打开 Microsoft 365 Defender](/microsoft-365/security/defender/m365d-enable)
