...
WPR 是 Windows评估和部署工具包 (Windows ADK) 的一部分，可以从下载并安装 Windows [ADK 下载](/windows-hardware/get-started/adk-install)。 也可以下载它作为 Windows 10 SDK 中软件开发工具包Windows 10[的一部分](https://developer.microsoft.com/windows/downloads/windows-10-sdk/)。

可以按照使用 WPR UI 捕获性能日志中的步骤使用 [WPR 用户界面](#capture-performance-logs-using-the-wpr-ui)。

或者，您也可以使用命令行工具 *wpr.exe*，该工具在 Windows 8 和更高版本中可用，具体方法为按照使用 WPR CLI 捕获性能日志 [中的步骤操作](#capture-performance-logs-using-the-wpr-cli)。
...
2. 右键单击"**开始**"菜单图标，Windows PowerShell (**管理员**) 或命令提示符 (**管理员**) 打开管理员命令提示符窗口。
...
8. 在提交到 Microsoft 支持人员时，同时包括 文件和 文件夹。
