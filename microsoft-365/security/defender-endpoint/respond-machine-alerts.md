...
2. 在操作中心飞出中，选择 **可用于** 下载 zip 文件的程序包集合包。
   ...
   :::image type="content" source="images/collect-package.png" alt-text="下载程序包选项" lightbox="images/collect-package.png":::
...
|Windows SMB 会话 (服务器) 块|列出对文件、打印机和串行端口的共享访问，以及网络上节点之间的其他通信。 这可以帮助识别数据外排或横向移动。 <p> 包含 SMBInboundSessions 和 SMBOutboundSession 的文件。 <p> <div class="alert"><b>注意：</b> 如果入站或出站 (会话，) 一个文本文件，告知您未找到 SMB 会话。</div>|
...
选择"运行 **防病毒** 扫描"之一，选择希望快速或完整 (运行扫描类型) 在确认扫描之前添加注释。

:::image type="content" source="images/run-antivirus.png" alt-text="选择快速扫描或完全扫描并添加注释的通知" lightbox="images/run-antivirus.png":::
