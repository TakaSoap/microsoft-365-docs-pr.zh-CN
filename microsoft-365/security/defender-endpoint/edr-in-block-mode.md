...
3. 在结果列表中，在 **AMRunningMode** 行中查找下列值之一：
   - `Normal` - 正常运行的 Defender 服务。 不启用任何特殊模式。
   - `Passive Mode`- 如果你的组织将 Microsoft Defender for Endpoint 与非 Microsoft 防病毒/反恶意软件解决方案一同使用，Microsoft Defender 防病毒自动进入被动模式。  (实时保护和威胁不会由 Microsoft Defender 防病毒.) 
   - `SxS Passive Mode`- 类似于被动模式，但可以选择打开有限定期扫描。
