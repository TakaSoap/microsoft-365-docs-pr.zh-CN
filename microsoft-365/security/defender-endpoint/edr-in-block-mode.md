...
[当终结点检测和响应](overview-endpoint-detection-response.md) (EDR) 在非主要防病毒产品且在被动模式下运行时，Microsoft Defender 防病毒保护，防止恶意项目。 EDR阻止模式在后台工作，以修正由这些功能检测到的EDR项目。 此类项目可能已被主要的非 Microsoft 防病毒产品错过。 对于运行 Microsoft Defender 防病毒 作为主要防病毒的设备，EDR模式中的设备通过允许 Microsoft Defender 防病毒 自动对泄露后的行为检测采取EDR防护。
...
EDR模式中的应用已与威胁& 漏洞管理[集成](next-gen-threat-and-vuln-mgt.md)。 如果组织的安全团队尚未启用EDR，则[](tvm-security-recommendation.md)安全团队会获得一条安全建议，以在阻止模式下打开它。
...
> [!NOTE]
> EDR阻止模式只能打开 Microsoft 365 Defender 门户 () [https://security.microsoft.com](https://security.microsoft.com) 或之前 [https://securitycenter.windows.com](https://securitycenter.windows.com) Microsoft Defender 安全中心 () 。 不能使用注册表项、Microsoft Intune或组策略来启用或禁用EDR阻止模式。
...
- 当Microsoft Defender 防病毒处于活动状态时，EDR阻止模式中进行的操作不提供额外扫描，但它允许 Microsoft Defender 防病毒 对泄露后的行为或行为EDR自动操作。
