...
[当终结点检测和响应](overview-endpoint-detection-response.md) (EDR) 在阻止模式下运行，当终结点Microsoft Defender 防病毒防病毒产品并且正在被动模式下运行时，可针对恶意项目提供额外的保护。 EDR阻止模式在后台工作，以修正由功能检测到的EDR项目。 此类项目可能已被主要的非 Microsoft 防病毒产品错过。 对于运行 Microsoft Defender 防病毒 作为主要防病毒的设备，EDR 模式允许 Microsoft Defender 防病毒 对攻破后的行为和检测采取自动操作，从而提供一层额外的EDR防御。
...
EDR模式中的应用已与威胁& 漏洞管理[集成](next-gen-threat-and-vuln-mgt.md)。 如果组织的安全团队尚未启用EDR，安全[](tvm-security-recommendation.md)团队将获取一个安全建议，以在阻止模式下将其打开。
...
> [!NOTE]
> EDR阻止模式只能打开 Microsoft 365 Defender 门户 () 或之前 [https://security.microsoft.com](https://security.microsoft.com) [https://securitycenter.windows.com](https://securitycenter.windows.com) Microsoft Defender 安全中心 () 。 不能使用注册表项、Microsoft Intune或组策略来启用或禁用EDR阻止模式。

## <a name="requirements-for-edr-in-block-mode"></a>在阻止EDR下运行的要求
...
EDR模式访问不会影响在用户设备上运行的第三方防病毒保护。 EDR如果主防病毒解决方案遗漏了某些内容，或者存在攻破后检测，则阻止模式下的阻止模式有效。 EDR模式配置的工作方式与被动模式下Microsoft Defender 防病毒一样，只不过EDR模式下的阻止模式也会阻止和修正检测到的恶意项目或行为。
...
可以使用 PowerShell 确认在EDR模式下处于打开状态，Microsoft Defender 防病毒被动模式下运行。
