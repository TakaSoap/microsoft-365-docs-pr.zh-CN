...
| [缓解](#apply-additional-mitigations) | 可以停止或帮助降低威胁影响的建议。 本部分还包括作为威胁分析报告的一部分未动态跟踪的缓解。 |
...
对于载入到 Microsoft Defender [for Endpoint](onboard-configure.md)的设备，将引发 EDR 警报。 这些警报通常依赖于 Microsoft Defender for Endpoint 传感器收集的安全信号和其他充当强大信号源的终结点功能（如防病毒、网络保护、防篡改保护）。
