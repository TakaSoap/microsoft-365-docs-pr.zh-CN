---
title: 在 Linux Microsoft Defender for Endpoint部署部署
ms.reviewer: ''
description: 介绍如何使用 Microsoft Defender for Endpoint 在 Linux 上部署应用。
...
ms.locfileid: "64568371"
---
# <a name="deploy-microsoft-defender-for-endpoint-on-linux-with-puppet"></a>在 Linux Microsoft Defender for Endpoint部署部署
...
本文介绍如何使用部署在 Linux 上使用部署适用于终结点的 Defender。 要成功部署，需要完成以下所有任务：
...
## <a name="prerequisites-and-system-requirements"></a>先决条件和系统要求
 ...
 有关当前软件版本的先决条件和系统要求的说明，请参阅 Linux 上的 Defender [for Endpoint 主页面](microsoft-defender-endpoint-linux.md)。
...
3. 选择 **下载载入程序包**。 将文件另存为WindowsDefenderATPOnboardingPackage.zip。
   ...
   :::image type="content" source="images/portal-onboarding-linux-2.png" alt-text="下载已载入程序包的选项" lightbox="images/portal-onboarding-linux-2.png":::
...
你需要创建一个清单，用于将 Linux 上的 Defender for Endpoint 部署到由开发工具服务器管理的设备上。 此示例使用了从 *labs中提供的 apt* 和 *yumrepo* 模块，并假定模块已安装在了您的开发工具服务器上。

在安装 *Install_mdatp模块文件夹* 下install_mdatp */* 文件和清单文件夹。 此文件夹通常位于安装服务器 */etc/moduleslabs/code/environments/production/modules* 中。 将上面创建的 mdatp_onboard.json 文件复制到 *install_mdatp/files* 文件夹。 创建 *init.pp* 包含部署说明的文件：
