...
---
# <a name="deploy-microsoft-defender-for-endpoint-for-linux-with-puppet"></a>部署适用于 Linux 的 Microsoft Defender for Endpoint with 安装
...
本文介绍如何使用部署适用于 Linux 的 Defender for Linux。 成功的部署需要完成以下所有任务：
...
## <a name="prerequisites-and-system-requirements"></a>先决条件和系统要求
 ...
 有关当前软件版本的先决条件和系统要求的说明，请参阅适用于 Linux 的 [Defender for Endpoint 主页](microsoft-defender-endpoint-linux.md)。
...
你需要创建一个显示清单，用于将适用于 Linux 的 Defender for Endpoint 部署到由开发工具服务器管理的设备。 此示例使用了从 *labs中* 提供的 apt 和 *yumrepo* 模块，并假定模块已安装在了您的开发工具服务器上。
...
可以从以下频道之一部署适用于 Linux 的 Defender (表示为 *[channel]* *) ：insiders-fast、insiders-slow* 或 *prod*。每个通道对应于 Linux 软件存储库。
