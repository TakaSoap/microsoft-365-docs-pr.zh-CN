...
| [TVM &威胁 (漏洞管理) ](next-gen-threat-and-vuln-mgt.md) | 威胁 & 漏洞管理是 Microsoft Defender for Endpoint 的一个组件，可为安全管理员和安全运营团队提供独特的价值，包括：- 实时终结点检测和响应 (EDR) 与终结点漏洞相关的见解 - 事件调查期间最有价值的设备漏洞上下文 - 通过 Microsoft Intune 执行内置修正过程 和 Microsoft System Center Configuration Manager。  |
...
| [Microsoft 威胁专家 (MTE) ](microsoft-threat-experts.md) | Microsoft 威胁专家 是一项托管搜寻服务，可为安全操作中心 (SOC) 提供专家级别的监视和分析，以帮助它们确保不会错过独特环境中的关键威胁。      |
