...
Microsoft 建议使用最小特权的概念。 Defender for Endpoint 利用用户内内置Azure Active Directory。 Microsoft [建议查看可用的不同角色](/azure/active-directory/roles/permissions-reference) ，并选择适当的角色来解决你对此应用程序的每个角色的需求。 一些角色可能需要在部署完成后临时应用和删除。
...
- **基本权限管理**：将权限设置为完全访问或只读。 对于基本权限管理，具有 Globa Administrator 或 Security Administrator 角色的用户Azure Active Directory具有完全访问权限，而安全读者角色具有只读访问权限。
...
|Microsoft 威胁专家 (MTE) |Microsoft 威胁专家是一项托管搜寻服务，可为安全操作中心 (SOC) 提供专家级别的监视和分析，以帮助它们确保不会错过独特环境中的关键威胁。 <p> [了解详细信息。](/windows/security/threat-protection/windows-defender-atp/microsoft-threat-experts)|不适用|
