...
示例Power BI模板可用于可用于高级搜寻查询的 Microsoft Defender for Endpoint。 借助这些示例模板（包括用于设备控件的模板）可以将高级搜寻功能集成到Power BI。 有关详细信息[，GitHub模板Power BI存储库](https://github.com/microsoft/MDATP-PowerBI-Templates)。 请参阅[使用自定义Power BI，](/microsoft-365/security/defender-endpoint/api-power-bi)了解有关Power BI集成。
...
使用 Intune，可以通过"允许的服务"蓝牙[使用蓝牙服务](/windows/client-management/mdm/policy-csp-bluetooth#servicesallowedlist-usage-guide)。 默认状态"蓝牙允许的服务"设置意味着允许一切。  一旦添加服务，即成为允许列表。 如果客户添加了 Keyboards 和鼠标值，但未添加文件传输 GUID，应阻止文件传输。
...
使用安全机制Microsoft Defender 防病毒可移动[存储需要启用](/microsoft-365/security/defender-endpoint/configure-real-time-protection-microsoft-defender-antivirus)实时保护或计划扫描，并配置可移动驱动器进行扫描。
...
1. 从 Windows 10版本 1803 开始，Microsoft 引入了[适用于 Thunderbolt 的内核 DMA](/windows/security/information-protection/kernel-dma-protection-for-thunderbolt.md)保护，以提供通过 Thunderbolt 端口抵御 DMA 攻击的本机保护。 适用于 Thunderbolt 的内核 DMA 保护由系统制造商启用，用户无法打开或关闭。
   ...
   从 Windows 10版本 1809 开始，可以通过配置 DMA Guard CSP 来调整内核[DMA 保护级别](/windows/client-management/mdm/policy-csp-dmaguard#dmaguard-deviceenumerationpolicy)。 这是对不支持设备内存隔离的外围设备的额外 (也称为 DMA 重新映射) 。 内存隔离允许操作系统利用设备的 I/O 内存管理单元 (IOMMU) 阻止外围设备内存沙盒 (不允许的 I/O 或内存) 。 换句话说，操作系统向外设分配特定内存范围。 如果外设尝试读取/写入指定范围之外的内存，操作系统将阻止它。

   支持设备内存隔离的外围设备始终可以连接。 只有在用户登录默认登录后才能阻止、允许或允许 (外设) 。
...
可以使用 Microsoft Defender for Endpoint 自定义检测规则创建自定义警报和 [自动响应操作](/microsoft-365/security/defender-endpoint/custom-detection-rules)。 自定义检测中的响应操作涵盖计算机和文件级别操作。 还可使用 Microsoft Defender for Endpoint 连接器创建[Power Apps和](https://powerapps.microsoft.com/)Flow[和自动](https://flow.microsoft.com/)响应[操作](/connectors/wdatp/)。 连接器支持调查、威胁扫描和限制正在运行的应用程序的操作。 它是 200 多个预定义连接器之一，包括 Outlook、Teams、Slack 等。 还可以构建自定义连接器。 请参阅 [连接器](/connectors/) ，详细了解连接器。
