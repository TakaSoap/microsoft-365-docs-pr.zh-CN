...
2. 配置为仅允许或阻止某些可移动设备并防止威胁。
    ...
    2. [通过启用：防止](#prevent-threats-from-removable-storage) 由可移动存储设备引入的可移动存储的威胁：
        - Microsoft Defender 防病毒 RTP (实时) 扫描可移动存储中恶意软件。
...
示例Power BI模板可用于可用于高级搜寻查询的 Microsoft Defender for Endpoint。 借助这些示例模板（包括用于设备控件的模板）可以将高级搜寻功能集成到Power BI。 有关详细信息[，GitHub模板Power BI存储库](https://github.com/microsoft/MDATP-PowerBI-Templates)。 请参阅[使用自定义Power BI，](/microsoft-365/security/defender-endpoint/api-power-bi)详细了解Power BI集成。
...
|[限制使用 蓝牙](#limit-services-that-use-bluetooth)|您可以限制可以使用此蓝牙。|
|
...
![管理员模板列表的屏幕截图。](images/admintemplates.png)
...
![按连接的设备。](images/devicesbyconnection.png)
...
通过 **"阻止安装与这些设备任何** 一个匹配的设备"策略，你可以指定阻止安装Windows设备列表。
...
有关设备 ID 格式的信息，请参阅 [标准 USB 标识符](/windows-hardware/drivers/install/standard-usb-identifiers)。
...
5. 对于 **"仅移动版** (可移动存储和 **USB) ，请选择"阻止****"。** **可移动存储** 包括 USB 驱动器，而 **USB 连接 (移动设备**) USB 充电，但仅包括移动设备上的其他 USB 连接。
...
允许安装的外围设备可以通过其设备实例 [ID 指定](/windows-hardware/drivers/install/device-instance-ids)。 在推出配置之前测试该配置，以确保它支持预期的设备。 理想情况下，测试硬件的各种实例。 例如，测试多个 U 盘，而不是只测试一个 U 盘。
...
禁止安装的外围设备可以通过其设备实例 [ID 指定](/windows-hardware/drivers/install/device-instance-ids)。 在推出配置之前测试该配置，以确保它支持预期的设备。 理想情况下，测试硬件的各种实例。 例如，测试多个 U 盘，而不是只测试一个 U 盘。
...
使用 Intune，可以通过"允许的服务"蓝牙使用蓝牙[服务](/windows/client-management/mdm/policy-csp-bluetooth#servicesallowedlist-usage-guide)。 默认状态"允许蓝牙"设置意味着允许一切。  一旦添加服务，即成为允许列表。 如果客户添加了 Keyboards 和鼠标值，但未添加文件传输 GUID，应阻止文件传输。
...
使用安全机制保护Microsoft Defender 防病毒移动存储需要启用实时保护或[](/microsoft-365/security/defender-endpoint/configure-real-time-protection-microsoft-defender-antivirus)计划扫描并配置可移动驱动器进行扫描。
...
- 如果使用计划扫描，则需要禁用默认启用 (的 DisableRemovableDriveScanning 设置) 以在完全扫描期间扫描可移动设备。 无论 DisableRemovableDriveScanning 设置如何，均可在快速或自定义扫描过程中扫描可移动设备。
...
1. 从 Windows 10版本 1803 开始，Microsoft 引入了[适用于 Thunderbolt 的内核 DMA](/windows/security/information-protection/kernel-dma-protection-for-thunderbolt.md)保护，以提供通过 Thunderbolt 端口抵御 DMA 攻击的本机保护。 适用于 Thunderbolt 的内核 DMA 保护由系统制造商启用，用户无法打开或关闭。
   ...
   从 Windows 10版本 1809 开始，可以通过配置 DMA Guard CSP 来调整内核[DMA 保护级别](/windows/client-management/mdm/policy-csp-dmaguard#dmaguard-deviceenumerationpolicy)。 这是对不支持设备内存隔离的外围设备的额外控制 (也称为 DMA 重新映射) 。 内存隔离允许操作系统利用设备的 I/O 内存管理单元 (IOMMU) 来阻止外设 (内存沙盒阻止不允许的 I/O 或内存) 。 换句话说，操作系统向外设分配特定内存范围。 如果外设尝试读取/写入指定范围之外的内存，操作系统将阻止它。
...
有关设备控制相关高级搜寻事件和如何创建自定义警报的示例的信息，请参阅高级搜寻 [更新：USB](https://techcommunity.microsoft.com/t5/Microsoft-Defender-ATP/Advanced-hunting-updates-USB-events-machine-level-actions-and/ba-p/824152)事件、计算机级操作和架构更改。

## <a name="respond-to-threats"></a>响应威胁

可以使用 Microsoft Defender 终结点自定义检测规则创建自定义警报和 [自动响应操作](/microsoft-365/security/defender-endpoint/custom-detection-rules)。 自定义检测中的响应操作涵盖计算机和文件级别操作。 还可使用 Microsoft Defender for Endpoint[](https://powerapps.microsoft.com/)连接器创建Power Apps和Flow[和自动](https://flow.microsoft.com/)响应[操作](/connectors/wdatp/)。 连接器支持调查、威胁扫描和限制正在运行的应用程序的操作。 它是 200 多个预定义连接器之一，包括 Outlook、Teams、Slack 等。 还可以构建自定义连接器。 请参阅 [连接器](/connectors/) ，详细了解连接器。
...
- [用于自定义Power BI的设备控件模板](https://github.com/microsoft/MDATP-PowerBI-Templates)
