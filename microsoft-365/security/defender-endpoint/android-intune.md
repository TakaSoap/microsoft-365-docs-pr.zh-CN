...
3. 在" *分配"* 部分，转到" **必需"** 部分并选择" **添加组"。** 然后，你可以选择要 (Android) Defender for Endpoint 的用户组策略。 选择 **"选择**"，然后选择"下一 **步"。**
...
3. 成功载入后，设备将开始显示在 Microsoft Defender 安全中心 中的"设备"列表中。
...
9. Defender for Endpoint 通过 Intune 支持托管设备的应用配置策略。 此功能可用于自动授予适用的 Android () ，因此最终用户无需接受这些 (权限) 。
    ...
    1. 在" **创建应用配置策略"** 页中，输入以下详细信息：
        ...
        > [!div class="mx-imgBorder"]
        > ![创建应用配置策略页面的图像。](images/android-create-app.png)
