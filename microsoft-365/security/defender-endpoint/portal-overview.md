...
---
# <a name="microsoft-defender-security-center-portal-overview"></a>Microsoft Defender 安全中心门户概述
...
企业安全团队可以使用 Microsoft Defender 安全中心监视和协助响应潜在高级永久性威胁活动或数据泄露的警报。

可以使用 [Microsoft Defender 安全中心](https://securitycenter.windows.com/) ：
...
-  (2) 搜索， 社区中心， 本地化， 帮助和支持， 反馈

 ![适用于终结点的 Microsoft Defender 门户](images/mdatp-portal-overview.png)

> [!NOTE]
> 仅在你的设备将 Microsoft Defender 防病毒用作默认的反恶意软件实时保护产品时，才显示与恶意软件相关的检测。

可以使用所有部分中可用的菜单选项在门户中导航。 有关每个部分的说明，请参阅下表。

区域 | 说明
:---|:---
**(1) 导航窗格** | 使用导航窗格在仪表板、事件、**设备** 列表、**警报** 队列、**自动** 调查、高级搜寻、**报告**、合作伙伴 &  **API、** 威胁 &**漏洞** 管理、评估和教程、**服务** 运行状况、配置管理和设置之间移动。   选择导航窗格顶部的水平线以显示或隐藏它。
**仪表板** | 访问活动的自动调查、活动警报、自动调查统计信息、处于风险中的设备、处于风险中的用户、传感器问题的设备、服务运行状况、检测源和每日设备报告仪表板。
**事件** | 查看已聚合为事件的警报。
...
**合作伙伴& API** | 查看受支持的合作伙伴连接，以增强平台的检测、调查和威胁智能功能。 还可以查看连接的应用程序、API 资源管理器、API 使用情况概述和数据导出设置。
...
**设置** | 显示你在载入期间选择的设置，并允许你更新你的行业首选项和保留策略期限。 还可以设置其他配置设置，如权限、API、规则、设备管理、IT 服务管理和网络评估。
**(2) 搜索， 社区中心， 本地化， 帮助和支持， 反馈** | **搜索** - 按设备、文件、用户、URL、IP、漏洞、软件和建议进行搜索。 </br></br> **社区中心** - 访问社区中心，了解、协作和共享产品体验。 </br></br>  **本地化** - 设置时区。 </br></br>  **帮助和支持** - 访问适用于终结点的 Defender 指南、Microsoft 和 Microsoft Premier 支持、许可证信息、模拟 & 教程、适用于终结点的 Defender 评估实验室，请咨询威胁专家。</br></br> **反馈** - 提供有关您喜欢或我们可以更好地执行哪些工作的评论。

> [!NOTE]
> 对于具有高分辨率 DPI 缩放问题的设备，请参阅适用于高 DPI 设备的 [Windows](https://support.microsoft.com/help/3025083/windows-scaling-issues-for-high-dpi-devices) 缩放问题，了解可能的解决方案。
...
Icon | 说明
:---|:---
![ATP 徽标图标](images/atp-logo-icon.png)| 适用于终结点的 Microsoft Defender 徽标
![警报图标](images/alert-icon.png)| 警报 – 与高级攻击相关的活动指示。
![检测图标](images/detection-icon.png)| 检测 – 恶意软件威胁检测的指示。
![活动威胁图标](images/active-threat-icon.png)| 活动威胁 – 检测时主动执行的威胁。
![修正图标1](images/remediated-icon.png)| 已修复 – 已从设备中删除的威胁。
![未修复的图标](images/not-remediated-icon.png)| 未修复 – 未从设备中删除的威胁。
![Thunderbolt 图标](images/atp-thunderbolt-icon.png)| 指示在警报进程树中触发 **警报的事件**。
![设备图标](images/atp-machine-icon.png)| 设备图标
![Microsoft Defender AV 事件图标](images/atp-windows-defender-av-events-icon.png)| Microsoft Defender 防病毒事件
![应用程序防护事件图标](images/atp-Application-Guard-events-icon.png)| Windows Defender应用程序防护事件
![Device Guard 事件图标](images/atp-Device-Guard-events-icon.png)| Windows Defender Device Guard 事件
![攻击防护事件图标](images/atp-Exploit-Guard-events-icon.png)| Windows Defender攻击防护事件
![SmartScreen 事件图标](images/atp-Smart-Screen-events-icon.png)| Windows Defender SmartScreen 事件
![防火墙事件图标](images/atp-Firewall-events-icon.png)| Windows 防火墙事件
...
![社区中心图标](images/atp-community-center.png) | 社区中心
![通知图标](images/atp-notifications.png) | 通知
![未发现威胁](images/no-threats-found.png) | 自动调查 - 未找到威胁
![失败图标](images/failed.png) | 自动调查 - 失败
![部分修复的图标](images/partially-investigated.png) | 自动调查 - 部分调查
![已由系统终止](images/terminated-by-system.png) | 自动调查 - 由系统终止
![挂起图标](images/pending.png) | 自动调查 - 挂起
![正在运行图标](images/running.png) | 自动调查 - 运行
![修正的图标2](images/remediated.png) | 自动调查 - 已修正
