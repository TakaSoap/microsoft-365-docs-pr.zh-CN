...
- [在设备上使用管理 Microsoft Defender for Endpoint Microsoft Endpoint Manager](/mem/intune/protect/mde-security-integration)
...
- Azure AD连接以同步计算机对象。 默认情况下，计算机 US 在连接Azure AD作用域中。 如果计算机对象属于特定组织单位 (组织单位) ，请配置要同步到Azure AD 连接。 若要了解有关如何使用计算机对象同步计算机对象Azure AD 连接，请参阅基于组织[单位的筛选](/azure/active-directory/hybrid/how-to-connect-sync-configure-filtering#organizational-unitbased-filtering)。
...
在 **终结点** \> **设备清单中**，"托管者"列已添加到按管理通道 (例如 MEM) 。
...
3. 将显示 **"编辑入站同步规则** "窗口。 更新规则说明，注意Windows Server 2012R2 将使用此规则进行同步。 除"优先级"值外，保留所有其他选项不变。 为"优先级"输入一个值，该值高于原始规则 (如规则列表中的值) 。
