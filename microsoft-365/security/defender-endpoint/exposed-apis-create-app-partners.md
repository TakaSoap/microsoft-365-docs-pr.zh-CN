...
此页面介绍如何创建 Azure AD Azure Active Directory (应用程序) 客户以编程方式访问 Microsoft Defender for Endpoint。
...
- 复制/粘贴应用程序中的以下 (请不要忘记更新三个变量：、和 `tenantId` `appId` `appSecret`) 
...
- 将你发送的 Http 请求中的授权标头设置为"Bearer {token}" (Bearer 是授权方案) 
