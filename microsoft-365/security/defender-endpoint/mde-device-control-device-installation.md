...
> [!NOTE]
> "阻止由其他策略设置 **描述** 的设备安装"策略设置已替换为支持的目标 Windows 10版本的"允许"和"阻止跨所有设备匹配条件策略"的"应用分层评估顺序"策略设置。 建议尽可能在所有设备匹配 **条件** 策略设置中对"允许"和"阻止设备安装策略"使用应用分层评估顺序。

### <a name="allow-installation-of-devices-that-match-any-of-these-device-instance-ids"></a>允许安装与这些设备实例任何一个匹配的设备 

此策略设置允许你为允许安装的设备指定即Windows设备实例 ID 的列表。 此策略设置仅在启用"跨所有设备匹配条件应用设备安装策略的分层评估顺序"策略设置时使用。 
...
此策略设置允许你为允许安装的驱动程序包指定设备设置 (全局唯一标识符) GUID Windows GUID。 此策略设置仅在启用"跨所有设备匹配条件应用设备安装策略的分层评估顺序"策略设置时使用。
...
此策略设置允许你为阻止安装 (的驱动程序包 (Windows GUID) 设备设置类全局唯一标识符列表。 默认情况下，此策略设置优先于任何其他允许Windows安装设备的策略设置。
