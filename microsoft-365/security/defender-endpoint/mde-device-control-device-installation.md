...
> [!NOTE]
> "阻止由其他策略设置描述的设备安装"策略设置已替换为受支持的目标 Windows 10版本和 Windows 11 的所有设备匹配条件策略的"允许"和"阻止设备安装策略"的"应用分层评估顺序"。 建议尽可能在所有设备匹配 **条件** 策略设置中对"允许"和"阻止设备安装策略"使用应用分层评估顺序。
...
此策略设置允许你为允许安装的驱动程序包指定设备设置 (全局唯一标识符) GUID Windows。 此策略设置仅在启用"跨所有设备匹配条件应用设备安装策略的分层评估顺序"策略设置时使用。
...
1. 配置 **使用与这些设备设置类匹配的驱动程序阻止安装设备**。
    ...
    - 打开终结点安全>攻击面减少>创建策略>平台：Windows 10 (及) &配置文件：设备控制。
