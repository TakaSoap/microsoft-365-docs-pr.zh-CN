...
description: 如有必要，请获取 (进一步) Microsoft Defender for Endpoint 服务报告的所有事件的说明和进一步疑难解答步骤。
...
3. 服务记录的事件将显示在日志中。 有关服务记录的事件的列表，请参阅下表。
   ...
   ****
   ...
   |19|OOBE (Windows欢迎) 尚未完成。|只有在任何更新完成安装Windows服务才能启动。|正常操作通知;无需任何操作。 <p> 如果在系统重新启动后此错误仍然存在，请确保所有Windows更新都已安装完整。|
   |20|无法等待 OOBE (Windows欢迎) 完成。 失败代码 `variable` ：。|内部错误。|如果在系统重新启动后此错误仍然存在，请确保所有Windows更新都已安装完整。|
   ...
   |27|Microsoft Defender for Endpoint 服务在客户端中无法启用 SENSE 感知Microsoft Defender 防病毒。 载入过程失败。 失败代码 `variable` ：。|通常，Microsoft Defender 防病毒如果另一个实时反恶意软件产品在设备上正常运行，并且设备向 Defender for Endpoint 报告，则此设备将进入特殊的被动状态。|检查载入设置和脚本是否正确部署。 尝试重新部署配置包。 <p> 请参阅[载入Windows 10设备](configure-endpoints.md)。 <p> 确保实时反恶意软件保护运行正常。|
   ...
   |30|Microsoft Defender for Endpoint 服务在终结点服务中无法禁用 SENSE 感知Microsoft Defender 防病毒。 失败代码 `variable` ：。|通常，Microsoft Defender 防病毒如果另一个实时反恶意软件产品在设备上正常运行，并且设备向 Defender for Endpoint 报告，则此设备将进入特殊的被动状态。|检查载入设置和脚本是否正确部署。 尝试重新部署配置包。 <p> 请参阅[载入Windows 10设备](configure-endpoints.md)。 <p> 确保实时反恶意软件保护运行正常。|
