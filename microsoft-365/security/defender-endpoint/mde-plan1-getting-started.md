...
通过Microsoft 365 Defender门户 () 查看有关检测到的威胁的信息、管理警报和事件、对检测到的威胁采取任何所需操作以及 [https://security.microsoft.com](https://security.microsoft.com) 管理设备。 在Microsoft 365 Defender门户中，你可以开始使用 Defender for Endpoint Plan 1 (预览版获得的威胁防护) 。 以下各节介绍如何开始：
...
> [!NOTE]
> 本文中所示的示例可能不同于在门户中Microsoft 365 Defender的内容。 在门户中看到的内容取决于许可证和权限。 此外，安全团队可以通过添加、删除和重新设置卡片来自定义组织的门户。
...
| **运行状况**  > **消息中心** | 导航到消息中心中的Microsoft 365 管理中心。 消息中心提供有关计划更改的信息。 每条消息都描述即将发生的变化、它可能会如何影响用户以及如何管理更改。 |  
...
若要访问威胁防护报告，在Microsoft 365 Defender门户中，选择"报告"，然后选择"**威胁防护"。**  威胁防护报告显示警报趋势、状态、类别等。 视图按两列排列： **警报趋势** 和 **警报状态**，如下图所示：
