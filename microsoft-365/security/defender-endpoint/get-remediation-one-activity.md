...
ms.locfileid: "62073386"
---
# <a name="get-one-remediation-activity-by-id"></a>按 ID 获取修正活动
...
completerId|字符串|如果修正活动是由某人手动完成的，则此列包含其对象 ID|NULL
completionMethod|String|如果所有 (设备都由选择"标记为已完成") 或"手动"进行修补，则修正活动可以"自动"完成。|自动
createdOn|DateTime|创建此修正活动的时间|2021-01-12T18：54：11.5499478Z
说明|String|此修正活动的说明|将 Microsoft Silverlight 更新到更高版本，以减少影响设备的已知漏洞。
dueOn|DateTime|此修正活动的创建者设置的截止日期|2021-01-13T00：00：00Z
...
productivityImpactRemediationType|字符串|只能请求对不影响用户的设备进行一些配置更改。 此值指示"所有公开的设备"或"仅不会影响用户的设备"之间的选择。|AllExposedAssets
rbacGroupNames|字符串|相关设备组名称|[ "Windows Servers"、"Windows 11"、"Windows 10" ]
recommendedProgram|字符串|要升级到的推荐程序|NULL
recommendedVendor|字符串|建议升级到的供应商|NULL
recommendedVersion|字符串|要更新/升级到的建议版本|NULL
relatedComponent|String|此修正活动的相关组件 (安全建议计划的相关组件) |Microsoft Silverlight
...
requesterNotes|String|注释 (此) 活动添加的自定义文本|NULL
Scid|字符串|相关安全建议 SCID|NULL
Status|String|修正活动状态 (/已完成) |活动
statusLastModifiedOn|DateTime|更新状态字段的日期|2021-01-12T18：54：11.5499487Z
