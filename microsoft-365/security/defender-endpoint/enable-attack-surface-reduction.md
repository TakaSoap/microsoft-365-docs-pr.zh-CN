...
> [!TIP]
> 若要了解有关许可Windows，请参阅Windows 10[许可](https://www.microsoft.com/licensing/product-licensing/windows10?activetab=windows10-pivot:primaryr5)并获取适用于 Windows 10 [的批量许可Windows 10](https://download.microsoft.com/download/2/D/1/2D14FE17-66C2-4D4C-AF73-E122930B60F6/Windows-10-Volume-Licensing-Guide.pdf)。
...
可以使用文件夹路径或完全限定的资源 (指定单个文件或文件夹) ，但无法指定排除项所应用的规则。 仅在已排除的应用程序或服务启动时应用排除。 例如，如果为已在运行的更新服务添加排除项，则更新服务将继续触发事件，直到停止并重新启动该服务。
...
2. 在终结点 **保护窗格中**，选择"**Windows Defender攻击防护"**，然后选择"**攻击面减少"**。 选择每个 ASR 规则所需的设置。
...
2. 在 **"创建配置文件"** 中的以下两个下拉列表中，选择以下选项：
   ...
   - 在 **"配置文件类型**"中， **选择"模板"**
   - 如果已经通过终结点安全性设置了 ASR 规则，请在"配置文件类型"中选择"设置 **目录"**。
