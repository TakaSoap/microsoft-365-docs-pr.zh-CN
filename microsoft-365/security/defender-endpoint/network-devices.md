...
1. 确定具有与计划扫描 (网络设备的管理端口的网络连接的 () 或服务器客户端或服务器终结点的 Defender。 
...
显示结果后，你可以选择哪些设备将包含在定期扫描中。 如果跳过查看扫描结果，则所有配置的 IP 地址都将添加到网络评估作业 (无论设备的响应) 。 也可以导出扫描结果。
