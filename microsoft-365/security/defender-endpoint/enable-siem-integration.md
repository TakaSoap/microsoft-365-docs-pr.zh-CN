...
在 SIEM (启用安全信息和事件) 集成，以便你可以从 Microsoft Defender 安全中心拉取检测。 使用 SIEM 解决方案或直接连接到检测 REST API 拉取检测。
...
2. 选择 **启用 SIEM 集成**。 这会使用预填充的值激活 **SIEM** 连接器访问详细信息部分，并且应用程序是在 Azure AD 租户的 Azure Active Directory (创建的) 部分。
...
你现在可以继续配置 SIEM 解决方案，或者通过编程访问连接到检测 REST API。 配置 SIEM 解决方案时，你需要使用令牌，以允许它接收来自 Microsoft Defender 安全中心的检测。
