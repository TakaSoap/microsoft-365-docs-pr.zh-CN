...
> [!NOTE]
> R2 Windows Server 2012篡改保护适用于使用新式统一解决方案包载入的设备。 有关详细信息，请参阅适用于 [Windows Server 2012 R2 和 2016 Preview](/microsoft-365/security/defender-endpoint/configure-server-endpoints#new-functionality-in-the-modern-unified-solution-for-windows-server-2012-r2-and-2016-preview) 的新式统一解决方案中的新功能。
...
篡改保护不会阻止您查看安全设置。 而且，防篡改保护不会影响非 Microsoft 防病毒应用向 Windows 安全中心 注册。 如果您的组织使用 Windows 10 企业版 E5，则单个用户不能更改篡改保护设置;在这种情况下，防篡改保护由安全团队进行管理。
...
|打开或关闭 (设备的) 篡改保护|[在单个设备上管理篡改保护](#manage-tamper-protection-on-an-individual-device)|
...
- 目前，对于新部署，Microsoft 365 Defender门户中管理篡改保护的选项处于打开状态。 对于现有部署，可选择性地使用防篡改保护。 若要选择加入，在 Microsoft 365 Defender <a href="https://go.microsoft.com/fwlink/p/?linkid=2077139" target="_blank">门户中</a>，\>设置 **终结点** \> **高级功能** \> **篡改保护**"。

- 当你使用 Microsoft 365 Defender 门户管理篡改保护时，你不需要使用 Intune 或租户附加方法。

- 在 Microsoft 365 Defender 门户中管理篡改保护时，该设置将应用于租户范围，从而影响运行 Windows 10、Windows 10 企业版 多会话、Windows 11、Windows 11 Enterprise 多会话、Windows Server 2012 R2、Windows Server 2016、Windows Server 2019 或 Windows Server 2022。 若要微调防篡改保护 (例如某些设备对防篡改保护，但对另一些设备) ，请使用带租户附加Microsoft Endpoint Manager或 [Configuration Manager](#manage-tamper-protection-for-your-organization-with-configuration-manager-version-2006)。[](#manage-tamper-protection-for-your-organization-using-microsoft-endpoint-manager)
...
- 你的设备必须 [载入到 Microsoft Defender for Endpoint](/microsoft-365/security/defender-endpoint/onboarding)。

- 你的设备必须使用反恶意软件平台 `4.18.2010.7` 版本 (或 `1.1.17600.5`) 或 (或) 。  ([管理Microsoft Defender 防病毒更新并应用比较基准](manage-updates-baselines-microsoft-defender-antivirus.md)。) 
...
:::image type="content" source="../../media/mde-turn-tamperprotectionon.png" alt-text="在门户中打开防篡改Microsoft 365 Defender功能" lightbox="../../media/mde-turn-tamperprotectionon.png":::

1. 转到"Microsoft 365 Defender门户 () [https://security.microsoft.com](https://security.microsoft.com) 并登录。

2. 选择 **设置** \> **终结点"**。

3. 转到常规 **高级** \> **功能**，然后打开防篡改保护。
...
如果你的组织使用 Microsoft Endpoint Manager (MEM) 可以在 (管理中心中为组织打开或) Microsoft Endpoint Manager防 () [https://endpoint.microsoft.com](https://endpoint.microsoft.com) 。 当你想要微调防篡改保护设置时，请使用 Intune。 例如，如果你希望在某些设备上（而不是所有设备）启用篡改保护，请使用 Intune。

### <a name="requirements-for-managing-tamper-protection-in-endpoint-manager"></a>管理篡改保护在Endpoint Manager

- 你的设备必须 [载入到 Microsoft Defender for Endpoint](/microsoft-365/security/defender-endpoint/onboarding)。

- 必须分配有 [适当的](/microsoft-365/security/defender-endpoint/assign-portal-access) 权限，例如全局管理员、安全管理员或安全操作。

- 你的组织[Microsoft Endpoint Manager管理设备](/mem/endpoint-manager-getting-started)。  (Microsoft Endpoint Manager (MEM) 许可证是必需的;MEM 包含在 Microsoft 365 E3/E5、企业移动性 + 安全性 E3/E5、Microsoft 365 商业高级版、Microsoft 365 F1/F3、Microsoft 365 政府版 G3/G5 以及相应的教育许可证中。) 

- 你的Windows设备必须运行 Windows 11 [1709](/windows/release-health/status-windows-10-1709) Windows 10 [1803](/windows/release-health/status-windows-10-1803)、[1809](/windows/release-health/status-windows-10-1809-and-windows-server-2019) 或更高版本。  (有关版本详细信息，请参阅Windows 10[发布信息](/windows/release-health/release-information)。) 

- 必须使用安全Windows更新到版本 1.287.60.0 或 (或) 。[](https://www.microsoft.com/wdsi/definitions)

- 你的设备必须使用反恶意软件平台版本 4.18.1906.3 (或) 及 (`1.1.15500.X` 或) 以上版本的反恶意软件引擎。  ([管理Microsoft Defender 防病毒更新并应用比较基准](manage-updates-baselines-microsoft-defender-antivirus.md)。) 

### <a name="turn-tamper-protection-on-or-off-in-microsoft-endpoint-manager"></a>打开或关闭防 (或) 篡改Microsoft Endpoint Manager

:::image type="content" source="images/turnontamperprotectinmem.png" alt-text="使用 Intune 打开防篡改保护" lightbox="images/turnontamperprotectinmem.png":::

1. 在"[Microsoft Endpoint Manager中心](https://go.microsoft.com/fwlink/?linkid=2109431)"中，转到"**终结点安全** \> **防病毒**"，然后选择"**+ 创建策略"**。
...
如果你使用的是 Configuration [Manager 版本 2006](/mem/configmgr/core/plan-design/changes/whats-new-in-version-2006)，可以在 Windows 10、Windows 10 企业版 多会话、Windows 11、Windows 11 Enterprise上管理防篡改保护设置。Windows Server 2012租户附加Windows Server 2016，Windows R2、Windows Server 2019 和 Windows Server 2022 *。* 租户附加使你能够将仅本地 Configuration Manager 设备同步到 Microsoft Endpoint Manager 管理中心，然后将终结点安全配置策略&设备。
...
2. 在"[Microsoft Endpoint Manager中心](https://go.microsoft.com/fwlink/?linkid=2109431)"中，转到"**终结点安全** \> **防病毒**"，然后选择"**+ 创建策略"**。

   - 在"**平台"** 列表中，Windows 10 **、Windows 11和 Windows Server (ConfigMgr)**。
   - 在配置文件 **列表中**，选择Windows 安全中心 **预览 (体验)**。
...
> [!NOTE]
> 篡改保护会阻止尝试Microsoft Defender 防病毒注册表修改这些设置。
>
...
:::image type="content" source="images/tamperprotectionturnedon.png" alt-text="打开防篡改保护Windows 10 家庭版" lightbox="images/tamperprotectionturnedon.png":::
...
如果你使用的是 Windows Server 2016、Windows 10 1709、1803 或 [1809](/windows/release-health/status-windows-10-1809-and-windows-server-2019) 版本，则将不会在 Windows 安全中心 应用中看到防篡改保护。 相反，可以使用 PowerShell 确定是否已启用防篡改保护。
...
使用 Microsoft Defender [for](overview-endpoint-detection-response.md) Endpoint 中的[](advanced-hunting-overview.md)终结点检测和响应以及高级搜寻功能，安全运营团队可以调查和处理此类尝试。

## <a name="review-your-security-recommendations"></a>查看安全建议

防篡改保护与 [威胁&漏洞](next-gen-threat-and-vuln-mgt.md) 管理功能集成在一起。 [安全建议](tvm-security-recommendation.md) 包括确保打开防篡改保护。 例如，您可以 *搜索篡改。* 在结果中，可以选择" **打开防** 篡改保护"了解更多信息并打开它。

:::image type="content" source="images/tamperprotectsecurityrecos.png" alt-text="在门户中打开防篡改Microsoft Defender 安全中心保护" lightbox="images/tamperprotectsecurityrecos.png":::

若要了解有关威胁和漏洞&，请参阅仪表板[见解 -](tvm-dashboard-insights.md#dashboard-insights---threat-and-vulnerability-management) 危险和漏洞管理。

## <a name="frequently-asked-questions"></a>常见问题解答

### <a name="on-which-versions-of-windows-can-i-configure-tamper-protection"></a>我可以在哪些版本的Windows配置篡改保护？

Windows 10操作系统 [1709](/windows/release-health/status-windows-10-1709)、[1803](/windows/release-health/status-windows-10-1803)、[1809](/windows/release-health/status-windows-10-1809-and-windows-server-2019) 或更高版本以及 [Microsoft Defender for Endpoint](/microsoft-365/security/defender-endpoint)。
...
如果使用的是具有租户附加的 Configuration Manager 版本 2006，则防篡改保护可以扩展到 Windows Server 2012 R2、Windows Server 2016、Windows Server 2019 和 Windows Server 2022。 请参阅 [租户附加：从管理中心 ](/mem/configmgr/tenant-attach/deploy-antivirus-policy)创建和部署终结点安全防病毒策略 (预览) 。

### <a name="will-tamper-protection-affect-non-microsoft-antivirus-registration-in-the-windows-security-app"></a>防篡改保护是否会影响非 Microsoft 防病毒注册在 Windows 安全中心 应用中？
...
载入到 Microsoft Defender for Endpoint 的设备将Microsoft Defender 防病毒被动模式运行。 在这些情况下，防篡改保护将继续保护服务及其功能。
...
如果你是使用 [Microsoft Defender for Endpoint 的组织](/microsoft-365/security/defender-endpoint)，你应该能够在 Intune 中管理篡改保护，类似于管理其他终结点保护功能的方式。 请参阅本文的以下部分：

- [使用工具管理防篡改Microsoft Endpoint Manager](#manage-tamper-protection-for-your-organization-using-microsoft-endpoint-manager)
- [使用管理门户管理Microsoft 365 Defender保护](#manage-tamper-protection-for-your-organization-using-the-microsoft-365-defender-portal)

### <a name="how-does-configuring-tamper-protection-in-intune-affect-how-i-manage-microsoft-defender-antivirus-with-group-policy"></a>在 Intune 中配置篡改保护将如何影响我Microsoft Defender 防病毒组策略？
