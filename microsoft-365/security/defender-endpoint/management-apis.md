...
当你在 SIEM) 集成中启用安全 (和事件管理时，它允许你使用 SIEM 解决方案或直接连接到检测 REST API 从 Microsoft Defender 安全中心 拉取检测。 这会使用预填充的值激活 SIEM 连接器访问详细信息部分，并且应用程序是在 Azure AD Azure Active Directory (租户) 创建的。 有关详细信息，请参阅 [SIEM 集成](enable-siem-integration.md)。
