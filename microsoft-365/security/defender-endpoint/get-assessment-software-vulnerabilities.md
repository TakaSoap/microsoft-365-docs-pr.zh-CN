...
SoftwareVendor | string | 软件供应商的名称。 | google  
SoftwareVersion | string | 软件产品的版本号。 | 81.0.4044.138  
状态 | String | **新建**   (如果设备上不再存在此漏洞（这意味着已修复此漏洞)  (1) **修复** (，则说明在设备上引入的新漏洞  ) 。  (2) 更新 (如果设备上 ****   漏洞已更改。 可能的更改包括：CVSS 分数、攻击性级别、严重性级别、DiskPaths、RegistryPaths、RecommendedSecurityUpdate) 。 | Fixed
VulnerabilitySeverityLevel | string | 根据 CVSS 分数和受威胁环境影响的动态因素为安全漏洞分配的严重性级别。 | 中  
