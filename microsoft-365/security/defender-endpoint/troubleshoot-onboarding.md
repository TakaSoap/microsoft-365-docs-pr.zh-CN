...
|`15`|无法启动 SENSE 服务|检查服务运行状况 (`sc query sense` 命令) 。 请确保它未在中间状态 ("Pending_Stopped"、"Pending_Running") 并尝试使用管理员权限 (再次运行) 。  <p> 如果设备在运行Windows 10版本 1607 并运行命令 `sc query sense` ，则 `START_PENDING` 重新启动设备。 如果重新启动设备无法解决问题，请升级到 KB4015217 并再次尝试载入。|
|`15`|无法启动 SENSE 服务|如果错误的消息是：系统错误 577 或错误 1058 已发生，你需要启用 Microsoft Defender 防病毒 ELAM 驱动程序，请参阅确保策略[](#ensure-that-microsoft-defender-antivirus-is-not-disabled-by-a-policy)未禁用 Microsoft Defender 防病毒，了解说明。|
...
|0x87D101A9|-2016345687|SyncML (425) ：请求的命令失败，因为发件人对收件人没有足够的访问控制权限 (ACL) 权限。|全部|**可能的原因：** 尝试在不支持的 SKU/平台上部署 Microsoft Defender for Endpoint，尤其是全息 SKU。<p> 当前支持的平台： <p> Enterprise、教育以及Professional。|
|
...
|情况|症状|可能的原因和疑难解答步骤|
|:---:|---|---|
|`1`|设备符合 SenseIsRunning OMA-URI。 但不符合 OrgId、Onboarding 和 OnboardingState OMA-URI。|**可能的原因：** 检查用户在安装或升级后是否Windows OOBE。 在 OOBE 载入期间无法完成，但 SENSE 已在运行。 <p> **疑难解答步骤：** 等待 OOBE 完成。|
...
6. 可指示问题的事件将显示在"操作" **窗格中** 。 您可以尝试根据下表中的解决方案进行疑难解答：
   ...
   ****
   ...
   |`30`|在 Microsoft Defender for Endpoint ($ (build.sense.productDisplayName) 模式失败。 失败代码：%1|请联系支持人员。|
...
> [!IMPORTANT]
> 从 2020 年 8 月 (版本 `disableAntiSpyware` 4.18.2007.8 开始，该设置将在所有 Windows 10 设备上都被忽略，) 更新到 Microsoft Defender 防病毒。
...
1. 在应用程序中创建Microsoft Endpoint Configuration Manager。
    ...
    ![配置 1 Microsoft Endpoint Configuration Manager的图像。](images/mecm-1.png)
...
9. 在 **检测方法** 中，**选择"配置规则以检测此部署类型是否存在"，** 然后选择"**添加子句"。**
    ...
    ![Microsoft Endpoint Configuration Manager configuration9 的图像。](images/mecm-9.png)
...
15. 在 **"摘要"中**，选择"下一 **步"。**
    ...
    ![Microsoft Endpoint Configuration Manager配置15 的图像。](images/mecm-15.png)
...
25. 在 **用户体验中，** 选择"在截止时间或维护时段提交更改 (需要重新启动) ，然后选择"下 **一****步"。**
...
28. 在 **"完成"** 中，选择"**关闭"。**
    ...
    ![Microsoft Endpoint Configuration Manager configuration30 的图像。](images/mecm-30.png)
