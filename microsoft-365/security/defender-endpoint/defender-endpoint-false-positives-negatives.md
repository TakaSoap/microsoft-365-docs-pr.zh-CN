...
幸运的是，可以采取措施来解决并减少此类问题。 如果在邮件中看到误报/负Microsoft 365 Defender，安全操作可以通过[](/microsoft-365/security/defender/microsoft-365-defender)以下过程采取一些措施解决这些问题：
...
3. 选择警报以了解有关警报的更多详细信息。  (请参阅[查看 Microsoft Defender for Endpoint.) ](review-alerts.md)
...
> [!TIP]
> 有关抑制警报的信息，请参阅管理 [适用于终结点的 Microsoft Defender 警报](/microsoft-365/security/defender-endpoint/manage-alerts)。 此外，如果你的组织正在使用 SIEM (安全信息和事件) ，请确保也定义抑制规则。
...
[修正操作](manage-auto-investigation.md#remediation-actions)（例如，将文件发送到隔离或停止进程）对 (实体（如) 检测到威胁的文件）执行。 多种类型的修正操作自动通过自动调查和Microsoft Defender 防病毒：
...
2. 选择 **"终结点安全** \> **防病毒"**，然后选择现有策略。  (如果没有现有策略，或者想要创建新策略，请 [跳至下](#use-microsoft-endpoint-manager-to-create-a-new-antivirus-policy-with-exclusions) 一步) 。

3. 选择 **"属性**"，在" **配置设置"旁边**，选择"编辑 **"**。

4. 展开 **Microsoft Defender 防病毒排除** 项"，然后指定排除项。
...
[尤其是 (](/microsoft-365/security/defender-endpoint/manage-indicators) 威胁指示器或 IoCs) 使安全运营团队能够定义实体的检测、防护和排除。 例如，可以在 Microsoft Defender for Endpoint 的扫描和修正操作中指定要省略的某些文件。 或者，指示器可用于对某些文件、IP 地址或 URL 生成警报。
...
自定义网络指示器在自定义[Microsoft 365 Defender。](/microsoft-365/security/defender/microsoft-365-defender) 若要了解更多信息，请参阅 [高级功能](/microsoft-365/security/defender-endpoint/advanced-features)。
...
2. 访问Microsoft 安全智能提交网站 (<https://www.microsoft.com/wdsi/filesubmission>) ，然后提交你的 (提交) 。

### <a name="submit-a-fileless-detection-for-analysis"></a>提交无文件检测进行分析

如果根据行为检测到恶意软件，并且您没有文件，您可以提交文件 `Mpsupport.cab` 进行分析。 您可以使用 Microsoft *恶意软件*.cab工具 (MPCmdRun.exe Command-Line 工具获取) 或 Windows 10 Windows 11。
...
4. 访问Microsoft 安全智能提交网站 () <https://www.microsoft.com/wdsi/filesubmission> ，并提交.cab文件。
...
我们建议你[Microsoft Endpoint Manager](/mem/endpoint-manager-overview)或设置云保护设置;但是，可以使用其他方法，如组策略 (请参阅管理 [Microsoft Defender for Endpoint](manage-mde-post-migration.md)。[](/azure/active-directory-domain-services/manage-group-policy)
...
2. 选择 **"终结点安全** \> **防病毒"** ，然后选择现有策略。  (如果没有现有策略，或者想要创建新策略，请 [跳至下](#use-microsoft-endpoint-manager-to-set-cloud-delivered-protection-settings-for-a-new-policy) 一步) 。
...
#### <a name="use-microsoft-endpoint-manager-to-set-cloud-delivered-protection-settings-for-a-new-policy"></a>使用Microsoft Endpoint Manager设置云提供的保护设置 (策略设置) 

1. 转到管理Microsoft Endpoint Manager中心 () <https://endpoint.microsoft.com> 登录。

2. 选择 **"终结点安全** \> **防病毒** \> **+ 创建策略"**。

3. 对于 **"** 平台"，选择一个选项，然后对于"配置文件"，选择"防病毒"或"Microsoft Defender 防病毒 (具体取决于为 **Platform** 选择的选项。) "创建 **"**。
...
5. 在" **配置设置"** 选项卡上，展开 **"云保护**"，并指定以下设置：
   ...
   - 将 **"启用云保护"设置为****"是"**。
   - 将“**云端保护级别**”设为“**未配置**”。  (此级别默认情况下提供强大的保护级别，同时减少误报的可能性。) 
