...
幸运的是，可以采取措施来解决并减少此类问题。 如果在以前的 Microsoft 365 Defender (Microsoft Defender 安全中心) 中看到误报[](microsoft-defender-security-center.md)/负数，则安全操作可以通过以下过程采取措施来解决这些误报/负数：
...
[修正操作](manage-auto-investigation.md#remediation-actions)（例如，将文件发送到隔离或停止进程）将 (实体（如) 检测到威胁的文件）。 几种类型的修正操作通过自动调查和自动修复Microsoft Defender 防病毒：
...
1. 在导航门户的左侧 <a href="https://go.microsoft.com/fwlink/p/?linkid=2077139" target="_blank">导航窗格中</a>，Microsoft 365 Defender操作 **中心"。**
...
> [!NOTE]
> Microsoft Defender 防病毒仅应用于防病毒保护，不适用于其他 Microsoft Defender for Endpoint 功能。 若要广泛排除文件，请为 Microsoft Defender for [](/microsoft-365/security/defender-endpoint/manage-indicators) Endpoint Microsoft Defender 防病毒和自定义指示器使用排除项。

本节中的过程介绍如何定义排除项和指示器。

### <a name="exclusions-for-microsoft-defender-antivirus"></a>排除项Microsoft Defender 防病毒

一般情况下，你无需为项目定义排除Microsoft Defender 防病毒。 请确保谨慎定义排除项，并且只包含导致误报的文件、文件夹、进程和进程打开的文件。 此外，请确保定期查看定义的排除项。 我们建议使用[Microsoft Endpoint Manager](/mem/endpoint-manager-overview)定义或编辑防病毒排除项;但是，可以使用其他方法，如组策略 (管理 Microsoft Defender for [](/azure/active-directory-domain-services/manage-group-policy) [Endpoint](manage-mde-post-migration.md)) 。
...
如果根据行为检测到恶意软件，并且您没有文件，您可以提交 `Mpsupport.cab` 文件进行分析。 您可以使用 Microsoft *恶意软件*.cabCommand-Line工具 MPCmdRun.exeMPCmdRun.exe) 工具获取Windows 10或Windows 11。
...
我们建议[Microsoft Endpoint Manager或](/mem/endpoint-manager-overview)设置云保护设置;但是，可以使用其他方法，如组策略 (请参阅管理 Microsoft Defender for [Endpoint](manage-mde-post-migration.md)) 。 [](/azure/active-directory-domain-services/manage-group-policy)
...
#### <a name="use-microsoft-endpoint-manager-to-set-cloud-delivered-protection-settings-for-a-new-policy"></a>使用Microsoft Endpoint Manager设置云保护设置 (策略策略设置) 

1. 转到管理Microsoft Endpoint Manager中心 <https://endpoint.microsoft.com> () 登录。

2. 选择 **"终结点安全** \> **防病毒** \> **+ 创建策略"。**

3. 对于 **"** 平台"，选择一个选项，然后对于"配置文件"，选择"防病毒"或"Microsoft Defender 防病毒 (特定选项取决于你为 **平台** 选择) 然后选择"创建 **"。**
...
7. 在"**分配**"选项卡上，指定应应用策略的用户和组，然后选择"下一步 **"。**  (如果需要分配帮助，请参阅在 .Microsoft Intune 中分配用户[和设备](/mem/intune/configuration/device-profile-assign)) 
...
我们建议你[Microsoft Endpoint Manager](/mem/endpoint-manager-overview)或设置 PUA 保护设置;但是，可以使用其他方法，如组策略 (管理 Microsoft [](/azure/active-directory-domain-services/manage-group-policy) [Defender for Endpoint](manage-mde-post-migration.md)) 。
