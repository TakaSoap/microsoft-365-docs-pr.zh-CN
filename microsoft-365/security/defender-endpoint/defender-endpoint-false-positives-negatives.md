...
3. 选择警报以了解有关警报的更多详细信息。  (查看 [Microsoft Defender for Endpoint.) 中的](review-alerts.md)警报
...
4. 根据警报状态，执行下表中所述的步骤：
   ...
   |警报为误报|1 [. 将警报](#classify-an-alert) 分类为误报。<br/><br/>2. [抑制警报](#suppress-an-alert)。<br/><br/>3. [为](#indicators-for-microsoft-defender-for-endpoint) Microsoft Defender 终结点创建指示器。<br/><br/>4. [将文件提交给 Microsoft 进行分析](#part-4-submit-a-file-for-analysis)。|
   |警报准确无误，但 (不重要) |[将警报分类](#classify-an-alert) 为真正的正数，然后 [抑制该警报](#suppress-an-alert)。|

### <a name="classify-an-alert"></a>对警报进行分类

警报在警报中可分类为误报或Microsoft 365 Defender。 对警报进行分类有助于培训 Microsoft Defender for Endpoint，以便随着时间的推移，你将看到更多真实警报和更少的假警报。
...
如果您具有误报或真误报的警报，但是对于不重要的事件，可以在非Microsoft 365 Defender。 抑制警报有助于减少安全操作仪表板中的噪音。
...
[修正操作](manage-auto-investigation.md#remediation-actions)（如将文件发送到隔离或停止进程）对已 (实体执行，) 被检测为威胁的文件。 多种类型的修正操作通过自动调查和自动修复Microsoft Defender 防病毒：
...
其他操作（如启动防病毒扫描或收集调查包）是手动发生的，或是通过 [实时响应发生的](live-response.md)。 无法撤消通过实时响应采取的操作。

查看警报后，下一步是 [查看修正操作](manage-auto-investigation.md)。 如果由于误报而进行了任何操作，您可以撤消大多数种类的修正操作。 具体而言，你可以：
...
检查并撤消由于误报而采取的操作后，请继续查看 [或定义排除项](#part-3-review-or-define-exclusions)。
...
3. 在飞出窗格中，选择"撤消 **"**。 如果使用此方法无法撤消该操作，则看不到"撤消 **"** 按钮。  (若要了解更多信息，请参阅 Undo [completed actions](manage-auto-investigation.md#undo-completed-actions).) 
...
> [!NOTE]
> Microsoft Defender 防病毒仅应用于防病毒保护，不适用于其他 Microsoft Defender for Endpoint 功能。 若要广泛排除文件，请使用 Microsoft Defender for Endpoint Microsoft Defender 防病毒自定义指示器的排除[](/microsoft-365/security/defender-endpoint/manage-indicators)项。

本节中的过程介绍如何定义排除项和指示器。

### <a name="exclusions-for-microsoft-defender-antivirus"></a>排除项Microsoft Defender 防病毒

通常，你无需为自定义项定义Microsoft Defender 防病毒。 请确保谨慎定义排除项，并且只包含导致误报的文件、文件夹、进程和进程打开的文件。 此外，请确保定期查看定义的排除项。 我们建议使用 [Microsoft Endpoint Manager](/mem/endpoint-manager-overview)定义或编辑防病毒排除项;但是，可以使用其他方法，如组策略 (管理 [Microsoft Defender for Endpoint](manage-mde-post-migration.md)。[](/azure/active-directory-domain-services/manage-group-policy)

> [!TIP]
> 是否需要有关防病毒排除的帮助？ 请参阅[配置并验证扫描的Microsoft Defender 防病毒项](configure-exclusions-microsoft-defender-antivirus.md)。
...
1. 转到管理Microsoft Endpoint Manager中心 () <https://endpoint.microsoft.com> 并登录。

2. 选择 **"终结点安全** \> **防病毒"**，然后选择现有策略。  (如果没有现有策略，或者要创建新策略，请 [跳至下](#use-microsoft-endpoint-manager-to-create-a-new-antivirus-policy-with-exclusions) 一步) 。
...
7. 在" **范围标记** "选项卡上，如果您在组织中使用范围标记，请为要创建的策略指定范围标记。  (Scope [tags](/mem/intune/fundamentals/scope-tags).) 

8. 在" **分配** "选项卡上，指定应应用策略的用户和组，然后选择"下一步 **"**。  (如果需要分配帮助，请参阅在 [Microsoft Intune](/mem/intune/configuration/device-profile-assign).) 
...
若要将实体指定为 Microsoft Defender for Endpoint 的排除项，请为这些实体创建"允许"指示器。 Microsoft Defender for Endpoint 中的此类"允许"指示器适用于下一[](microsoft-defender-antivirus-in-windows-10.md)代保护、终结点检测和[](overview-endpoint-detection-response.md)响应以及自动调查[&修正](/microsoft-365/security/defender-endpoint/automated-investigations)。
...
为 [文件（](/microsoft-365/security/defender-endpoint/indicator-file)如可执行文件）创建"允许"指示器时，它有助于防止组织使用的文件被阻止。 文件可以包括可移植的可执行 (PE) 文件，如 `.exe` 和 `.dll` 文件。

创建文件指示器之前，请确保满足以下要求：

- Microsoft Defender 防病毒基于云的保护配置 (请参阅管理基于云的[保护](/windows/security/threat-protection/microsoft-defender-antivirus/deploy-manage-report-microsoft-defender-antivirus)) 
- 反恶意软件客户端版本为 4.18.1901.x 或更高版本
- 设备正在运行Windows 10版本 1703 或更高版本或 Windows 11;Windows Server 2016、Windows Server 2019 或 Windows Server 2022
...
- 设备正在运行Windows 10版本 1709 或更高版本，或Windows 11

自定义网络指示器在设备中[Microsoft 365 Defender。](/microsoft-365/security/defender/microsoft-365-defender) 若要了解更多信息，请参阅 [高级功能](/microsoft-365/security/defender-endpoint/advanced-features)。
...
- Microsoft Defender 防病毒基于云的保护配置 (请参阅管理[基于云的保护](deploy-manage-report-microsoft-defender-antivirus.md)
- 反恶意软件客户端版本为 4.18.1901.x 或更高版本
- 设备正在运行Windows 10版本 1703 或更高版本或 Windows 11;Windows Server 2016、Windows Server 2019 或 Windows Server 2022
- 病毒和威胁防护定义是最新的
...
1. 查看此处的指南： [提交文件进行分析](/windows/security/threat-protection/intelligence/submission-guide)。

2. 访问[Microsoft 安全智能提交网站](https://www.microsoft.com/wdsi/filesubmission) (https://www.microsoft.com/wdsi/filesubmission)，然后提交你的 (提交) 。

### <a name="submit-a-fileless-detection-for-analysis"></a>提交无文件检测进行分析

如果根据行为检测到恶意软件，并且您没有文件，您可以提交文件 `Mpsupport.cab` 进行分析。 您可以使用 Microsoft *恶意软件.cab* Command-Line实用工具 MPCmdRun.exeMPCmdRun.exe) 工具获取 Windows 10 或 Windows 11。

1. 转到 ， ` C:\ProgramData\Microsoft\Windows Defender\Platform\<version>`然后以管理员 `MpCmdRun.exe` 角色运行。
...
3. 查看此处的指南： [提交文件进行分析](/windows/security/threat-protection/intelligence/submission-guide)。

4. 访问Microsoft 安全智能[提交网站](https://www.microsoft.com/wdsi/filesubmission) (https://www.microsoft.com/wdsi/filesubmission)，然后提交.cab文件。
...
- 经过身份验证的客户（尤其是具有有效软件 [保障 (或) 的 ](https://www.microsoft.com/licensing/licensing-programs/software-assurance-default.aspx)企业级客户）的优先级更高。
- 由 SAID 持有者标记为高优先级的提交将被立即关注。

若要检查你的提交更新，请在提交Microsoft 安全智能[登录](https://www.microsoft.com/wdsi/filesubmission)。
...
检查云提供的保护级别，了解Microsoft Defender 防病毒。 默认情况下，云提供的保护设置为"未配置"，这与大多数组织的正常保护级别相对应。 如果云提供的保护设置为 **高**、高 **+** 或 **零** 容限，则可能会遇到更多误报。
...
我们建议你[Microsoft Endpoint Manager](/mem/endpoint-manager-overview)或设置云保护设置;但是，可以使用其他方法，如组策略 (管理 [Microsoft Defender for Endpoint](manage-mde-post-migration.md)。[](/azure/active-directory-domain-services/manage-group-policy)
...
2. 选择 **"终结点安全** \> **防病毒"** ，然后选择现有策略。  (如果没有现有策略，或者要创建新策略，请 [跳至下](#use-microsoft-endpoint-manager-to-set-cloud-delivered-protection-settings-for-a-new-policy) 一步) 。

3. 在 **"管理"** 下，选择 **"属性"**。 然后，在"配置 **设置"旁边，** 选择"编辑 **"**。

4. 展开 **云保护**，并查看云提供的保护 **级别行中的当前** 设置。 我们建议将云提供的保护设置为"未配置 **"**，这将提供强大的保护，同时降低误报的可能性。

5. 选择 **"审阅 + 保存"**，然后选择" **保存"**。

#### <a name="use-microsoft-endpoint-manager-to-set-cloud-delivered-protection-settings-for-a-new-policy"></a>使用Microsoft Endpoint Manager新策略设置设置云 (保护设置) 

1. 转到管理Microsoft Endpoint Manager中心 () <https://endpoint.microsoft.com> 并登录。

2. 选择 **"终结点安全** \> **防病毒** \> **+ 创建策略"**。

3. 对于 **"** 平台"，选择一个选项，然后为"配置文件"选择"防病毒"或"Microsoft Defender 防病毒 (特定选项取决于为 **Platform** 选择的选项。) 然后选择"创建 **"**。
...
6. 在" **范围标记** "选项卡上，如果您在组织中使用范围标记，请为策略指定范围标记。  (Scope [tags](/mem/intune/fundamentals/scope-tags).) 

7. 在" **分配** "选项卡上，指定应应用策略的用户和组，然后选择"下一步 **"**。  (如果需要分配帮助，请参阅在 [Microsoft Intune](/mem/intune/configuration/device-profile-assign).) 
...
我们建议你[Microsoft Endpoint Manager](/mem/endpoint-manager-overview)或设置 PUA 保护设置;但是，可以使用其他方法，如组策略 (管理 [Microsoft Defender for Endpoint](manage-mde-post-migration.md)。[](/azure/active-directory-domain-services/manage-group-policy)

#### <a name="use-microsoft-endpoint-manager-to-edit-pua-protection-for-existing-configuration-profiles"></a>使用Microsoft Endpoint Manager编辑现有配置文件 (PUA 保护) 
...
4. 在"**配置设置"** 选项卡上，向下滚动 **并展开Microsoft Defender 防病毒**。
...
3. 对于 **"平台**"，**Windows 10** 和更高版本，对于 **"配置文件**"，选择"**设备限制"**。

4. 在 **"基本信息"** 选项卡上，指定策略的名称和说明。 然后选择“**下一步**”。

5. 在"**配置设置"** 选项卡上，向下滚动 **并展开Microsoft Defender 防病毒**。
...
[AIR (](automated-investigations.md) 自动调查和) 功能旨在检查警报并立即采取措施来解决违规问题。 触发警报并运行自动调查时，将针对调查的每个证据生成裁定。 裁定可以是 *恶意、**可疑或**未找到威胁*。

根据为 [组织](/microsoft-365/security/defender-endpoint/automation-levels)设置的自动化级别和其他安全设置，将针对被视为恶意或 *可疑的项目采取**修正操作*。 在某些情况下，将自动执行修正操作;在其他情形下，修正操作是手动采取的，或仅在安全运营团队批准后执行。
...
> [!IMPORTANT]
> 我们建议使用 *完全自动化* 进行自动调查和修正。 不要因为误报而关闭这些功能。 相反， [使用"允许"指示器定义](#indicators-for-microsoft-defender-for-endpoint)例外，并保留自动调查和修正集以自动采取适当操作。 遵循 [本指南](automation-levels.md#levels-of-automation) 有助于减少安全运营团队必须处理的警报数。
...
[管理 Microsoft Defender for Endpoint](manage-mde-post-migration.md)
