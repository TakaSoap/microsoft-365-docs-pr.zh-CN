...
幸运的是，可以采取措施来解决并减少此类问题。 如果在以前的 Microsoft 365 Defender (中看到误报/负Microsoft Defender 安全中心) ，安全操作可以通过以下过程采取一些措施解决这些问题： [](microsoft-defender-security-center.md)
...
3. 选择警报以了解有关警报的更多详细信息。  (请参阅 [查看 Microsoft Defender for Endpoint .) 中的](review-alerts.md)警报
...
> [!TIP]
> 有关抑制警报的信息，请参阅管理 [适用于终结点的 Microsoft Defender 警报](/microsoft-365/security/defender-endpoint/manage-alerts)。 此外，如果你的组织使用 SIEM (SIEM) ，请确保也定义抑制规则。
...
[修正操作](manage-auto-investigation.md#remediation-actions)（如将文件发送到隔离或停止进程）对 (实体执行，) 被检测为威胁的文件。 几种类型的修正操作通过自动调查和调查自动Microsoft Defender 防病毒：
...
> [!NOTE]
> Microsoft Defender 防病毒仅应用于防病毒保护，不适用于其他 Microsoft Defender for Endpoint 功能。 若要广泛排除文件，请使用 Microsoft Defender for Endpoint [](/microsoft-365/security/defender-endpoint/manage-indicators) Microsoft Defender 防病毒和自定义指示器的排除项。

本节中的过程介绍如何定义排除项和指示器。

### <a name="exclusions-for-microsoft-defender-antivirus"></a>排除项Microsoft Defender 防病毒

通常，你无需为自定义项定义Microsoft Defender 防病毒。 请确保谨慎定义排除项，并且只包含导致误报的文件、文件夹、进程和进程打开的文件。 此外，请确保定期查看定义的排除项。 我们建议使用[Microsoft Endpoint Manager](/mem/endpoint-manager-overview)定义或编辑防病毒排除项;但是，可以使用其他方法，如组策略 (请参阅管理 Microsoft Defender [](/azure/active-directory-domain-services/manage-group-policy) [for Endpoint](manage-mde-post-migration.md)) 。
...
#### <a name="use-microsoft-endpoint-manager-to-create-a-new-antivirus-policy-with-exclusions"></a>使用 Microsoft Endpoint Manager创建排除项的新防病毒策略
...
如果根据行为检测到恶意软件，并且您没有文件，您可以提交 `Mpsupport.cab` 文件进行分析。 您可以使用 Microsoft *恶意软件.cab* Command-Line工具 (MPCmdRun.exe) Windows 10工具获取.cab或 Windows 11。
...
若要检查你的提交更新，请在你的提交Microsoft 安全智能[登录](https://www.microsoft.com/wdsi/filesubmission)。
...
我们建议你[Microsoft Endpoint Manager](/mem/endpoint-manager-overview)或设置云保护设置;但是，可以使用其他方法，如组策略 (请参阅管理[Microsoft Defender for](manage-mde-post-migration.md) [](/azure/active-directory-domain-services/manage-group-policy) Endpoint) 。
...
3. 对于 **"** 平台"，选择一个选项，然后对于"配置文件"，Microsoft Defender 防病毒 (选择"防病毒"或"Microsoft Defender 防病毒 (具体选项取决于你为 **平台** 选择) 然后选择"创建 **"。** 
...
我们建议你[Microsoft Endpoint Manager](/mem/endpoint-manager-overview)或设置 PUA 保护设置;但是，可以使用其他方法，如组策略 (请参阅管理 Microsoft [](/azure/active-directory-domain-services/manage-group-policy) [Defender for Endpoint](manage-mde-post-migration.md)) 。
