...
静态代理通过组策略 (GP) 配置，组策略值下的两个设置都应配置为代理服务器以使用EDR。 组策略在管理模板中可用。
...
2. 将设置为 **"已启用"** 并定义代理服务器。 请注意，URL 必须包含 http:// 或 https://。 有关支持的版本，https:// [管理Microsoft Defender 防病毒更新](manage-updates-baselines-microsoft-defender-antivirus.md)。
   ...
   :::image type="content" source="images/proxy-server-mdav.png" alt-text="客户端的代理服务器Microsoft Defender 防病毒" lightbox="images/proxy-server-mdav.png":::
...
与早期版本的 Windows（如 Windows 7 SP1、Windows 8.1 和 Windows Server 2008 R2*）的 Log Analytics 代理通信时，需要代理和防火墙配置信息列表中的信息 (通常称为 Microsoft Monitoring Agent) 。
...
## <a name="confirm-microsoft-monitoring-agent-mma-service-url-requirements"></a>确认Microsoft Monitoring Agent (MMA) 服务 URL 要求 
 ...
 请参阅以下指南，在将 Microsoft Monitoring Agent (MMA) 用于早期版本的 Microsoft Monitoring Agent () 时，消除特定环境的通配符 (*Windows。

1. 有关使用 Microsoft Monitoring Agent (MMA) 的以前操作系统载入到 Defender for Endpoint (中有关详细信息，请参阅在 [Defender for Endpoint](https://go.microsoft.com/fwlink/p/?linkid=2010326) 上载入早期版本的 Windows 和载入 [Windows 服务器](configure-server-endpoints.md)) 。
