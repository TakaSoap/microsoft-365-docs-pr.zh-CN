...
- 审核模式未启用。 使用组策略将规则设置为禁用 ( 值：**0**) 启用 [攻击面减少规则中所述](enable-attack-surface-reduction.md)。
...
2. 如果攻击面减少规则未阻止应阻止 (也称为漏报) ，你可以立即继续执行最后一步，收集诊断数据，并向我们 [提交](#collect-diagnostic-data-for-file-submissions)问题。
