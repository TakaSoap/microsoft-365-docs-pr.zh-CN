...
评估 Microsoft Defender Office 365时，你可以选择先试点特定用户，然后再为整个组织启用和实施策略。 创建通讯组可帮助管理部署过程。 例如，为 *Office 365 用户创建 Defender -* 标准保护 *，Defender for Office 365 Users - 严格保护**，Defender for Office 365 Users - 自定义保护*，或 *Defender for Office 365 Users - Exceptions*。
...
通讯组可以直接在内部部署 Active Directory 中创建Exchange Online或同步。
...
下面是概述[步骤的 Preset security policies in EOP and Microsoft Defender for Office 365](../office-365-security/preset-security-policies.md)一文。
...
必须 *注意这些* 保护策略在应用和强制执行时优先级，如电子邮件保护的顺序和优先级- Office 365 [说明。](../office-365-security/how-policies-and-protections-are-combined.md)
...
|攻击模拟器|可以使用攻击门户中的攻击模拟Microsoft 365 Defender在组织中运行实际攻击方案，这可以帮助你在真实攻击影响你的环境之前识别并查找易受攻击的用户。|[开始使用攻击模拟培训](../office-365-security/attack-simulation-training-get-started.md)|
|报表仪表板|在左侧导航菜单上，单击"报告"，然后展开"电子邮件&协作"标题。 电子邮件 & 协作报告关于防止安全趋势，其中一些安全趋势允许你通过按钮（如"转到提交") ）采取操作 (，而其他报告将显示趋势，如邮件流状态摘要、热门恶意软件、欺骗检测、遭到入侵的用户、邮件延迟、保险箱 链接和 保险箱 附件报告。 这些指标是自动生成的。|[查看报表](../office-365-security/view-email-security-reports.md)|
|
