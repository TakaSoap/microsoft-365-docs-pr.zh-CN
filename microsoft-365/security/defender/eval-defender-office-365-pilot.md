...
评估 Microsoft Defender Office 365，你可以选择先试点特定用户，然后再为整个组织启用和强制执行策略。 创建通讯组可帮助管理部署过程。 例如，创建 Defender *for Office 365 Users - Standard Protection、Defender* for Office 365 Users - Strict *Protection、Defender* *for Office 365 Users - Custom Protection* 或 Defender for Office 365 Users *- Exceptions 等* 组。

为何"标准"和"严格"是用于此术语的术语可能并不明显，但在你浏览更多有关 Defender for Office 365安全预设时，这一点将变得清晰。 命名组"自定义"和"例外"代表自己，尽管大多数用户都应在标准和严格的标准下，但自定义和例外组将收集关于管理风险的有价值的数据。

## <a name="step-1-create-pilot-groups"></a>步骤 1：创建试点组

通讯组可以直接在内部部署 Active Directory Exchange Online或同步。

1. 使用已Exchange收件人管理员角色或 (组管理权限) 帐户登录到 EAC 管理中心。
2. 从导航菜单中，展开 *"收件人"，* 然后选择"组 *"。*

:::image type="content" source="../../media/mdo-eval/1_mdo-eval-pilot.png" alt-text="Exchange导航菜单上的管理中心 (快速启动) 一个指向&quot;组&quot;的箭头。单击&quot;组&quot;。":::
...
默认情况下，Defender for Office 365中的某些功能已配置并打开，但安全操作可能需要提高默认保护级别。
...
8. 根据需要添加条件以将基线 ***MDO** _ 保护应用于特定试点用户或用户组。 单击_Next*继续。
    - 例如，如果收件人是 Office 365 Standard Protection 组的定义 Defender 的成员，然后只需通过组添加/删除帐户 *进行管理，* 则可能会应用适用于试点评估的 Defender for Office 365 条件。
...
|安全附件     |    保护用户免受电子邮件附件中恶意内容的攻击，以及电子邮件SharePoint、OneDrive和Teams。     |    [在 Defender for Office 365](../office-365-security/set-up-safe-attachments-policies.md)     |
