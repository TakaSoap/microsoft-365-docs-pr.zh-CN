...
8. 根据需要添加条件以将基线 ***MDO** _ 保护应用于特定试点用户或用户组。 单击_Next*继续。
   ...
   例如，如果收件人是 Office 365 Standard Protection 组定义的 Defender 的成员，然后只需通过组添加/删除帐户进行管理，则可能会应用适用于试点评估的 Defender for *Office 365* 条件。
...
|攻击模拟器|可以使用 Microsoft Defender 365 门户中的攻击模拟培训在组织中运行真实的攻击方案，帮助你在真实攻击影响你的环境之前识别和查找易受攻击的用户。|[开始使用攻击模拟培训](../office-365-security/attack-simulation-training-get-started.md)|
|报表仪表板|在左侧导航菜单上，单击"报告"，然后展开"电子邮件&协作"标题。 电子邮件 & 协作报告会指出安全趋势，其中一些安全趋势允许你通过"转到提交") 等按钮执行 (操作，其他报告将显示趋势，如邮件流状态摘要、热门恶意软件、欺骗检测、遭到入侵的用户、邮件延迟、保险箱 链接和 保险箱 附件报告。 这些指标是自动生成的。|[查看报表](../office-365-security/view-email-security-reports.md)|
|
