...
为了与 Microsoft 使用 SIEM 进行威胁防护的跨域方法以及扩展检测和响应 (XDR) 一致，我们已将 Microsoft Defender 高级威胁防护 重新品牌化为 Microsoft Defender for Endpoint，并统一到单个集成门户-Microsoft 365 安全中心。

本指南介绍如何通过启用从以前的 Microsoft Defender for Endpoint 门户 (securitycenter.windows.com 或 securitycenter.microsoft.com) 到 Microsoft 365 安全中心门户门户 (security.microsoft.com) 的自动重定向，将帐户路由到 Microsoft 365 安全中心。
...
启用自动重定向后，在 securitycenter.windows.com 或 securitycenter.microsoft.com 访问以前的 Microsoft Defender 终结点门户的帐户将自动路由到位于 security.microsoft.com 的 Microsoft 365 安全中心门户。
 
详细了解更改了哪些功能[：Microsoft Defender for Endpoint Microsoft 365安全中心](microsoft-365-security-center-mde.md)。

这包括通过浏览器直接访问以前的门户的重定向，包括指向以前的 securitycenter.windows.com 门户的链接（如电子邮件通知中的链接，以及 SIEM API 调用返回的链接）。  

 电子邮件通知或 SIEM API 中的外部链接当前包含指向这两个门户的链接。 启用重定向后，这两个链接将Microsoft 365安全中心，直到最终删除旧链接。 我们鼓励你采用指向安全中心Microsoft 365链接。
...
>[!IMPORTANT]
>启用此设置不会终止活动用户会话。 应用此设置时，活动会话中的帐户在结束当前会话并再次登录Microsoft 365将仅定向到安全中心。

>[!NOTE]
>您必须是全局管理员或具有安全管理员权限Azure Active Directory才能启用或禁用此设置。  

## <a name="can-i-go-back-to-using-the-former-portal"></a>我能否返回到使用以前的门户？
如果无法通过安全中心门户完成某些Microsoft 365，我们希望听到有关它的信息。 如果遇到重定向问题，我们鼓励你使用"提供反馈提交"表单告知我们。
