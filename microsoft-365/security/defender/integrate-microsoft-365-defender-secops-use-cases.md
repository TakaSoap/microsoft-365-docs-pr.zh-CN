...
| 监视Teams | 通过公司 Intranet 门户报告威胁和漏洞状态 | 第 2 层 SOC 分析师 | 报告中自动生成的报告Microsoft 365 Defender资产修正进度 | [安全与合规&警报](/microsoft-365/security/office-365-security/alerts) <br><br> 安全分数监视 | 未就资产的威胁和漏洞状态向资产所有者传达任何视图或仪表板报告。 | 创建自动化脚本以填充组织的高风险和关键资产漏洞修正状态。 | 网络 |
|||||||||
...
针对所有缺陷修复了用例测试后，可以将所总结的经验和收集的指标合并到 SOC 团队的生产运行手册 (操作流程) 以及 (事件响应和上报过程) 中。 
