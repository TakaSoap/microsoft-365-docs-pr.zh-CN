...
- 答： 云应用发现与 Microsoft Defender for Endpoint 本地集成。 Defender for Endpoint 报告从 IT 托管的设备访问的云应用Windows 10 Windows 11设备。 
...
你可能已经将 SaaS 应用添加到 Azure AD 租户以强制执行多重身份验证和其他条件访问策略。 Microsoft Defender for Cloud Apps 在本机与 Azure AD 集成。 你只需在云应用中配置策略Azure AD在 Defender for Cloud Apps 中使用条件访问应用控制。 这将通过 Defender for Cloud Apps 作为代理路由这些托管 SaaS 应用的网络流量，从而允许云应用的 Defender 监视此流量并应用会话控件。 
...
你可能已使用 Defender for Cloud Apps 发现和批准的云应用，这些应用尚未添加到Azure AD。 通过将这些云应用添加到你的应用程序租户和条件访问规则的范围，Azure AD条件访问应用控制。
...
从概述到本试点评估和试点指南，值得Microsoft 365 Defender此图。 
...
| 条件访问应用控制    | 与 Identity Provider (IdP) 的反向代理体系结构Azure AD条件访问策略，并选择性地强制执行会话控制。        |  [使用 Microsoft Defender for Cloud Apps 条件访问应用控制保护应用](/cloud-app-security/proxy-intro-aad)       |
...
### <a name="applying-azure-ad-conditional-access-policies-to-cloud-apps"></a>将Azure AD访问策略应用到云应用
