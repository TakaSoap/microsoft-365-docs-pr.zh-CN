...
description: 了解高级搜寻架构的 DeviceEvents (设备事件) 、防火墙和其他事件类型
...
高级搜寻架构中的杂项设备事件或表包含有关各种事件类型的信息，包括安全控件触发的事件， `DeviceEvents` 如Windows Defender 防病毒和 Exploit Protection。 [](advanced-hunting-overview.md) 使用此参考来构建从此表返回信息的查询。
