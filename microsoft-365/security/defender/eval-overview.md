...
|Azure AD 标识保护|Azure AD Identity Protection 评估数十亿次登录尝试的风险数据，并使用此数据评估每次登录环境的风险。 此数据由 Azure AD 用于允许或阻止帐户访问，具体取决于条件访问策略的配置方式。 Azure AD Identity Protection 与用户单独获得Microsoft 365 Defender。 它包含在Azure Active Directory Premium P2。|[什么是标识保护？](/azure/active-directory/identity-protection/overview-identity-protection)|
| | | |
...
- Microsoft Defender 可保护你的组织免受电子邮件、链接 (URL) 和协作工具造成的恶意威胁。 它将与活动共享由这些活动Microsoft 365 Defender。 Exchange Online Protection (EOP) 集成，为传入电子邮件和附件提供端到端保护。
...
- Azure AD Identity Protection 评估数十亿次登录尝试的风险数据，并使用此数据评估每次登录环境的风险。 此数据由 Azure AD 用于允许或阻止帐户访问，具体取决于条件访问策略的配置方式。 Azure AD Identity Protection 与用户单独获得Microsoft 365 Defender。 它包含在Azure Active Directory Premium P2。  
