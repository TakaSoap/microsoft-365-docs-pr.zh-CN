...
Microsoft 365 Defender是一种扩展检测和响应 (XDR) 解决方案，可自动收集、关联和分析来自 Microsoft 365 环境中的信号、威胁和警报数据，包括终结点、电子邮件、应用程序和标识。 它利用广泛的 AI 和自动化来自动停止攻击，将受影响的资产修正为安全状态。 以下文章将逐步介绍设置试用环境的过程，以便评估 Microsoft 365 Defender。 
...
Microsoft 365 Defender由这些安全技术共同操作。 无需所有这些组件都受益于 XDR 和 Microsoft 365 Defender。 你还将通过使用一个或两个实现收益和效率。 
...
|Exchange Online Protection     |      Exchange Online Protection是本机基于云的 SMTP 中继和筛选服务，可帮助保护组织免受垃圾邮件和恶意软件的攻击。      |   [Exchange Online Protection (EOP) 概述 - Office 365](../office-365-security/overview.md)     |
...
|Azure AD 标识保护|Azure AD Identity Protection 评估来自数十亿次登录尝试的风险数据，并使用此数据评估每次登录环境的风险。 此数据由 Azure AD 用于允许或阻止帐户访问，具体取决于条件访问策略的配置方式。 Azure AD Identity Protection 与 Microsoft 365 Defender 分开许可。 它包含在Azure Active Directory Premium P2。|[什么是标识保护？](/azure/active-directory/identity-protection/overview-identity-protection)|
| | | |
...
- Microsoft Defender 可保护你的组织免受电子邮件、链接 (URL) 和协作工具造成的恶意威胁。 它将与组织共享由这些活动Microsoft 365 Defender。 Exchange Online Protection (EOP) 集成，为传入电子邮件和附件提供端到端保护。
...
- Azure AD Identity Protection 评估来自数十亿次登录尝试的风险数据，并使用此数据评估每次登录环境的风险。 此数据由 Azure AD 用于允许或阻止帐户访问，具体取决于条件访问策略的配置方式。 Azure AD Identity Protection 与 Microsoft 365 Defender 分开许可。 它包含在Azure Active Directory Premium P2。  
