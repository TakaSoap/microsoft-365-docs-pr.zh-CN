...
在具有 Exchange Online 邮箱的 Microsoft 365 组织或独立 Exchange Online Protection (EOP) 组织中，自 2018 年 10 月起，入站电子邮件将自动受到 EOP 的欺骗保护。 EOP 使用欺骗智能作为组织抵御网络钓鱼的整体防护的一部分。 有关详细信息，请参阅 [EOP 中的反欺骗保护](anti-spoofing-protection.md)。
...
5. 单击 ![ " ](../../media/scc-expand-icon.png) 允许欺骗 **？"列中的"展开"** 图标。 选择 **"是** "以允许欺骗发件人，或选择" **否** "将邮件标记为欺骗邮件。 操作由默认防钓鱼策略或自定义防钓鱼策略控制， (默认值为"将邮件移动到垃圾邮件"文件夹) 。  有关详细信息，请参阅反网络钓鱼策略 [中的欺骗设置](set-up-anti-phishing-policies.md#spoof-settings)。
   ...
   - **欺骗用户**：被欺骗的用户帐户。 这是"发件人"地址 (也称为在) `5322.From` 客户端中显示的地址发件人。 SPF 不检查此地址的有效性。
   ...
   - **允许欺骗？：** 你在此处看到的值是：
     ...
     - **否**：来自欺骗用户和发送基础结构组合的邮件被标记为欺骗。 操作由默认防钓鱼策略或自定义防钓鱼策略控制， (默认值为"将邮件移动到垃圾邮件"文件夹) 。  有关详细信息，请参阅下一节。
...
2. 编辑 CSV 文件以添加或修改 **SpoofedUser** (电子邮件地址) **AllowedToSpoof** (是或否) 值。 保存文件，读取文件，将内容存储为一个名为 `$UpdateSpoofedSenders` ：
...
您可以在默认反网络钓鱼策略和自定义策略中配置欺骗智能设置。 有关基于订阅的说明，请参阅下列主题之一：
...
在欺骗和网络钓鱼保护方面努力。 以下是检查发件人欺骗域并有助于防止他们破坏组织的相关方法：
