...
有关角色和角色组详细信息，请参阅独立 [EOP 中的权限](feature-permissions-in-eop.md)。
...
创建新的角色组时，可以在创建组期间或 (配置所有设置) 。 或者，您可以复制并修改现有角色组。
...
创建新的角色组时，可以在创建组期间或 (之后手动配置所有) 。 或者，您可以复制并修改现有角色组。
...
- 若要 _选择性地修改_ 现有成员列表，请使用以下 `@{Add="Member1","Member2"...; Remove="Member3","Member4"...}` 语法：
