...
- **测试**：ASF 将相应的 X 标头字段添加到邮件。 对邮件发生的情况由 **TestModeAction** (*TestModeAction*) 确定：
...
以下增加垃圾邮件得分 **ASF** 设置将检测到的邮件的垃圾邮件可信度 (SCL) 设置为 5 或 6，对应于垃圾邮件筛选器裁定和反垃圾邮件策略中的相应操作。
...
|---|---|---|
|**指向远程网站的图像链接** <p> *IncreaseScoreWithImageLinks*|包含指向远程网站的 HTML 标记链接的邮件 (例如，使用 http) `<Img>` 标记为垃圾邮件。|`X-CustomSpam: Image links to remote sites`|
...
|**HTML 中的嵌入标记** <p> *MarkAsSpamEmbedTagsInHtml*|包含 `<embed>` HTML 标记的邮件被标记为高可信度垃圾邮件。 <p> 此标记允许在 HTML 文档中嵌入不同类型的文档 (例如声音、视频或图片) 。|`X-CustomSpam: Embed tag in html`|
|**HTML 格式 的 JavaScript 或 VBScript** <p> *MarkAsSpamJavaScriptInHtml*|使用 JAVAScript 或 html Visual Basic Script Edition 的邮件被标记为高可信度垃圾邮件。 <p> 这些脚本语言在电子邮件中用于使特定操作自动发生。|`X-CustomSpam: Javascript or VBscript tags in HTML`|
