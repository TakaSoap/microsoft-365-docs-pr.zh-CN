...
- 向外部收件人转发规则通常被攻击者用来提取数据。 使用 [Microsoft](../mtp/microsoft-secure-score.md) **安全分数中的"** 审阅邮箱转发规则"信息查找甚至阻止将规则转发给外部收件人。 有关详细信息，请参阅使用安全分数缓解客户端 [外部转发规则](/archive/blogs/office365security/mitigating-client-external-forwarding-rules-with-secure-score)。