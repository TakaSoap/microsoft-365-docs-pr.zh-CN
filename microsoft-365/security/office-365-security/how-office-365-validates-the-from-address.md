...
- 地址 `5321.MailFrom` (**MAIL FROM** 地址、P1 发件人或信封发件人) 是在邮件的 SMTP 传输中使用的电子邮件地址。 虽然发件人可以指定不同的返回路径电子邮件地址，但此电子邮件地址通常记录在邮件头 (的"返回路径"头字段中) 。 
...
- **显示名称**：描述电子邮件地址所有者的可选短语。
  ...
  - 我们建议您始终使用双引号显示名称双引号 (") ，如下所示。 如果显示名称逗号， _则必须_ 将每个 RFC 5322 的字符串括在双引号中。
...
有关设置 MX 记录的信息，请参阅在任何[DNS](../../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md)托管提供商上为用户Microsoft 365。
...
不能覆盖从邮箱发送的出站电子邮件的"Microsoft 365"。 此外，Outlook.com 将不允许进行任何类型的覆盖，即使通过支持。
