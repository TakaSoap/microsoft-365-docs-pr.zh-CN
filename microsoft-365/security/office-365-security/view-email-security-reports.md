...
![电子邮件&门户中的"电子邮件Microsoft 365 Defender协作报告"页](../../media/email-collaboration-reports.png)
...
![Exchange"电子邮件和协作报告"页上的&传输规则小组件](../../media/transport-rule-report-widget.png)
...
- **按以下传输Exchange** 查看数据：高严重性、中等严重性和 **低严重性邮件** 的数量。 你可以将严重性级别设置为规则中的操作 (严重性级别审核此规则或 _SetAuditSeverity_) 。 有关详细信息，请参阅邮件[流规则操作Exchange Online。](/Exchange/security-and-compliance/mail-flow-rules/mail-flow-rule-actions)
...
- **用户和域模拟后的电子邮件**
  ...
  - **域模拟**：由于尝试模拟在 Defender for Office 365 中反网络钓鱼策略的模拟保护设置中定义的域而筛选Office 365。
- **文件和 URL 触发后的电子邮件**
  - **文件触发**：由附件策略保险箱的邮件。
  - **URL 触发**：由链接策略保险箱的邮件。
...
图表下方的详细信息表包含的信息与图表相同，但信息 (较短) 说明：
...
- **反垃圾邮件、批量邮件筛选**
- **用户和域模拟**
- **ATP (** 的文件和 URL) ：Defender for Office 365
...
![电子邮件门户中的欺骗邮件Microsoft 365 Defender页](../../media/spoof-detections-report-page.png)
...
威胁 **防护状态报告** 在 EOP 和 Defender for Office 365;但是，报告包含不同的数据。 例如，EOP 客户可以查看有关电子邮件中检测到的恶意软件的信息，但不能查看有关 SharePoint、OneDrive 和 Microsoft Teams 的[保险箱 附件检测到的恶意文件的信息](mdo-for-spo-odb-and-teams.md)。
...
!["电子邮件和协作报告"页上的威胁&小组件](../../media/threat-protection-status-report-widget.png)

默认情况下，图表显示过去 7 天的数据。 如果 **单击"****威胁** 防护状态报告"页上的"筛选"，可以选择 90 天的日期范围 (试用版订阅可能限制为) 。 详细信息表允许筛选 30 天。
...
**注意**：向 Azure Active Directory 中的相应 Azure Active Directory 角色添加用户Microsoft 365 管理中心为用户提供 Microsoft 365 Defender 门户中所需的权限以及 Microsoft 365 中其他功能的权限。 有关详细信息，请参阅 [关于管理员角色](../../admin/add-users/about-admin-roles.md)。
