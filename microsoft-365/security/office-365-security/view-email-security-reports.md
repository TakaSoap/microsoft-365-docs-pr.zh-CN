...
**邮件延迟报告** 包含有关您组织中的邮件传递和沙箱延迟的信息。 有关详细信息，请参阅 [邮件延迟报告](view-reports-for-atp.md#mail-latency-report)。
...
" **威胁防护状态** 报告" 在 EOP 和 Microsoft Defender for Office 365 中均可用;但是，报告包含不同的数据。 例如，EOP 客户可以查看有关在电子邮件中检测到的恶意软件的信息，但不是关于由 [SharePoint、OneDrive 或 Microsoft 团队的 ATP](atp-for-spo-odb-and-teams.md)检测到的恶意文件的信息。

该报告提供了包含恶意内容的电子邮件（如文件或网站地址 (Url) 被反恶意软件引擎阻止、 [零小时自动清除 (ZAP) ](zero-hour-auto-purge.md)和 Defender for Office 365 功能（如 [安全链接](atp-safe-links.md)、 [安全附件](atp-safe-attachments.md)和 [反网络钓鱼](set-up-anti-phishing-policies.md)）等电子邮件的计数。 您可以使用此信息来确定趋势或确定组织策略是否需要调整。

**注意**：如果将邮件发送给5个收件人，请务必先将其计数为五个不同的邮件，而不是一封邮件。
...
- **数据查看依据：内容 \> 恶意软件**<sup>1</sup>：为 Microsoft Defender for Office 365 组织显示以下信息：
  ...
  - **反恶意软件引擎**： [Microsoft 365 中内置的病毒检测](virus-detection-in-spo.md)在 Sharepoint、OneDrive 和 Microsoft 团队中检测到的恶意文件。
  - **文件沙箱**：由 [Sharepoint、OneDrive 和 Microsoft 团队的 ATP](atp-for-spo-odb-and-teams.md)检测到的恶意文件。
...
- **分解方式：检测技术** 和 **查看数据的方式：电子邮件 \> 网络钓鱼**：将显示以下信息：
  ...
  - **ATP 生成的 url 信誉**<sup>1</sup>：在其他 Microsoft 365 客户中从适用于 Office 365 detonations 的 DEFENDER 生成的恶意 url 信誉。
...
- 对于 **View data by by： Content \> 恶意软件**，可以按 **开始日期** 和 **结束日期** 以及 **检测** 值修改报告。

- 对于 **View data by by： Message Override**，可以使用以下筛选器修改报告：

  - **开始日期** 和 **结束日期**
  - **重写原因**
  - **标记**：按已应用指定用户标记的用户或组筛选结果 (包括优先级帐户) 。 有关用户标记的详细信息，请参阅 [用户标记](user-tags.md)。
  - **域**
