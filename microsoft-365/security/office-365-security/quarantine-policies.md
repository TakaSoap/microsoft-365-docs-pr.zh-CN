...
对于 [受支持的保护](#step-2-assign-a-quarantine-policy-to-supported-features)功能，隔离策略指定允许用户对其自己的邮件执行哪些操作 (用户作为收件人的邮件) 隔离 _和隔离通知_ 中。 [隔离](use-spam-notifications-to-release-and-report-quarantined-messages.md) 通知是最终用户垃圾邮件通知的替换。 这些通知现在由隔离策略控制，并包含有关所有受支持的保护功能的隔离邮件的信息 (不仅仅是反垃圾邮件策略和反网络钓鱼策略裁定) 。
...
- 在 2021 年 7 月 (/8 月早期启用隔离策略功能之前，组织已存在) 。
...
|[保险箱附件保护](safe-attachments.md)： <ul><li>附件被"启用"和"操作"策略保险箱 _隔离为恶意软件__(电子邮件)_</li><li>被附件隔离为恶意软件的文件[保险箱SharePoint、OneDrive](mdo-for-spo-odb-and-teams.md)和Microsoft Teams</li></ul>|<ul><li>是</li><li>否</li></ul>|<ul><li>AdminOnlyAccessPolicy (无法访问) </li><li>不适用</li></ul>|
...
> [!NOTE]
> 如果对默认隔离策略提供的默认最终用户权限和隔离通知 (或未) ，则无需执行任何操作。 如果要为隔离邮件添加或删除最终用户功能 (或为隔离邮件添加或删除) 按钮) ，或者启用隔离通知，以及添加或删除隔离通知中的相同功能，可以将不同的隔离策略分配给隔离操作。
...
1. 在 Microsoft 365 Defender [门户](https://security.microsoft.com)中，转到"策略"部分中的"电子邮件&**协作** 策略& \> **策略** \>  \> **""** 威胁策略""防钓鱼"。
...
1. 在 Microsoft 365 Defender [门户中](https://security.microsoft.com)，转到"策略"&电子邮件& \> **策略""** 威胁策略 \>  \> **""** 反恶意软件"部分。 
...
1. 在 Microsoft 365 Defender [门户](https://security.microsoft.com)中，转到"策略 **"&** 中的"电子邮件&策略"保险箱"附件"中的"威胁策略 \>  \>  \> **"。**
