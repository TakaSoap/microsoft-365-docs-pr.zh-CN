...
对于 [受支持的保护](#step-2-assign-a-quarantine-policy-to-supported-features)功能，隔离策略指定允许哪些用户对其自己的邮件执行哪些操作 (用户是隔离和隔离通知中的) 收件人 _的邮件_。 隔离通知是最终用户垃圾邮件通知的替换。 这些通知现在由隔离策略控制，其中包含有关所有受支持保护功能的隔离邮件的信息 (不仅仅是反垃圾邮件策略裁定) 。
...
在邮箱门户中或在 PowerShell Microsoft 365 Defender PowerShell 中为 (Exchange Online邮箱Microsoft 365组织创建Exchange Online隔离策略;EOP 组织中独立的 EOP PowerShell，Exchange Online邮箱) 。
...
5. 在"**最终用户垃圾邮件** 通知"页上，选择"启用"以启用隔离 (之前称为最终用户垃圾邮件通知) 。 完成后，请单击“**下一步**”。
...
|防钓鱼策略: <ul><li>[AuthenticationFailAction](set-up-anti-phishing-policies.md#spoof-settings) (_欺骗智能_) </li><li>[Defender for Office 365 中的模拟Office 365：](set-up-anti-phishing-policies.md#impersonation-settings-in-anti-phishing-policies-in-microsoft-defender-for-office-365)<ul><li>**如果邮件被检测为** _TargetedUserProtectionAction_ (模拟) </li><li>**如果邮件被检测为** _TargetedDomainProtectionAction_ (模拟) </li><li>**如果邮箱智能检测到并模拟了** _MailboxIntelligenceProtectionAction (MailboxIntelligenceProtectionAction_) </li></ul></li></ul>|是|<ul><li>DefaultFullAccessPolicy (完全访问权限) </li><li>模拟保护：<ul><li>DefaultFullAccessPolicy (完全访问权限) </li><li>DefaultFullAccessPolicy (完全访问权限) </li><li>DefaultFullAccessPolicy (完全访问权限) </li></ul></li></ul>|
|[反恶意软件策略](configure-anti-malware-policies.md)：始终隔离所有检测到的邮件。|是|AdminOnlyAccessPolicy (无法访问) |
|[保险箱附件保护](safe-attachments.md)： <ul><li>包含附件被"启用"和"操作"策略保险箱为恶意软件 (_的电子邮件)_ </li><li>由附件中隔离为保险箱[恶意软件的文件SharePoint、OneDrive和Microsoft Teams](mdo-for-spo-odb-and-teams.md)</li></ul>|<ul><li>是</li><li>否</li></ul>|<ul><li>AdminOnlyAccessPolicy (无法访问) </li><li>不适用</li></ul>|
...
如果您更希望使用 PowerShell 在反垃圾邮件策略中分配隔离策略，请连接到 Exchange Online PowerShell 或 Exchange Online Protection PowerShell，然后使用以下语法：
...
如果你想要使用 PowerShell 在反恶意软件策略中分配隔离策略，请连接到 Exchange Online PowerShell 或 Exchange Online Protection PowerShell，然后使用以下语法：
...
如果你想要使用 PowerShell 在 保险箱 附件策略中分配隔离策略，请连接到 Exchange Online PowerShell 或 Exchange Online Protection PowerShell，然后使用以下语法：
...
- PowerShell 中的新 保险箱 附件策略要求使用 **New-SafeAttachmentPolicy** cmdlet 的安全附件策略 (设置) ，以及使用 **New-SafeAttachmentRule** cmdlet 的独占安全附件规则 (收件人筛选器) 。 有关说明，请参阅使用[Exchange Online PowerShell 或独立 EOP PowerShell](set-up-safe-attachments-policies.md#use-exchange-online-powershell-or-standalone-eop-powershell-to-configure-safe-attachments-policies)配置保险箱附件策略。
...
#### <a name="delete-permission"></a>删除权限
 ...
 _PermissionToDelete (PermissionToDelete_) 控制用户删除其邮件的能力 (用户是隔离的收件人) 邮件。
