...
以下示例脚本允许 Microsoft Exchange Online Protection (EOP) 管理员管理多个租户 (公司) 使用 Exchange Online PowerShell 查看和/或将配置设置应用到其租户。
...
2. 使用电子表格应用程序 (例如 Excel) ，创建具有以下详细信息的 .csv 文件：
   ...
   - UserName 列：用于连接用户的帐户 (例如 `admin@contoso.onmicrosoft.com` ，) 。
