...
> [!NOTE]
> 仅可将设置稳定可靠地从一个 EOP 独立 (Standard) 组织移动到另一个 EOP Standard 或 Exchange Enterprise CAL with Services (EOP Premium) 组织，或从一个 EOP Premium 组织移动到另一个 EOP Premium 组织。 由于某些高级功能在 EOP Standard 组织中不受支持，因此，从 EOP Premium 组织移动到 EOP 标准组织可能是失败的。 <br><br> 这些说明是针对 EOP 仅筛选组织。从一个 Exchange Online 组织移动到另一个 Exchange Online 组织还有其他一些注意事项。Exchange Online 组织不在这些说明的范畴内。
...
将域从一个组织移动到另一个组织的挑战性在于，同一个经验证的域不能同时存在于两个组织中。以下步骤将帮助您逐步完成这一过程。

## <a name="step-1-collect-data-from-the-source-organization"></a>步骤 1：收集源组织数据
...
- 邮件流规则 (也称为传输规则) 
  ...
  > [!NOTE]
  > 对邮件流规则集合导出和导入操作的 cmdlet 支持当前仅支持 EOP Premium 订阅计划。

收集您的所有设置最简单的方法是使用 PowerShell。 若要连接到独立 EOP PowerShell，请参阅[连接到 Exchange Online Protection PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-protection-powershell)。
...
在独立 EOP PowerShell 中，在易于找到并进行更改的位置创建一个名为"Export"的目录。 例如：
...
可以使用以下脚本收集源组织中的所有邮件用户、组、反垃圾邮件设置、反恶意软件设置、连接器以及邮件流规则。 将下面的文本复制并粘贴到记事本等文本编辑器，将文件在刚刚创建的"Export"目录中另存为 Source_EOP_Settings.ps1，然后运行以下命令：
...
## <a name="step-2-add-domains-to-the-target-organization"></a>步骤 2：将域添加到目标组织
...
现在，你可以从目标组织的 Microsoft 365 管理中心查看并收集信息，就可以在合适时间后快速验证域：

1. 登录 Microsoft 365 管理中心 <https://portal.office.com> 。

2. Click **Domains**.

   如果您看不到域，单击"自定义**导航"，** 选择"**设置"，** 然后单击"保存 **"。**
...
6. 将验证 TXT 记录添加到你的 DNS 记录。 这将使你在从目标组织中删除域后，可以更快速地验证源组织中的域。 有关配置 DNS 的详细信息，请参阅任何 [DNS 托管提供商处 Microsoft 365 创建 DNS 记录](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider)。

## <a name="step-3-force-senders-to-queue-mail"></a>步骤 3：强制发件人对邮件进行排队

在将域从一个租户移动到另一个租户时，您需要从源组织中删除这些域，然后在目标组织中验证这些域。在此期间，您不能通过 EOP 路由邮件。

强制发件人对邮件进行排队的一个选择是将 MX 记录更新为直接指向您的内部部署邮件服务器。
...
有关配置 DNS 的详细信息，请参阅任何 [DNS 托管提供商处 Microsoft 365 创建 DNS 记录](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider)。
...
以下脚本将使用 Azure Active Directory PowerShell 从源租户删除用户、组和域。 将以下文本复制并粘贴到记事本等文本编辑器，将文件另存为 C:\EOP\Export\Remove_Users_and_Groups.ps1，然后运行以下命令：
...
1. 登录到下面的管理中心 [https://portal.office.com](https://portal.office.com) 。
...
适用于 EOP 的最佳实践是使用 Azure Active Directory 将本地 Active Directory 同步到目标租户。 有关如何执行此操作的详细信息，请参阅"在 EOP 中管理邮件用户"中的"使用[目录同步管理邮件用户"。](manage-mail-users-in-eop.md) 还可使用以下脚本从源租户重新创建用户和组。 注意：无法移动用户密码。 新用户密码创建并保存在名为 UsersAndGroups.ps1。
...
## <a name="step-7-add-protection-settings-to-the-target-organization"></a>步骤 7：将保护设置添加到目标组织

您可以在登录目标组织时从“Export”目录运行以下脚本，重新创建早期从源组织导出到 .xml 文件的设置。
...
如果你选择将 MX 记录设置为无效地址，使发件人在转换期间对邮件进行排队，你将需要将其设置回在管理中心中指定的 [正确值](https://admin.microsoft.com)。 有关配置 DNS 的详细信息，请参阅任何 [DNS 托管提供商处 Microsoft 365 创建 DNS 记录](https://docs.microsoft.com/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider)。
