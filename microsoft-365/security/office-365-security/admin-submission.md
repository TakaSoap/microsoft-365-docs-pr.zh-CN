...
在 Exchange Online 中具有邮箱的 Microsoft & 365 组织中，管理员可以使用安全与合规中心中的提交门户将电子邮件、URL 和附件提交给 Microsoft 进行扫描。

提交电子邮件时，将获取有关可能允许传入电子邮件进入租户的任何策略的信息，以及检查邮件中任何 URL 和附件的信息。 允许邮件的策略包括单个用户的安全发件人列表以及租户级别策略（如 Exchange 邮件流规则 (也称为传输规则) 。
...
- 若要将邮件和文件提交到 Microsoft，你需要是以下角色组之一的成员：
  ...
  - **Exchange** [Online 中的组织管理](https://docs.microsoft.com/Exchange/permissions-exo/permissions-exo#role-groups)。
    ...
    请注意，查看自定义邮箱的用户提交需要此角色[](#view-user-submissions-to-the-custom-mailbox)组的成员身份，如本文稍后所述。
...
1. 在"**对象类型"** 部分，选择"**电子邮件"。** 在" **提交格式** "部分中，使用以下选项之一：
   ...
   > [!NOTE]
   > 使用 Defender for Office 365 计划 1 或计划 2 的管理员可以提交超过 30 天的邮件。 其他管理员将只能返回 7 天。
...
如果发现覆盖，则重新扫描应在几分钟后完成。 如果电子邮件身份验证没有问题，或者传递不受替代影响，则评分人员的反馈最多可能需要一天。
...
在页面顶部附近，默认情况下，您可以输入开始日期、结束日期和 () 可以通过在框中输入值并单击"刷新"按钮按发件人进行 ![ 筛选 ](../../media/scc-quarantine-refresh.png) 。 Update
...
**如果**[已配置自定义](user-submission.md)邮箱以接收用户报告的邮件，则还可以查看并提交已传递到报告邮箱的邮件。
