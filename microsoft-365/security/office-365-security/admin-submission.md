...
在管理员提交中提交的邮件将重新进行扫描，并显示在 "详细信息" 浮出控件中的结果：

- 传递时，如果发件人的电子邮件身份验证失败。
- 有关可能会受到影响或重写邮件的结论的任何策略命中的信息。
- 当前沙箱结果，以查看邮件中包含的 Url 或文件是否为恶意的。
- 来自 graders 的反馈。

如果找到替代，则重新扫描应在几分钟内完成。 如果电子邮件身份验证中没有问题或传递不受覆盖的影响，则来自 graders 的反馈可能需要一天时间。
