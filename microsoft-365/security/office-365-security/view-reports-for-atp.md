...
- **查看数据者：文件**：图表包含以下信息：
  ...
  当您将鼠标悬停在 (数据点) 上时，可以看到 EOP 中的安全附件和反恶意软件保护检测到的恶意文件类型[](atp-safe-attachments.md)[的细目](anti-malware-protection.md)。
...
- **查看数据者：消息**：图表包含以下信息：
  ...
  - **邮件被替换**
  - **监视的邮件**
  - **替换为动态电子邮件传递**：有关详细信息，请参阅动态 [传递在安全附件策略](atp-safe-attachments.md#dynamic-delivery-in-safe-attachments-policies)中。
...
URL **威胁防护报告** 提供有关检测到的威胁以及作为安全链接的一部分对 URL 单击采取的操作的摘要和 [趋势视图](atp-safe-links.md)。 如果应用了安全链接策略的用户选择了"不跟踪用户单击"选项，则此报告 **将不会包含来自这些用户的单击** 数据。
...
如果在 Defender for Office 365 报告中看不到数据，请仔细检查策略是否正确设置。 你的组织必须[定义安全链接策略](set-up-atp-safe-links-policies.md)[和安全](set-up-atp-safe-attachments-policies.md)附件策略，以便实施 Defender for Office 365 保护。 另请参阅 [反垃圾邮件和反恶意软件保护](anti-spam-and-anti-malware-protection.md)。
