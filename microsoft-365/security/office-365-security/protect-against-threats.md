...
3. 在出现的策略详细信息飞出中，配置以下设置：
   ...
   <sup>\*</sup>此设置仅适用于 Defender for Office 365。
...
1. 在门户 **中打开"反垃圾邮件策略**"Microsoft 365 Defender位于 <https://security.microsoft.com/antispam> 。
...
3. 在出现的策略详细信息飞出中，配置以下设置：
   ...
   - **"操作**"部分：单击 **"编辑操作"。** 在出现的"飞出"中，配置以下设置：
     ...
     - **启用零时差自动清除 (ZAP) ：** 验证此设置是否 (启用) 。
       - **启用网络钓鱼邮件**：验证是否选中此设置 (启用) 。 有关详细信息，请参阅适用于网络钓鱼的零[时差自动清除 (ZAP) 。](zero-hour-auto-purge.md#zero-hour-auto-purge-zap-for-phishing)
       - **启用垃圾邮件**：验证是否选中此设置 (启用) 。 有关详细信息，请参阅垃圾邮件的零[时差自动清除 (ZAP) 。](zero-hour-auto-purge.md#zero-hour-auto-purge-zap-for-spam)
     - **通知** 部分：
       - 选择 **"启用最终用户垃圾邮件通知"。**
         - **每两天发送一次最终用户 () ：** 验证值 **3** 天。
...
2. On the **保险箱 Attachments** page， click **Global settings**， and then configure the following settings on the flyout that appears：
   ...
   - **打开 保险箱** 客户端Office文档：打开此设置 (![ 打开 ](../../media/scc-toggle-on.png) 。) 。 请注意，此功能仅对许可证Microsoft 365 E5或Microsoft 365 E5 安全性有意义。
   - **允许用户单击"受保护的** 视图"，即使保险箱文档将文件标识为恶意文件：验证此设置是否 (![ 关闭 ](../../media/scc-toggle-off.png) 。) 。
