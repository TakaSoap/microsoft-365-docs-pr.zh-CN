...
以下过程介绍如何配置默认的防钓鱼策略。 设置 Defender for Office 365中提供的信息会进行明确标记。
...
3. 在出现的策略详细信息飞出中，配置以下设置：
   - **网络钓鱼阈值&保护** 部分：单击" **编辑保护** 设置"，然后配置打开的飞出控件中的以下设置：
     ...
     - 添加受信任发件人和域部分：单击"管理 <sup>\*</sup> (**nn)** 受信任发件人 () 和域 () "，根据需要将发件人和发件人域例外配置为模拟保护。
   ...
   - **"** 操作 **"部分：** 单击"编辑操作"，在打开的飞出控件中配置以下设置：
     - **"邮件** 操作"部分：配置以下设置：
       ...
       - **如果邮箱智能检测到模拟** 用户：选择"将邮件移动到收件人的垃圾邮件文件夹" (<sup>\*</sup> Standard) "或"隔离 (严格) "。  
...
3. 在出现的策略详细信息飞出中，配置以下设置：
   ...
   - **"操作**"部分：单击 **"编辑操作"。** 在出现的"飞出"中，配置以下设置：
     ...
     - **启用零时差自动清除 (ZAP) ：** 验证此设置是否 (启用) 。
       - **启用网络钓鱼邮件**：验证是否选中此设置 (是否) 。 有关详细信息，请参阅适用于网络钓鱼的零[时差自动清除 (ZAP) 。](zero-hour-auto-purge.md#zero-hour-auto-purge-zap-for-phishing)
...
2. On the **保险箱 Attachments** page， click **Global settings**， and then configure the following settings on the flyout that appears：
   ...
   - **打开 保险箱 客户端Office** 文档：打开此设置 (![ 打开。) 。 ](../../media/scc-toggle-on.png) 请注意，此功能仅对许可证Microsoft 365 E5或Microsoft 365 E5 安全性有意义。
   - **即使文档将** 文件标识为恶意保险箱，也允许用户单击"受保护的视图"：验证此设置是否 (![ 关闭 ](../../media/scc-toggle-off.png) 。) 。
...
2. 在 **"保险箱"** 页上，单击"全局设置"，然后在出现的飞出页面上配置以下设置：
   - **设置支持的应用部分中的内容Office 365应用**：
     ...
     - **请勿让用户单击浏览** 到 Office 365 应用中的原始 URL：验证此设置是否 (![ 打开。) 。 ](../../media/scc-toggle-on.png)
