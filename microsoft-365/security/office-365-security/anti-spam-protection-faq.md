...
本主题提供了有关使用 Exchange Online 中的邮箱的 Microsoft 365 组织的反恶意软件保护的常见问题和解答，或者没有 Exchange Online 邮箱的独立 Exchange Online Protection （EOP）组织。
...
注册服务并添加域后，垃圾邮件筛选功能将自动启用。 默认情况下，将对垃圾邮件筛选进行调整，以保护您无需任何其他配置（除了前面提到的针对混合环境中独立 EOP 独立客户的例外情况）。 作为管理员，您可以编辑默认垃圾邮件筛选设置，以最大限度地满足组织的需求。 为了获得更多的粒度，您还可以创建应用于组织中的指定用户、组或域的反垃圾邮件策略和出站反垃圾邮件策略。 虽然自定义策略的优先级始终高于默认策略，但可以更改自定义策略的优先级（即运行顺序）。
...
## <a name="how-can-customers-using-the-service-send-false-negative-spam-and-false-positive-non-spam-messages-to-microsoft"></a>客户如何使用服务向 Microsoft 发送假否定（垃圾邮件）和误报（非垃圾邮件）邮件？
...
## <a name="will-the-service-throttle-rate-limit-my-mail-if-my-users-send-outbound-spam"></a>如果我的用户发送出站垃圾邮件，服务限制（速率限制）我的邮件？

如果在特定时间段内通过服务从用户发送的邮件超过一半（例如，每小时），则由 EOP 确定为垃圾邮件，将阻止该用户发送邮件。 在大多数情况下，如果将出站邮件确定为垃圾邮件，则会通过高风险传递池路由此邮件，这会降低将正常的出站 IP 池添加到阻止列表的可能性。
