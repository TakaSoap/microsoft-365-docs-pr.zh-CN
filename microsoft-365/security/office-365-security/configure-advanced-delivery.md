...
- 专用于第三方网络钓鱼模拟：
  ...
  - 当用户使用报告邮件或报告网络钓鱼外接程序报告网络钓鱼模拟[](enable-the-report-message-add-in.md)邮件时，系统不会生成警报、调查或事件。 链接或文件不会触发，但邮件也会显示在"提交"页面的"用户报告的邮件 **"** 选项卡上。
...
- 威胁[防护状态报告](view-email-security-reports.md#threat-protection-status-report)：管理员可以在下拉菜单中按"系统覆盖"查看数据进行筛选，并选择查看由于网络钓鱼模拟系统覆盖而允许的邮件。 To see messages allowed by the SecOps mailbox override， you can select **chart breakdown by delivery location** in the **chart breakdown by reason** drop down menu.
- [Microsoft Defender for Endpoint 中的](../defender-endpoint/advanced-hunting-overview.md)高级搜寻：EmailEvents 中的网络钓鱼模拟和 SecOps 邮箱系统覆盖将显示为 OrgLevelPolicy 中的选项。
...
- 您需获得权限，然后才能执行本文中的过程：
  ...
  - 若要对高级传递策略进行只读访问，你需要是全局读者或安全读者 **角色组的成员**。
...
1. In the Microsoft 365 Defender portal at <https://security.microsoft.com>， go to **Email & Collaboration** \> **Policies & Rules** \> **Threat policies** \> **Advanced delivery** in the **Rules** section. 若要直接转到高级 **传递页面，** 请使用 <https://security.microsoft.com/advanceddelivery>。
...
3. 在打开的"编辑 **SecOps** 邮箱"飞出控件上，通过执行下列步骤之一输入要指定为 SecOps Exchange Online现有邮箱：
   ...
   - 单击此框，开始键入邮箱 (名称、显示名称、别名、电子邮件地址、帐户名等 ) 的标识符，然后从结果中选择邮箱 (显示名称) 。
...
您配置的 SecOps 邮箱条目显示在 **SecOps 邮箱选项卡** 上。若要进行更改，请单击"编辑 !["图标。](../../media/m365-cc-sc-edit-icon.png) **在** 选项卡上编辑。
...
3. 在打开 **的"编辑第三方网络钓鱼** 模拟"飞出控件上，配置以下设置：
   ...
   - 域：展开此设置并输入至少一个电子邮件地址域 (例如，contoso.com) 方法是单击该框，输入值，然后按 Enter 或选择显示在框下方的值。 根据需要重复执行此步骤（次数不限）。 您最多可以添加 20 个条目。
     ...
     > [!NOTE]
     > 使用地址 (也称为 **邮件发件人** 地址、P1 发件人或信封发件人) ，用于邮件的 SMTP 传输或由网络钓鱼模拟供应商指定的域密钥识别邮件 (DKIM) 域。 `5321.MailFrom` 

   - **发送 IP**：展开此设置，并输入至少一个有效的 IPv4 地址，方法是单击该框，输入值，然后按 Enter 或选择框下方显示的值。 根据需要重复执行此步骤（次数不限）。 您最多可以添加 10 个条目。 有效值为：
     - 单个 IP：例如，192.168.1.1。
     - IP 范围：例如，192.168.0.1-192.168.0.254。
     - CIDR IP：例如，192.168.0.1/25。
   - 要允许的模拟 URL：展开此设置，并选择输入属于网络钓鱼模拟活动的特定 URL，这些 URL 不应被阻止或触发，方法是单击框，输入值，然后按 Enter 或选择框下方显示的值。 您最多可以添加 10 个条目。 有关 URL 语法格式，请参阅 [租户允许/阻止列表的 URL 语法](tenant-allow-block-list.md#url-syntax-for-the-tenant-allowblock-list)。 这些 URL 在单击时封装，但不被阻止。
   ...
   > 每个字段最多可以指定 10 个条目。
   > 在至少一个域和一个发送  **IP** 上必须存在匹配，但值之间没有关联。
...
- **第三方筛选器**：如果域的 MX 记录没有指向Office 365 (邮件将路由到其他位置) ，默认情况下，安全 [](secure-by-default.md) *不可用*。 如果要添加保护，则需要启用连接器的增强筛选 (也称为跳过 *列表) 。* 有关详细信息，请参阅[使用第三](/exchange/mail-flow-best-practices/manage-mail-flow-using-third-party-cloud)方云服务和邮件流管理Exchange Online。 如果您不希望增强连接器筛选，请使用邮件流规则 (也称为传输规则) ，以绕过 Microsoft 筛选已由第三方筛选评估的邮件。 有关详细信息，请参阅使用 [邮件流规则在邮件中设置 SCL](/exchange/security-and-compliance/mail-flow-rules/use-rules-to-set-scl)。

- 正在审查 **的** 误报：你可能希望暂时允许 Microsoft 通过管理员提交仍在分析的某些邮件，以报告被错误地标记为 [](admin-submission.md)对 Microsoft (错误误报的已知) 。 与所有替代一样， **_我们强烈建议这些_** 允许是临时的。
...
> [!NOTE]
> 无论您指定的 Name 值如何，策略名称都将为 _SecOpsOverridePolicy_，因此您可能还使用该值。
...
> [!NOTE]
> 无论指定 Name 值如何，规则名称都将为 _SecOpsOverrideRule_\<GUID\>\<GUID\>，其中是唯一的 GUID 值 (例如，6fed4b63-3563-495d-a481-b24a311f8329) 。
...
此示例将添加到 `secops2@contoso.com` SecOps 替代策略。
...
## <a name="security--compliance-center-powershell-procedures-for-third-party-phishing-simulations-in-the-advanced-delivery-policy"></a>安全&高级传递策略中第三方网络钓鱼模拟的 PowerShell 过程
...
**注意**：无论指定 Name 值如何，策略名称将为 _PhishSimOverridePolicy_，因此您可能还使用该值。
...
无论指定 Name 值如何，规则名称都将是 _PhishSimOverrideRule_\<GUID\>\<GUID\>，其中是唯一的 GUID 值 (例如，a0eae53e-d755-4a42-9320-b9c6b55c5011) 。
...
确定无效规则后，可以使用 **Remove-PhishSimOverrideRule** cmdlet 删除它们，如本文稍后 [所述](#use-powershell-to-remove-phishing-simulation-override-rules)。
...
通过 _Entries_ 参数) 或 **Get-TenantAllowBlockListItems** cmdlet 的输出中的 Identity (，可以标识要修改的条目 (_Ids_ 参数) 。
