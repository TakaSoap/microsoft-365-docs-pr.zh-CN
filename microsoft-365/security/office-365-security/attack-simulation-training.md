...
- **单击率**：单击此有效负载的人。
- **预测泄露率**：预测此负载Microsoft 365威胁的负载的历史数据。
...
- **复杂性**：根据有效负载中指示可能的攻击和拼写错误 (紧急性等的指示器) 。 更多指示器更易于识别为攻击，并指示复杂度更低。 可用值有：
  - **低**
  - **中**
  - **高**
- **源**：指示有效负载是在组织中创建的，还是 Microsoft 预先存在的有效负载目录的一部分。 有效值包含:
  ...
  - **租户** (自定义) 
  - **全部**
- 语言 **：可用** 值为：中文 (简体 **) 、** 中文 (**繁体) 、****英语**、**法语**、**德语**、**意大利语**、**日语**、**朝鲜** 语、葡萄牙语、**俄语**、**西班牙语** 和 **荷兰语**。
- **添加 (标记)**
...
单击" ![ 发送测试"图标。](../../media/m365-cc-sc-send-icon.png) **发送测试** 按钮，将有效负载电子邮件的副本发送给自己 (当前登录的用户) 进行检查。
