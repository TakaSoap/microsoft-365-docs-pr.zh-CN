...
Microsoft Defender for Office 365 中的攻击模拟培训允许你在组织上运行恶意网络攻击模拟，以测试安全策略和做法，并培训员工提高认知度并减少他们遭受攻击的可能性。 本文将引导你完成使用攻击模拟培训创建模拟网络钓鱼攻击。
...
> [!NOTE]
> 在模拟创建期间的任何时间点，你可以保存并关闭，以稍后继续配置模拟。

## <a name="selecting-a-social-engineering-technique"></a>选择社交工程技术

从 4 种不同的技术中选择，由[MITRE ATT&CK®。](https://attack.mitre.org/techniques/enterprise/) 不同的有效负载可用于不同的技术：

- **凭据收集** 尝试通过让用户访问具有输入框的已知网站来提交用户名和密码来收集凭据。
- **恶意软件附件** 向邮件添加恶意附件。 当用户打开附件时，将运行任意代码，帮助攻击者破坏目标设备。
- **附件中的链接** 是凭据获取混合的一种类型。 攻击者在电子邮件附件中插入 URL。 附件中的 URL 遵循与凭据获取相同的技术。
- **指向恶意软件** 的链接将运行一些来自已知文件共享服务上托管的文件的任意代码。 发送给用户的邮件将包含指向此恶意文件的链接。 打开文件并帮助攻击者破坏目标设备。
- **"按 URL"** 是消息中的恶意 URL 将用户指向一个看起来熟悉的网站，该网站以静默方式运行和/或在用户的设备上安装代码代码。
...
- **复杂性**（通过 **筛选器提供**）基于有效负载中的指示器数量计算，这些指示器可提示目标受到攻击。 指标越多，复杂性越低。
- **源**（通过 **筛选器提供**）指示有效负载是在租户上创建的，还是 Microsoft 预先存在的有效负载目录的一部分， (全局) 。

![Microsoft 365 安全中心的攻击模拟培训中选定的有效负载](../../media/attack-sim-preview-select-payload.png)

从列表中选择一个有效负载，以查看有效负载的预览，并提供有关它的其他信息。

如果你想要创建自己的有效负载，请阅读为攻击模拟培训 [创建有效负载](attack-simulation-training-payloads.md)。

## <a name="audience-targeting"></a>访问群体设定

现在，该选择此模拟的受众了。 可以选择包括 **组织中所有用户或** 仅包括 **特定用户和组**。

当你选择仅 **包含特定用户和组时** ，你可以：

- **添加** 用户，这允许您利用租户的搜索以及高级搜索和筛选功能，例如面向过去 3 个月内未被模拟定位的用户。
...
我们建议你为每个模拟分配培训，因为经过培训的员工不太容易受到类似攻击。
...
在以下步骤中，如果选择自己选择培训，则需要添加培训，并自定义培训登陆页面。 你将能够预览培训登陆页面，以及更改其标头和正文。

## <a name="launch-details-and-review"></a>启动详细信息和查看

现在，所有内容已配置完成，你可以立即启动此模拟或安排在以后日期进行。 你还需要选择何时结束此模拟。 我们将停止捕获与所选时间后此模拟的交互。
