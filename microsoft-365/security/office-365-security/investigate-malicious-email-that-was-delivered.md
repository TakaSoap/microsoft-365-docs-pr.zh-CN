...
[Microsoft Defender For Office 365](office-365-atp.md) 使你能够调查将组织中的人员面临风险的活动，并采取措施保护你的组织。 例如，如果您是组织的安全团队的一部分，则可以查找并调查已传递的可疑电子邮件。 您可以使用 [威胁资源管理器 (或) 的实时检测 ](threat-explorer.md)来执行此操作。
...
- 您的组织具有 [Microsoft Defender For Office 365](office-365-atp.md) ，并且 [许可证已分配给用户](../../admin/manage/assign-licenses-to-users.md)。
...
- 您是全局管理员，或者您具有安全管理员或在安全 & 合规中心中分配的搜索和清除角色。 请参阅 [安全性 & 合规性中心中的权限](permissions-in-the-security-and-compliance-center.md)。 对于某些操作，还必须分配新的预览角色。
...
3. **威胁资源管理器中的视图**：在 " **视图** " 菜单中选择 " **所有电子邮件**"。
    ...
    *恶意软件* 视图当前是默认的，并捕获检测到恶意软件威胁的电子邮件。 *网络钓鱼* 视图的工作方式与网络钓鱼的操作方式相同。
    ...
    但是， *所有的电子邮件视图都会* 列出组织收到的每封邮件，无论是否检测到威胁。 正如您所想到的，这是大量数据，这就是为什么此视图显示一个要求应用筛选器的占位符。  (此视图仅适用于 Office 365 P2 客户的 Defender。 ) 
