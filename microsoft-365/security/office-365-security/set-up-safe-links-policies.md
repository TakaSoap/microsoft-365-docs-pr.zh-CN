...
可以在 保险箱 门户中或在 PowerShell (Exchange Online Microsoft 365 Defender PowerShell 中为Microsoft 365邮箱的合格组织配置 Exchange Online 链接Exchange Online;适用于没有邮箱，但Exchange Online Microsoft Defender for Office 365加载项订阅的组织的独立 EOP PowerShell) 。
...
当你在网站门户中管理"链接"保险箱，这两个元素Microsoft 365 Defender并不明显：
...
- 有关推荐用于链接策略保险箱，请参阅保险箱[链接策略设置。](recommended-settings-for-eop-and-office365.md#safe-links-policy-settings)
...
在 保险箱 门户中创建自定义链接Microsoft 365 Defender将同时创建安全链接规则和相关的安全链接策略，并使用相同的名称。
...
>   - 在 `$false` **New-SafeLinksRule** cmdlet (上创建新策略为已禁用) 。
