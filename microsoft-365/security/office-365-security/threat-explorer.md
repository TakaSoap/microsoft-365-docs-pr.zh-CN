...
|使用 Microsoft Defender for Office 365 计划 2，可以看到：|使用 Microsoft Defender for Office 365 计划 1，可以看到：|
|---|---|
...
这些更新后，你将看到每封邮件的单个条目，无论影响邮件的不同传递后事件如何。 操作可以包括 ZAP、手动修正 (这意味着管理员) 操作、动态传递等。

除了显示恶意软件和网络钓鱼威胁之外，还可以看到与电子邮件关联的垃圾邮件裁定。 在电子邮件中，查看与电子邮件关联的所有威胁以及相应的检测技术。 电子邮件可以具有零、一或多个威胁。 你将在电子邮件飞出的" **详细信息"部分** 看到当前威胁。 对于恶意软件 (网络钓鱼) ，检测技术字段显示威胁检测映射，这是标识威胁的检测技术。
...
>
> - 如果邮件已送达，您可能会将传递位置视为"已送达"，并且传递位置为"未知"，但收件箱规则将邮件移动到默认文件夹 (例如"草稿"或"存档") ，而不是"收件箱"或"垃圾邮件"文件夹。
>
...
*在电子邮件* 传递后应用了其他操作。 它们可以包括 *ZAP、* 管理员 (手动修正操作（如软删除) 、动态传递和重新处理 (，这些操作被反向检测为良好) 。 

> [!NOTE]
> - 作为挂起更改的一部分，"由 ZAP 删除"值当前在传递操作筛选器中显示将消失。 你将具有通过其他操作搜索 ZAP 尝试的所有 **电子邮件的方法**。
>
...
- 跨 URL 筛选器 (*URL* 域与 *URL* 域和路径) ：更新会影响搜索包含 URL/单击裁定的邮件。 我们启用了与协议无关的搜索支持，因此无需使用即可搜索 `http` URL。 默认情况下，除非明确指定了其他值，否则 URL 搜索将映射到 http。 例如：
...
> [!IMPORTANT]
> 按用户标记进行筛选和排序当前处于公共预览阶段。 在商业发布之前，可能会对此功能进行重大修改。 Microsoft 对提供的信息不做出明示或暗示的担保。
...
例如，一些用户评论了与自动刷新 (混淆，例如，一旦更改日期，页面就会刷新) ，并手动刷新 (其他筛选器) 。 同样，删除筛选器会导致自动刷新。 在修改查询时更改筛选器可能会导致搜索体验不一致。 为了解决这些问题，我们将迁移到手动筛选机制。

从体验的角度来看，用户可以从筛选器集和日期) 应用和删除不同的筛选器 (范围，并选择刷新按钮以在定义查询后筛选结果。 屏幕上现在也强调刷新按钮。 我们还更新了相关工具提示和产品内文档。
...
你将能够导出目标用户列表（最多 3，000 个）以及每个电子邮件视图的脱机分析尝试次数。 此外，选择尝试次数（例如 (，) 下图中的 13 次尝试将在威胁资源管理器中打开筛选视图，因此可以查看该用户的电子邮件和威胁的更多详细信息。
...
连接器是一组说明，用于自定义电子邮件在 Microsoft 365 或 Office 365 组织之间流动的流式。 它们使您能够应用任何安全限制或控件。 在威胁资源管理器中，你现在可以查看与电子邮件相关的连接器，然后使用连接器名称搜索电子邮件。
...
现在，你可以预览电子邮件头，在威胁资源管理器中下载电子邮件正文，管理员可以分析下载的邮件头/电子邮件中的威胁。 由于下载电子邮件可能会暴露信息的风险，因此此过程由基于角色的访问控制 (RBAC) 。 必须将新角色 *"预览*"添加到另一个角色组 (如安全操作或安全管理员) ，以授予在"所有电子邮件"视图中下载邮件的能力。 但是，查看电子邮件头不需要任何其他角色 (在威胁资源管理器中查看邮件) 。
...
|---|---|---|---|
|电子邮件已传递到用户的收件箱或文件夹，用户可以访问它。|电子邮件已发送到用户的"垃圾邮件"或"已删除"文件夹，用户可以访问它。|已隔离、失败或已丢弃的电子邮件。 用户无法访问这些邮件。|电子邮件的恶意附件替换为 .txt 文件，这些文件指出附件是恶意附件。|
...
现在可以将 URL 单击报告导出到 Microsoft Excel 以查看其网络消息 **ID** 和单击裁定，这有助于说明 URL 单击流量的来源。 操作方法如下：在 Office 365 快速启动栏上的威胁管理中，按照以下链操作：
...
2. 在"**查看"** 菜单中，选择 **"电子邮件** \> **钓鱼邮件"。**
   ...
   > [!div class="mx-imgBorder"]
   > ![网络钓鱼上下文中资源管理器的视图菜单](../../media/ExplorerViewEmailPhishMenu.png)
...
4. 选择一个或多个选项，如"阻止"和"阻止 **覆盖**"，然后选择与应用该筛选器的选项位于同一行上的"刷新"按钮。  (不刷新浏览器窗口。) 
   ...
   - **顶部单击** 是单击的安全链接包装 URL，按总点击计数排序。 为了简化视图，也不显示此列。 按列的总计数指示每个单击 URL 的安全链接单击裁定计数。 在网络钓鱼电子邮件视图中，这些 URL 通常是可疑或恶意的 URL。 但是，该视图可能包含不是威胁但包含钓鱼邮件的 URL。 此处不会显示对未包装链接的 URL 单击。
...
- **已阻止：** 阻止用户导航到 URL。
- **待定裁定：** 用户已显示等待触发的页面。
- **被阻止重写：** 阻止用户直接导航到 URL。 但是，用户会过度设置此块以导航到 URL。
