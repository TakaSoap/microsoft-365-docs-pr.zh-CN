...
使用本文中的步骤针对基线、敏感和高度保密的团队及其基础 SharePoint 网站设计并部署保留标签和数据丢失防护 (DLP) 策略。 有关这三层保护的详细信息，请参阅[保护 Microsoft Teams 中的文件](secure-files-in-teams.md)。
  
## <a name="how-this-works"></a>工作原理

1. 创建所需的保留标签并发布这些标签。 最多可能需要 12 个小时来发布这些标签。
2. 对于所需的基础 SharePoint 网站，将文档库设置编辑为向库中项目应用所需的保留标签。
3. 创建 DLP 策略以根据保留标签执行相关操作。

当用户将文档添加到团队的基础 SharePoint 网站库时，默认情况下该文档将接收分配的保留标签。 用户可以根据需要更改标签。 当用户在组织外共享文档时，DLP 将检查是否已分配标签，并在 DLP 策略与标签匹配时执行相关操作。 DLP 还将查找其他策略匹配，例如，如果配置了此类策略，则使用信用卡号保护文件。 

## <a name="retention-labels-for-your-underlying-sharepoint-sites"></a>基础 SharePoint 网站的保留标签

为基础 SharePoint 网站创建并分配保留标签分为三个阶段。
  
### <a name="step-1-determine-the-retention-label-names"></a>阶段 1：确定保留标签名称

在此阶段，对于应用到基础 SharePoint 网站的四个级别的信息保护，确定保留标签的名称。 下表列出了针对每个级别建议的名称。
  
|**基础 SharePoint 网站保护级别**|**标签名称**|
|:-----|:-----|
|基线 - 公用  <br/> |内部公用  <br/> |
|基线 - 专用  <br/> |Private  <br/> |
|敏感  <br/> |敏感  <br/> |
...
### <a name="step-2-create-the-retention-labels"></a>阶段 2：创建保留标签
...
8. 对于其他标签，请单击“创建标签”****，然后根据需要重复此过程的步骤 3-7。
...
### <a name="step-3-apply-the-retention-labels-to-your-underlying-sharepoint-sites"></a>步骤 3：将保留标签应用于基础 SharePoint 网站

使用这些步骤将保留标签应用于基础 SharePoint 网站的文档文件夹。
  
1.  在团队中，单击“文件”，然后单击“在 SharePoint 中打开”********。

2. 在浏览器的新“SharePoint 网站”标签页中，单击“文档”****。
    
3. 单击设置图标，然后单击“库设置”****。
    
4. 在“权限和管理”下，单击“向此库中的项应用标签”********。
    
5. 在“**设置-应用标签**”中，选择相应的保留标签，然后单击“**保存**”。
    
6. 关闭 SharePoint 网站的选项卡。
    
7. 重复步骤 1-6，将保留标签分配给其他基础 SharePoint 网站。
...
使用以下步骤配置 DLP 策略，该策略可在用户在组织外共享关于基础 SharePoint 网站的文档时进行通知。

1. 使用具有安全管理员或公司管理员角色的帐户登录 [Microsoft 365 合规性门户](https://compliance.microsoft.com/)。
...
[使用灵敏度标签保护团队中的文件](deploy-teams-sensitivity-labels.md)
