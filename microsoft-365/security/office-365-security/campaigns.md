...
- **筛选器裁定**：此处的值与[反垃圾邮件标头](anti-spam-message-headers.md)中所述的可用反钓鱼和反垃圾邮件筛选器裁定相关。 下表描述了可用的值：

  |值|垃圾邮件筛选器判定|说明|
  |:-----|:-----|:-----|
  | **Allowed**|`SFV:SKN` <br/><br/> `SFV:SKI`|垃圾邮件筛选器（例如，通过邮件流规则（也称为传输规则）进行评估之前，邮件被标记为 "非垃圾邮件" 和/或 "已跳过筛选"。<br/><br/>邮件因其他原因而跳过垃圾邮件筛选（例如，发件人和收件人似乎位于同一组织中）。|
  |**Blocked**|`SFV:SKS`|垃圾邮件筛选器（例如，通过邮件流规则）对邮件进行评估之前，邮件被标记为垃圾邮件。|
  |**已检测**|`SFV:SPM`|垃圾邮件筛选器已将邮件标记为垃圾邮件。|
  |**未检测到**|`SFV:NSPM`|垃圾邮件筛选器已将邮件标记为 "非垃圾邮件"。|
  |**以后**|`SFV:SKQ`|邮件跳过垃圾邮件筛选，因为它已从隔离区中释放。|
  |**租户允许**<sup>\*</sup>|`SFV:SKA`|邮件因垃圾邮件筛选器策略配置而跳过垃圾邮件筛选（例如，发件人或域位于**发件人允许**列表中）。|
  |**租户块**<sup>\*\*</sup>|`SFV:SKA`|由于垃圾邮件筛选器策略配置（例如，发件人或域位于**发件人阻止**列表中），垃圾邮件筛选功能阻止了邮件。|
  |**用户允许**<sup>\*</sup>|`SFV:SFE`|邮件跳过垃圾邮件筛选，因为发件人位于 Outlook 的用户安全发件人列表中。|
  |**用户块**<sup>\*\*</sup>|`SFV:BLK`|邮件被垃圾邮件筛选阻止，因为发件人位于 Outlook 中的用户阻止的发件人列表中。|
  |**ZAP**|无|[零小时自动清除（ZAP）](zero-hour-auto-purge.md)根据垃圾邮件筛选器策略配置（移至 "垃圾邮件" 文件夹或隔离）对已发送邮件采取操作。|

  <sup>\*</sup>检查垃圾邮件筛选器策略配置设置，因为服务可能阻止了允许的邮件。

  <sup>\*\*</sup>检查垃圾邮件筛选器策略配置设置，因为应隔离和不传递这些邮件。
