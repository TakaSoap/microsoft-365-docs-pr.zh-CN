...
- 为欺骗的未经身份验证的发件人显示 **(？？) ：** 如果邮件未通过 SPF 或 DKIM 检查且邮件未通过 DMARC 或复合身份验证，则此通知会将问号添加到发件人的照片的"发件人"[框中。](email-validation-and-authentication.md#composite-authentication) 关闭此设置后，不会将问号添加到发件人的照片中。

- 显示"通过 <u>"标记</u>**？：** 如果发件人地址 (电子邮件客户端) 中显示的邮件发件人与 DKIM 签名或 **MAIL FROM** 地址中的域不同，此通知将在"发件人"框中通过 fabrikam.com) 添加 via 标记 (chris@contoso.com。 有关这些地址详细信息，请参阅 [电子邮件标准概述](how-office-365-validates-the-from-address.md#an-overview-of-email-message-standards)。
...
"**显示第一安全提示** 联系人"设置在 EOP 和 Defender 中可用于 Office 365 组织，并且对欺骗智能或模拟保护设置没有依赖性。 以下安全提示向收件人显示此策略：
...
![具有安全提示的邮件的第一个联系人联系人。](../../media/safety-tip-first-contact-multiple-recipients.png)
...
- **允许用户保护**：防止指定的内部或外部电子邮件地址被模拟 **为邮件发件人**。 例如，您收到一封来自公司副总裁的电子邮件，要求您向她发送一些内部公司信息。 是否执行？ 许多人在未思考的情况下发送回复。
  ...
  > [!NOTE]
  >
  > - 在每个防钓鱼策略中，最多可指定 350 个受保护的用户 (发件人电子邮件地址) 。 不能在多个策略中指定同一受保护的用户。 因此，无论向收件人应用多少策略，每个收件人 (发件人电子邮件地址) 用户的最大数量为 350。 有关策略优先级以及策略处理如何在应用第一个策略后停止的信息，请参阅电子邮件保护的顺序 [和优先级](how-policies-and-protections-are-combined.md)。
  ...
  当您将内部或外部电子邮件地址添加到"用户保护"列表时，来自这些发件人的邮件将接受模拟保护检查。 如果将邮件发送到应用于默认策略的所有收件人的收件人，则检查 (是否模拟;**自定义策略中的** 用户、组和域) 。 如果在发件人的电子邮件地址中检测到模拟，则用户模拟保护操作将应用于邮件 (以及对邮件执行哪些操作、是否显示模拟用户安全提示等) 。

- **启用域进行保护**：阻止在邮件发件人的域中 **模拟指定的域**。 例如，你拥有的所有域 (接受) 或特定自定义[](/exchange/mail-flow-best-practices/manage-accepted-domains/manage-accepted-domains) (或合作伙伴域) 。 此防止 **模拟** 的发件人域列表不同于策略应用于默认策略的所有 (收件人的列表; 特定收件人，如"常见策略设置"部分"用户、组和域"[](#common-policy-settings)设置) 。
  ...
  将域添加到"**启用域保护"列表** 时，来自这些域中发件人的邮件将接受模拟保护检查。 如果将邮件发送到应用于默认策略的所有收件人的收件人，则检查 (是否模拟;**自定义策略中的** 用户、组和域) 。 如果在发件人的域中检测到模拟，则域的模拟保护操作将应用于邮件 (该对邮件执行哪些操作，是否显示模拟用户安全提示等) 。
...
当增加此设置时， (标记为错误) 误报的可能性会增加。 有关建议设置的信息，请参阅 Microsoft Defender 中的防钓鱼策略[Office 365设置](recommended-settings-for-eop-and-office365.md#anti-phishing-policy-settings-in-microsoft-defender-for-office-365)。
