...
description: Microsoft Defender for Office 365 E5 P1 和 P2 客户现在可以 360 度查看每个包含电子邮件实体的电子邮件页面。
ms.openlocfilehash: 8bd5f152ff945a6a60941e836e7bdc34890e974b
ms.sourcegitcommit: f88a0ec621e7d9bc5f376eeaf70c8a9800711f88
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/15/2021
ms.locfileid: "59356509"
---
...
电子邮件实体页面在电子邮件Microsoft 365 Defender门户 () <https://security.microsoft.com> 在&**资源管理器** \> **中提供**。 或者，若要直接转到资源管理器 **页面** ，请使用 <https://security.microsoft.com/threatexplorer> 。
...
1. **时间线**：每个资源管理器时间线 (电子邮件时间线视图) 电子邮件上发生的传递后事件的原始传递时间。 对于没有传递后操作的电子邮件，视图在日程表视图中显示原始传递行。 来自系统、管理员和用户等来源的零时差自动清除 (ZAP) 、修正、URL 单击、et cetera 等事件按发生顺序显示在此处。
...
如果邮件仍存在于云中，管理员可以预览云邮箱中的电子邮件。 如果管理员、 (或用户) 或 ZAP (对) 执行软删除操作，则云位置中不再存在电子邮件。 在这种情况下，管理员将无法预览这些特定邮件。 已丢弃或传递失败的电子邮件永远不会进入邮箱。 因此，管理员也将无法预览这些电子邮件。
...
1. 行为 *详细信息* 是一个导出，用于显示行为详细信息，如触发期间发生的确切事件，以及包含在爆炸期间发现的 URL、IP、域和文件的可观测项 (它们可能是有问题的或不良的) 。 请注意，可能没有以下行为详细信息：
...
*最新送达位置*：最新的送达位置是电子邮件在系统操作（如 ZAP）或管理员操作（如"移动到已删除项目"）之后登录的位置。 最新送达位置并不用于通知管理员邮件 *的当前位置。* 例如，如果用户删除邮件或将其移动到存档，则传递位置将不会更新。 但是，如果已执行系统操作并更新位置 (如 ZAP 导致电子邮件移动到隔离邮箱) 这会将"最新传递位置"更新为"隔离"。
...
- *垃圾邮件规则*："垃圾邮件"是隐藏的收件箱规则，默认在邮箱中启用。
  - 在邮箱上启用垃圾邮件规则后，Exchange Online Protection (EOP) 根据某些条件将邮件移动到垃圾邮件。 移动可以基于垃圾邮件筛选裁定操作 将 *邮件移动到垃圾邮件文件夹*，或邮箱上的阻止发件人列表。 禁用垃圾邮件规则会基于邮箱上的"发件人保险箱 *阻止* 将邮件发送到"垃圾邮件"文件夹。
