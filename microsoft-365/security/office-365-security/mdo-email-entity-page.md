...
*最新送达位置*：最新的送达位置是电子邮件在系统操作（如 ZAP）或管理员操作（如"移动到已删除邮件"）之后登录的位置。 最新送达位置并不用于通知管理员邮件 *的当前位置。* 例如，如果用户删除邮件或将其移动到存档，则传递位置将不会更新。 但是，如果已执行系统操作并更新位置 (如 ZAP 导致电子邮件移动到隔离邮箱) 这会将"最新送达位置"更新为隔离。
...
- *批量投诉级别 (BCL) ：* 邮件的批量 () 级别。 BCL 越高，如果电子邮件可能是垃圾邮件，则 () 产生投诉的可能性更大。
