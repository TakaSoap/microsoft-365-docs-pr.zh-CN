...
Microsoft Defender for Office 365 保护您的组织免受电子邮件、链接和 (URL) 和协作工具造成的恶意威胁。 Defender for Office 365 包括：
...
Microsoft Defender for Office 365 试用版是试用适用于 Office 365 的 Defender 功能的最简单方法，设置它只需单击几下。 试用设置完成后，所有 Defender for Office 365 计划 1 和计划 2 功能在组织中最多可用 90 天。
...
注册试用版后，可能需要最多 2 小时更改和更新才能可用。 管理员必须注销并重新登录才能查看更改。

管理员可以随时通过访问用户卡片禁用 <> 。

## <a name="availability"></a>供应情况

Defender for Office 365 试用版将逐步向满足特定条件 (（包括地理位置) ）且其订阅中不包含现有 Defender for Office 365 计划 1 或计划 2 许可证 (或作为附加) 的现有客户推出。
...
可靠的筛选堆栈可防止各种基于卷和目标的攻击，包括商业电子邮件泄露、凭据钓鱼、勒索软件以及高级恶意软件。
...
丰富的模拟和培训功能以及客户端应用程序中的集成体验可增强用户意识。
