...
ms.locfileid: "62074722"
---
# <a name="migrate-to-microsoft-defender-for-office-365---phase-2-setup"></a>迁移到 Microsoft Defender for Office 365 - 阶段 2：设置
...
可以指定一个Exchange Online邮箱，以接收用户报告为恶意或不恶意的邮件。 有关更多说明，请参阅 [用户报告的邮件设置](user-submission.md)。 此邮箱可以接收用户提交到 Microsoft 的邮件副本，或者邮箱可以截获邮件，而无需将其报告给 Microsoft (安全团队可以手动分析和提交邮件) 。 但是，这种拦截方法不允许服务自动调整和学习。
...
由于入站电子邮件通过位于 Microsoft 365 前面的另一个保护服务进行路由，因此在 Exchange Online 中，您可能已有一个邮件流规则 (也称为传输规则) ，它将所有传入邮件的垃圾邮件可信度 (SCL) 都设置为值 -1 (会绕过垃圾邮件筛选) 。 大多数第三方保护服务都鼓励使用此 SCL=-1 邮件流规则Microsoft 365想要使用其服务的客户使用此 SCL=-1 邮件流规则。
...
有关详细信息，请参阅使用邮件流规则将垃圾邮件可信度设置为 (中的) [SCL Exchange Online。](/exchange/security-and-compliance/mail-flow-rules/use-rules-to-set-scl)
...
- 如果计划允许 Internet 邮件通过现有保护服务并同时直接流入 Microsoft 365，则需要将绕过垃圾邮件筛选) 的 SCL=-1 邮件流规则 (邮件限制为仅通过现有保护服务的邮件。 您不希望未筛选的 Internet 邮件登录 Microsoft 365。
  ...
  - **对于基于云的保护服务**：可以使用组织独有的标头和标头值。 具有标头的邮件不会由邮件头Microsoft 365。 没有标头的邮件由邮件头Microsoft 365
...
首先要执行的是配置增强的连接器[筛选 (也称为](/exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/enhanced-filtering-for-connectors)跳过连接器上用于从现有保护服务流向) 的邮件的 Microsoft 365。 可以使用入站 [邮件报告来帮助](/exchange/monitoring/mail-flow-reports/mfr-inbound-messages-and-outbound-messages-reports) 标识连接器。
...
若要正确启用连接器的增强筛选功能，需要添加所有第三方服务和/或本地电子邮件系统主机的公共 **IP** \* \* **\* \*** 地址，以将入站邮件路由到 Microsoft 365。
...
有关建议设置，请参阅建议保险箱[附件策略设置。](recommended-settings-for-eop-and-office365.md#safe-attachments-policy-settings) 请注意，"标准"和"严格"建议是相同的。 若要创建策略，请参阅设置保险箱[附件策略。](set-up-safe-attachments-policies.md) 请务必将 **MDOPilot \_ SafeAttachments** 组用作策略应用于 (策略的策略) 。
...
有关推荐设置，请参阅推荐保险箱[链接策略设置](recommended-settings-for-eop-and-office365.md#safe-links-settings)。 请注意，"标准"和"严格"建议是相同的。 若要创建策略，请参阅设置链接[保险箱策略](set-up-safe-links-policies.md)。 请务必将 **MDOPilot \_ SafeLinks** 组用作策略应用于 (策略的策略条件) 。

> [!IMPORTANT]
> 目前，没有默认的"链接保险箱策略。 在切换任何 MX 记录之前，建议您使用一个保险箱链接策略来保护整个组织。
...
- 使用严格设置的策略。 使用组 **MDOPilot \_ SpamPhish \_ Strict** 作为策略 (策略应用于组的条件) 。 此策略的优先级应高于 (标准) 策略的优先级。
...
- 使用严格设置的策略，模拟检测操作除外，如下所述。 使用组 **MDOPilot \_ SpamPhish \_ Strict** 作为策略 (策略应用于组的条件) 。 此策略的优先级应高于 (标准) 策略的优先级。
...
根据标准或严格 (，你可以调整欺骗保护 (允许和阻止) 并启用每个模拟保护操作来隔离邮件或将邮件移动到垃圾邮件文件夹 () 。 你可以观察结果并根据需要调整其设置。
...
- [在 Defender for Office 365 中配置防钓鱼策略](configure-mdo-anti-phishing-policies.md)。
