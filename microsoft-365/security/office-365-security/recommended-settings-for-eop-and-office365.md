...
若要自动将"标准"或"严格"设置应用于用户，请参阅预设[EOP](preset-security-policies.md)和 Microsoft Defender 中的Office 365。

本文介绍了默认设置，以及建议的标准和严格设置，以帮助保护用户。 这些表包含 Microsoft 365 Defender 门户和 PowerShell (Exchange Online PowerShell 中的设置，或适用于没有 Exchange Online 邮箱的组织的独立 Exchange Online Protection PowerShell) 。

> [!TIP]
> 你无法更改应用门户中建议的"标准"和"严格Microsoft 365 Defender设置。 若要更改建议的值 **，如允许用户保护**，你需要使用 [Exchange Online PowerShell](/powershell/exchange/connect-to-exchange-online-powershell)。
>
...
|为网络钓鱼邮件启用零 (ZAP) 清除 <p> _PhishZapEnabled_|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`||
...
|**HTML 表单中的 Web** (_MarkAsSpamWebBugsInHtml_) ||
...
| _TestModeAction_ (测试) |对于支持"测试为操作"的 ASF 设置，可以将测试模式操作配置为"无"、添加默认 **X 标头** 文本或"发送 **Bcc** `None` (、或 `AddXHeader` `BccMessage`) "。 有关详细信息，请参阅启用 [、禁用或测试 ASF 设置](advanced-spam-filtering-asf-options.md#enable-disable-or-test-asf-settings)。|
|
...
|**显示 () 欺骗的未经身份验证的发件人的发件人身份** <p> _EnableUnauthenticatedSender_|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`|向 () 发件人的照片添加Outlook问号。 有关详细信息，请参阅未经 [身份验证的发件人](set-up-anti-phishing-policies.md#unauthenticated-sender)。|
...
如果你的订阅包括 Microsoft Defender for Office 365或者你已购买 Defender for Office 365 作为加载项，请设置以下标准或严格配置。
...
如前文所述，EOP 客户会获得基本的防钓鱼功能，Office 365 Defender for Office 365更多的功能和控制，以帮助防止、检测和修正攻击。 若要创建和配置这些策略，请参阅在 Defender for [Office 365 中配置防钓鱼Office 365。](configure-mdo-anti-phishing-policies.md)
...
|---|:---:|:---:|:---:|---|
|**网络钓鱼阈值&保护**|||||
|**允许用户保护 (** 的用户保护)  <p> _EnableTargetedUserProtection_ <p> _TargetedUsersToProtect_|未选定 <p> `$false` <p> 无|已选中 <p> `$true` <p> \<list of users\>|已选中 <p> `$true` <p> \<list of users\>|我们建议在 (角色) 用户和邮件发件人。 在内部，受保护的发件人可能是 CEO、CFO 和其他高级领导。 从外部来说，受保护的发件人可以包含会员或你的委员会。 无法修改预设安全策略;如果要根据建议在关键角色中添加用户，则必须禁用这些角色。|
...
|**操作**||||无论您选择"隔离 **邮件"，"****选择隔离策略"** 框都可用。 隔离策略定义允许用户对隔离邮件执行哪些操作。 <p> 创建新的防钓鱼策略时，空值表示默认隔离策略用于定义该裁定 (DefaultFullAccessPolicy 针对所有模拟检测类型) 隔离的邮件的历史功能。 <p> 管理员可以创建和选择自定义隔离策略，这些策略为用户定义限制性较低或限制更严格的功能。 有关详细信息，请参阅 [隔离策略](quarantine-policies.md)。|
...
|**启用欺骗智能** <p> _EnableSpoofIntelligence_|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`||
|**操作**|||||
|**如果邮件被检测为欺骗邮件** <p> _AuthenticationFailAction_|**将邮件移动到收件人的"垃圾邮件"文件夹** <p> `MoveToJmf`|**将邮件移动到收件人的"垃圾邮件"文件夹** <p> `MoveToJmf`|**隔离邮件** <p> `Quarantine`|此设置适用于欺骗性发件人，这些发件人被自动阻止，如欺骗智能见解中所示，[](learn-about-spoof-intelligence.md)或在租户允许/阻止列表中手动[阻止](tenant-allow-block-list.md)。 <p> 如果选择"**隔离邮件"，** 则"应用隔离策略"框可用于选择用于定义允许哪些用户对隔离邮件执行哪些操作的隔离策略。 创建新的防钓鱼策略时，空值表示默认隔离策略用于定义 DefaultFullAccessPolicy (欺骗隔离邮件的历史) 。 <p> 管理员可以创建和选择自定义隔离策略，该策略定义允许哪些收件人对隔离邮件执行哪些操作。 有关详细信息，请参阅 [隔离策略](quarantine-policies.md)。|
...
保险箱 Microsoft Defender for Office 365 中的附件包括与 保险箱 附件策略无关系的全局设置，以及特定于每个 保险箱 链接策略的设置。 有关详细信息，请参阅 defender 中的保险箱[附件Office 365。](safe-attachments.md)

尽管没有默认的"保险箱 附件"策略，但内置保护预设安全策略向未在自定义 保险箱 附件策略) 中定义的所有收件人 (提供 保险箱 附件保护。 有关详细信息，请参阅[Preset security policies in EOP and Microsoft Defender for Office 365](preset-security-policies.md)。
...
若要配置这些设置，请参阅在 Microsoft 365 E5 中保险箱启用[SharePoint、OneDrive](turn-on-mdo-for-spo-odb-and-teams.md)和 Microsoft Teams 保险箱[文档Microsoft 365 E5。](safe-docs.md)
...
|**为客户端保险箱文档Office文档** <p> _EnableSafeDocs_|关 <p> `$false`|开 <p> `$true`|此功能仅在 Defender for Office 365 (（例如，Microsoft 365 E5 或 Microsoft 365 E5 安全性) ）中不包含的许可证Microsoft 365 E5 安全性) 。 有关详细信息，请参阅 保险箱[Documents in Microsoft 365 E5](safe-docs.md)。|
...
|**如果扫描保险箱超时或错误，请应用附件检测 (响应)** <p> _ActionOnError_|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`||
|
...
|**在保险箱应用中使用Office 365链接** <p> _EnableSafeLinksForO365Clients_|打开 <p> `$true`|打开 <p> `$true`|使用保险箱 iOS 和 Android Office 365 应用支持 (移动链接) 链接。 有关详细信息，请参阅保险箱[应用的链接Office 365设置](safe-links.md#safe-links-settings-for-office-365-apps)。|
...
>
> "**自定义中的默认值**"列引用您创建的新保险箱链接策略中的默认值。 其余列 (，除非另有) 预设安全策略中配置的值。
...
|**不跟踪用户单击** <p> _DoNotTrackUserClicks_|未选定 <p> `$false`|未选定 <p> `$false`|未选定 <p> `$false`|未选定 <p> `$false`|关闭此设置后 (_DoNotTrackUserClicks_ 设置为) `$false` 用户单击。|
...
- 使用这些链接获取有关如何设置[EOP](/exchange/standalone-eop/set-up-your-eop-service)服务的信息，并配置[Microsoft Defender Office 365。](defender-for-office-365.md)  不要忘记'防范威胁中'中的有用[Office 365。](protect-against-threats.md)
