...
description: 什么是适用于 EOP Exchange Online Protection (和 Defender) 安全Office 365最佳做法？ 标准保护的当前建议是什么？ 如果要更加严格，应该使用什么？ 如果你还使用 Defender for Office 365？
ms.technology: mdo
...
若要自动将标准或严格设置应用于用户，请参阅预设 EOP 中的安全策略[和 Microsoft Defender for Office 365。](preset-security-policies.md)
...
|为网络钓鱼邮件启用零时差 (ZAP) 清除 <p> _PhishZapEnabled_|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`||
...
>
> - 没有可自动保险箱组织所有收件人的默认保险箱链接策略或附件策略。 若要获取保护，需要创建至少一个"链接保险箱策略保险箱附件"策略。
>
> - [保险箱文档](mdo-for-spo-odb-and-teams.md)SharePoint、OneDrive、Microsoft Teams 和 保险箱[文档的](safe-docs.md)附件不依赖于 保险箱 链接策略。

如果你的订阅包括 Microsoft Defender for Office 365或者如果你已购买 Defender for Office 365 作为加载项，请设置以下标准或严格配置。
...
有关此设置详细信息，请参阅 Microsoft Defender for Office 365 中的反网络钓鱼策略[中的高级网络钓鱼Office 365。](set-up-anti-phishing-policies.md#advanced-phishing-thresholds-in-anti-phishing-policies-in-microsoft-defender-for-office-365) 若要配置此设置，请参阅在 Defender for Office 365 中[配置防钓鱼Office 365。](configure-mdo-anti-phishing-policies.md)
...
|---|:---:|:---:|:---:|---|
|**网络钓鱼阈值&保护**|||||
|**允许用户保护 (** 的用户保护)  <p> _EnableTargetedUserProtection_ <p> _TargetedUsersToProtect_|未选定 <p> `$false` <p> 无|已选中 <p> `$true` <p> \<list of users\>|已选中 <p> `$true` <p> \<list of users\>|我们建议在主要角色 (用户) 发件人。 在内部，受保护的发件人可能是 CEO、CFO 和其他高级领导。 在外部，受保护的发件人可能包括会员或你的委员会。|
...
|**启用欺骗智能** <p> _EnableSpoofIntelligence_|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`||
|**操作**|||||
|**如果邮件被检测为欺骗邮件** <p> _AuthenticationFailAction_|**将邮件移动到收件人的"垃圾邮件"文件夹** <p> `MoveToJmf`|**将邮件移动到收件人的"垃圾邮件"文件夹** <p> `MoveToJmf`|**隔离邮件** <p> `Quarantine`|此设置适用于欺骗性发件人，这些发件人被自动阻止，如欺骗智能见解中所示，[](learn-about-spoof-intelligence.md)或在租户允许/阻止列表中手动[阻止](tenant-allow-block-list.md)。 <p> 如果选择"**隔离邮件"，** 则"应用隔离策略"框可用于选择用于定义允许哪些用户对隔离邮件执行哪些操作的隔离策略。 创建新的防钓鱼策略时，空值值意味着默认隔离策略用于定义欺骗隔离邮件的历史功能 (DefaultFullAccessPolicy) 。 <p> 管理员可以创建和选择自定义隔离策略，该策略定义允许哪些收件人对隔离邮件执行哪些操作。 有关详细信息，请参阅 [隔离策略](quarantine-policies.md)。|
...
|**显示 () 欺骗的未经身份验证的发件人的发件人身份** <p> _EnableUnauthenticatedSender_|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`|向发件人 () 发件人的照片添加问号Outlook欺骗发件人。 有关详细信息，请参阅未经 [身份验证的发件人](set-up-anti-phishing-policies.md#unauthenticated-sender)。|
|**显示"via"标记** <p> _EnableViaTag_|已选中 <p> `$true`|已选中 <p> `$true`|已选中 <p> `$true`|如果通过标签 (chris@contoso.com 地址 fabrikam.com) DKIM 签名或 MAIL FROM 地址中的域不同，则通过 fabrikam.com) 将 **标签添加到"收件人"** 地址。 <p> 有关详细信息，请参阅未经 [身份验证的发件人](set-up-anti-phishing-policies.md#unauthenticated-sender)。|
|
...
|**打开保险箱客户端的 Office 文档** <p> _EnableSafeDocs_|关 <p> `$false`|开 <p> `$true`|打开 <p> `$true`|此功能仅在 Defender for Office 365 (（例如，Microsoft 365 E5 或 Microsoft 365 E5 安全性) ）中未包含的许可证Microsoft 365 E5有意义。 有关详细信息，请参阅 保险箱[Documents in Microsoft 365 E5](safe-docs.md)。|
...
|**不允许用户单击访问应用中的原始 URL Office 365 URL** <p> _AllowClickThrough_|打开 <p> `$false`|打开 <p> `$false`|打开 <p> `$false`|打开此设置 (_将 AllowClickThrough_ 设置为) 阻止在受支持的应用中单击到 `$false` Office 365 URL。|
|
...
|**不允许用户单击访问原始 URL** <p> _DoNotAllowClickThrough_|未选定 <p> `$false`|已选中 <p> `$true`|已选中 <p> `$true`|打开此设置 (_DoNotAllowClickThrough_ 设置为) `$true` 阻止单击访问原始 URL。|
...
- 使用这些链接获取有关如何设置[EOP](/exchange/standalone-eop/set-up-your-eop-service)服务以及如何配置[Microsoft Defender Office 365。](defender-for-office-365.md)  不要忘记'防范威胁在威胁中'中的Office 365[说明。](protect-against-threats.md)
