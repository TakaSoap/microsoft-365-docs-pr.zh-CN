---
title: 在 Microsoft Defender 中配置防钓鱼策略以Office 365
f1.keywords:
- NOCSH
ms.author: chrisda
author: chrisda
...
---
# <a name="configure-anti-phishing-policies-in-microsoft-defender-for-office-365"></a>在 Microsoft Defender 中配置防钓鱼策略以Office 365
...
在 Microsoft 365 Defender 门户中创建自定义防钓鱼策略可同时创建反网络钓鱼规则和相关反网络钓鱼策略，并使用相同的名称。
...
5. 在出现的 **"网络钓鱼&** 保护"页上，配置以下设置：
   ...
   - **模拟**：这些设置是策略的一个条件，用于标识要单独或按域 (在入站邮件的"发件人"地址) 查找邮件的特定发件人。 有关详细信息，请参阅 Microsoft Defender for Office 365 中的防钓鱼[策略中的模拟Office 365。](set-up-anti-phishing-policies.md#impersonation-settings-in-anti-phishing-policies-in-microsoft-defender-for-office-365)
     ...
     > [!NOTE]
     >
     > - 在每个防钓鱼策略中，最多可指定 350 个受保护的用户 (发件人电子邮件地址) 。 不能在多个策略中指定同一受保护的用户。
     > - 如果发件人和收件人之前通过电子邮件进行通信，则用户模拟保护不起作用。 如果发件人和收件人从未通过电子邮件进行通信，则邮件将被标识为模拟尝试。

     - **允许用户保护**：默认值为 off， (未) 。 若要打开它，请选中该复选框，然后单击"管理 (**nn) 发件人 ()** 显示的链接。
   ...
   - **欺骗**：在此部分中，使用" **启用欺骗** 智能"复选框打开或关闭欺骗智能。 默认值在选中 (时) ，建议保持打开状态。 在下一页的"如果邮件被检测为欺骗"设置中，指定对来自被阻止的欺骗发件人的邮件要采取的操作。
...
6. 在出现的“**操作**”页面上，配置下列设置：
   ...
   - **安全&：** 配置以下设置：
     ...
     - **显示域模拟安全提示：** 只有在上一页上选择了"启用要保护的域"时，此设置才可用。
     ...
     - **显示"通过"标记**：仅在你在上一页上选择了" **启用** 欺骗智能"时，此设置才可用。 如果通过标签 (chris@contoso.com 通过 fabrikam.com) 添加到"收件人"地址（如果它不同于 DKIM 签名或 **MAIL FROM** 地址中的域）。 默认值在选定 (上) 。 若要将其关闭，请清除该复选框。

       > [!NOTE]
       > 如果你没有"显示 **""** 通过"标记设置，则问号和通过标记都由组织中"显示 **(？)** 欺骗的未经身份验证的发件人"设置控制。
...
- 反网络钓鱼策略按照其显示顺序进行处理 (第一个策略的优先级值为 0) 。  默认防钓鱼策略的优先级值为 **"** 最低"，你无法更改它。
...
在 PowerShell 中启用或禁用反网络钓鱼规则会启用或禁用整个防钓鱼策略 (反网络钓鱼规则以及分配的反网络钓鱼策略) 。 无法启用或禁用默认防钓鱼策略 (它始终应用于所有收件人) 。
...
若要验证您是否已成功在 Defender for Office 365 中配置防钓鱼策略，请执行以下步骤之一：

- 在 Microsoft 365 Defender 门户中，在"策略"部分&"电子邮件&协作策略"&"规则威胁策略 \>  \>  \> **""** 防钓鱼 **"。** 验证策略列表、 **策略的 Status** 值及其 **Priority** 值。 若要查看更多详细信息，请从列表中选择策略，方法是单击名称并查看出现的飞出内容中的详细信息。
