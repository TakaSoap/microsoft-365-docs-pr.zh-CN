...
> [!NOTE]
>
> 本文中所述的攻击模拟器现在是只读的，已替换为 [Microsoft 365](https://security.microsoft.com)安全中心中电子邮件&**协作** 节点中的攻击模拟培训。  有关详细信息，请参阅使用 [攻击模拟培训入门](attack-simulation-training-get-started.md)。
>
> 已禁用从此版本的攻击模拟器启动新模拟的能力。 但是，您仍可在 2021 年 1 月 24 日起最多 90 天内访问报告。

## <a name="what-do-you-need-to-know-before-you-begin"></a>开始前，有必要了解什么？

- 若要打开安全与合规中心，请转到 <https://protection.office.com/>。 攻击模拟器在威胁管理 **攻击** \> **模拟器中可用**。 转到直接攻击模拟器，打开 <https://protection.office.com/attacksimulator> 。
...
*网络钓鱼* 是电子邮件攻击的一般术语，它尝试窃取看似来自合法或受信任的发件人的邮件中的敏感信息。 *网络钓鱼* 是一种有针对性的网络钓鱼攻击，使用专门针对目标收件人定制的重点和自定义内容 (通常，攻击者在重新识别收件人后) 。
...
- **网络钓鱼 (附件**) ：攻击试图说服收件人在邮件中打开 .docx 或 .pdf 附件。 附件包含来自默认网络钓鱼链接的相同内容，但第一句以"开头，你将看到此邮件为您打开的最近 \<Display Name\> 电子邮件..."。
...
- **使用内置电子邮件模板**：有两个内置模板可用： **产品** /服务奖励和 **工资单更新**。 创建和启动市场活动时，可以进一步自定义模板中的部分、全部或无电子邮件属性。
...
3. 在打开 **的**"攻击详细信息"页的"网络钓鱼模板"部分，在 **"创建模板**"区域中，单击 **"新建模板"。**
...
#### <a name="step-2-create-and-launch-the-spear-phishing-campaign"></a>步骤 2：创建并启动钓鱼活动
...
3. " **配置网络钓鱼攻击"** 向导从新的飞出控件开始。 在 **"开始** "步骤中，执行以下步骤之一：
   ...
   - 单击 **"使用** 模板"，然后选择内置或自定义电子邮件模板。 选择模板后，将基于模板自动填充"名称"框，但可以更改名称。
...
4. 在 **"目标收件人"** 步骤中，执行下列步骤之一：
   ...
   - 单击 **"** 导入"，再单击"文件导入"以将逗号分隔 (CSV) 或电子邮件地址的行分隔文件中导入。 每行必须包含收件人的电子邮件地址。
...
5. 在 **"配置电子邮件详细信息** "步骤中，配置以下设置：
   ...
   - **网络钓鱼登录服务器 URL：** 单击下拉列表，然后从列表中选择一个可用的 URL。 这是用户将尝试单击的 URL。 ）这三个选项包括：
     ...
     > - URL 信誉服务可能会将其中一个或多个 URL 标识为不安全。 在网络钓鱼活动中使用 URL 之前，请检查受支持的 Web 浏览器中 URL 的可用性。
     >
     > - 您需要选择 URL。 对于 **Spear 钓鱼** (附件) 活动，可以在下一步 (中删除邮件正文中的链接，否则邮件将同时包含链接和附件) 。 
...
6. 在 **"撰写电子邮件** "步骤中，创建电子邮件的邮件正文。 如果在"开始"步骤中选择了模板，则邮件正文已配置，但您可以对其进行自定义。 可以使用"电子邮件"**选项卡 (** 富 HTML 编辑器) 或源选项卡 (原始 HTML代码) 。
   ...
   对于 **Spear 网络钓鱼 (** 附件) 活动，应该从邮件正文中删除链接 (否则，邮件将同时包含链接和附件，并且不会在附件市场活动) 中跟踪链接单击。
...
2. 在 **"模拟攻击** "页上，根据要创建的市场活动类型进行以下选择之一：
   ...
   - 在 **"暴力密码 (字典** 攻击) 部分，单击"启动攻击 **"或单击**"**攻击详细信息** \> **启动攻击"。**
...
5. 在 **"选择攻击设置** "步骤中，根据市场活动类型选择要执行哪些操作：
   ...
   - **暴力使用密码 (字典**) ：执行下列任一步骤：
     ...
     - **从字典文件** 上载密码 **：单击"** 上载"以导入每行包含一个密码和一个空最后一行的现有文本文件。 该文本文件的大小必须小于或小于 10 MB，并且不能包含超过 30000 个密码。
...
- **附件打开成功率**：由打开附件 (/目标用户总数) 计算得出的 **百分比**。
