...
如果您的组织具有 Microsoft Defender for Office 365 计划2，其中包括 [威胁调查和响应功能](office-365-ti.md)，则您可以使用安全 & 合规性中心中的攻击模拟器在您的组织中运行实际的攻击方案。 这些模拟攻击可帮助您识别易受攻击的用户，并在真正的攻击影响你的底线之前找到这些用户。 阅读本文以了解详细信息。
...
- 若要打开安全与合规中心，请转到 <https://protection.office.com/>。 攻击模拟器在 **威胁管理** \> **攻击模拟器** 中可用。 请直接转到 "攻击模拟器"，打开 <https://protection.office.com/attacksimulator> 。

- 有关跨不同 Microsoft 365 订阅的攻击模拟器的可用性的详细信息，请参阅 [Microsoft Defender For Office 365 服务说明](https://docs.microsoft.com/office365/servicedescriptions/office-365-advanced-threat-protection-service-description)。

- 您必须是 " **组织管理** " 或 " **安全管理员** " 角色组的成员。 若要详细了解安全与合规中心内的角色组，请参阅[安全与合规中心内的权限](permissions-in-the-security-and-compliance-center.md)。

- 需要为你的帐户配置 (MFA) 的多重身份验证，以在攻击模拟器中创建和管理市场活动。 有关说明，请参阅 [设置多因素身份验证](https://docs.microsoft.com/microsoft-365/admin/security-and-compliance/set-up-multi-factor-authentication)。
...
*仿冒* 是电子邮件攻击的一般性术语，试图窃取看似合法或受信任发件人的邮件中的敏感信息。 *Spear 仿冒* 是一种目标仿冒攻击，它使用专门为目标收件人量身定制的重点和自定义内容 (通常是由攻击者) 的收件人的侦测之后。

在攻击模拟器中，可以使用两种不同类型的 spear 网络钓鱼活动：

- **Spear 网络钓鱼 (凭据搜集)**：攻击试图说服收件人单击邮件中的 URL。 如果他们单击该链接，系统将要求他们输入凭据。 如果是这样，则会将它们转到以下位置之一：
...
- **Spear 网络钓鱼 (附件)**：攻击试图说服收件人打开邮件中的 .docx 或 .pdf 附件。 附件包含来自默认仿冒链接的相同内容，但第一句以 "" \<Display Name\> ，您将看到此邮件是您打开的最近的电子邮件 ... "。
...
2. 在 " **模拟攻击** " 页上的 " **Spear 仿冒 (凭据搜集)** 或 **Spear 仿冒 (附件)** 部分中，单击" **攻击详细信息**"。
...
5. 在 " **配置电子邮件详细信息** " 步骤中，配置以下设置：
   ...
   - **网络钓鱼登录服务器 URL**：单击下拉列表，并从列表中选择一个可用的 url。 这是用户将被引诱单击的 URL。 ）这三个选项包括：
     ...
     >
     > - URL 信誉服务可能会将这些 Url 中的一个或多个标识为不安全。 在网络仿冒活动中使用 URL 之前，请先检查受支持的 web 浏览器中的 URL 是否可用。

   - **自定义登陆页面 URL**：输入用户在单击 "仿冒" 链接并输入其凭据时使用的可选登录页。 此链接将替换默认登录页。 例如，如果您有内部意识培训，则可以在此指定 URL。

   - **类别**：当前) 忽略输入的任何内容，则不使用此设置 (。

   - **主题**：电子邮件的 " **主题** " 字段。

   完成后，单击“下一步”。

6. 在 " **撰写电子邮件** " 步骤中，创建电子邮件的邮件正文。 您可以使用 " **电子邮件** " 选项卡 (丰富的 html 编辑器) 或 (原始 HTML 代码) 的 " **源** " 选项卡。
...
2. 在 " **模拟攻击** " 页面上，根据您要创建的市场活动类型，进行下列选择之一：
   ...
   - 在 " **Spear 仿冒 (凭据收集)** " 部分，单击 " **启动攻击** " 或 " **攻击详细信息** \> **启动攻击**"。

   - 在 " **Spear 仿冒 (附件)** " 部分，单击 " **启动攻击** " 或 " **攻击详细信息** \> **启动攻击**"。
...
4. 在 " **目标收件人** " 步骤中，执行下列步骤之一：
   ...
   - 单击 " **通讯簿** " 以选择市场活动 (用户或组) 的收件人。 每个目标收件人都必须有一个 Exchange Online 邮箱。 如果在未输入搜索条件的情况下单击 " **筛选** 和 **应用** "，则将返回所有收件人并将其添加到市场活动中。

   - 单击 " **导** 入"，然后单击 " **文件导** 入" 以将逗号分隔的值导入 (CSV) 或电子邮件地址的行分隔文件。 每行必须包含收件人的电子邮件地址。
...
5. 在 " **配置电子邮件详细信息** " 步骤中，配置以下设置：
   ...
   - **从 (电子邮件)**：发件人的电子邮件地址。 您可以输入组织的电子邮件域中的真实或虚假电子邮件地址，也可以输入真实或虚假的外部电子邮件地址。 组织中的有效发件人电子邮件地址将在收件人的电子邮件客户端中实际解析。
   ...
   - **网络钓鱼登录服务器 URL**：单击下拉列表，并从列表中选择一个可用的 url。 这是用户将被引诱单击的 URL。 ）这三个选项包括：
     ...
     > - URL 信誉服务可能会将这些 Url 中的一个或多个标识为不安全。 在网络仿冒活动中使用 URL 之前，请先检查受支持的 web 浏览器中的 URL 是否可用。
     >
     > - 您需要选择一个 URL。 对于 **Spear 网络钓鱼 (附件)** 市场活动，您可以在下一步中删除邮件正文中的链接 (否则，邮件将同时包含链接 **和** 附件) 。
...
6. 在 " **撰写电子邮件** " 步骤中，创建电子邮件的邮件正文。 如果在 " **开始** " 步骤中选择了一个模板，则表示已配置邮件正文，但您可以对其进行自定义。 您可以使用 " **电子邮件** " 选项卡 (丰富的 html 编辑器) 或 (原始 HTML 代码) 的 " **源** " 选项卡。
   ...
   对于 **Spear 网络钓鱼 (附件)** 市场活动，应删除邮件正文中的链接 (否则，邮件将同时包含链接 **和** 附件，并且不会在附件活动) 中跟踪链接单击。
...
- **强力密码 (字典攻击)**： *强力* 攻击或 *字典* 攻击对用户帐户使用大量的密码字典文件，希望其中的一个用户 (可以在一个帐户) 中使用多个密码。 不正确的密码锁定帮助阻止强力密码攻击。

  对于字典攻击，您可以指定一个或多个密码来尝试 (手动输入或上载的文件) ，并且您可以指定一个或多个用户。

- **密码喷涂攻击**： *密码喷涂* 攻击对用户帐户列表使用相同的密码仔细考虑， (对许多帐户) 的一个密码。 密码喷涂攻击比强力密码攻击更难检测 (如果攻击者在几十个或数百个帐户之间尝试一个密码，而不会导致用户不正确的密码锁定) 的风险，则会导致成功的概率。

  对于密码喷涂攻击，只能指定一个密码进行尝试，并且您可以指定一个或多个用户。

> [!NOTE]
> 攻击模拟器中的密码攻击会将用户名和密码的基本身份验证请求传递给终结点，因此它们还可以使用 (AD FS、密码哈希同步、传递 PingFederate 等其他身份验证方法 ) 。 对于已启用 MFA 的用户（即使密码攻击尝试其实际密码），尝试将始终注册为故障 (换句话说，MFA 用户永远不会出现在市场活动) 的 **成功尝试** 计数中。 这是预期的结果。 MFA 是一种帮助防止密码攻击的主要方法。
...
2. 在 " **模拟攻击** " 页面上，根据您要创建的市场活动类型，进行下列选择之一：
   ...
   - 在 **强力密码 (字典攻击)** 部分，单击 " **启动攻击** " 或 " **攻击详细信息** \> **启动攻击**"。
...
5. 在 " **选择攻击设置** " 步骤中，选择要基于市场活动类型执行的操作：
   ...
   - **密码喷涂攻击**：在 " **攻击 (s) " 的密码中使用。在 "攻击** " 框中，输入一个密码。
...
活动的市场活动将显示状态栏、已完成的百分比值和 " (已完成用户的用户)  (总用户数) " 计数。 单击 " **刷新** " 按钮将更新任何活动活动的进度。 您还可以单击 " **终止** " 以停止活动的市场活动。
...
### <a name="spear-phishing-credentials-harvest-campaign-results"></a>Spear 仿冒 (凭据收集) 市场活动结果
...
- **成功的尝试**：单击链接 **并** 输入凭据的用户数 () 的 *任何* 用户名和密码值。
...
- **单击 "成功率**：由单击链接的用户数) / **目标用户** 数 (计算的百分比。

- **最快的凭据**：在启动市场活动后，第一个用户输入其凭据所需的时间。

- **平均凭据**：每个人输入其凭据所花的时间除以输入其凭据的用户数量的总和。

- **凭据成功率**：由 (个用户输入其凭据的用户数) / **目标用户** 数计算得出的百分比。
...
- **附件打开成功率**：由 (打开附件的用户数) / **目标的用户** 数计算的百分比。
