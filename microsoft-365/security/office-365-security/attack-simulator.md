...
如果你的组织拥有 Microsoft Defender for Office 365 计划 2（[](office-365-ti.md)包括威胁调查和响应功能）。可以使用安全 & 合规中心中的攻击模拟器在组织中运行真实的攻击方案。 这些模拟攻击可以帮助你在真实攻击影响你的最后一线之前识别和查找易受攻击的用户。 阅读本文可了解更多信息。
...
- 攻击模拟和培训相关的数据与其他客户数据一起存储，Microsoft 365服务。 有关详细信息，请参阅Microsoft 365[位置](../../enterprise/o365-data-locations.md)。
...
- **网络钓鱼 (** 获取) ：攻击试图诱使收件人单击邮件中的 URL。 如果用户单击该链接，则要求他们输入其凭据。 如果这样做，他们会被带至以下位置之一：
...
2. 在 **"模拟攻击** "页上，根据要创建的市场活动类型进行以下选择之一：
   ...
   - 在 **"Spear Phishing (Credentials Harvest) "** 部分中，单击"启动攻击"**或单击"****攻击详细信息** 启动攻击 \> **"。**

   - 在"**网络网络钓鱼 (附件**) ，单击"启动攻击"**或单击"** 攻击 **详细信息** 启动攻击 \> **"。**
...
5. 在" **配置电子邮件详细信息** "步骤中，配置以下设置：
   ...
   - **网络钓鱼登录服务器 URL：** 单击下拉列表，然后从列表中选择一个可用的 URL。 这是用户将尝试单击的 URL。 ）这三个选项包括：
     ...
     > - URL 信誉服务可能会将其中一个或多个 URL 标识为不安全。 在网络钓鱼活动中使用该 URL 之前，请检查 URL 在受支持的 Web 浏览器中的可用性。
     >
     > - 您需要选择 URL。 对于 **网络钓鱼** (附件) 活动，可以在下一步 (中删除邮件正文中的链接，否则邮件将同时包含链接和) 。 

   - **附件类型**：此设置仅在网络网络钓鱼和附件 (**活动中)** 可用。 单击下拉列表，**然后从**.DOCX.PDF选择"选项"。
...
- 暴力密码 (字典攻击 **) ：** 暴力或字典攻击在用户帐户上使用一个大型密码字典文件，希望其中一个能够 (一个帐户设置多个密码) 。 不正确的密码锁定有助于阻止暴力密码攻击。

  对于字典攻击，你可以指定一个或多个密码来尝试手动输入 (或上传的文件) ，你可以指定一个或多个用户。

- **密码加密** 攻击：密码攻击对用户帐户列表使用同样谨慎考虑的密码， (多个帐户使用一) 。 密码释放攻击比暴力密码攻击更难检测 (攻击者在数十个或数百个帐户上尝试一个密码时成功的可能性会提高，而不会降低用户错误密码锁定) 的风险。

  对于密码攻击，你只能指定一个要尝试的密码，并且可以指定一个或多个用户。

> [!NOTE]
> 攻击模拟器中的密码攻击将用户名和密码基本身份验证请求传递到终结点，因此它们还与其他身份验证方法 (AD FS、密码哈希同步、传递、PingFederate 等) 。 对于启用了 MFA 的用户，即使密码攻击尝试其实际密码，尝试将始终注册为失败 (换句话说，MFA 用户永远不会显示在活动成功尝试计数) 。  这是预期结果。 MFA 是帮助防止密码攻击的主要方法。
...
2. 在 **"模拟攻击** "页上，根据要创建的市场活动类型进行以下选择之一：
   ...
   - 在"**暴力密码和** 字典 (攻击) ，单击"启动攻击"或 **单击"** 攻击 **详细信息** 启动攻击 \> **"。**
...
4. 在 **"目标用户"** 步骤中，执行下列步骤之一：
   ...
   - 单击 **"** 导入 **"，** 然后单击"文件导入"，以 (CSV) 或电子邮件地址的行分隔文件中导入逗号分隔值。 每行必须包含收件人的电子邮件地址。
...
5. 在 **"选择攻击设置"** 步骤中，根据市场活动类型选择要执行哪些操作：
   ...
   - **暴力密码 (字典) ：** 执行以下步骤之一：
     ...
     - **Upload词典文件添加** 密码：单击"Upload导入现有文本文件，其中包含每行一个密码和一个空最后一行。 文本文件的大小必须小于或小于 10 MB，并且不能超过 30000 个密码。

   - **密码攻击**：在 **"密码 () 密码"框中** ，输入一个密码。
...
### <a name="brute-force-password-dictionary-attack-campaign-results"></a>暴力密码 (字典) 攻击活动结果
