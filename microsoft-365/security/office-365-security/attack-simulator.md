...
2. 在 **"模拟攻击** "页上，根据要创建的市场活动类型进行以下选择之一：
   ...
   - 在 **"Spear Phishing (Credentials Harvest) "** 部分中，单击"启动攻击"或 **单击"****攻击详细信息** 启动攻击 \> **"。**
...
6. 在 **"撰写电子邮件** "步骤中，创建电子邮件的邮件正文。 如果在"开始"步骤 **中** 选择了模板，则消息正文已配置，但您可以对其进行自定义。 You can use the **Email** tab (a rich HTML editor) or the **Source** tab (raw HTML code) .
   ...
   对于网络钓鱼 (附件 **)** 活动，应从邮件正文中删除链接 (否则，邮件将同时包含链接和附件，并且附件市场活动) 中不会跟踪链接单击。 
...
- 暴力密码 (字典攻击 **) ：** 暴力或字典攻击使用用户帐户上的大型字典密码文件，希望其中一个能够 (一个帐户密码) 。 不正确的密码锁定有助于阻止暴力密码攻击。

  对于字典攻击，你可以指定一个或多个密码来尝试手动输入 (或上传的文件) ，你可以指定一个或多个用户。

- **密码加密***攻击：密码* 攻击针对用户帐户列表使用同一个仔细考虑的密码， (多个帐户使用一) 。 密码增强攻击比暴力密码攻击更难检测 (攻击者在数十个或数百个帐户上尝试一个密码时，成功的可能性会提高，而不会消除用户错误的密码锁定) 。

  对于密码攻击，你只能指定一个要尝试的密码，并且可以指定一个或多个用户。

> [!NOTE]
> 攻击模拟器中的密码攻击将用户名和密码 基本身份验证请求传递到终结点，因此它们还与其他身份验证方法 (AD FS、密码哈希同步、传递、PingFederate 等一起) 。 对于启用了 MFA 的用户，即使密码攻击尝试其实际密码，尝试将始终注册为失败 (换句话说，MFA 用户永远不会显示在活动) 的成功尝试计数中。  这是预期结果。 MFA 是帮助防止密码攻击的主要方法。
...
2. 在 **"模拟攻击** "页上，根据要创建的市场活动类型进行以下选择之一：
   ...
   - 在"**暴力密码 (字典**) 攻击"部分，单击"启动攻击"或单击"攻击 **详细信息** 启动攻击 \> **"。**
