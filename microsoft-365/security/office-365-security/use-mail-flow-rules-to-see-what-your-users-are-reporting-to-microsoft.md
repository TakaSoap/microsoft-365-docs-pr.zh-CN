...
在 Exchange Online 中有邮箱或独立 Exchange Online 保护的 Microsoft 365 组织中 (EOP) 不含 Exchange Online 邮箱的组织中，用户可以通过多种方式将邮件报告给 Microsoft 进行分析，如 [报告消息和文件到 microsoft](report-junk-email-messages-to-microsoft.md)中所述。

您可以创建邮件流规则 (也称为传输规则) ，用于查找用户向 Microsoft 报告的邮件，并且您可以配置密件抄送收件人以接收这些报告的邮件的副本。

您可以使用 Exchange Online 中的邮箱在 Exchange 管理中心 (EAC) 和 PowerShell (Exchange Online 365 PowerShell 中创建邮件流规则;没有 Exchange Online 邮箱) 组织的独立 EOP PowerShell。
