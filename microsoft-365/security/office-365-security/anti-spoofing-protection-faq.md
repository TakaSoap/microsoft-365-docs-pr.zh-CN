...
## <a name="is-spoof-intelligence-available-to-microsoft-365-customers-without-defender-for-office-365"></a>没有 Office 365 Defender 的 Microsoft 365 客户是否可以使用欺骗智能？

是。 截至 2018 年 10 月，欺骗智能适用于在 Exchange Online 中拥有邮箱的所有组织和没有 Exchange Online 邮箱的独立 EOP 组织。
...
## <a name="im-an-admin-and-i-dont-know-all-of-sources-for-messages-in-my-email-domain"></a>我是管理员，不知道我电子邮件域中的邮件的所有源！
...
## <a name="does-anti-spoofing-protection-mean-i-will-be-protected-from-all-phishing"></a>反欺骗保护是否意味着我将免受所有网络钓鱼的防护？
...
## <a name="do-i-still-need-to-enable-the-advanced-spam-filter-setting-spf-record-hard-fail-_markasspamspfrecordhardfail_-if-i-enable-anti-spoofing"></a>如果启用反欺骗，是否需要启用 _MarkAsSpamSpfRecordHardFail ("_ 高级垃圾邮件筛选器"设置"SPF 记录：硬失败) MarkAsSpamSpfRecordHardFail？

不正确。 不再需要此 ASF 设置。 反欺骗保护会考虑 SPF 硬失败和更广泛的条件集。 如果已启用防欺骗和“SPF 记录:硬故障”(_MarkAsSpamSpfRecordHardFail_)，可能会收到更多误报。
...
SRS 仅部分修复了转发电子邮件的问题。 通过重写 SMTP MAIL **FROM，SRS** 可以确保转发的邮件通过下一个目的地的 SPF。 但是，由于反欺骗基于 From 地址以及 **MAIL FROM** 或 DKIM 签名域 (或其他信号) ，因此不足以防止 SRS 转发的电子邮件被标记为欺骗邮件。