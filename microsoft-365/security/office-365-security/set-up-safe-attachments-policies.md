...
您可以在 保险箱 门户或 PowerShell Microsoft 365 Defender PowerShell 中为在 Exchange Online 中拥有邮箱的符合条件的 (Exchange Online Microsoft 365 配置附件Exchange Online;独立 EOP PowerShell，适用于Exchange Online邮箱，但使用 Defender for Office 365 加载项订阅的组织) 。
...
> [!NOTE]
> 在"附件"设置的全局保险箱区域中，配置不依赖于"附件"保险箱的功能。 有关说明，请参阅[在](turn-on-mdo-for-spo-odb-and-teams.md)保险箱中SharePoint、OneDrive和Microsoft Teams保险箱[文档](safe-docs.md)Microsoft 365 E5。
...
- 您需具有权限，然后才能执行本文中的过程：
  ...
  - 若要对附件策略保险箱只读访问权限，你需要是全局读取者角色组或安全读者角色组的成员，Microsoft 365 Defender组。
...
1. 在 Microsoft 365 Defender 门户中，转到"策略"部分中的"电子邮件&协作策略 \> **"&"** 规则保险箱 \>  \> **附件****"。**
...
## <a name="use-the-microsoft-365-defender-portal-to-remove-safe-attachments-policies"></a>使用 Microsoft 365 Defender门户删除保险箱附件策略
...
- 可以在 PowerShell 中的新安全附件策略上配置以下设置，这些设置在创建新策略Microsoft 365 Defender后才能在 Microsoft 365 Defender 门户中可用：
...
如果 **Set-SafeAttachmentPolicy** cmdlet 没有 _Name_ 参数，则 (在 PowerShell 中重命名安全) 。 在安全保险箱中重命名"附件"Microsoft 365 Defender，只需重命名安全附件 _规则_。
...
- 在 Microsoft 365 Defender 门户中，转到"策略"部分中的"电子邮件&协作策略 \> **"&"** 规则保险箱 \>  \> **附件****"。** 验证策略列表、 **策略的 Status** 值及其 **Priority** 值。 若要查看更多详细信息，请通过单击名称从列表中选择策略，然后查看飞出中的详细信息。
