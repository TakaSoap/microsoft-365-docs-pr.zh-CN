...
您可以在 保险箱 Microsoft 365 Defender 门户中或在 PowerShell (Exchange Online PowerShell 中为在 Microsoft 365 中拥有邮箱的合格 Exchange Online 组织配置附件Exchange Online;独立 EOP PowerShell，适用于Exchange Online邮箱，但使用 Defender for Office 365 加载项订阅的组织) 。

"附件"策略保险箱元素包括：

- 安全附件策略：指定未知恶意软件检测的操作、是否将带恶意软件附件的邮件发送到指定的电子邮件地址，以及是否传递邮件（如果保险箱附件扫描无法完成）。
...
- 有关附件策略保险箱，请参阅附件[保险箱设置](recommended-settings-for-eop-and-office365.md#safe-attachments-settings)。
...
5. 在“**设置**”页上，配置下列设置：
   ...
   - **保险箱附件未知恶意软件响应**：选择下列值之一：
     ...
     这些值在附件策略[保险箱进行了解释](safe-attachments.md#safe-attachments-policy-settings)。
...
## <a name="use-the-microsoft-365-defender-portal-to-view-safe-attachments-policies"></a>使用Microsoft 365 Defender查看附件保险箱策略
...
- 阻止在未使用 _Action_ 参数保险箱文档扫描 (发现包含恶意软件的邮件，默认值为 `Block`) 。
...
若要验证附件保险箱扫描邮件，请检查可用的 Defender，查看Office 365报告。 有关详细信息，请参阅查看 Defender [for Office 365](view-reports-for-mdo.md)报告和使用浏览器[在](threat-explorer.md)Microsoft 365 Defender 门户。
