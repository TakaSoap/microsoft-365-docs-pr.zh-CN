...
2. 运行以下命令以将内容经过筛选的垃圾邮件路由到垃圾邮件文件夹：
   ...
   其中 _NameForRule_ 是新规则的名称，例如，JunkContentFilteredMail。
...
3. 运行以下命令以将在到达内容筛选器之前标记为垃圾邮件的邮件路由到垃圾邮件文件夹：
   ...
   其中 _NameForRule_ 是新规则的名称，例如，JunkMailBeforeReachingContentFilter。
...
4. 运行以下命令，以确保来自垃圾邮件筛选器策略中阻止列表的发件人的邮件（如**发件人阻止**列表）都会被路由到垃圾邮件文件夹：
   ...
   其中 _NameForRule_ 是新规则的名称，例如，JunkMailInSenderBlockList。
...
[新 New-transportrule](https://docs.microsoft.com/powershell/module/exchange/policy-and-compliance/new-transportrule)
