...
description: Office 365 中的安全性，从 EOP 到适用于 Office 365 计划 1 和 2 的 Defender，Standard 与严格安全配置等。 了解你拥有哪些功能以及如何保护你的属性。
ms.technology: mdo
...
本文将向你介绍云中的新安全属性。 无论你是安全操作中心的一员，你是空间的新安全管理员，还是想要刷新一下，让我们开始吧。

> [!CAUTION]
> 如果你使用的是 **Outlook.com、Microsoft** **365** 家庭版或 Microsoft **365 个人** 版，并且需要安全链接或安全附件信息，请单击此 ***链接***[：Microsoft 365](https://support.microsoft.com/office/advanced-outlook-com-security-for-office-365-subscribers-882d2243-eab9-4545-a58a-b36fee4a46e2)订阅者的高级 Outlook.com 安全。

## <a name="office-365-security-spelled-out"></a>拼写了 Office 365 安全

每个 Office 365 订阅都附带了安全性功能。 您可以采取的目标和操作取决于这些不同订阅的焦点。 在 Office 365 安全中，有三种主要 (或产品) 绑定到订阅类型：
...
尽管其中每个服务都强调保护、检测、调查和响应中的一个目标，但 ***所有** _ 服务 *_都可以执行保护_*、检测、调查和响应的任何 * 目标。
...
尽管电子邮件身份验证配置发生在公共 DNS 中，但配置此功能以帮助防御欺骗很重要。 *如果您有 EOP，****则应该 [配置电子邮件身份验证](email-validation-and-authentication.md)***。
...
|阻止/检测|调查|响应|
|---|---|---|
|技术包括：<ul><li>垃圾邮件 (spam)</li><li>phish</li><li>恶意软件</li><li>批量邮件</li><li>欺骗智能</li><li>模拟检测</li><li>管理员隔离</li><li>误报和误报的管理员和用户提交</li><li>允许/阻止 URL 和文件</li><li>报告</li></u1>|<li>审核日志搜索</li><li>邮件跟踪</li>|<li>零时差自动清除 (ZAP) </li><li>允许列表和阻止列表的精简和测试</li>|
|
...
Microsoft Defender for Office 365 P1 **还添加了** 调查实时检测。 此威胁搜寻工具的名称以粗体显示，因为拥有它就是了解 Office  365 P1 的 Defender。 它不会在 Defender for Office 365 P2 中显示。
...
|阻止/检测|调查|响应|
|---|---|---|
|技术包括 EOP 和 Microsoft Defender for Office 365 P1 中所有功能以及：<u1><li>相同</li>|<li>**威胁资源管理器**</li><li>威胁跟踪器</li><li>市场活动视图</li>|<li>AIR 自动调查和响应 (AIR) </li><li>来自威胁资源管理器的 AIR</li><li>针对受损用户的 AIR</li><li>用于自动调查的 SIEM 集成 API</li>

因此，Microsoft Defender for Office 365 P2 扩展了内部调查和响应端，并添加了新的搜寻强度。  自动化。

在 Microsoft Defender for Office 365 P2 中，主要搜寻工具称为威胁 **资源管理器** ，而不是实时检测。 如果在导航到安全中心时看到威胁资源管理器，则位于 Microsoft Defender for Office 365 P2 中。
...
> [!TIP]
> ***预览体验成员提示** _. 可以使用目录docs.microsoft.com了解 EOP 和 Microsoft Defender for Office 365。 导航回此页面 [，即 Office 365](index.md)安全概述，你将注意到侧栏中的目录组织。 它首先部署部署 (迁移) ，然后继续进入预防、检测、调查和响应。 <p> 此结构分为两部分，以便 _ *Security Administration** 主题后跟 **安全操作** 主题。 如果你是任一工作角色的新增成员，请使用此提示中的链接以及你对目录的知识，以帮助了解空间。 请记住， *在访问时* 使用反馈 *链接和评分* 文章。 反馈可帮助我们改进我们为您提供的信息。

## <a name="where-to-go-next"></a>下一步将转到何处

如果你是安全管理员，可能需要为邮件配置 DKIM 或 DMARC。 你可能想要为优先用户推出"严格"安全预设，或查找产品中的新增功能。 或者，如果你使用安全操作，你可能想要利用实时检测或威胁资源管理器调查和响应，或者使用攻击模拟器训练最终用户检测。 无论使用哪种方式，下面是有关下一步要查看的一些其他建议。
...
了解[Microsoft Defender for Office 365](whats-new-in-office-365-atp.md) (的新增功能，包括 EOP) 
