...
管理员可使用多个垃圾邮件筛选器设置，以避免将垃圾邮件发送到用户收件箱。如果你采用本文列出的设置，垃圾邮件筛选器将能更好地阻止垃圾邮件，并避免出现漏报邮件。在这种情况下，漏报是指垃圾邮件发送到用户收件箱。

## <a name="block-ip-addresses-with-a-connection-filter"></a>使用连接筛选器阻止 IP 地址

通过将发件人 IP 地址添加到连接筛选器 IP 阻止列表，自定义垃圾邮件筛选器：
...
垃圾邮件主要是大量邮件（例如，新闻稿或促销邮件）吗？如果[使用邮件流规则配置大量电子邮件筛选](use-transport-rules-to-configure-bulk-email-filtering.md)，或启用垃圾邮件筛选器的 [Office 365 高级垃圾邮件筛选 (ASF) 设置](advanced-spam-filtering-asf-options.md)中的“大量邮件”**** 设置，可以自定义垃圾邮件筛选器。在 Exchange 管理中心内，首先依次单击“保护”****\>“内容筛选器”****，再双击要调整的筛选策略。单击“垃圾邮件和大量邮件操作”****，以调整设置，如下所示。
...
[在 Office 365 中配置反垃圾邮件策略](configure-your-spam-filter-policies.md)，以在垃圾邮件筛选器中将发件人地址添加到阻止的发件人列表，或将域添加到阻止的域列表。发送自垃圾邮件筛选器阻止列表中发件人或域的电子邮件会被标记为“垃圾邮件”。

## <a name="email-users-can-also-help-ensure-that-false-negative-and-email-spam-is-blocked-with-spam-filter"></a>电子邮件用户还可有助于确保通过垃圾邮件筛选器避免漏报和阻止垃圾邮件

如果让用户在 [Outlook](https://support.office.com/article/5ae3ea8e-cf41-4fa0-b02a-3b96e21de089) 或 [Outlook 网页版](https://support.office.com/article/db786e79-54e2-40cc-904f-d89d57b7f41d)中将垃圾邮件发件人地址添加到阻止的发件人名单中，将有助于反垃圾邮件筛选器避免漏报并阻止垃圾邮件。在 Outlook 网页版中，首先依次单击“**设置**”\>“**选项**”\>“**阻止或允许**”，再将地址添加到“**阻止的发件人**”名单中，如下所示。

![在 Outlook 网页版中阻止发件人](../../media/fdf51381-2527-4819-ac2a-5dff84d2a36d.png)

> [!NOTE]
> 若要详细了解安全发件人列表，请参阅[在 Office 365 中创建安全发件人列表](create-safe-sender-lists-in-office-365.md)。
...
[Microsoft 电子邮件反垃圾邮件保护](anti-spam-protection.md)
