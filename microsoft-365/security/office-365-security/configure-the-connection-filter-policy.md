...
如果你是在 Exchange Online 中拥有邮箱的 Microsoft 365 客户，或者没有 Exchange Online 邮箱的独立 Exchange Online Protection (EOP) 客户，请使用 EOP (中的连接筛选，尤其是默认连接筛选器策略) 通过 IP 地址标识好或坏的源电子邮件服务器。 默认连接筛选器策略的关键组件包括：
...
本文介绍如何在 Microsoft 365 Microsoft 365 Defender 门户或 PowerShell (Exchange Online PowerShell 中为在 Exchange Online 中拥有邮箱的 Microsoft 365 组织配置默认连接筛选器Exchange Online;独立 EOP PowerShell，适用于Exchange Online邮箱) 。 有关 EOP 如何使用连接筛选是组织整体反垃圾邮件设置的一部分，请参阅 [反垃圾邮件保护](anti-spam-protection.md)。
...
2. 在 **"反垃圾邮件策略**"页上，单击该策略 (，从) 选择"默认"选项。
...
- IP 允许列表中的 IP 地址也配置在 Microsoft 365 (中任何租户中基于 IP 的入站内部部署连接器中) 让我们调用此租户 A) ，租户 A 和首次遇到邮件的 EOP 服务器正好位于 Microsoft 数据中心中的同一 *Active* Directory 林中。 在此方案中 **，IPV：CAL** 将添加到邮件的反垃圾邮件邮件头 [ (指示](anti-spam-message-headers.md)邮件绕过了垃圾邮件筛选) ，但邮件仍受垃圾邮件筛选限制。
...
- 规则条件 **：如果** 发件人 IP 地址位于以下任一范围内，或与 IP 地址 (完全匹配，则应用此 \>  \>  \>) 。
- 规则操作 **：修改邮件属性** 将垃圾邮件可信度设置为 \> **(SCL**) \> **绕过垃圾邮件筛选。**
