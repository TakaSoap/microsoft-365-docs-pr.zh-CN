...
- ** \<site name> SharePoint 组的成员**（可包含用户帐户和组）被分配有**编辑**权限级别

- ** \<site name> 访问者**SharePoint 组的成员（可包含用户帐户和组）被分配为 "**读取**" 权限级别

- SharePoint 组** \<site name> 所有者**（可包含用户帐户和组）的成员被分配了 "**完全控制**" 权限级别
...
- ** \<site name> 管理员**访问组中应该只有少数成员，与管理团队网站的少数 SharePoint Online 管理员相对应。

- 大多数网站成员都在 " ** \<site name> 成员**" 或 " ** \<site name> 查看者**" 访问组中。 由于** \<site name> 成员**访问组中的网站成员具有删除或修改网站中的资源的能力，因此请仔细考虑其成员资格。 如果不确定，请将网站成员添加到** \<site name> 查看者**访问组。
