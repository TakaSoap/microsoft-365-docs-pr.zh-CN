...
## <a name="phase-1-determine-your-sharepoint-groups-and-permission-levels"></a>阶段 1：确定 SharePoint 组和权限级别
...
确定 SharePoint 组的成员可以在网站中执行哪些操作的特定权限集是权限级别。 默认情况下，SharePoint Online 团队网站有三个权限级别：编辑、读取和完全控制。 下表显示了 SharePoint 组的默认关联和分配的权限级别：
...
## <a name="phase-2-assign-permissions-to-users-with-access-groups"></a>阶段 2：向具有访问组的用户分配权限
...
- 为 **\<site name> Visitors** SharePoint 组的成员（可同时包括用户帐户和组）分配 **读取** 权限级别
...
Microsoft 365 的 Azure AD 组是不同的 Microsoft 365 组。 Azure AD 组显示在 Microsoft 365 管理中心中，其类型设置为 **"** 安全"，并且没有电子邮件地址。 Azure AD 组可在：

- Active Directory 域服务 (AD DS)

    这些组是在内部部署 AD DS 基础结构中创建并同步到 Microsoft 365 订阅的组。 在 Microsoft 365 管理中心中，这些组的状态与 **Active directory 同步**。

- Office 365

    这些组是使用 Microsoft 365 管理中心、Azure 门户或 Microsoft PowerShell 创建的组。 在 Microsoft 365 管理中心中，这些 **组的状态为****云**。

 **最佳做法：** 如果你使用的是本地 AD DS 并与 Microsoft 365 订阅同步，请通过 AD DS 执行用户和组管理。
...
对于仅限于少数人员的项目，添加到网站的 SharePoint 组的单个级别的基于 Azure AD 的访问组将适合大多数方案。 但是，如果你拥有大量人员，并且这些人员已经是已建立的 Azure AD 组的成员，则通过使用嵌套组或包含其他组作为成员的组，可以更轻松地分配 SharePoint 权限。

例如，您希望创建独立的 SharePoint Online 团队网站，以在销售、市场营销、工程、法律和支持部门的主管人员以及已具有执行用户帐户成员身份的部门之间协作。 不要为新网站成员创建新组，并放入所有单独的行政用户帐户，而是将每个部门的现有执行组放入新组中。

 如果在多个组织之间共享 Microsoft 365 订阅，则组织独立网站的单个级别的组成员身份可能由于用户帐户数量而变得难以管理。 在这种情况下，你可以将嵌套的 Azure AD 组用于每个包含其组织中组的组织来管理权限。

若要使用嵌套的 Azure AD 组：

1. 确定或创建将包含用户帐户的 Azure AD 组，并添加相应的用户帐户作为成员。

2. 创建将包含其他 Azure AD 组的基于 Azure AD 的访问组的容器，并添加这些组作为成员。

3. 对于容器访问组的相应访问级别，请确定 SharePoint 组和相应的权限级别。
...
由于研究、工程和项目小组中所有用户帐户都是网站成员，因此将其 Azure AD 组添加到 ProjectX 成员访问组会更容易。
