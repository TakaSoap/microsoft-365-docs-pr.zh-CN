...
ms.locfileid: "62886398"
---
# <a name="migrate-to-microsoft-defender-for-office-365---phase-1-prepare"></a>迁移到 Microsoft Defender for Office 365 - 阶段 1：准备
...
**但是，不要自动或任意重新创建 Defender for Office 365 中现有的所有自定义Office 365。** 您最好引入不再需要、相关或功能性的设置。 更糟糕的是，以前的一些自定义设置可能会实际在 Defender for Office 365。

对 Defender for Office 365 的本机功能和行为的测试和观察将最终确定所需的替代和设置。 您可能会发现，将现有保护服务中的设置分为以下类别会很有帮助：

- **连接或内容筛选**：你会发现，在 Defender for Office 365 中不需要大部分自定义。
- **业务路由**：需要重新创建的大多数自定义项可能属于此类别。 例如，可以在邮件流规则Microsoft 365重新创建这些设置Exchange规则 (传输规则) 、连接器和欺骗智能的例外。

建议采用一种包含不断增多的用户成员资格的试验阶段和基于观察的调整（基于平衡安全注意事项与组织业务需求）的瀑布式方法，而不是将旧设置完全移动到 Microsoft 365 中。

## <a name="check-your-existing-protection-configuration-in-microsoft-365"></a>检查现有保护配置Microsoft 365

如前面所述，不可能完全关闭传递到 Microsoft 365 的邮件的所有保护功能，即使您使用第三方保护服务。 因此，Microsoft 365至少配置了一些电子邮件保护功能，这一点并不少见。 例如：

- 过去，你未将第三方保护服务与 Microsoft 365。 你可能已使用和配置了当前Microsoft 365中的一些保护功能。 但是，当你"打开拨号盘"以在拨号盘中启用保护功能时，这些设置Microsoft 365。
- 您可能对误报Microsoft 365保护 (通过现有保护服务) 或误报 (错误邮件) 错误邮件。
...
- 如果您使用的是任何复杂的路由方法，例如 [ (邮件传输](/exchange/transport-options)) ，则应考虑简化您的路由并全面记录它。 外部跃点（尤其是在Microsoft 365消息后）会使配置和故障排除变得复杂。
...
- 本文未介绍出站和中继邮件流。 但是，请注意，您可能需要执行下列一个或多个步骤：
  ...
  - 我们强烈建议你在登录时设置 DKIM Microsoft 365。 有关详细信息，请参阅使用 [DKIM 验证出站电子邮件](use-dkim-to-validate-outbound-email.md)。
...
- Defender for Office 365 没有聚合 DMARC 报告的功能。 访问 [Microsoft 智能安全 (MISA) 目录](https://www.microsoft.com/misapartnercatalog)，查看提供 DMARC 报告的第三方供应商Microsoft 365。

## <a name="move-features-that-modify-messages-into-microsoft-365"></a>将修改邮件的功能移动到Microsoft 365

您需要将以任何方式修改邮件的任何自定义项或功能转移到Microsoft 365。 例如，现有的保护服务向来自外部发件人的邮件的主题或邮件正文添加一个 **External** 标记。 任何链接环绕功能也会导致某些邮件出现问题。 如果当前正在使用此功能，应优先考虑如何优先部署保险箱链接，以尽可能减少问题。

如果未关闭现有保护服务中的邮件修改功能，则可能会看到以下负面影响Microsoft 365：

- DKIM 将中断。 并非所有发件人都依赖于 DKIM，但那些无法通过身份验证的发件人。
- [本指南稍后](anti-spoofing-protection.md) 介绍的欺骗智能和调整步骤将无法正常工作。
- 你可能会收到大量误报， (标记为错误邮件) 。

若要在邮件中重新创建Microsoft 365发件人标识，可以使用以下选项：
...
- 邮件流规则 (传输规则) 。 有关详细信息，请参阅组织范围内的邮件免责声明、[签名](/exchange/security-and-compliance/mail-flow-rules/disclaimers-signatures-footers-or-headers)、页脚或 Exchange Online。

Microsoft 正在与行业合作，以在近期支持经过身份验证的接收 (ARC) 标准。 如果要使当前邮件网关提供商启用任何邮件修改功能，建议就支持此标准的计划与用户联系。
...
- **隔离与发送到"垃圾邮件"文件夹**：恶意邮件和肯定存在风险的邮件的自然和推荐响应是隔离邮件。 但是，您希望用户如何处理不太有害的邮件（如垃圾邮件）和批量邮件 (也称为灰色 *邮件) 。* 这些类型的邮件应传递到用户垃圾邮件文件夹吗？
...
- **需要批量邮件与不需要的批量邮件**：许多保护系统允许用户自行允许或阻止批量电子邮件。 这些设置无法轻松迁移到 Microsoft 365，因此应考虑与 ISP 及其员工合作，以在 Microsoft 365 中重新创建现有配置。

  现在，Microsoft 365一些批量邮件 (，例如，新闻稿) 邮件源的安全。 来自这些"安全"源的邮件当前未标记为批量 (批量投诉级别或 BCL 为 0 或 1) ，因此难以全局阻止来自这些源的邮件。 对于大多数用户，解决方案是要求他们单独取消订阅这些批量邮件，或使用Outlook阻止发件人。 但是，某些用户不会喜欢阻止或取消订阅批量邮件本身。
...
如果此功能可供你使用，优先级帐户和用户标记可以帮助识别你的重要Microsoft 365用户，以便他们在报告中脱颖而出。 有关详细信息，请参阅 [Microsoft Defender 中的用户标记Office 365](user-tags.md)管理和监视[优先级帐户](/microsoft-365/admin/setup/priority-accounts)。

## <a name="next-step"></a>后续步骤

**恭喜！** 你已完成迁移到 Microsoft Defender **的"** 准备"[阶段，Office 365](migrate-to-defender-for-office-365.md#the-migration-process)！
