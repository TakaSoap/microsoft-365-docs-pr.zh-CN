...
2. 在 **"防钓鱼"** 页面上，模拟见解如下所示：
   ...
   见解有两种模式：
    ...
    - 见解模式：如果在任何反网络钓鱼策略中启用和配置了模拟保护，则此见解将显示过去七天内从模拟域和模拟用户 (发件人) 检测到的邮件数。 这是所有反网络钓鱼策略中所有检测到的模拟发件人总数。
...
- **模拟类型**：此值显示模拟邮件的检测 (例如，**地址中的域) 。**
- **模拟 (域) ：** 模拟域，它应非常类似于在反网络钓鱼策略中配置为进行模拟保护的域。
- **域类型**：此值是 **内部域** 的公司域或自定义 **域** 的自定义域。
- **策略**：检测到模拟域的防钓鱼策略。
- **允许模拟**：下列值之一：
  - **是**：域配置为受信任的域， (防钓鱼策略中) 保护策略中的一个例外。 检测到来自模拟域中发件人的邮件，但允许。
