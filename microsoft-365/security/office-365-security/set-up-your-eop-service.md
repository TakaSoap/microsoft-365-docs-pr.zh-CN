...
本主题介绍如何在 EOP Exchange Online Protection (设置) 。 如果您已从 Office 365 域向导登录到这里，则假如您不想使用 Exchange Online Protection，请返回到 Office 365 域向导。 若要详细了解如何配置连接器，请参阅[Configure mail flow using connectors in Office 365](/exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/use-connectors-to-configure-mail-flow)。

> [!NOTE]
> 此主题假设你拥有内部部署邮箱，并且想使用 EOP 对其进行保护，这称为独立方案。 如果要在云中托管所有邮箱，Exchange Online，则不必完成本文中所有步骤。 转到比较[Exchange Online](https://products.office.com/exchange/compare-microsoft-exchange-online-plans)注册和购买云邮箱的计划。
>
...
- 您需在 Exchange Online Protection权限，然后才能执行本文中的过程。 具体来说，您需要远程域和接受域角色，默认情况下，该角色分配给组织管理 (全局管理员) 和邮件Flow **管理员** 角色组。 有关详细信息，请参阅 [Permissions in standalone EOP](feature-permissions-in-eop.md) 和 [Use the EAC modify the list of members in role groups](manage-admin-role-group-permissions-in-eop.md#use-the-eac-modify-the-list-of-members-in-role-groups)。
...
1. 在Microsoft 365 [中心中](../../admin/admin-overview/about-the-admin-center.md)，转到"**设置**"以将域添加到服务。

2. 按照该步骤将适用的 DNS 记录添加到您的 DNS 托管提供程序以验证域所有权。

> [!TIP]
> [将域添加到](../../admin/setup/add-domain.md)Office 365和在任何 DNS 托管提供商处为 Office 365 创建[DNS](../../admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider.md)记录是当您将域添加到服务和配置 DNS 时要引用的有用资源。
...
> [!NOTE]
> 如果必须将 MX 记录指向位于 EOP 前面的另一台服务器或服务，请参阅增强的连接器[筛选](/Exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/enhanced-filtering-for-connectors)Exchange Online。
