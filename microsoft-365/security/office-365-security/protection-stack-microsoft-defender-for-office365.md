...
Microsoft Defender for Office 365保护或筛选堆栈可以分为 4 个阶段，如本文所介绍。 一般来说，传入邮件在传递之前会通过所有这些阶段，但电子邮件的实际路径需要受组织的 Defender Office 365配置。
...
6. **连接器的增强** 筛选功能可保留身份验证信息，即使通信在到达其他设备之前Office 365。 这提高了筛选堆栈的准确性，包括启发式群集、反欺骗和防钓鱼机器学习模型，即使在复杂或混合路由方案中也可以。

## <a name="phase-2---sender-intelligence"></a>阶段 2 - 发件人智能

发件人智能中的功能对于捕获垃圾邮件、批量、模拟和未经授权的欺骗邮件至关重要，还会成为网络钓鱼检测的因素。 这些功能中的大多数是可单独配置的。

:::image type="content" source="../../media/mdo-filtering-stack/mdo-filter-stack-phase2.png" alt-text="Defender 中筛选邮件的第 2 阶段Office 365发件人智能。":::
...
3. 防病毒防病毒 (AV) 引擎也用于真正键入所有附件，以便类型阻止可以阻止管理员指定类型的所有附件。 
...
8. **对 OneDrive、SharePoint** 和 Teams 的保护保险箱针对恶意文件（本地、OneDrive、SharePoint 和 Microsoft Teams 内部）提供相同的 Microsoft Teams 附件保护。
