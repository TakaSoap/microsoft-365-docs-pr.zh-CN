...
- **验证是否已在用户邮箱中启用垃圾邮件规则**：默认情况下已启用该规则，但如果不是标记为垃圾邮件的邮件不能移动到 "垃圾邮件" 文件夹中。 有关详细信息，请参阅[Microsoft 365 中的 Exchange Online 邮箱上的配置垃圾邮件设置](configure-junk-email-settings-on-exo-mailboxes.md)。
