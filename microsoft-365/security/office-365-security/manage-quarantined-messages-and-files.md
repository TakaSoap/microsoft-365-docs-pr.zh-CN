...
默认情况下，只有管理员才能管理被隔离为恶意软件、高可信度网络钓鱼或由于邮件流规则 (也称为传输规则) 。 但是，管理员可以使用 _隔离_ 策略根据邮件被隔离的原因来定义允许用户对隔离邮件执行哪些操作 (支持的功能) 。 有关详细信息，请参阅 [隔离策略](quarantine-policies.md)。
...
在 Microsoft 365 Defender 门户中或在 PowerShell 中查看和管理隔离邮件 (Exchange Online PowerShell，Microsoft 365邮箱在 Exchange Online;适用于没有邮箱或邮箱Exchange Online的独立 EOP PowerShell) 。
...
## <a name="use-the-microsoft-365-defender-portal-to-manage-quarantined-files-in-defender-for-office-365"></a>使用 Microsoft 365 Defender 门户在 Defender for Office 365 中管理隔离Office 365
