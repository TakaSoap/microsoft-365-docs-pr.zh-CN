...
在独立 Exchange Online Protection (EOP) 不含 Exchange Online 邮箱的组织中，邮件用户是用户帐户的基本类型。 邮件用户在独立 EOP 组织中具有帐户凭据，并且可以访问) 分配权限的资源 (。 邮件用户的电子邮件地址是外部 (例如，在本地电子邮件环境) 中。
...
- 必须先分配有权限，然后才能执行这些过程。 具体来说，您需要创建邮件收件人 (创建) 和邮件收件人 (修改) 角色，默认情况下这些角色分配给 OrganizationManagement (全局管理员) 和 RecipientManagement 角色组。 有关详细信息，请参阅 [独立 EOP 中的权限](feature-permissions-in-eop.md) 和 [使用 EAC 修改角色组中的成员列表](manage-admin-role-group-permissions-in-eop.md#use-the-eac-modify-the-list-of-members-in-role-groups)。
...
2. 单击 " **新建** ![ 新图标" ](../../media/ITPro-EAC-AddIcon.png) 。 在打开的 " **新建邮件用户** " 页上，配置以下设置。 使用标记为的设置 <sup>\*</sup> 是必需的。
   ...
   - <sup>\*</sup>**用户 ID**：输入此人将用于登录服务的帐户。 用户 ID 由位于 ( @ ) 符号 ( @ ) 的左侧的用户名和右侧的域组成。
...
- 在 EAC 中，转到 " **收件人**" " \> **联系人**"。 验证邮件用户是否列出 (或是否未列出) 。 选择邮件用户并查看详细信息窗格中的信息，或单击 " **编辑** ![ 编辑图标 ](../../media/ITPro-EAC-AddIcon.png) " 以查看设置。

- 在独立 EOP PowerShell 中，运行以下命令来验证邮件用户是否 (列出，或者是否未列出) ：
...
在独立 EOP 中，目录同步适用于具有本地 Active Directory 的客户。 你可以将这些帐户同步到 Azure Active Directory (Azure AD) ，其中，帐户副本存储在云中。 将现有用户帐户同步到 Azure Active Directory 时，可以在 Exchange 管理中心的 " **收件人** " 窗格中 (EAC) 或独立 EOP PowerShell 中查看这些用户。
