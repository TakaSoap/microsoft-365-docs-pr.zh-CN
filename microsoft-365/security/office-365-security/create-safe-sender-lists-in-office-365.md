...
最不理想的选择是在反垃圾邮件策略中使用允许的发件人列表或允许的域列表。 *如果可能*，应尽可能避免使用此选项，因为发件人绕过所有垃圾邮件、欺骗和网络钓鱼防护以及发件人身份验证（SPF、DKIM、DMARC）。 此方法最适合仅用于临时测试。 详细步骤可在 EOP 主题中的 "[配置反垃圾邮件策略](configure-your-spam-filter-policies.md)" 中找到。
...
EOP 中的反垃圾邮件策略中的安全发件人列表和安全域列表检查 `5321.MailFrom` 和 `5322.From` 地址。 Outlook 安全发件人仅使用该 `5322.From` 地址。
