...
本文是一个分步部署指南，用于创建和配置 365 中的独立 SharePoint Online Microsoft Office网站。 这些步骤假定使用三个默认 SharePoint 组和相应的权限级别，并针对每个访问级别使用一个基于 AD) 的 Azure Active Directory (AD 组。

## <a name="phase-1-create-and-populate-the-team-site-access-groups"></a>第 1 阶段：创建和填充团队网站访问组
...
确定与独立团队网站的 SharePoint Online 管理员对应的用户帐户集。

如果要通过 Microsoft 365 管理用户帐户和组，并且想要使用 Windows PowerShell，请从 UPN 示例 UPN (UPN)  (用户主体belindan@contoso.com) 。

### <a name="step-2-list-the-members-for-the-site"></a>步骤 2：列出网站的成员

确定与独立团队网站的成员（将协作处理网站中存储的资源的成员）对应的用户帐户集。

如果要通过 Microsoft 365 管理用户帐户和组，并且想要使用 PowerShell，请列出其 UPN。 如果有很多网站成员，可以将 UPN 列表存储在文本文件中，然后使用一个 PowerShell 命令将它们全部添加。

### <a name="step-3-list-the-viewers-for-the-site"></a>步骤 3：列出网站的查看者

确定与独立团队网站的查看者对应的用户帐户集，这些用户帐户可以查看网站中存储的资源，但不能修改它们或直接协作处理其内容。
...
1. 在浏览器中，转到 Azure 门户，然后使用已分配有用户管理管理员或公司管理员角色的帐户的 <https://portal.azure.com> 凭据登录。
...
> [!NOTE]
> 你需要使用 Azure 门户创建组，以便启用 Office 功能。 如果 SharePoint Online 独立网站稍后被配置为具有 Azure 信息保护标签的"高度机密"网站，以加密文件并将权限分配给特定组，则必须在启用 Office 功能的情况下创建允许的组。 创建 Azure AD 组的 Office 功能设置后，无法更改该设置。
...
如果要通过 Active Directory 域服务 (AD DS) 管理用户帐户和组，则使用正常的 AD DS 用户和组管理过程将用户添加到相应的访问组，并等待与 Microsoft 365 订阅同步。
...
如果将任何访问组的用户帐户的 UPN 存储在文本文件中，可以使用以下 PowerShell 命令块一次添加它们：
...
在此阶段，创建独立的 SharePoint Online 网站，并配置默认 SharePoint Online 权限级别的权限，以使用新的基于 Azure AD 的访问组。 默认情况下，新的团队网站包括 Microsoft 365 组和其他相关资源，但在这种情况下，我们将创建不含 Microsoft 365 组的团队网站。 这允许完全通过 SharePoint 维护权限。

首先，使用以下步骤创建 SharePoint Online 团队网站。

1. 使用还将用于管理 SharePoint Online 团队网站的帐户登录到 Microsoft 365 管理中心 (SharePoint Online) 。 如需帮助，请参阅[如何登录到 Office 365](https://support.microsoft.com/office/e9eb7d51-5430-4929-91ab-6157c5a050b4)。
...
17. 在 **"共享**"对话框中，键入网站查看者访问组的名称，选择它，然后单击"**共享"。**

18. 关闭浏览器的“权限”标签页。

以下是这些权限设置的结果：

- Owners **\<site name>** SharePoint 组包含网站管理员访问组，其中所有成员均具有 **完全控制** 权限级别。
...
下面是针对配置为使用三个访问组（使用用户帐户或 Azure AD 组填充）的网站的三个 SharePoint 组生成的配置。

![具有访问组和用户帐户的隔离 SharePoint Online 网站的最终配置。](../../media/e7618971-06ab-447b-90ff-d8be3790fe63.png)

通过其中一个访问组的组成员身份，您和网站的成员现在可以使用网站的资源进行协作。
