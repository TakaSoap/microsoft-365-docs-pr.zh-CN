...
> [!NOTE]
> 如果你正在 Microsoft 365 Defender 门户 () 中，你可以在此处启动 Defender for Office 365 评估：其他部分中的电子邮件 & 协作策略 & 规则威胁策略评估 <https://security.microsoft.com>  \>  \>  \> 模式。 
...
使用评估模式 [保险箱，保险箱](safe-attachments.md)代表您设置 [](safe-links.md)基于附件、保险箱链接和基于 [](set-up-anti-phishing-policies.md#impersonation-settings-in-anti-phishing-policies-in-microsoft-defender-for-office-365)邮箱智能的模拟策略。 所有 Defender for Office 365 策略均在后台的非强制模式下创建，并且对不可见。

作为设置的一部分，评估模式还配置 [连接器的增强筛选](/exchange/mail-flow-best-practices/use-connectors-to-configure-mail-flow/enhanced-filtering-for-connectors)。 它通过保留 IP 地址和发件人信息提高筛选准确度，否则当邮件通过 Defender for Office 365 前面的电子邮件安全网关 (ESG) 时，这些信息会丢失。 增强的连接器筛选功能还提高了 EOP 策略Exchange Online Protection (EOP) 反垃圾邮件和防钓鱼策略的筛选准确度。
...
你的Exchange Online Protection策略（如批量和垃圾邮件保护）将保持不变。 但是，评估会启用对连接器的增强筛选，这可能会影响邮件流Exchange Online Protection策略，除非绕过。
...
如果使用第三方电子邮件安全网关 (ESG) ，则需要知道提供商的名称。 如果你使用的是本地或不支持的 ESG 供应商，你需要知道设备的公共 IP 地址 () 地址。
...
从以下三个Office 365访问点，在 Microsoft 365 Defender 门户 () Microsoft Defender <https://security.microsoft.com>) 评估设置卡：
...
如果已有网关，启用评估模式将激活连接器的增强筛选。 此功能通过更改传入发件人 IP 地址提高筛选准确度。 此功能可能会更改筛选裁定，如果您未绕过Exchange Online Protection，则可能会更改某些邮件的可传递性。 在这种情况下，您可能需要暂时绕过筛选来分析影响。 若要绕过筛选，请打开 Exchange 管理中心 (EAC) ，并创建一个邮件流规则，将邮件的 SCL 设置为 <https://admin.exchange.microsoft.com> -1 (如果还没有邮件) 。 有关说明，请参阅[使用邮件流规则将](/exchange/security-and-compliance/mail-flow-rules/use-rules-to-set-scl)垃圾邮件可信度设置为 (中的) SCL Exchange Online。
