...
并非所有用户帐户都有权访问相同的公司信息。 某些帐户有权访问敏感信息，例如财务数据、产品开发信息、合作伙伴对关键生成系统的访问权限等。 有权访问高度机密信息的帐户如果受到威胁，将构成严重的威胁。 我们将这些类型的帐户称为 _优先级帐户_。 优先级帐户包括 CEOS、COS、CFOS、基础结构管理员帐户、生成系统帐户等。
...
作为 Microsoft 365 E5 或附加订阅) 的一部分，Microsoft Defender for Office 365 计划 2 (中的用户标记是一种在报告和事件调查中快速识别和分类特定用户或用户组的方法。
