...
在独立 Exchange Online Protection (EOP) 不含 Exchange Online 邮箱的组织中，您可以创建、修改和删除以下类型的组：
...
- **启用邮件的安全组**：邮件用户和其他需要管理员角色访问权限的安全组的集合。 例如，您可能希望向特定用户组授予管理员权限，以便他们可以配置反垃圾邮件和反恶意软件设置。
    ...
    > [!NOTE]
    >
    > - 默认情况下，启用邮件的新安全组会拒绝来自外部 (未经身份验证的) 发件人的邮件。
    >
    > - 不向启用邮件的安全组添加通讯组。
...
- 必须先分配有权限，然后才能执行这些过程。 具体来说，您需要 "通讯组" 角色，默认情况下，该角色分配给 OrganizationManagement (全局管理员) 和 RecipientManagement 角色组。 有关详细信息，请参阅 [独立 EOP 中的权限](feature-permissions-in-eop.md) 和 [使用 EAC 修改角色组中的成员列表](manage-admin-role-group-permissions-in-eop.md#use-the-eac-modify-the-list-of-members-in-role-groups)。
...
3. 在打开的 "新建组" 页中，配置以下设置。 使用标记为的设置 <sup>\*</sup> 是必需的。
   ...
   - <sup>\*</sup>**电子邮件地址**：电子邮件地址由 ( @ ) 符号的左侧的别名和右侧的域组成。 默认情况下， **别名** 的值用于别名值，但您可以对其进行更改。 对于 "域" 值，请单击下拉，并在组织中选择并接受域。
...
- **别名**：这是电子邮件地址中显示在 ( @ ) 符号左侧的部分。 如果更改别名，组的主 SMTP 地址也会更改，并包含新的别名。 同时，带有之前别名的电子邮件地址将作为该组的代理地址保留。

- **电子邮件地址**：电子邮件地址由 ( @ ) 符号的左侧的别名和右侧的域组成。 默认情况下， **别名** 的值用于别名值，但您可以对其进行更改。 对于 "域" 值，请单击下拉，并在组织中选择并接受域。
...
- 如果不使用 _alias_ 参数，则 _Name_ 参数将用于别名值。 删除空格并将不受支持的字符转换为问号 (？ ) 。

- 如果不使用 _PrimarySmtpAddress_ 参数，则在 _PrimarySmtpAddress_ 参数中使用别名值。

- 如果不使用 _Type_ 参数，则默认值为分布。
...
本示例使用更改主 SMTP 地址 (也称为 "西雅图员工" 组的 "答复地址) " 到 "sea.employees@contoso.com"。
...
- 在 EAC 中，转到 " **收件人** \> **组**"。 确认组已列出 (或未列出) ，并验证 " **组类型** " 值。 选择该组并查看详细信息窗格中的信息，或单击 " **编辑** ![ 编辑图标 ](../../media/ITPro-EAC-AddIcon.png) " 以查看设置。

- 在独立 EOP PowerShell 中，运行以下命令来验证组是否 (或未在) 中列出：
