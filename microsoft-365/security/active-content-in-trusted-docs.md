...
以前，当用户将文档标识为受信任的文档时，用户的选择将允许运行活动内容，即使管理员配置了策略来阻止文档中的活动内容Office内容。 现在，管理员设置的策略优先于受信任文档的用户标识。 此行为更改可能会导致用户问题。
...
3. 这是更新的行为生效的地方：
   ...
   - 现在，对于用户是否将文档标识为受信任的文档，现在 (步骤 8) 。

     这是行为的基本更改：云策略 (步骤 4) 、组策略 (步骤 6) 和本地设置 (step 7) ，然后再考虑用户指定的受信任文档。  如果其中任一步骤阻止访问活动内容，并且任何步骤均不允许用户替代，则用户将文档标识为受信任的文档基本上不相关。
...
- **Office云** 策略服务：设置基于用户的策略，该策略适用于使用其 Azure AD 帐户访问 Office 应用中文件的任何设备上的用户。 请参阅在云[策略服务Office云策略Office](/DeployOffice/overview-office-cloud-policy-service)[的步骤](https://config.office.com/officeSettings/officePolicies)。
- **Office Intune** 中的策略：使用 Intune 设置 目录或管理模板将 HKCU 策略部署到 Windows 10 电脑：在 [MEM](https://endpoint.microsoft.com/#blade/Microsoft_Intune_DeviceSettings/DevicesMenu/configurationProfiles)管理中心的 **"设备** 配置文件" \> **下**。
  ...
  - ***设置预览 (目录) ：*** 请参阅使用预览设置目录 [ () 。](/mem/intune/configuration/settings-catalog)
...
|外接程序&可扩展性|PowerPoint|运行程序|**禁用 (运行任何程序)**|**值** "是"在 **(用户之前启用"提示)**|
...
|宏|Outlook|Outlook安全模式|**使用Outlook安全组策略**|需要启用所有 Outlook GPO 设置。 <p> 作为依赖关系 (此策略不会阻止活动内容本身) 。|
...
|外接程序&可扩展性|Office|阻止在文档中Office Flash 激活|请参阅 Microsoft 安全指南 ADMX/ADML 文件，查看 COM 删除位列表，以阻止在应用中对 Flash Microsoft 365激活。 企业安全基线的 ADMX/ADML 文件在安全与合规[Toolkit。](https://www.microsoft.com/download/details.aspx?id=55319)|否|
