...
列表事件 API 允许你对事件进行排序，以创建明智的网络安全响应。 它公开在环境保留策略中指定的时间范围内网络中标记的事件的集合。 最新事件显示在列表顶部。 每个事件都包含一组相关警报及其相关实体。
...
如果成功，此方法返回 `200 OK` 响应正文 [中的](api-incident.md) 事件列表。
...
状态 | 将事件分类 (*活动或* 已解决 *) 。* 它可以帮助你组织和管理对事件的响应。 | 活动文件
severity | 指示对资产可能的影响。 严重性越高，影响越大。 通常，严重性较高的项目需要最直接的注意。<br /><br />下列值之一：*信息性*、*低*、**中等和高。* | 中
...
“类别” | 攻击沿击杀链的进度的可视化和数值视图。 与 [MITRE ATT&CK™对齐](https://attack.mitre.org/)。 | 影响
状态 | 将警报分类为 *(、活动或* 已解决 *) 。* 它可以帮助你组织和管理对警报的响应。 | 新式
severity | 指示对资产可能的影响。 严重性越高，影响越大。 通常，严重性较高的项目需要最直接的注意。<br>下列值之一：*信息性*、*低*、**中等和高。* | 中
...
url | 如果 entityType 为 *Url，则可用*。 <br>与网络事件（如到恶意网络目标的通信）关联的 *警报的 URL。* | down.esales360.cn
