...
虽然可以构造 [高级](advanced-hunting-overview.md) 搜寻查询以返回非常精确的信息，但您也可以使用查询结果来进一步深入了解和调查特定活动和指示器。 您可以对查询结果执行以下操作：
...
| **堆积柱形图** | 将 x 轴上的一系列唯一项呈现为堆积垂直条，其高度代表来自一个或多个其他字段的数值 |
| **饼图** | 呈现表示唯一项的节饼图。 每个饼图的大小表示另一个字段的数值。 |
| **Donut 图表** | 呈现表示唯一项的节弧。 每个弧的长度表示来自另一个字段的数值。 |
...
![显示为柱形图的按严重性显示的警报的柱形图查询结果的高级搜寻 ](../../media/advanced-hunting-column-chart.jpg)
 *查询结果的图像，显示为柱形图*

#### <a name="alert-severity-by-operating-system"></a>按操作系统的警报严重性
您还可以使用运算符 `summarize` 为来自多个字段的图表值准备结果。 例如，你可能希望了解警报严重性如何分布在操作系统和操作系统 (操作系统) 。 
...
![显示为按操作系统显示的警报和严重性的堆叠图表查询结果的高级搜寻查询结果的图像，显示为堆积 ](../../media/advanced-hunting-stacked-chart.jpg)
 *图表*

#### <a name="phishing-emails-across-top-ten-sender-domains"></a>跨前十个发件人域的网络钓鱼电子邮件
如果处理的值列表并不有限，可以使用运算符仅绘制包含最多实例 `Top` 的值的图表。 例如，若要获取具有网络钓鱼电子邮件最多的前十个发件人域，请使用以下查询：
...
![以饼图饼图显示高级搜寻查询结果的图像，显示钓鱼电子邮件在 ](../../media/advanced-hunting-pie-chart.jpg)
 *顶部发件人域中的分布*

#### <a name="file-activities-over-time"></a>一段时间的文件活动
将 `summarize` 运算符与 `bin()` 函数一同使用，可以检查一段时间是否涉及特定指示器的事件。 下面的查询按 30 分钟的间隔计算涉及文件的事件，以显示与该文件相关的活动 `invoice.doc` 峰值：
...
```
下面的图表清楚地突出显示了具有更多活动的时间段，包括 `invoice.doc` ： 

![高级搜寻查询结果的图像，显示为显示一段时间涉及文件的事件数的直线图表 ](../../media/advanced-hunting-line-chart.jpg)
 


## <a name="export-tables-and-charts"></a>导出表格和图表
运行查询后，选择" **导出** "将结果保存到本地文件。 所选的视图确定结果的导出方式：
...
- **进程树** — 为包含流程信息的记录生成，并且使用可用上下文信息进行扩充;通常，返回更多列的查询可以产生更丰富的进程树。
- **所有详细信息** - 记录中列的所有值  
...
通过选择要包含或排除的值上的或按钮，然后选择"运行查询"来 `+` `-` **优化查询**。
