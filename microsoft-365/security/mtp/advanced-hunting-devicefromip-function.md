...
使用高级搜寻查询中的函数可快速获取给定时间点已分配给特定 IP 地址 `DeviceFromIP()` 的设备列表。 [](advanced-hunting-overview.md) 
...
- **x**— 第一个参数通常是查询中的列。 在这种情况下，它是名为"IP 地址"的列，您希望查看已分配给它的设备 `IP` 列表。 它应该是本地 IP 地址。 不支持外部 IP 地址。
- **y**—第二个可选参数是 ，指示函数从特定时间获取最新 `Timestamp` 分配的设备。 如果未指定，函数将返回最新可用记录。
