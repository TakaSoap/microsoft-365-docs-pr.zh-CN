...
Microsoft 365 Defender 聚合来自设备、用户和邮箱的一切相关警报、资产、调查和证据，让你全面了解整个攻击范围。

调查影响网络的警报，了解其含义，整理与事件相关的证据，以便可以制定出有效的补救计划。

## <a name="investigate-an-incident"></a>调查事件

1. 从事件队列中选择一个事件。 <BR> 侧面板将打开，并预览重要信息，如状态、严重性、类别和受到影响的实体。

    ![事件侧面板的图像](../../media/incident-side-panel.png)

2. 选择“**打开事件页面**”。 <BR> 这将打开事件页面，你将在此找到事件详细信息、注释和操作、选项卡 (概述、警报、设备、用户、调查、证据) 。
...
通过攻击类别，你可以直观和数字地了解攻击对击杀链的进度。 与其他 Microsoft 安全产品一样，Microsoft 365 Defender 与[MITRE ATT&&trade; CK](https://attack.mitre.org/)框架一致。
...
可以查看与事件相关的所有警报及其其他信息，例如严重性、警报中涉及的实体、警报的来源 (Microsoft Defender for Identity、Microsoft Defender for Endpoint、Microsoft Defender for Office 365) 以及将它们链接在一起的原因。

![事件警报页面图像](../../media/incident-alerts.png)

警报默认按时间顺序排序，可以首先查看攻击如何随时间推移。 单击每个警报将引导您进入相关警报页面，您可以在其中对此警报进行深入调查。
...
调查标记为属于事件或与事件相关的邮箱。 若要执行进一步调查工作，选择与邮件相关的警报将打开 Microsoft Defender for Office 365，可在其中采取修正操作。
...
选择“调查”，以导航到调查详细信息页面，获取有关调查和修复状态的完整信息。 如果作为调查的一部分有任何等待审批的操作，它们将显示在"挂起的操作"选项卡中。采取措施作为事件修正的一部分。

## <a name="evidence"></a>证据

Microsoft 365 Defender 自动调查警报中所有事件支持的事件和可疑实体，为您提供自动响应和有关重要文件、进程、服务、电子邮件等的信息。 这有助于快速检测并阻止事件中的潜在威胁。
