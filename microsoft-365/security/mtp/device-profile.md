...
可以自定义显示的项目数和列表中的列。 默认列列出事件时间、活动用户、操作类型、实体 (进程) 以及有关事件的其他信息。
...
" **发现漏洞** " 选项卡列出了可能会影响设备的任何常见漏洞和漏洞 (cve) 。

![设备配置文件的 "发现漏洞" 选项卡的图像](../../media/mtp-device-profile/hybrid-device-tab-discovered-vulnerabilities.png)

默认视图列出了 CVE 的严重性、常见漏洞分数 (CVS) 、与 CVE 有关的软件、发布 CVE 时、何时最后更新 CVE 以及与 CVE 相关的威胁。
