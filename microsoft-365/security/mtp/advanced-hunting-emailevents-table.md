...
| `ConfidenceLevel` | string | 任何垃圾邮件或网络钓鱼裁定的可信度列表。 对于垃圾邮件，此列显示垃圾邮件可信度 (SCL) ，指示电子邮件是否跳过了 (-1) 、发现不是垃圾邮件 (0，1) 、被识别为可信度中等的垃圾邮件 (5，6) ，或发现是高可信度 (9) 中的垃圾邮件。 对于网络钓鱼，此列显示可信度是"高"还是"低"。 |
