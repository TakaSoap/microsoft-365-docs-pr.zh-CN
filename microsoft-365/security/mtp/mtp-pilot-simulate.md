...
切换到 SOC 分析师的观点，现在可以开始在 Microsoft 威胁防护门户中调查攻击。 

1.  从任何设备打开 [Microsoft 威胁防护门户](https://security.microsoft.com/incidents) 事件队列。
...
导航回 Microsoft 威胁防护门户中的事件。 "**事件**" 页面中的 "**调查**" 选项卡显示由 Azure ATP 和 Microsoft Defender ATP 触发的自动调查。 下面的屏幕截图仅显示由 Microsoft Defender ATP 触发的自动调查。 默认情况下，Microsoft Defender ATP 将自动 remediates 在队列中找到的、需要修正的项目。
...
一旦事件得到解决，它将关闭 Microsoft 威胁防护和相关门户中的所有关联警报。
