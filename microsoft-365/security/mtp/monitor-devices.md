...
每个 Microsoft Defender ATP 警报都有相应的严重性（高、中、低或信息），指示其对网络的潜在影响（如果无人值守）。  
...
Microsoft Defender ATP 将每个警报分类到一个类别，表示攻击链中的某个阶段或威胁组件的类型。 例如，检测到的威胁活动可能会被分类为 "横向移动"，以指示有人尝试访问网络上的其他设备。 在攻击者获取初始 foothold 之后，也可能发生此活动。 检测到威胁组件时，可能会将其广泛划分为恶意软件，或者更具体地说是作为勒索软件、凭据偷窃或其他类型的恶意软件或不需要的软件。
