...
信息安全注册的评估员计划（IRAP）为针对澳大利亚政府策略和指南的系统安全的独立评估提供了一个全面的过程。 IRAP 目标是通过重点介绍存储、处理和传达澳大利亚的信息和通信技术基础结构，最大限度地提高澳大利亚联邦、州和本地政府数据的安全性。

## <a name="irap-overview"></a>IRAP 概述

信息安全注册的评估员程序（IRAP）由 ACSC 进行管理和管理。 IRAP 提供了用于从私有和公共扇区中为个人背书，以向澳大利亚政府提供网络安全评估服务的框架。 已背书的 IRAP 评估员可提供对 ICT 安全的独立评估，建议缓解措施并突出显示剩余风险。 IRAP 提供了针对澳大利亚政府策略和准则独立评估系统安全的综合过程。 IRAP 目标是通过重点介绍存储、处理和传达澳大利亚的信息和通信技术基础结构，最大限度地提高澳大利亚联邦、州和本地政府数据的安全性。
...
Microsoft 的 Azure、Dynamics 365 CRM 和 Office 365 将继续在 CCSL 中包含未分类：散播限制标记（DLM）（现在为官方：敏感）和受保护的数据（基于以前的 IRAP 评估和证书）澳大利亚网络安全中心（ACSC）。
...
对于每个评估，Microsoft 按照 ACSC 资格鉴定的 IRAP 评估员负责检查 Microsoft IT 运营团队、物理数据中心、入侵检测、加密、跨域和网络安全性所使用的安全控制和流程。范围内服务的访问控制和信息安全风险管理。 IRAP 评估发现，Microsoft 系统体系结构基于合理的安全原则，并且适用的澳大利亚政府信息安全手册（ISM）控件在评估服务中已就绪且完全有效。

ISM 使用的风险管理框架是从[国家标准和技术协会（NIST）特殊出版物（SP） 800-37](https://csrc.nist.gov/publications/detail/sp/800-37/rev-2/final)的 "风险管理框架" （针对信息系统和组织的风险管理框架）中进行绘制的，即安全和隐私的系统生命周期方法。 在此风险管理框架中，可以使用各种风险管理标准（如[国际标准化组织（ISO）31000:2018、风险管理–指南）](https://www.iso.org/standard/65694.html)来确定风险和选择安全控制措施。 ISM 广泛使用的风险管理框架包含六个步骤：
...
- [Microsoft 服务信任门户（STP）的 "澳大利亚" 页面](https://aka.ms/au-irap)
