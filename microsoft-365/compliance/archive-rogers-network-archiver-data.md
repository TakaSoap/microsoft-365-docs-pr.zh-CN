...
使用企业版中的 TeleMessage 连接器Microsoft 365 合规中心并存档来自为用户而发送的短信和彩信数据。 在设置和配置一个一个设置和配置[了一个设置和配置了一个设置](https://www.telemessage.com/mobile-archiver/network-archiver/rogers/)后，该连接器将连接到您组织的为 Your Organization 的一个 Mobile Network，并导入短信和彩信数据到 Microsoft 365 中的邮箱。

在用户邮箱中存储了来自用户邮箱的用于 Microsoft 365 移动网络的数据后，可以将 Microsoft 365 合规性功能（如诉讼保留、内容搜索和Microsoft 365保留策略）应用于数据。 例如，您可以使用内容搜索或与核心电子数据展示案例关联的搜索，搜索来自具有 Mobiles 移动网络的短信和彩信。 使用一个为组织导入和存档数据的 Microsoft 365 可帮助组织遵守公司管理法规和法规策略。

## <a name="overview-of-archiving-rogers-mobile-network-data"></a>存档用户移动网络数据概述

以下概述介绍使用连接器在电子邮件中存档一条短信和彩信Microsoft 365。
...
- 必须分配有在步骤 3 中创建一个为用户创建一个"数据连接器""存档器"的管理员角色。 若要在"数据连接器"页上添加连接器，需要此Microsoft 365 合规中心。 默认情况下，此角色添加到多个角色组。 有关这些角色组的列表，请参阅安全与合规中心内的权限中的"安全与合规& ["部分](../security/office-365-security/permissions-in-the-security-and-compliance-center.md#roles-in-the-security--compliance-center)。 或者，您组织的管理员可以创建自定义角色组，分配数据连接器管理员角色，然后将相应的用户添加为成员。 有关说明，请参阅"权限"部分中的"创建自定义[角色Microsoft 365 合规中心](microsoft-365-compliance-center-permissions.md#create-a-custom-role-group)。
...
完成上一部分中所述的先决条件后，可以在部署中创建一个Microsoft 365 合规中心。 连接器使用你提供的信息连接到 TeleMessage 站点，将一条短信/彩信数据传输给 Microsoft 365 中的相应用户邮箱框。

1. 转到 ， <https://compliance.microsoft.com> 然后单击"数据 **连接器** > **""数据连接器""网络存档器"**。

2. 在 **"一线网络存档器** 产品说明"页上，单击" **添加连接器"**。
