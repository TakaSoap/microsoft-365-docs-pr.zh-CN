...
- **[就地保留](/Exchange/security-and-compliance/create-or-remove-in-place-holds)：** 通过使用 Exchange Online 中 Exchange 管理中心中的In-Place电子数据展示&保留"工具应用于用户邮箱的保留Exchange Online。 
...
- **[Microsoft 365保留策略](retention.md)：** 可以配置为保留 (，或保留然后删除 Exchange Online 中用户邮箱中的) 内容，以及 Microsoft 365 组和邮箱Microsoft Teams。 您还可以创建保留策略，以保留Skype for Business对话，这些对话存储在用户邮箱中。
...
若要管理处于保留状态中的邮箱，可能需要确定邮箱上所放置的保留类型，以便执行以下任务：更改保留期、临时或永久删除保留，或者从 Microsoft 365 保留策略中排除邮箱。 在这些情况下，第一步是标识置于邮箱上的保留类型。 由于多个 (和不同类型的) 可以置于单个邮箱上，因此如果要删除或更改保留，必须标识邮箱上设置的所有保留。
...
运行以下命令，获取有关应用于邮箱Microsoft 365保留策略的信息。
...
若要在延迟保留过期之前删除延迟保留，可以在 (PowerShell 中运行) 或同时运行Exchange Online命令，具体取决于要更改的属性：
