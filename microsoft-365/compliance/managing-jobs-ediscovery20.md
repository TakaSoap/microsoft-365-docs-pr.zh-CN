...
| 作业类型            | 说明     |
| :----------------- | :----------     |
|正在添加数据到审阅集。 | 用户将搜索结果添加到审阅集。 此作业包括 2 个子作业： </br>• **GatheringItems** - 生成匹配搜索查询（以及其所在 Microsoft 365 数据源）的项目列表。 </br>• **引入和编制索引** - 将匹配搜索查询的项目（在名为 *引入* 的进程中 ）复制到 Azure Storage 位置，然后在Azure Storage 位置重新编制这些项目的索引。 新索引将用于在数据集中查询和分析项目。 </br></br>有关详细信息，请参阅 [添加搜索结果到审阅集](add-data-to-review-set.md)。 |
...
|预估搜索结果 | 用户创建并运行新搜索（或重新运行已有搜索）后，搜索工具将搜索与搜索结果相匹配项目的索引，并预估搜索到的全部项目的数目和总量，以及所搜索数据源的数目。  有关详细信息，请参阅 [收集案例数据](collecting-data-for-ediscovery.md)。 | 
...
|准备搜索预览 | 用户创建并运行新搜索（或重新运行已有搜索）后，搜索工具将给出可预览项目（与搜索查询相匹配）的示例子集。 预览搜索结果有助于确定搜索是否有效。  有关详细信息，请参阅 [收集案例数据](collecting-data-for-ediscovery.md#view-search-results-and-statistics)。 | 
