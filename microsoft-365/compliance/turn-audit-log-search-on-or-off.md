...
启用 Microsoft 365 合规中心 中的审核时，组织的用户和管理员活动将记录在 审核日志 中，并保留 90 天，最多保留一年，具体取决于分配给用户的许可证。 但是，您的组织可能出于不希望记录并保留数据审核日志的原因。 在这种情况下，全局管理员可能会决定关闭 Microsoft 365。

> [!IMPORTANT]
> 如果在组织中关闭Microsoft 365，则不能Office 365管理活动 API 或 Azure Sentinel 访问组织的审核数据。 按照本文中的步骤关闭审核意味着在使用 审核日志 搜索 Microsoft 365 合规中心 或在 Exchange Online PowerShell 中运行 **Search-UnifiedAuditLog** cmdlet 时不会返回任何结果。 这也意味着审核日志无法通过 Office 365管理活动 API 或 Azure Sentinel 提供。
...
如果未为组织启用审核，可以在 Microsoft 365 合规中心中或通过使用 PowerShell Exchange Online审核。 在打开审核后，可能需要几个小时才能在搜索查询时返回审核日志。
...
有关搜索管理员帐户Exchange详细信息审核日志[Search-AdminAuditLog](/powershell/module/exchange/search-adminauditlog)。