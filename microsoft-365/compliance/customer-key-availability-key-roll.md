...
Microsoft 不会向客户公开对可用性密钥的直接控制。 例如，您只能在 Azure Key Vault 中滚动（旋转）您拥有的密钥。 Office 365 按内部定义的计划滚动可用性密钥。 没有面向客户的服务级别协议（SLA），这些密钥将会回滚。 Office 365 在自动、非手动过程中使用 Office 365 服务代码旋转可用性密钥。 Microsoft 管理员可能会启动滚动过程。 使用自动机制对密钥进行滚动，而无需直接访问密钥存储。 无法为 Microsoft 管理员预配对可用性密钥机密存储的访问权限。 可用性密钥滚动利用用于最初生成密钥的相同机制。 有关可用性密钥的详细信息，请参阅[了解可用性密钥](customer-key-availability-key-understand.md)。
