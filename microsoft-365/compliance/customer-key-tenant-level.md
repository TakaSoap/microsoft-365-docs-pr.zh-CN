...
对于 Microsoft Teams，从 DEP 分配给租户起，租户级别的客户密钥将加密新数据。 公共预览版不支持加密过去的数据。 对于 Exchange Online，客户密钥加密所有现有和新数据。

每个租户可以创建多个 DEP，但在任何时间点只能分配一个 DEP。 分配 DEP 时，加密将自动开始，但可能需要一些时间才能完成，具体取决于租户的大小。
...
创建的租户级加密策略对 Microsoft 365 中的 Microsoft Teams 和 Exchange Online 工作负载的所有数据进行加密。 此策略不会干扰已在客户密钥中创建的微调的 DEP。
...
## <a name="set-up-customer-key-at-the-tenant-level-public-preview"></a>在租户级别设置客户密钥 (预览版) 
...
根加密密钥的临时或永久丢失可能会中断甚至对服务操作造成灾难性影响，并可能导致数据丢失。 因此，用于客户密钥的资源需要强大的保护。 用于客户密钥的所有 Azure 资源都提供默认配置以外的保护机制。 Azure 订阅可以标记或注册，以防止立即取消和不可撤销的取消。 这称为注册强制保留期。 注册 Azure 订阅以保留强制保留期所需的步骤需要与 Microsoft 协作。 此过程最多可能需要 5 个工作日。 以前，这有时称为"不取消"。
...
当您可以快速恢复密钥时，您不太可能因意外或恶意删除的密钥而遇到扩展的服务中断。 启用此配置（称为"软删除"）后，才能将密钥与客户密钥一同使用。 启用软删除允许您在删除后 90 天内恢复密钥或保管库，而无需从备份中还原它们。
  
若要对密钥保管库启用软删除，请完成以下步骤：
...
3. 通过运行 **Get-AzKeyVault** cmdlet 确认为密钥保管库配置软删除。 如果为密钥保管库正确配置软删除，则 _启用软_ 删除的属性将返回 **True 值**：
...
有两种方法可以将密钥添加到 Azure 密钥保管库;可以直接在 Key Vault 中创建密钥，也可以导入密钥。 直接在 Key Vault 中创建密钥是不太复杂的方法，而导入密钥可提供对密钥生成方式的完全控制。 使用 RSA 密钥。 Azure Key Vault 不支持使用椭圆曲线键环绕和解包。
...
如果您打算使用 HSM 保护密钥，请确保将 **HSM** 指定为 _Destination_ 参数的值，否则，请指定 **Software**。
...
如果恢复级别属性返回除 **"可恢复+ProtectedSubscription"** 值外的其他值，则需要查看本文，并确保已按照所有步骤将订阅置于"不取消"列表中，并确保在每个密钥保管库上启用了"软删除"。 接下来，将电子邮件输出的屏幕截图 `(Get-AzKeyVaultKey -VaultName <vault name> -Name <key name>).Attributes` 发送到m365ck@microsoft.com。
...
在 DEP 中使用密钥之前执行验证是可选的，但强烈建议这样做。 特别是，如果使用除本主题中所述的步骤设置密钥和保管库外的步骤，应在配置客户密钥之前验证 Azure Key Vault 资源的运行状况。
...
客户密钥无法使用过期密钥，并且尝试使用过期密钥的操作将失败，并可能导致服务中断。 我们强烈建议用于客户密钥的密钥没有过期日期。 一旦设置过期日期，将无法删除，但可更改为其他日期。 如果必须使用设置了过期日期的密钥，将过期值更改为 12/31/9999。 到期日期设置为 12/31/9999 日期的密钥无法通过 Microsoft 365 验证。
...
您需获得权限，然后才能运行这些 cmdlet。 尽管本文列出了 cmdlet 的所有参数，但如果这些参数未包含在分配给您的权限中，则您可能无法访问这些参数。 若要查找在贵组织中运行任何 cmdlet 或参数所需的权限，请参阅 [Find the permissions required to run any Exchange cmdlet](https://docs.microsoft.com/powershell/exchange/exchange-server/find-exchange-cmdlet-permissions)。
...
说明：允许合规性管理员使用两个 AKV 根密钥 (DEP) 数据加密策略。 创建后，可以使用 cmdlet 分配Set-M365DataAtRestEncryptionPolicy策略。 首次分配密钥或旋转密钥后，新密钥最多可能需要 24 小时才能生效。 如果新的 DEP 需要 24 小时以上才能生效，请与 Microsoft 联系。
...
说明：该 cmdlet 可用于修改或刷新现有策略。 它还可用于启用或禁用策略。 首次分配密钥或旋转密钥后，新密钥最多可能需要 24 小时才能生效。 如果新的 DEP 需要 24 小时以上才能生效，请与 Microsoft 联系。
...
如果需要恢复为 Microsoft 托管的密钥，可以。 当你离开时，你的数据会使用每个单独工作负载支持的默认加密重新加密。 例如，Exchange Online 支持使用 Microsoft 托管密钥进行默认加密。
...
有关用于客户密钥的旋转或滚动键的信息，请参阅滚动或 [旋转客户密钥或可用性密钥](customer-key-availability-key-roll.md)。 更新 DEP 以使用新版本的密钥时，将运行 Set-M365DataAtRestEncryptionPolicy cmdlet，如本文前面所述。
