...
- [内容搜索](limits-for-content-search.md)限制，例如一次可以运行的最大搜索数和可以包含在单个搜索中的最大内容位置数
...
有时，您必须执行更高级、更复杂且重复的内容搜索任务。 在这些情况下，使用安全与合规中心 PowerShell 中的命令&快捷。 为了帮助简化这一过程，我们创建了多个安全与合规& PowerShell 脚本，以帮助你完成与内容搜索相关的复杂任务。

- [当您确信响应](use-content-search-for-targeted-collections.md) (的邮件位于该文件夹中时) 特定邮箱和网站文件夹将搜索称为目标集合的邮件
