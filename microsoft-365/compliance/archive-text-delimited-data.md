...
使用 Microsoft 365 合规中心中的一个 Microsoft 365 连接器将文本分隔的数据导入并存档到 Microsoft 365 组织的用户邮箱。 Microsoft 提供 [一个](https://globanet.com/text-delimited) 文本分隔连接器，配置为定期捕获第三方数据源 (中的项目) 将这些项目导入到 Microsoft 365。 连接器将内容从文本分隔的数据源转换为电子邮件格式，然后将这些项目导入到 Microsoft 365 中的用户邮箱。
...
3. 在 Microsoft 365 合规中心内创建的以文本分隔的连接器每天连接到 Microsoft Merge1 网站，将邮件传输至 Microsoft 云中的安全 Azure 存储位置。
