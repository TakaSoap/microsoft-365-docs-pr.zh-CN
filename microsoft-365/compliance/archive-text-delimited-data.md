...
使用 Microsoft 365 合规性中心中的 Globanet 连接器将文本分隔的数据导入到 Microsoft 365 组织中的用户邮箱并进行存档。 Globanet 提供了一个 [文本分隔的连接器](https://globanet.com/text-delimited) ，该连接器配置为定期捕获第三方数据源中的项目 () 并将这些项目导入到 Microsoft 365。 连接器将文本分隔的数据源中的内容转换为电子邮件格式，然后将这些项目导入到 Microsoft 365 中的用户邮箱中。

将以文本分隔的数据存储在用户邮箱中之后，可以应用 Microsoft 365 合规性功能，如诉讼保留、电子数据展示和保留策略以及保留标签。 使用以文本分隔的数据连接器以在 Microsoft 365 中导入和存档数据可帮助您的组织遵守政府和法规策略。

## <a name="overview-of-archiving-the-text-delimited-data"></a>存档以文本分隔的数据概述

以下概述说明使用连接器在 Microsoft 365 中存档以文本分隔的源信息的过程。
...
4. 连接器使用自动用户映射的 *电子邮件* 属性的值将已转换的邮件项导入到特定用户的邮箱中，如步骤3中所述。 在用户邮箱中 **创建名为** "收件箱" 文件夹中的新子文件夹，并将邮件项目导入该文件夹中。 连接器通过使用 *Email* 属性的值确定要将项目导入到哪个邮箱。 每封邮件都包含此属性，该属性由每个参与者的电子邮件地址填充。
...
第二步是在 Merge1 网站上配置文本分隔的连接器。 有关在 Globanet Merge1 网站上配置文本分隔的连接器的信息，请参阅 [Merge1 第三方连接器用户指南](https://docs.ms.merge1.globanetportal.com/Merge1%20Third-Party%20Connectors%20text-delimited%20User%20Guide%20.pdf)。
...
2. 单击 " **连接线** " 选项卡，然后选择以 **文本分隔** 的连接线以显示弹出页面。 此页面包含有关连接器的属性和信息。
