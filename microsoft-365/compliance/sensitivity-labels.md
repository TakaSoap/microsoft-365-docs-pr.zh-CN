...
示例显示 Excel 中可用的灵敏度标签来自于功能区上的 **主页** 选项卡。在此示例中，已应用的标签将显示在状态栏上：
...
- **跨越不同平台和设备保护 Office 应用程序中的内容。** 若要获取有关支持的应用程序的列表，请参阅 [在 Office 应用程序中使用灵敏度标签](sensitivity-labels-office-apps.md)。

- 使用 Microsoft Cloud App Security **保护第三方应用程序和服务中的内容**。通过云应用程序安全，即使第三方应用程序或服务无法读取或不支持敏感性标签，也可以检测、分类、标记和保护第三方应用程序和服务（如 SalesForce、Box 或 DropBox）中的内容。

- **保护包括 Teams、Microsoft 365 组和 SharePoint 网站的容器**。例如，设置隐私设置、外部用户访问权限和来自非托管设备的访问。

- **将敏感度标签扩展到第三方应用程序和服务。** 使用 Microsoft 信息保护 SDK，则第三方应用程序可读取敏感度标签并应用保护设置。

- **无需使用任何保护设置即可对内容进行分类。** 也可以简单地对内容进行分类（如贴纸），该分类在内容使用和共享时将随内容一起持续存在和漫游。使用此分类以生成使用情况报告，并查看敏感内容的活动数据。根据此类信息，可随时选择以后应用程序的保护设置。
...
- **清除清除。** 由于该标签以清晰的文本存储在内容的元数据中，因此第三方应用程序和服务可以读取它，然后根据需要应用其保护操作。

- **永久性。** 在向内容应用敏感度标签后，该标签将存储在相应电子邮件或文档的元数据中。这意味着标签会随内容（包括保护设置）一起漫游，这些数据会成为应用和执行策略的基础。

在 Office 应用中，敏感度标签就像是电子邮件或文档上向用户显示的标记。

支持灵敏度标签的每个项目都可以应用一个灵敏度标签。文件和电子邮件可以同时应用敏感标签和[保留标签](retention.md#retention-labels)。
...
当电子邮件或文档应用有敏感度标签后，系统便会对内容强制执行相应标签的任何已配置的保护设置。敏感度标签可用于：

- 仅 **加密** 电子邮件或同时加密电子邮件和文档。可选择哪些用户或组在多长时间内有权执行哪些操作。例如，可选择允许另一个组织的特定域中的用户在内容有标签后仅有 7 天的权限来审查相应内容。另外，可不采用管理员定义的权限，而是允许用户在应用标签时为内容分配权限。 
    
    有关创建或编辑敏感度标签时的“**加密**”设置的详细信息，请参阅 [使用敏感度标签中的加密限制对内容的访问](encryption-sensitivity-labels.md)。

- 当使用 Office 应用程序时，通过向已应用标签的电子邮件或文档添加水印、页眉或页脚来 **标记内容**。水印可以应用到文档，但不能应用到电子邮件。页眉和水印的示例：
    
    ![应用于文档的水印和页眉](../media/Sensitivity-label-watermark-header.png)
    
    需要检查何时应用了内容标记？请参阅 [Office 应用程序何时应用内容标记和加密](sensitivity-labels-office-apps.md#when-office-apps-apply-content-marking-and-encryption)。
    
    某些应用程序（但非所有应用程序）通过使用变量以支持动态标记。例如，在页眉、页脚或水印中插入标签名称或文档名称。更多相关详细信息，请参阅 [带有变量的动态标记](sensitivity-labels-office-apps.md#dynamic-markings-with-variables)。
    
    尽管可以为这些内容标记配置自定义字体名称，并配置包括 RGB 代码自定义的多种字体颜色，但这些设置仅由 [Azure 信息保护统一标签客户端](https://docs.microsoft.com/azure/information-protection/rms-client/aip-clientv2)支持。除非仅使用此客户端应用敏感度标签，否则不要使用自定义字体设置，并且不要选择以下颜色之一：黑色、黄色、蓝色、绿色、红色。
