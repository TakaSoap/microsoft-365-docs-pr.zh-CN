...
仅全局（公共）云中的租户支持敏感度标签。 其他云（如[国家云](https://docs.microsoft.com/azure/active-directory/develop/authentication-national-cloud)）中的租户不支持敏感度标签。
...
- **永久性**：向内容应用敏感度标签后，该标签将在相应电子邮件或文档的元数据中暂留。也就是说，标签（包括保护设置）随内容一起漫游，并成为应用和强制执行策略的基础。
...
除了电子邮件和文档以外，多个公共预览版本也提供了敏感度标签。 有关敏感度标签对文件、团队、组和网站有何作用的详细信息，请参阅以下文章：
...
一个文档或电子邮件只能应用有一个敏感度标签。 如果要求用户提供将标签更改为较低分类的理由，理由可以是此列表的排序，因为它会标识较低分类。
...
子标签只是向逻辑组中的用户显示标签的一种方式。 子标签不会从其父标签继承任何设置。 可以将子标签应用于内容；不能应用父标签。

不要选择父标签作为默认标签（请参阅下一节），或将父标签配置为自动应用或推荐使用，因为父标签不会应用于使用 Azure 信息保护统一标签客户端的 Office 应用程序中的内容。
...
敏感度标签显示在 Office 应用 UI 中。若要查看敏感度标签对特定应用和平台的最新适用情况，请参阅**[此功能目前在何处适用？](https://support.office.com/article/2f96e7cd-d5a4-403b-8bd7-4cc636bae0f9)**
...
有关 Web 上的 Office 应用的信息敏感度标签，请参阅[将敏感度标签应用于 Office Web 文档和电子邮件](https://support.office.com/article/2f96e7cd-d5a4-403b-8bd7-4cc636bae0f9)。
...
如果你已在 Office for Windows 中安装了 Azure 信息保护客户端，则 Office 365 将自动关闭敏感度标签。
有关如何解决此问题的详细信息，请参阅[关于 Office 内置标记客户端](sensitivity-labels-office-apps.md#about-the-office-built-in-labeling-client)。
...
将要创建敏感度标签的合规团队成员需要 Microsoft 365 合规中心、Microsoft 365 安全中心或 Office 365 安全与合规中心的访问权限。 默认情况下，你的租户管理员有权访问这些管理中心，并且可以向合规专员和其他人提供访问权限，而不为其提供租户管理员的所有权限。若要执行此操作。我们建议你转到其中一个管理中心的“**权限**”页面，然后将成员添加到**合规性管理员**或**安全管理员**角色组。

有关详细信息，请参阅[向用户授予对 Office 365 安全与合规中心的访问权限](https://docs.microsoft.com/microsoft-365/security/office-365-security/grant-access-to-the-security-and-compliance-center)。
