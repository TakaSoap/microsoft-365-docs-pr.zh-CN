...
此策略模板为内部用户启用风险评分，这些用户检测到与 EMR 系统中现有电子 (记录) 活动。 检测侧重于对患者数据的未经授权的访问、查看、修改和导出。 你需要将连接器配置为[ (Microsoft 医疗保健](import-healthcare-data.md)连接器或) 连接器) 以支持在[](import-epic-data.md)EMR 系统中检测访问、过滤或模糊处理活动。
