...
|所有信息屏障策略应用程序|使用 `Get-InformationBarrierPoliciesApplicationStatus -All`<p>这将显示有关策略应用程序是已完成、失败还是正在进行的信息。|
