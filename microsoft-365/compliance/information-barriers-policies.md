...
- 用户帐户属性在 Azure Active Directory (Exchange Online) 。 这些属性可以包括部门、职务、位置、工作组名称和其他工作配置文件详细信息。 
- 分段是使用所选用户帐户属性在安全与合规&定义 **的用户集**。  (请参阅[受支持的属性 .) ](information-barriers-attributes.md)
...
| 阶段 | 所涉及的内容 |
|:--------|:------------------|
| [确保满足先决条件](#prerequisites) | - 验证您是否具有 [所需的许可证和权限](information-barriers.md#required-licenses-and-permissions)<br/>- 验证目录是否包含用于分段用户的数据<br/>- 为 Microsoft Teams 启用作用域内目录搜索<br/>- 确保审核日志记录已打开<br/>- 确保没有 Exchange 通讯簿策略<br/>- 使用 PowerShell (提供了示例) <br/>- 向 Microsoft Teams 提供管理员 (步骤包含在)  |
| [第 1 部分：划分组织中用户](#part-1-segment-users) | - 确定需要哪些策略<br/>- 创建要定义的线段列表<br/>- 确定要使用哪些属性<br/>- 根据策略筛选器定义分段 |
| [第 2 部分：定义信息屏障策略](#part-2-define-information-barrier-policies) | - 定义策略 (尚未应用) <br/>- 从两种类型的 (选择或允许)  |
...
- 无通讯簿策略 - 在定义和应用信息屏障策略之前，请确保没有 Exchange 通讯簿策略。 信息屏障基于通讯簿策略，但两种类型的策略不兼容。 如果您具有此类策略，请确保首先 [删除通讯簿](/exchange/address-books/address-book-policies/remove-an-address-book-policy) 策略。 启用信息屏障策略并启用分层通讯簿后，未包含在信息屏障段中的所有用户都将在 Exchange Online 中查看分层通讯簿[](/exchange/address-books/hierarchical-address-books/hierarchical-address-books)。

- PowerShell - 目前，使用 PowerShell cmdlet 在 Office 365 安全与合规中心&和管理信息屏障策略。 尽管本文提供了几个示例，但您需要熟悉 PowerShell cmdlet 和参数。 你还需要 Azure PowerShell 模块。
...
- Microsoft Teams 中信息屏障的管理员同意 - 当你的 IBM 策略就位后，他们可以从组中删除非 (-IBM 合规性用户，即基于组策略的 Teams) 。 此配置有助于确保组织符合策略和法规。 使用以下过程使信息屏障策略在 Microsoft Teams 中正常工作。

   1. 先决条件：从安装 Azure PowerShell 安装[Azure PowerShell。](https://docs.microsoft.com/powershell/azure/install-az-ps)
   ...
   1. 系统提示时，使用 Office 365 的工作或学校帐户登录。
...
> [!TIP]
> 为了帮助你准备计划，本文中包括了一个示例方案。 [请参阅 Contoso 的部门、部门和策略](#example-contosos-departments-segments-and-policies)。<p>此外，可下载的 Excel 工作簿还可用于帮助你计划和定义分段和策略 (并创建 PowerShell cmdlet) 。 [获取工作簿](https://github.com/MicrosoftDocs/OfficeDocs-O365SecComp/raw/public/SecurityCompliance/media/InfoBarriers-PowerShellGenerator.xlsx)。
...
考虑法律和行业法规，贵组织中哪些组需要信息屏障策略？ 创建列表。 应阻止任何组与另一个组通信？ 是否允许任何组仅与一个或两个其他组通信？ 将所需的策略视为属于两个组之一：
...
除了初始策略列表之外，请为组织创建一个分段列表。 将包含在信息屏障策略中的用户应属于某个类别。 仔细规划你的分段，因为用户只能在一个段中。 每个分段只能应用一个信息屏障策略。
...
当你想要阻止分段相互通信时，你可以定义两个策略：每个方向一个策略。 每个策略仅阻止单向通信。
...
1. 若要定义第一个阻止策略，请使用 **New-InformationBarrierPolicy** cmdlet 和 **SegmentsBlocked** 参数。
    ...
    | 语法 | 示例 |
    |:--------|:----------|
    | `New-InformationBarrierPolicy -Name "policyname" -AssignedSegment "segment1name" -SegmentsBlocked "segment2name"` | `New-InformationBarrierPolicy -Name "Sales-Research" -AssignedSegment "Sales" -SegmentsBlocked "Research" -State Inactive` <p> 在此例中，我们为名为 Sales 的段定义了名为 *Sales-Research* *的策略*。 当处于活动状态和应用时，此策略会阻止 *销售* 部门中的人员与名为 Research 的部门中的 *人员通信*。 |

2. 若要定义第二个阻止段，请再次使用带 **SegmentsBlocked** 参数的 **New-InformationBarrierPolicy** cmdlet，这次反向使用分段。
...
在您将信息屏障策略设置为活动状态，然后应用这些策略之前，这些策略才会生效。
...
Contoso 有五个部门：HR、Sales、Marketing、Research 和 Manufacturing。 为了符合行业法规，某些部门中的人员不应与其他部门进行通信，如下表所示：

| Segment | 可以与用户交谈 | 无法与用户交谈 |
|:----------|:--------------|:-----------------|
| HR | 每个人 |  (无限制)  |
...
1. 一种旨在阻止销售部门与研究 (通信的策略，以及另一个阻止研究与销售) 。

2. 旨在允许制造部门仅与 HR 和市场营销部门进行通信的策略。

对于此方案，不需要为 HR 或 Marketing 定义策略。

### <a name="contosos-defined-segments"></a>Contoso 定义的线段
...
| Policy | 策略定义 |
|:---------|:--------------------|
| **策略 1：阻止销售与研究进行通信** | `New-InformationBarrierPolicy -Name "Sales-Research" -AssignedSegment "Sales" -SegmentsBlocked "Research" -State Inactive` <p> 本示例中，信息屏障策略称为 *Sales-Research。* 当此策略处于活动状态并应用时，它将有助于防止销售部门中的用户与"研究"部分的用户通信。 此策略是单向策略;它不会阻止 Research 与销售部门通信。 为此，需要策略 2。 |
| **策略 2：阻止研究与销售部门通信** | `New-InformationBarrierPolicy -Name "Research-Sales" -AssignedSegment "Research" -SegmentsBlocked "Sales" -State Inactive` <p> 本示例中，信息屏障策略名为 *Research-Sales。* 当此策略处于活动状态并应用时，它将有助于防止"研究"部分的用户与销售部门中的用户通信。 |
| **策略 3：仅允许制造与 HR 和市场营销部门进行通信** | `New-InformationBarrierPolicy -Name "Manufacturing-HRMarketing" -AssignedSegment "Manufacturing" -SegmentsAllowed "HR","Marketing","Manufacturing" -State Inactive` <p> 在这种情况下，信息屏障策略称为 *"制造-HRMarketing"。* 当此策略处于活动状态并应用时，制造部门只能与人力资源和市场营销部门进行通信。 人力资源和市场营销部不限制与其他部门进行通信。 |
