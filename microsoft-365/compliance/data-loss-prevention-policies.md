...
当规则匹配时，你可以向合规部主管 (或你选择 () 的任何) 发送一封包含警报详细信息的警报电子邮件。 此警报电子邮件将提供 [DLP 警报管理仪表板](dlp-configure-view-alerts-policies.md)的链接，法律人员可以使用该链接查看警报和事件的详细信息。 仪表板包含触发警报的事件的详细信息，以及匹配的 DLP 策略和检测到的敏感内容的详细信息。
