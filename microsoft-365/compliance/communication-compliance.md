...
ms.locfileid: "49613104"
---
# <a name="learn-about-communication-compliance-in-microsoft-365"></a>了解 Microsoft 365 中的通信合规性

通信合规性是 Microsoft 365 中的内幕风险解决方案，可帮助您检测、捕获和操作组织中不适当的邮件，从而帮助最大限度地减少通信风险。 通过预定义和自定义策略，可以扫描策略匹配的内部和外部通信，以便指定的审阅者可以对其进行检查。 审阅者可以调查组织中扫描的电子邮件、Microsoft 团队、Yammer 或第三方通信，并采取适当的措施以确保它们符合组织的邮件标准。
...
此外，IT 管理员与合规性管理团队之间可能存在责任分离。 通信合规性支持策略配置和邮件的调查和审阅之间的分离。 例如，您的组织的 IT 组可能负责设置通信合规性角色权限、组和策略，并且调查人员和审阅者可能负责邮件会审、审阅和缓解操作。
...
- **公司策略**
    ...
    用户必须遵守其所有与业务相关的通信中的可接受使用、道德标准和其他公司策略。 通信合规性策略可以检测策略匹配，并帮助您采取纠正措施来帮助缓解这些类型的事件。 例如，可以对组织中的用户通信进行扫描，以查找潜在的人为资源问题，如骚扰或使用不当或攻击性的语言。
...
- **模式检测到通知 (预览)**：许多 harassing 和威胁操作在一段时间后发生，并涉及到用户相同行为的重复实例。 警报详细信息中显示的检测到的新模式通知可帮助您注意这些警报和此类行为。
...
- **冒犯性或威胁语言**：使用此模板可以快速创建使用内置分类器的策略，以自动检测可能被视为滥用或攻击性的内容。
...
- **解决** 方法：查看问题后，可以通过解析警报进行修正。 解析警报会将其从待处理的警报队列中删除，并将操作保留为匹配策略的已解析队列中的条目。 在将警报标记为误报、向用户发送通知或打开警报的新事例之后，将自动解决通知。
...
- **监视和报告**：使用通信合规性仪表板小部件、导出日志和在统一审核日志中记录的事件，以不断评估和改进合规性状态。
