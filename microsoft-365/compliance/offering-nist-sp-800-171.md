...
美国国家标准和技术协会 (NIST) 促进并维护度量标准和准则，以帮助保护联邦机构的信息和信息系统。 为了响应执行官订单13556的 (管理 CUI) ，it 发布了 [NIST SP 800 – 171](https://csrc.nist.gov/publications/detail/sp/800-171/rev-1/final)，从而 *保护了 Nonfederal 信息系统和组织中的受控制的未分类信息*。 CUI 定义为由政府 (或代表其代表的实体创建的信息（数字和物理）) ，而不是保密的信息仍是敏感的，需要保护。
...
经资格鉴定的第三方评估组织、Kratos Secureinfo 和 Coalfire 与 Microsoft 合作，以证明其范围内的云服务符合 NIST SP 800 –171中的条件，在 *Nonfederal 信息系统和组织中保护受控制的未分类信息 () CUI 在*处理 CUI 时。 [Microsoft FedRAMP](offering-fedramp.md)要求的实现有助于确保 microsoft 范围内的云服务符合或超过 NIST SP 800 –171的要求，而这些系统和实践已准备就绪。
...
是。 Microsoft 客户可以使用来自独立第三方评估组织的报告中所述的审核的控件 (3PAO) 在 FedRAMP 标准中，作为其自己的 FedRAMP 和 NIST 风险分析和资格努力的一部分。 这些报告证明 Microsoft 已在其范围内的云服务中实现的控制措施的有效性。 客户负责确保其 CUI 工作负载符合 NIST SP 800 –171准则。
