...
> [!NOTE]
> 最近将数据丢失防护功能添加到 Microsoft 团队聊天和频道消息，以供用户许可的 Office 365 E5/A5、Microsoft 365 E5/A5、Microsoft 365 信息保护和治理或 Office 365 高级合规性。 Office 365 和 Microsoft 365 E3 包括适用于 SharePoint Online、OneDrive 和 Exchange Online 的 DLP 保护。 这还包括通过团队共享的文件，因为团队使用 SharePoint Online 和 OneDrive 共享文件。
...
最近， [数据丢失防护](data-loss-prevention-policies.md) (DLP) 功能扩展为包括 Microsoft 团队聊天和频道消息， **包括专用通道消息**。


如果你的组织拥有 DLP，你现在可以定义策略，以防止用户在 Microsoft 团队频道或聊天会话中共享敏感信息。 下面是有关此保护工作方式的一些示例：

- **示例1：保护邮件中的敏感信息**。 假定有人尝试在团队聊天或频道中共享与来宾 (外部用户) 相关的敏感信息。 如果您定义了一个 DLP 策略来防止出现这种情况，则会删除包含发送给外部用户的敏感信息的邮件。 这将根据您的 DLP 策略的配置方式在几秒内自动发生。
...
- **示例2：保护文档中的敏感信息**。 假定有人尝试与 Microsoft 团队频道或聊天中的来宾共享文档，并且文档包含敏感信息。 如果您定义了一个 DLP 策略来阻止这种情况，则不会为这些用户打开该文档。 请注意，在这种情况下，您的 DLP 策略必须包括 SharePoint 和 OneDrive，以便保护就绪。  (这是一种在 Microsoft 团队中显示的 DLP for SharePoint 的示例，因此要求用户获得 office 365 E3) 中包含的 Office 365 DLP (许可，但不要求用户授予 Office 365 高级合规性许可。 ) 
...
6. 在 " **策略设置** " 选项卡上的 " **自定义要保护的内容类型**" 下，保留默认的简单设置，或选择 " **使用高级设置**"，然后选择 " **下一步**"。 如果选择 "高级设置"，则可以为策略创建或编辑规则。  (若要获取有关此的帮助，请参阅 [简单设置与高级设置](data-loss-prevention-policies.md#simple-settings-vs-advanced-settings)。 ) 

7.  在 " **策略设置** " 选项卡上的 " **如果检测到敏感信息，您希望执行什么操作？**" 下，查看设置。 在这里，可以选择保留默认 [策略提示和电子邮件通知](use-notifications-and-policy-tips.md)，也可以对其进行自定义 (。 ) <br/>![具有提示和通知的 DLP 策略设置](../media/dlp-teams-policysettings-tipsemails.png)<br/>完成查看或编辑设置后，选择 " **下一步**"。
...
若要确保在默认情况下，外部来宾无法从 SharePoint 或团队访问包含敏感信息的 SharePoint 文档，请选择以下选项：

- 您可以确保在 DLP 扫描之前文档受到保护，并将其标记为可安全共享，方法是[默认情况下将新文件标记为 "敏感](https://docs.microsoft.com/sharepoint/sensitive-by-default)"
- 建议的 DLP 策略结构
    - **条件**
        - 内容包含以下任意敏感信息类型： [选择所有适用项]
        - 内容是通过 Microsoft 365 与我的组织外部的人员共享的
        <br/>![检查敏感内容的外部共享的 DLP 条件](../media/dlp-teams-external-sharing/external-condition.png)<br/>
...
在尝试与外部来宾共享包含敏感信息的 SharePoint 中的文档时，操作中的 DLP 策略：
<br/>![阻止了外部共享](../media/dlp-teams-external-sharing/external-sharing-blocked.png)<br/>


当来宾尝试在具有阻止外部的团队中打开文档时，操作中的 DLP 策略：
