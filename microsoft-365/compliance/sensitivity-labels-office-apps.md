...
对于每个功能，下表列出了使用内置标签支持敏感度标签所需的最低 Office 版本。 或者，如果标签功能在公共预览版中或正在针对未来版本进行审核。 使用 [Microsoft 365 路线图](https://aka.ms/MIPC/Roadmap) 了解有关未来版本的详细信息。
...
Azure 信息保护统一标签客户端支持 Open XML 格式和 Microsoft Office 97-2003 格式。 有关详细信息，请参阅该客户端的管理员指南中 [Azure 信息保护统一标记](https://docs.microsoft.com/azure/information-protection/rms-client/clientv2-admin-guide-file-types) 客户端支持的文件类型。
...
- 用户将"机密 **"\"** 仅收件人"标签应用于电子邮件，并且此电子邮件被配置为应用"不要 **转发"的加密设置**。 然后，此用户手动配置 IRM 设置，以便电子邮件不受限制。 最终结果是收件人可以转发电子邮件，尽管具有"机密 **"\"仅收件人"** 标签。
...
> [!NOTE]
> 如果用户从存储在 SharePoint 或 OneDrive 中的已标记文档手动删除加密，并且你已启用 SharePoint 和 [OneDrive](sensitivity-labels-sharepoint-onedrive-files.md)中 Office 文件的敏感度标签，则下次访问或下载文档时将自动还原标签加密。 
...
使用 Azure 信息保护客户端：可以使用 **Azure** 信息保护客户端查看和更改应用于使用 Office 内置标签客户端的文档和电子邮件的敏感度标签，也可以采用其他方式。
...
但是，由于复制延迟，此方案中不会立即创建自动来宾帐户。 如果你指定个人电子邮件地址作为标签加密设置的一部分，我们建议你在 Azure Active Directory 中创建相应的来宾帐户。 然后让这些用户知道他们必须使用此帐户从组织打开加密文档。
...
> [!NOTE]
> 为了完整，包括 Outlook 说明，尽管当前仅受 Azure 信息保护统一标签客户端支持。
