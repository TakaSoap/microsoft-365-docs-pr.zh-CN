...
- **字段名称** 和 **显示字段名称：** 元数据字段的名称，以及在审阅集中查看所选文档的文件元数据时显示的字段的名称。 查看文档的文件元数据时，不包含一些元数据字段。 这些字段以星号 ( * ) 突出显示。
...
> [!NOTE]
> [审核集搜索](https://docs.microsoft.com/microsoft-365/compliance/review-set-search)中的**关键字**字段使用关键字查询语言 (KQL) 。 可以在 "可 **搜索字段名称** " 列中列出的字段在审阅集搜索中的 " **关键字** " 字段中使用，以形成复杂查询，而无需使用查询生成器。 有关 KQL 的详细信息，请参阅 [关键字查询语言语法参考](https://go.microsoft.com/fwlink/?LinkId=269603)。
...
|其他保管人|DedupedCustodians|Deduped_custodians|作为电子邮件完全重复 (的文档的保管人列表（基于内容）;对于文档，基于哈希) 。|
|其他文件 Id|DedupedFileIds|Deduped_file_IDs|作为电子邮件完全重复 (的文档的文件 Id 列表，基于内容;对于文档，基于哈希) 。|
...
|EmailThread*||Email_thread|邮件在电子邮件集中的位置;由从根到当前邮件的节点 Id 组成，并由 ( ) 的句点分隔。|
...
|FamilyDuplicateSet*||Family_duplicate_set|与每个其他 (相同内容和所有相同附件) 的完全相同的系列的数字标识符。|
...
|文件系统创建日期||File_system_date_created|从文件系统中创建的日期 (仅适用于非 Office 365 数据) 。|
|修改了文件系统日期||File_system_date_modified|从文件系统中修改的日期 (仅适用于非 Office 365 数据) 。|
...
|是新式附件| IsModernAttachment|  |此文件是新式附件或链接的文件。|
|来自文档版本 | IsFromDocumentVersion |  |当前文档来自其他版本的另一文档。|
|是电子邮件附件 | IsEmailAttachment|  |此项来自显示为邮件附加项目的电子邮件附件。|
|内嵌附件| IsInlineAttachment|  |这是内嵌附加的，并显示在邮件正文中。|
...
|NativeMD5||Native_MD5|) 文件流的 MD5 哈希 (128 位哈希值。|
|NativeSHA256||Native_SHA_256|SHA256 哈希 (256-文件流的位哈希值) 。|
|ND/ET 排序：排除附件|NdEtSortExclAttach|ND_ET_sort_excl_attach|将电子邮件线程的连接 (ET) 集和接近重复 (ND) set。 此字段用于在审阅时进行有效排序。 **D**的前缀为 ND 集， **E**的前缀设置为 ET。|
|ND/ET 排序：包括附件|NdEtSortInclAttach|ND_ET_sort_incl_attach|将电子邮件线程的连接 (ET) 集和接近重复 (ND) set。 此字段用于在审阅时进行有效排序。 **D**的前缀为 ND 集， **E**的前缀设置为 ET。 ET 集内的每个电子邮件项的后面都有相应的附件。|
...
|收件人|收件人|Email_recipients|邮件的所有收件人的列表 (到、抄送、密件抄送) 。|
...
|发件人|发件人|Email_sender|发件人 (来自邮件类型) 字段。 格式为**DisplayName \<SmtpAddress> **。|
