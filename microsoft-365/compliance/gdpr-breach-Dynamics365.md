...
对于要声明的 CRSI，Microsoft 必须确定已经或很可能已经发生对客户数据的未经授权访问，和/或存在必须发出通知的法律或合同承诺。 需要知道特定的客户影响、资源访问和修复步骤，但这不是必需的。 事件通常在安全事件的诊断阶段结束后声明为 CRSI。 但是，声明可能会在所有相关信息可用的任何时候发生。 安全事件经理必须无可置疑地证明发生了可报告事件，以开始执行客户事件通知流程。
