---
title: '在 Microsoft 隐私管理和预览版 (和管理) '
f1.keywords:
- CSH
...
> [!NOTE]
> 在公共预览版期间，某些运行数据传输策略以检测跨区域的传输的租户可能会遇到同步问题，这些问题会影响对 Exchange 和 Teams 数据的可见性。 建议在预览此SharePoint时OneDrive关注和更改数据。
...
- **回收/删除**：使用此选项软删除数据。 内容将移动到已删除邮件文件夹或回收站 (Exchange、SharePoint、OneDrive) 或已删除，并可以选择恢复 (Teams邮件) 。 可以在一段设定时间内撤销删除操作，具体取决于服务的设置。
