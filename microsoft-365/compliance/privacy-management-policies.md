...
> [!NOTE]
> 在公共预览版期间，某些运行数据传输策略以检测跨区域的传输的租户可能会遇到同步问题，这些问题会影响对 Exchange 和 Teams 数据的可见性。 建议在预览此策略SharePoint重点介绍OneDrive数据。
...
- **结果**：定义检测到策略匹配时的结果。 你的选项取决于你开始的策略类型。 可能的结果包括：
  ...
  - **Teams：** 为用户提供Teams策略提示和建议，以及指向相关培训的链接。 此选项可用于数据传输策略。
...
通过将人员添加为协作者，你可以通过安全的 Microsoft Teams 频道、公司电子邮件或直接在隐私管理中共享问题页面的链接来与企业的其他成员共享问题。 这些选项在"共享"按钮 **下** 可用。 通过 Teams 进行共享时，将要求您从组织中可用的团队中选择，选择特定频道，并留下与指定频道共享的问题消息。
