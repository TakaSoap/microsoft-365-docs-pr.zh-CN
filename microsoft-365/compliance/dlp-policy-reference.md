...
ms.locfileid: "60201237"
---
# <a name="data-loss-prevention-policy-reference"></a>数据丢失防护策略参考

DLP 策略 (数据丢失) 有很多可配置的组件。 为了创建有效的策略，您需要了解每个组件的用途及其配置如何改变策略的行为。 本文提供了 DLP 策略的详细分析。
...
|金融| 美国格雷姆-格雷姆-巴雷法案 (GLBA) 增强版|- [信用卡号](sensitive-information-type-entity-definitions.md#credit-card-number) </br> - [美国银行帐号](sensitive-information-type-entity-definitions.md#us-bank-account-number)</br> - [ITIN 用户的个人纳税 (标识号) ](sensitive-information-type-entity-definitions.md#us-individual-taxpayer-identification-number-itin)  </br> - [SSN (美国社会保险) ](sensitive-information-type-entity-definitions.md#us-social-security-number-ssn)</br> - [美国/英国护照号码](sensitive-information-type-entity-definitions.md#us--uk-passport-number) </br> -[美国驾驶证号码](sensitive-information-type-entity-definitions.md#us-drivers-license-number)|
...
|隐私| 沙特阿拉伯个人身份 (PII) 数据|- [沙特阿拉伯国家/地区 ID](sensitive-information-type-entity-definitions.md#saudi-arabia-national-id)|
...
|隐私| 美国个人身份信息 (PII) 数据增强|- [ITIN 中美国个人纳税 (号) ](sensitive-information-type-entity-definitions.md#us-individual-taxpayer-identification-number-itin)  </br> - [SSN (美国社会保险) ](sensitive-information-type-entity-definitions.md#us-social-security-number-ssn)</br> - [美国/英国护照号码](sensitive-information-type-entity-definitions.md#us--uk-passport-number)|
...
DLP 策略通过将其与敏感信息类型（SIT (、) 或保留标签）相匹配来检测敏感项目。 每个位置都支持定义敏感内容的不同方法。 此外，在策略中组合位置时，定义内容方式可能会与单个位置定义内容不同。 
...
|位置|  内容可通过 SIT 定义|  内容可以定义敏感度标签|   内容可通过保留标签定义|
|---------|---------|---------|---------|
...
> [!NOTE]
> DLP 支持检测电子邮件和附加邮件的敏感度标签 请参阅将敏感度标签 [用作 DLP 策略中的条件](dlp-sensitivity-label-as-condition.md#use-sensitivity-labels-as-conditions-in-dlp-policies)
...
每个规则都按其创建顺序分配一个优先级，这意味着创建的规则首先具有第一个优先级，第二个创建的规则具有第二个优先级，依此类比。 
...
SIT 具有预定义的 [**可信度，**](https://www.microsoft.com/videoplayer/embed/RE4Hx60) 你可根据需要更改该级别。 有关详细信息，请参阅 [有关可信度的更多信息](sensitive-information-type-learn-about.md#more-on-confidence-levels)。 SIT 还有一个预定义的 SIT 发生范围，必须找到该范围以匹配规则，例如，如果将实例计数范围设置为从 1 到 9，则 SIT 必须至少发生一次且最多发生九次，规则要匹配。
...
location 支持的例外条件与所有包含条件都相同，唯一的区别是在每个受支持的条件前预置"Except if"。
...
通过包含 ***条件** _ 和独占例外筛选器 _**_ 的任何项都将具有在应用于 _**_ 它的规则中定义的任何操作。 您必须配置支持该操作所需的选项。 例如，如果您选择"Exchange限制访问"或 *加密*"位置 *"操作Microsoft 365您需要从以下选项中进行选择：
...
可以选择每次活动与规则匹配时发送警报（这非常干扰）或根据设定时段内匹配项数或项目量将事件聚合为较少的警报。
