...
> [!CAUTION]
> 本文中所述的过程将导致数据永久删除 (从 Exchange Online 邮箱中清除) 。 这意味着无法恢复从 "可恢复的项目" 文件夹中删除的邮件，也不会提供用于法律查询或其他合规性的邮件。 如果要从作为诉讼保留的一部分的邮箱中删除邮件，请在 "安全与合规中心" 中创建的 In-Place 保留、电子数据展示保留或保留策略，在删除保留项之前，请与您的记录管理或法律部门进行确认。 您的组织可能有定义邮箱处于保留状态或 data 外泄事件是否优先的策略。
...
- 您无法对已为其分配保留设置的邮箱执行此过程，该策略已通过使用保留锁定进行锁定。 这是因为，这是因为此锁定会阻止您从策略中删除或排除邮箱，也无法在邮箱上禁用托管文件夹助理。 有关锁定策略以进行保留的详细信息，请参阅 [使用保留锁定来限制对保留策略和保留标签策略的更改](retention-preservation-lock.md)。
...
在 Exchange Online PowerShell 中运行以下命令，以标识邮箱中放置的 In-Place 保留。 使用您在步骤1中标识的 In-Place 保留的 GUID。

```powershell
Get-MailboxSearch -InPlaceHoldIdentity <hold GUID> | FL Name
```

在确定 In-Place 保留之后，可以使用 Exchange 管理中心 (EAC) 或 Exchange Online PowerShell 将邮箱从保留中删除。 有关详细信息，请参阅 [创建或删除 In-Place 保留](https://go.microsoft.com/fwlink/?linkid=852668)。
...
4. 重新应用您在步骤3中删除的保留。 根据保留的类型，使用以下过程之一。
    ...
    **In-Place Hold**
    ...
    使用 EAC (或 Exchange Online PowerShell) 将邮箱重新添加到 In-Place 保留。
...
|就地保留  <br/> | `c0ba3ce811b6432a8751430937152491` <br/> |*InPlaceHolds* 属性包含邮箱中放置的 In-Place 保留的 GUID。 你可以告诉这是一个 In-Place 的保留，因为 GUID 不以前缀开头。  <br/> 您可以使用  `Get-MailboxSearch -InPlaceHoldIdentity <hold GUID> | FL` Exchange Online PowerShell 中的命令来获取有关邮箱上的 In-Place 保留的信息。  <br/> |
