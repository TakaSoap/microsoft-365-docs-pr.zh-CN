...
4. 重新应用您在步骤3中删除的保留。 根据保留的类型，使用以下过程之一。
    ...
    **应用于特定邮箱的 Office 365 保留策略**
    ...
    使用安全 & 合规性中心将邮箱重新添加到保留策略。 转到 "安全 & 合规性中心" 中的 "**日期调控** \> "**保留**页，编辑保留策略，并将邮箱重新添加到应用保留策略的收件人列表中。 
    
    **组织范围内的 Office 365 保留策略**
    
    如果通过从策略中排除组织范围或 Exchange 范围内的保留策略，则使用安全 & 合规性中心将邮箱从排除的用户列表中删除。 转到 "安全 & 合规性中心" 中的 "**日期调控** \> "**保留**页，编辑组织范围的保留策略，然后从排除的收件人列表中删除邮箱。 执行此操作将把保留策略重新应用到用户的邮箱。 
