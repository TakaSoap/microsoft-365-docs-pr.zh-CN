...
- 禁用 **单个项目** (恢复，以便从步骤 5 中的"可恢复的项目"文件夹中删除) 在已删除项目保留期) 期间不会保留项目。
...
在 PowerShell Exchange Online以下命令In-Place邮箱上设置的邮件保留。 使用在步骤 1 中In-Place标识的保留的 GUID。
...
组织范围内、Exchange和Teams保留策略将应用于组织的每一个邮箱。 它们在组织级别应用， (邮箱级别) 运行步骤 1 中的 **Get-OrganizationConfig** cmdlet 时返回。 在安全与合规& [PowerShell 中](/powershell/exchange/exchange-online-powershell) 运行以下命令，以标识组织范围内的保留策略。 使用 GUID (未) 步骤 1 中标识的组织范围的保留策略的前缀  `mbx` 值。
...
在安全与&合规中心 [PowerShell](/powershell/exchange/connect-to-scc-powershell) 中运行以下命令，以标识与电子数据展示 (称为电子数据展示保留) 应用于邮箱 *的* 保留。 使用 GUID (步骤 1 中) 标识的电子数据展示保留的前缀  `UniH` 名称。 第二个命令显示与保留关联的电子数据展示案例的名称;第三个命令显示保留的名称。
...
在 PowerShell (按指定顺序) 以下步骤Exchange Online步骤。
