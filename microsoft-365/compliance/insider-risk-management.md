...
此外，新的审核 [ (预览 ](insider-risk-management-audit-log.md)) ，让你随时了解对内部风险管理功能采取的操作。 此资源允许独立审阅分配给一个或多个内部风险管理角色组的用户所采取的操作。
...
在更严重的情况下，可能需要与组织的其他审阅者或服务共享内部风险管理案例信息。 内部风险管理与其他合规性Microsoft 365紧密集成，帮助你解决端到端风险。
...
## <a name="actions-and-behaviors-by-disgruntled-users-preview"></a>解除注册的用户通过预览模式 (和行为) 

雇佣者事件可能会以多种与内部风险相关的方式影响用户行为。 这些压力因素可能是性能考核不佳、职位降级或被放在绩效考核计划中的用户。 虽然大多数用户不会恶意响应这些事件，但是这些操作的压力可能会导致某些用户采取在正常情况下通常不会考虑的操作。 为了帮助标识这些类型的风险活动，以下内部风险管理策略模板使用 Microsoft 365 HR 连接器，并开始对与可能在雇佣压力事件附近发生的行为相关的评分风险指示器：
