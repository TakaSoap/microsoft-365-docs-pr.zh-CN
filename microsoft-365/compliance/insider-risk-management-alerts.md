...
内部风险管理警报由内部风险管理策略中定义的风险指示器自动生成。 这些警报为合规性分析师和研究人员提供当前风险状态的所有视图，并允许你的组织对发现的风险进行会审和采取措施。 默认情况下，策略会生成一定数量的低、中和高严重性警报，但你可以增加或减少警报量以满足你[](insider-risk-management-settings.md#alert-volume)的需求。 此外，可以使用策略向导 [创建新](insider-risk-management-settings.md#indicator-level-settings-preview) 策略时配置策略指示器的警报阈值。
...
>[!NOTE]
>内部风险管理使用内置的警报限制来帮助保护和优化风险调查和审查体验。 此限制可抵御可能导致策略警报过载的问题，如错误配置的数据连接器或 DLP 策略。 因此，为用户显示新警报可能会存在延迟。
...
警报风险评分从多个风险活动指示器自动计算。 这些指标包括风险活动的类型、活动发生的数量和频率、用户风险活动的历史记录，以及增加可能提高活动严重程度的活动风险。 警报风险分数将推动每个警报的风险严重性级别的编程分配，并且无法进行自定义。 如果警报保持未重试状态，并且风险活动继续计入警报，则风险严重性级别可能会增加。 风险分析师和研究人员可以使用警报风险严重性来帮助根据组织的风险策略和标准对警报进行会审。

警报风险严重性级别为：

- **高严重性**：警报的活动和指示器会带来高风险。 相关的风险活动是严重的、重复的，并且对于其他重大风险因素影响很大。
- **中等严重性**：警报的活动和指示器会带来中等风险。 关联的风险活动是中等、频繁的，并且与其他风险因素有一些关联。
- **低严重性**：警报的活动和指示器构成较小的风险。 关联的风险活动是次要的、不频繁发生，并且不会与其他重大风险因素相关。

## <a name="filter-alerts-on-the-alert-dashboard"></a>筛选警报仪表板上的警报

根据组织中活动的内部风险管理策略的数量和类型，查看大型警报队列可能充满挑战。 使用警报筛选器可帮助分析员和研究人员按多个属性对警报进行排序。 若要筛选警报仪表板上的 **警报，****请选择筛选器控件**。 可以按一个或多个属性筛选警报：
...
若要搜索特定单词的警报名称，请选择 **"搜索** "控件并键入要搜索的单词。 搜索结果显示包含搜索中定义的单词的任何策略警报。
