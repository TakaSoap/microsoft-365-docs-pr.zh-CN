...
- MOE150
- MET150
description: 了解如何在合规中心创建、修改、删除和测试自定义敏感信息类型。
...
1. 在合规中心，转到 **数据分类** \> **敏感信息类型** ，然后选择 **“创建敏感信息”类型**。
...
5. 选择并定义 **主要元素**。 主要元素可以是带有可选验证程序的 **正则表达式**、**关键字列表**、**关键字字典** 或预先配置的 **函数** 之一。 有关 DLP 函数的详细信息，请参阅 [敏感信息类型函数](sit-functions.md)。 有关日期和校验和验证器的详细信息，请参阅 [敏感信息类型正则表达式验证程序](sit-regex-validators-additional-checks.md#sensitive-information-type-regular-expression-validators)。

6. 填写 **字符领近度** 的值。

7. （可选）添加支持元素（如有）。 支持元素可以是带有可选验证程序的正则表达式、关键字列表、关键字字典或预定义的函数之一。 支持元素可以有自己的 **字符邻近** 性配置。
...
8. 选择并定义 **主要元素**。 主要元素可以是 **正则表达式**、**关键字列表**、**关键字字典** 或预先配置的 **函数** 之一。 请参阅 [“敏感信息类型”函数](sit-functions.md)。

9. 填写 **字符领近度** 的值。

10.  (可选) 如果你有 **支持元素** 或任何其他 [**检查**](sit-regex-validators-additional-checks.md#sensitive-information-type-additional-checks) 添加它们。 如果需要，可以将你的 **支持元素** 分组。
...
为了确保高性能和更低延迟，自定义 SIT 配置存在限制。

|限制|值|
|---|---|
|通过合规中心创建的自定义 SIT 的最大数目| 500 |
|正则表达式的最大长度| 1024 个字符|
|关键字列表中给定术语的最大长度| 50 个字符|
|关键字列表中的最大术语数| 2048|
|每个敏感信息类型的非重复正则表达式的最大数目| 20|
|压缩后 (关键字字典的最大大小) | 1MB (~1，000，000 个字符) |
|租户中基于关键字词典的 SIT 的最大数目|50 |

> [!NOTE]
> 如果企业需要创建 500 多个自定义 SIT，请提出支持票证。

### <a name="instance-count-supported-values-for-sit"></a>SIT 的实例计数支持的值

在这些解决方案中使用 SIT 时，会应用 SIT 实例计数限制：
...
若要使扫描的项满足规则条件，任何单个项目中 SIT 的唯一实例数必须介于最小值和最大值之间。 这称为 **实例计数**。

- **最小** 字段：下限 (要触发匹配的项中必须找到的 SIT 唯一实例的最小) 数。 最小字段支持以下值：
  - 1 到 500
- **最大** 字段：可在项中找到并仍触发匹配项的 SIT 唯一实例数上限。 最大字段支持以下值：
  - 1 到 500 - 如果要设置项目中 SIT 实例数的 500 或更少的特定上限，请使用此值。
  - Any - 当在扫描的项中找到未定义的 SIT 唯一实例数，并且唯一实例数满足或超过最小唯一实例值数时，请使用 `Any` 唯一实例计数条件。 换句话说，只要满足最小值，就满足唯一实例计数条件。

例如，如果希望规则在单个项目中找到至少 500 个唯一 SIT 实例时触发匹配，请将 **最小** 值设置为`500`**最大** 值`Any`。
