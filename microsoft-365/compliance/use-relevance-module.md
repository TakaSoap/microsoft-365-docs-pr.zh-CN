...
在数据调查中 (预览) ，相关性模块包括与调查相关的相关文件的相关性培训和审核。 相关性工作流如下所示，如下所示：
...
- **培训和跟踪周期**
  ...
  - **标记**：数据调查 (preview) 根据专家对各个文件的迭代审核和标记学习特定于每个问题的相关性条件。
    
  - **跟踪**：计算并显示相关培训的中间结果，同时监视流程的统计有效性。 
    
- **批处理计算**：累积的和已知的相关性条件应用于整个文件集合，并为每个文件生成相关性分数。
    
- **决定**：应用于整个事例的分析结果在批量计算后显示，并显示用于进行文档审阅决定的数据。
...
- **标记和培训**： 
  ...
  - 仅在必要时使用 **跳过标签** 选项。 数据调查 (预览) 不基于跳过的文件进行定型。 在评估中，如果很难判断文件是否相关，最好在可能的情况下标记为相关 (R) 或不相关 (NR) ，而不是选择 " **跳过**"。 当数据调查 (预览) 评估培训时，可以看到这些类型的文件的处理方式。
    
  - 即使具有少量提取文本的文件，也应在训练中将其标记为 R/NR，而不是 "Skip" （如果可能）。 
...
- **问题和采样模式**：在完成工作时，将关闭问题 (相关性培训为 "稳定" 和 "批量计算" 执行) 、"取消" 或 "当其他用户正在处理问题"。
...
- 批量计算：相关性过程，其中的数据调查采用培训阶段获取的知识，并将其应用于整个文件填充。 将评估相关文件组中的所有文件的相关性，并为其分配相关性分数。
...
尽管数据调查引导您完成整个过程，但在不同的阶段使用建议的后续步骤，它还允许您在选项卡和页面之间导航，并做出选择，以解决与您的个人案例、问题或文档审阅过程相关的情况。 
