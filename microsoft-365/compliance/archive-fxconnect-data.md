...
使用 Microsoft 365 合规中心中的一个 Microsoft 365 连接器，将数据从 FX Connect 协作平台导入并存档到 Microsoft 365 组织的用户邮箱。 Microsoft 提供配置为捕获 FX Connect 项目以及将这些项目导入到 Microsoft 365 的 [FX Connect](https://globanet.com/fx-connect/) 连接器。 连接器将内容从 FX Connect（如会议、邮件和您组织的 FX Connect 帐户的其他详细信息）转换为电子邮件格式，然后将这些项目导入到 Microsoft 365 中的用户邮箱。
...
2. 每 24 小时一次，来自 FX Connect 帐户的项将复制到"改进合并 1"网站。 连接器还会将 FX Connect 项目转换为电子邮件格式。

3. 你在 Microsoft 365 合规中心创建的 FX Connect 连接器，每天连接到 Microsoft Merge1 网站，将 FX Connect 项目转移到 Microsoft 云中安全的 Azure 存储位置。
...
## <a name="step-2-configure-the-fx-connect-connector-on-the-veritas-merge1-site"></a>步骤 2：在"完成"合并 1 网站中配置 FX Connect 连接器
