...
使用 Microsoft 365 合规中心中的 Globanet 连接器，将数据从 FX Connect 协作平台导入并存档到 Microsoft 365 组织的用户邮箱。 Globanet 提供 [配置为](https://globanet.com/fx-connect/) 捕获 FX Connect 项目以及将这些项目导入到 Microsoft 365 的 FX Connect 连接器。 连接器将 FX Connect 中的内容（如费用、邮件和其他详细信息）从组织的 FX Connect 帐户转换为电子邮件格式，然后将这些项目导入到 Microsoft 365 中的用户邮箱。

在将 FX Connect 数据存储在用户邮箱中后，可以应用 Microsoft 365 合规性功能，如诉讼保留、电子数据展示、保留策略和保留标签以及通信合规性。 使用 FX Connect 连接器在 Microsoft 365 中导入和存档数据可帮助组织遵守政府法规策略。
...
2. 每 24 小时一次，FX Connect 帐户的项目将复制到 Globanet Merge1 网站。 连接器还会将 FX Connect 项目转换为电子邮件格式。

3. 在 Microsoft 365 合规中心创建的 FX Connect 连接器每天连接到 Globanet Merge1 网站，将 FX Connect 项目转移到 Microsoft 云中安全的 Azure 存储位置。

4. 连接器使用自动用户映射的 *Email* 属性值将项目导入特定用户的邮箱，如步骤 [3 中所述](#step-3-map-users-and-complete-the-connector-setup)。 在用户邮箱中创建名为 **FX Connect** 的收件箱文件夹中的子文件夹，项目将导入到该文件夹中。 连接器通过使用 Email *属性的值来实现* 此操作。 每个 FX Connect 项都包含此属性，此属性用项目每个参与者的电子邮件地址填充。
...
- 在步骤 1 中创建 FX Connect 连接器 (步骤 3) 必须分配给 Exchange Online 中的邮箱导入导出角色。 在 Microsoft 365 合规中心的"数据连接器"页上添加连接器需要此角色。 默认情况下，此角色不会分配给 Exchange Online 中的角色组。 可以将邮箱导入导出角色添加到 Exchange Online 中的组织管理角色组。 也可以创建一个角色组，分配邮箱导入导出角色，然后将相应的用户添加为成员。 有关详细信息，请参阅"在[](https://docs.microsoft.com/Exchange/permissions-exo/role-groups#create-role-groups)Exchange Online[](https://docs.microsoft.com/Exchange/permissions-exo/role-groups#modify-role-groups)中管理角色组"一文的"创建角色组或修改角色组"部分。

## <a name="step-1-set-up-the-fx-connect-connector"></a>步骤 1：设置 FX Connect 连接器

第一步是访问 Microsoft 365 合规中心中的"数据连接器"页，并创建用于 FX Connect 数据的连接器。 
...
第二步是在 Merge1 站点上配置 FX 连接连接器。 若要了解如何配置 FX Connect 连接器，请参阅 [Merge1 第三方连接器用户指南](https://docs.ms.merge1.globanetportal.com/Merge1%20Third-Party%20Connectors%20FX%20Connect%20User%20Guide%20.pdf)。
...
1. 在 **"将 FX Connect 用户映射到 Microsoft 365** 用户"页上，启用自动用户映射。 FX Connect 项目包括一个称为 *"电子邮件*"的属性，该属性包含组织中用户的电子邮件地址。 如果连接器可以将此地址与 Microsoft 365 用户关联，则项目将导入该用户的邮箱。
...
2. 单击 **"连接器"** 选项卡，然后选择 **FX Connect** 连接器以显示飞出页。 此页面包含有关连接器的属性和信息。
