...
组织通常受法规遵从性、法律或其他法规要求，这些法规要求他们在特定时间段内保留文档。 不过，如果文档的保留时间长于要求时间，则会给组织带来法律风险。 出于此原因，您的组织可能已为网站&mdash;创建了文档删除策略。例如，可能需要在创建一般业务文档五年后再将其删除。
...
- **默认** 默认策略自动应用于网站，但网站所有者可以： 
  ...
  - 如果该策略与网站中的内容不相关，则将其完全退出。
    
- **既不强制也不是默认** 在这种情况下，没有策略会自动应用于网站，并且网站所有者需要执行操作来应用一个策略。 
    
文档删除策略可能包含多个规则&mdash;例如，一个规则可能会在创建文档一年后将其删除一年，但另一条规则可能会在最后一次修改文档后一年的时间内指明删除文档。 如果策略包含多个规则，则可以选择最适用于您的网站的规则。 删除规则将应用于网站中的所有库。 网站中同时只能有一个策略和一个规则处于活动状态。 与策略一样，可以将规则设置为默认值，以便在应用策略时自动应用该规则。
...
这表示您应该对网站进行规划，以仅使用针对结构化内容（信息管理策略和内容类型策略）或非结构化内容（文件删除策略）的策略，而非同时使用。 如果您选择退出文档删除策略，则不会显示该警告，并且其他类型的策略将继续有效。
