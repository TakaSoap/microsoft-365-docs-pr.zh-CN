...
1. 转到<a href="https://go.microsoft.com/fwlink/p/?linkid=2077149" target="_blank">Microsoft 365 合规中心，</a>然后使用组织中管理员帐户的凭据登录。
...
5. 根据是否要剪裁导入到数据Microsoft 365，在"是否要筛选数据 **？"** 下，执行下列操作之一：
    ...
    "**将数据导入Office 365"** 页面将显示，并包含来自所执行分析的详细Microsoft 365见解。 
    ...
    可以配置以下筛选器：
      ...
      - **用户** - 可以排除特定人员发送或接收的邮件。 若要排除出现在"发件人："字段、"收件人："字段或邮件的"抄送："字段中的用户，请单击该 **收件人类型** 旁边的"排除用户"。 键入用户 (SMTP) 电子邮件地址，单击"**添加新** ![ 图标"。](../media/457cd93f-22c2-4571-9f83-1b129bcfb58e.gif) 以将其添加到该收件人类型的已排除用户列表中，然后单击"保存"以保存已排除用户的列表。 
...
- 下表列出了"更多选项"飞出页面 (请参阅上一过程中的步骤5b中"类型"筛选器中的"其他"类别中包含) 。  目前，在将 PST 导入到"其他"类别时，无法排除Office 365。 
