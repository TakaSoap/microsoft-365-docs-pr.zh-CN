...
您可以导出在导出搜索结果时生成的相同报告，而不是从安全性 & 合规性中心（和从与电子数据展示事例关联的内容搜索）中导出完整的搜索结果集。
...
- 导出报表时，数据将临时存储在 Microsoft 云中的唯一 Azure 存储区域中，然后再将其下载到本地计算机。 确保您的组织可以连接到 Azure 中的终结点， ** \*即 blob.core.windows.net** （通配符代表导出的唯一标识符）。 搜索结果数据在创建后两周从 Azure 存储区域中删除。 
