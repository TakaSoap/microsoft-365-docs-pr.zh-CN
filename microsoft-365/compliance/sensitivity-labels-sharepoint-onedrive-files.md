...
> [!NOTE]
> 当前存在一个问题，导致某些已标记和加密的文件在 Office 网页中无法打开：
>
> 当我们调查与特定文档属性相关的问题时，你将无法在 Office 网页中打开许多文件。 对于这些文件，你可以继续在桌面和移动 Office 应用中打开和编辑它们。 或者，执行以下操作：
>
> 1. 在 Office 桌面应用程序中打开文件。
> 2. 删除应用加密的标签。
> 3. 将文件保存在 SharePoint 或 OneDrive (的原始位置) ，然后关闭桌面应用。
> 4. 在 Office 网页版中打开文件，然后重新应用应用加密的原始标签。
> 
> 仅在 Office 网页中标记的文件不受影响。
...
2. 或者，如果从 Microsoft 下载中心安装了以前版本的 SharePoint Online 命令行管理程序，还可以转到添加或删除程序，然后卸载SharePoint Online 命令行管理程序。

3. 在 Web 浏览器中，转到“下载中心”页面，[下载最新的 SharePoint Online 命令行管理程序](https://go.microsoft.com/fwlink/p/?LinkId=255251)。

4. 选择语言，然后单击“**下载**”。

5. 在 x64 和 x86.msi 文件之间进行选择。 如果运行 64 位版本的 Windows，请下载 x64 文件;如果运行 32 位版本，则下载 x86 文件。 如果不知道，请参阅我Windows[操作系统的版本？](https://support.microsoft.com/help/13443/windows-which-operating-system)
...
例如：创建并发布应用加密的新敏感度标签，它很快就会显示在用户的桌面应用中。 用户将此标签应用于文档，然后将它上载到SharePoint或OneDrive。 如果尚未为服务完成标签复制，则上载时不会将新功能应用于该文档。 因此，不会在搜索或电子数据展示中返回文档，并且无法通过 Web Office打开文档。  

以下更改在一小时内复制：新的和已删除的敏感度标签，以及包含策略中哪些标签的敏感度标签策略设置。

以下更改在 24 小时内复制：更改现有标签的敏感度标签设置。

由于新敏感度标签的复制延迟仅为 1 小时，因此您不太可能遇到示例中的方案。 但作为一项安全措施，我们建议首先向一些测试用户发布新标签，等待一小时，然后验证 SharePoint 和 OneDrive。 作为最后一步，通过向现有标签策略添加更多用户或将标签添加到标准用户的现有标签策略，使标签可供更多用户使用。 当标准用户看到标签时，已同步到SharePoint OneDrive。

## <a name="sharepoint-information-rights-management-irm-and-sensitivity-labels"></a>SharePoint信息权限管理 (IRM) 和敏感度标签

[SharePoint信息](set-up-irm-in-sp-admin-center.md)权限管理 (IRM) 是一项较旧的技术，通过下载文件时应用加密和限制来保护列表和库级别的文件。 此较旧的保护技术旨在防止未经授权的用户在文件位于外部或SharePoint。
...
- 如果使用 Web Office标记文件，将强制执行该标签的任何加密设置。 对于这些文件，支持共同授权、电子数据展示、DLP 和搜索。

- 如果下载使用 Office 网页标签的文件，将保留标签，并强制执行标签中任何加密设置，而不是 IRM 限制设置。
...
有关使用托管属性的信息，请参阅管理 SharePoint 中的[搜索架构](/sharepoint/manage-search-schema)。
...
若要禁用这些新功能，必须使用 PowerShell。 使用 SharePoint 命令行管理程序和 [Set-SPOTenant](/powershell/module/sharepoint-online/set-spotenant) cmdlet，指定与使用 [PowerShell](#use-powershell-to-enable-support-for-sensitivity-labels)启用敏感度标签支持部分中所述的相同的 *EnableAIPIntegration* 参数。 但这次，将参数值设置为 false，然后按 **Y** 确认：
