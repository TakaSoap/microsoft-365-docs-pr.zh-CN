...
- SharePoint和 OneDrive不会自动将敏感度标签应用于已使用 Azure 信息保护标签加密的现有文件。 相反，为了让这些功能在启用 Office 和 OneDrive 中的Office标签后SharePoint，请完成以下任务：
    ...
    1. 确保你已[将 Azure 信息](/azure/information-protection/configure-policy-migrate-labels)保护标签迁移到敏感度标签，然后从[](create-sensitivity-labels.md#publish-sensitivity-labels-by-creating-a-label-policy)Microsoft 365中心或等效标签管理中心发布它们。
    2. 下载文件，然后将这些文件上载到SharePoint。
