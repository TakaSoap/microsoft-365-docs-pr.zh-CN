...
>
> SharePoint 中的现有文件也不会更改使用单个基于 Azure 的密钥以加密标记的 SharePoint 中的现有文件。 若要在为 SharePoint 和 OneDrive 中的 Office 文件启用敏感度标签后从新功能中获益，这些文件必须下载并再次上载或编辑。 例如，它们将在搜索和电子数据展示结果中返回。
...
您始终可以随时在 SharePoint 和 OneDrive 中禁用 Office 文件的敏感度标签， ([自愿退出](#how-to-disable-sensitivity-labels-for-sharepoint-and-onedrive-opt-out) 。
