...
保护静态和传输中的个人数据，检测并响应数据泄露，并推动安全措施的定期测试。这些配置是建立在之前工作基础上的重要安全措施。  
