...
当进度栏显示稳定培训状态、工具提示中的选中标记和稳定状态时，批计算将在成功完成 (后执行。) 批处理计算将相关性培训期间获取的知识应用于整个文件填充，以评估文件的相关性并分配相关性分数。
...
关键字是文件中唯一的字符串、字词、短语或字词序列，Advanced eDiscovery标识为文件是否相关的重要指标。 标记为"相关"的文件的"包含"列列表关键字和权重，"排除"列列出了标记为"不相关"的文件的关键字和权重。
...
"**培训统计信息**"窗格根据相关性Advanced eDiscovery显示统计信息和图形。 
