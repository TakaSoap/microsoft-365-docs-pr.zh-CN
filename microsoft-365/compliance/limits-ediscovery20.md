...
| 限制说明 | 限制 |
|:-----|:-----|
|可添加到案例审阅记录的文档总数 (审阅集的所有审阅) 。  <br/> |3 百万 <br/> |
...
| 限制说明 | 限制 |
  |:-----|:-----|
  |从单个文件中提取的最大字符数。  <br/> |1，000<sup>万 1</sup> <br/> |
...
>
> <sup>3</sup> 对于非短语查询 (不使用双引号的关键字值，) 特殊前缀索引。 这告诉我们一个单词在文档中出现，而不是出现在文档中。 若要使用双引号 (关键字值执行短语查询) ，我们需要比较短语中单词在文档中的位置。 这意味着不能将前缀索引用于短语查询。 在这种情况下，我们使用前缀扩展到的所有可能的单词在内部扩展查询;例如 **，time \* *_ 可以展开到 _*"time OR timer OR times OR timex OR timeboxed OR ..."。** 限制为 10，000 是单词可以扩展到的最大变体数，而不是与查询匹配的文档数。 非短语搜索词没有上限。
>
