...
ms.custom:
- seo-marvel-apr2020
description: 了解适用于 Microsoft 365 中解决方案案例限制、索引编制限制和Advanced eDiscovery搜索Microsoft 365。
ms.openlocfilehash: c07d53766a0542ebf9fd7753a211a305515e8444
ms.sourcegitcommit: df1ad7118c4a95a310a4f17124322a6ae6ace26f
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/11/2021
ms.locfileid: "60268678"
---
...
下表列出了事例和审阅集在Advanced eDiscovery。
...
|每个案例的唯一标记的最大数量。 <br/> |1，000<sup>6</sup> |
...
|单个案例保留中的最大邮箱数。 此限制包括用户邮箱总数，以及与组、组Microsoft 365组Microsoft Teams邮箱Yammer邮箱。 <br/> |1,000  <br/> |
|单个案例保留中的最大网站数。 此限制包括 OneDrive for Business 组、SharePoint 网站以及与 Microsoft 365 组、Microsoft Teams 和 Yammer 组关联的网站总数。  <br/> |100  <br/> |
...
|可以同时运行的最大搜索数。 |无限制 |
|单个用户可以同时启动的最大搜索数。 |10  | 
|搜索查询查询记录的最大字符数 (运算符和条件) 。 |10,000<sup>2</sup>|
...
|可针对SharePoint OneDrive for Business预览的最大网站数和最大网站数。 如果包含与搜索查询匹配的项目的网站超过 200 个，则仅具有最多结果的前 200 个网站可供预览。|200|
...
>
> <sup>3</sup> 对于非短语 (使用不使用双引号的关键字值，) 特殊前缀索引。 这将告诉我们一个单词出现在文档中，而不是它出现在文档中。 若要使用双引号 (查询短语) ，我们需要比较短语中单词在文档中的位置。 这意味着不能将前缀索引用于短语查询。 在这种情况下，我们使用前缀扩展到的所有可能的单词在内部扩展查询;例如 **，time \* *_ 可以展开到 _*"time OR timer OR times OR timex OR timeboxed OR ..."。** 限制 10，000 是单词可以扩展到的最大变体数，而不是与查询匹配的文档数。 非短语搜索词没有上限。
>
> <sup>4</sup> 此限制与其他电子数据展示工具中的内容导出共享。 这意味着内容搜索和核心电子数据展示中的并发导出 (以及将内容添加到内容审阅集Advanced eDiscovery) 都针对此限制应用。
>
> <sup>5</sup> 此限制适用于从审阅集下载所选文档。 它不适用于从审阅集导出文档。 有关下载和导出文档的信息[，请参阅导出](exporting-data-ediscover20.md)文档Advanced eDiscovery。
>
> <sup>6</sup> 这是一种情况下可以创建的最大标记数。 此限制与可标记的文档数不相关。
>
