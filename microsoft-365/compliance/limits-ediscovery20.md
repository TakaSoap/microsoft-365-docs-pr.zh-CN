...
下表列出了针对事例和审阅集的限制，Advanced eDiscovery。

| 限制说明 | 限制 |
|:-----|:-----|
|可以添加到案例的文档总数， (审阅集的所有审阅) 。  <br/> |3 百万 <br/> |
...
本节中所述的限制与使用"搜索"选项卡上的搜索工具收集案件集数据相关。 有关详细信息，请参阅在 Advanced eDiscovery[中收集案例Advanced eDiscovery。](collecting-data-for-ediscovery.md)
...
|可SharePoint搜索结果OneDrive for Business的最大网站数。 如果包含与搜索查询匹配的项目的网站超过 200 个，则只有结果最多的前 200 个网站可供预览。|200|
...
>   例如，包括搜索SharePoint网站和 OneDrive 帐户将计为六个字符，因为 SharePointLocation 和 OneDriveLocation 字段都将显示"ALL"一词。
>
> <sup>3</sup> 对于非短语 (使用不使用双引号的关键字) 我们使用特殊前缀索引。 这将告诉我们一个单词出现在文档中，而不是它出现在文档中。 若要将短语查询 (双引号) 关键字值，我们需要比较短语中单词在文档中的位置。 这意味着不能将前缀索引用于短语查询。 在这种情况下，我们使用前缀扩展到的所有可能的单词在内部扩展查询;例如 **，time \* *_ 可以展开到 _*"time OR timer OR times OR timex OR timeboxed OR ..."。** 限制 10，000 是单词可以扩展到的最大变体数，而不是与查询匹配的文档数。 非短语搜索词没有上限。
>
> <sup>4</sup> 此限制与其他电子数据展示工具中的内容导出共享。 这意味着，内容搜索和核心电子数据展示中的并发导出 (以及向内容检索中审阅Advanced eDiscovery) 都针对此限制应用。
>
> <sup>5</sup> 此限制适用于从审阅集下载所选文档。 它不适用于从审阅集导出文档。 有关下载和导出文档的信息，请参阅导出文档中[的](exporting-data-ediscover20.md)Advanced eDiscovery。
>
