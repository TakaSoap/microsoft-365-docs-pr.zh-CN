...
ms.custom:
- seo-marvel-apr2020
description: 了解适用于 Microsoft 365 中 Advanced eDiscovery 解决方案的情况限制、索引编制限制和Microsoft 365。
...
本节中所述的限制与使用"搜索"选项卡上的搜索工具收集案件集数据相关。 有关详细信息，请参阅在 Advanced eDiscovery 中收集[案例Advanced eDiscovery。](collecting-data-for-ediscovery.md)
...
|网站和网站搜索查询的最大字符数SharePoint OneDrive for Business， (运算符和条件) 。 |10,000<br>4，000（带通配符 &nbsp; <sup>2）</sup>|
...
>
> <sup>3</sup> 对于非短语 (使用不使用双引号的关键字) 我们使用了特殊的前缀索引。 这将告诉我们一个单词出现在文档中，而不是它出现在文档中。 若要使用双引号 (关键字值执行短语查询) ，我们需要比较短语中单词在文档中的位置。 这意味着不能将前缀索引用于短语查询。 在这种情况下，我们使用前缀扩展到的所有可能的单词在内部扩展查询;例如 **，time \* *_ 可以展开到 _*"time OR timer OR times OR timex OR timeboxed OR ..."。** 限制 10，000 是单词可以扩展到的最大变体数，而不是与查询匹配的文档数。 非短语搜索词没有上限。
>
> <sup>4</sup> 此限制与其他电子数据展示工具中的内容导出共享。 这意味着，内容搜索和核心电子数据展示中的并发导出 (以及添加要审阅的内容Advanced eDiscovery) 都针对此限制应用。
>
