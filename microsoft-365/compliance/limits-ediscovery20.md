...
ms.custom:
- seo-marvel-apr2020
description: 了解适用于 Microsoft 365 中 Advanced eDiscovery 解决方案的情况限制、索引编制限制和搜索Microsoft 365。
ms.openlocfilehash: e82b7d44a5733c245b00a1890b9d07dd95c679e2
ms.sourcegitcommit: 88c3b9758214936d283bad0321b826fb40a2e7e9
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/04/2021
ms.locfileid: "60088225"
---
# <a name="limits-in-advanced-ediscovery"></a>高级电子数据展示中的限制

本文介绍 Advanced eDiscovery 解决方案中的Microsoft 365。
...
| 限制说明 | 限制 |
|:-----|:-----|
|组织的最大案例保留数。 此限制包括核心电子数据展示和自动发现事例中保留Advanced eDiscovery总数。 <br/> |10,000  <br/> |
|单个案例保留中的最大邮箱数。 此限制包括用户邮箱总数，以及与组组、Microsoft 365 组Microsoft Teams组Yammer邮箱。 <br/> |1,000  <br/> |
...
本节中所述的限制与使用"搜索"选项卡上的搜索工具收集案件集数据相关。 有关详细信息，请参阅在 Advanced eDiscovery 中[收集案例Advanced eDiscovery。](collecting-data-for-ediscovery.md)
...
|网站和网站搜索查询的最大字符数SharePoint OneDrive for Business包括 (运算符和) 。 |10,000<br>4，000（带通配符 &nbsp; <sup>2）</sup>|
...
|预览页面上显示用于搜索SharePoint OneDrive for Business网站的最大项目数。 显示最新项目。 |200|
|可预览SharePoint搜索结果OneDrive for Business网站的最大网站数。 如果包含与搜索查询匹配的项目的网站超过 200 个，则只有结果最多的前 200 个网站可供预览。|200|
...
>   例如，包括搜索SharePoint网站和 OneDrive 帐户都将计为六个字符，因为 SharePointLocation 和 OneDriveLocation 字段都将显示"ALL"一词。
>
> <sup>3</sup> 对于非短语 (使用不使用双引号的关键字) 我们使用了特殊的前缀索引。 这将告诉我们一个单词出现在文档中，而不是它出现在文档中。 若要对包含双引号 (关键字值执行短语查询) ，我们需要比较短语中单词在文档中的位置。 这意味着不能将前缀索引用于短语查询。 在这种情况下，我们使用前缀扩展到的所有可能的单词在内部扩展查询;例如 **，time \* *_ 可以展开到 _*"time OR timer OR times OR timex OR timeboxed OR ..."。** 限制 10，000 是单词可以扩展到的最大变体数，而不是与查询匹配的文档数。 非短语搜索词没有上限。
>
> <sup>4</sup> 此限制与其他电子数据展示工具中的内容导出共享。 这意味着，内容搜索和核心电子数据展示中的并发导出 (以及将内容添加到 Advanced eDiscovery) 审阅集均针对此限制应用。
>
> <sup>5</sup> 此限制适用于从审阅集下载所选文档。 它不适用于从审阅集导出文档。 有关下载和导出文档的信息[，请参阅导出](exporting-data-ediscover20.md)文档Advanced eDiscovery。
>
