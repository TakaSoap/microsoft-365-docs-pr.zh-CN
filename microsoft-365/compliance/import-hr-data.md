...
4. 在 " **设置连接"** 页面上，执行以下操作，然后单击 " **下一步** "：
   ...
   1. 键入或粘贴您在步骤2中创建的 Azure 应用程序的 Azure AD 应用程序 ID。
...
8. 在 " **检查** " 页上，查看您的设置，然后单击 " **完成** " 以创建连接器。
   ...
   1. **作业 ID。** 在下一步中，你将需要此作业 ID 来运行脚本。 您可以从此页面或从 "连接线" 飞出页面复制它。

   1. **指向示例脚本的链接。** 单击 **此处** 链接转到 GitHub 站点以访问示例脚本 (链接将) 中打开新的窗口。 保持此窗口处于打开状态，以便您可以在第4步中复制脚本。 或者，也可以将目标做成书签或复制 URL，以便在运行脚本时可以再次访问它。 此链接在 "连接线" 弹出页面上也可用。
...
2. 单击 " **连接器** " 选项卡，然后选择 HR 连接器以显示弹出页面。 此页面包含有关连接器的属性和信息。
...
7. 选择 " **操作** " 选项卡，单击 " **新建** "，然后执行以下操作：
   ...
   1. 在 " **程序/脚本** " 框中，单击 " **浏览** "，然后转到以下位置并选择它，以使路径显示在框中： `C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe` 。

   1. 在 " **添加参数" (可选)** 框中，粘贴您在步骤4中运行的相同脚本命令。 例如，`.\HRConnector.ps1 -tenantId "d5723623-11cf-4e2e-b5a5-01d1506273g9" -appId "c12823b7-b55a-4989-faba-02de41bb97c3" -appSecret "MNubVGbcQDkGCnn"  -jobId "e081f4f4-3831-48d6-7bb3-fcfab1581458" -csvFilePath "C:\Users\contosoadmin\Desktop\Data\employee_termination_data.csv"`

   1. 在 " **开始 (可选)** " 框中，粘贴您在步骤4中运行的脚本的文件夹位置。 例如，`C:\Users\contosoadmin\Desktop\Scripts`。
