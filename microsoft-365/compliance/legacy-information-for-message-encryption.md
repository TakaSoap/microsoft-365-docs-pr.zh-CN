...
当某人在 Exchange Online 中发送与加密规则匹配的电子邮件时，该邮件将发送 HTML 附件。 收件人打开 HTML 附件，然后按照说明在 Office 365 邮件加密门户上查看加密邮件。 收件人可以选择通过使用 Microsoft 帐户或与 Office 365 关联的工作或学校登录，或者使用一次通过代码查看邮件。 两个选项均可以确保只有目标收件人可以查看加密邮件。 对于新的 OME 功能，此过程非常不同。
...
8. 若要启用加密而不使用新的 OME 功能，请在"执行以下操作"中，选择"修改邮件安全性 \> **应用以前版本的 OME"，** 然后选择"**保存"。**
   ...
   如果收到 IRM 许可未启用的错误，则表明你未使用旧版 OME。
...
2. 使用 **New-TransportRule** cmdlet 创建规则，将 _ApplyOME_ 参数设置为 `$true` 。
   ...
   本示例要求发送到此邮箱的所有DrToniRamos@hotmail.com必须加密。
...
您的电子邮件用户发送加密邮件后，这些邮件的收件人可以使用加密答复回复。 您可以创建邮件流规则，以自动从回复中删除加密，以便您的组织中的电子邮件用户不必登录加密门户来查看它们。 您可以使用 EAC 或 Windows PowerShell cmdlet 来定义这些规则。 您可以解密从组织内部发送的邮件，或对从组织内部发送的邮件的答复邮件。 无法解密来自组织外部的加密邮件。
...
收件人按照邮件中的说明打开附件，然后使用 Microsoft 帐户或与 Office 365 关联的工作或学校进行身份验证。 如果收件人没有任一帐户，则引导他们创建一个 Microsoft 帐户，以便他们登录以查看加密邮件。 或者，收件人可以选择获取一次传递代码来查看邮件。 登录或使用一次传递代码后，收件人可以查看解密的邮件并发送加密回复。
...
1. 使用远程 PowerShell 连接到 Exchange Online，如使用远程 [PowerShell](https://technet.microsoft.com/library/jj984289%28v=exchg.150%29.aspx)连接到 Exchange Online 中所述。

2. 使用 Set-OMEConfiguration cmdlet，如下所述 [：Set-OMEConfiguration](https://technet.microsoft.com/3ef0aec0-ce28-411d-abe8-7236f082af1b)。 若要从 DisclaimerText、EmailText 和 PortalText 值中删除组织的品牌自定义项，请将其值设置为空字符串  `""` 。 对于所有图像值（如徽标），将值设置为  `"$null"` 。
 ...
 **问：Office 365 邮件加密与权限管理服务和 RMS (如何) ？**
...
可以，可以通过从 Google Play 应用商店和 Apple App Store 下载 OME 查看器应用，在 Android 和 iOS 上查看消息。 在 OME 查看器应用中打开 HTML 附件，然后按照说明打开加密邮件。 对于其他移动设备，您可以打开 HTML 附件，只要您的邮件客户端支持"表单发布"。
...
是的。 您必须将 Exchange Online 的 URL 添加到组织的允许列表中，才能对由 Office 365 邮件加密加密的邮件启用身份验证。 有关 Exchange Online URL 的列表，请参阅 [Microsoft 365 URL 和 IP 地址范围](https://docs.microsoft.com/microsoft-365/enterprise/urls-and-ip-address-ranges)。
  
 **问：我可以向多少个收件人发送 Microsoft 365 加密邮件？**
  
收件人限制为每封邮件 500 个收件人，或者通讯组列表展开后，邮件的"收件人"字段中包含 11，980个字符，以先发生者为准。
