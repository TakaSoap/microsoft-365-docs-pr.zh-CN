...
第一步是访问 Microsoft 365 合规中心的"数据连接器"页面，并创建 EML 数据的连接器。 
...
1. 在" **将外部用户映射到 Microsoft 365** 用户"页上，启用自动用户映射。 EML 源项目包括名为 *Email* 的属性，该属性包含组织中用户的电子邮件地址。 如果连接器可以将此地址与 Microsoft 365 用户关联，EML 项目将导入该用户的邮箱。
