...
在某些情况下，您必须向法规或合规性审核人员提供信息，以证明员工活动和通信的监督。 这可能是与已定义的组织策略相关联的所有活动的摘要，或任何时候通信合规性策略更改。 通信合规性策略具有内置审核跟踪，可以实现内部或外部审核的完整就绪状态。 您的通信策略将捕获每个创建、编辑和删除操作的详细审核历史记录，以提供监督过程的证明。
