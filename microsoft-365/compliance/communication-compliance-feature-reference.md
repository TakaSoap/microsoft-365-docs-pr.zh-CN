...
在 Microsoft 365 合规中心为 Microsoft 365 组织创建通信合规性策略。 通信合规性策略定义组织中需要审阅哪些通信和用户，定义通信必须满足的自定义条件，并指定应进行审阅的用户。 分配了通信合规性 *管理员* 角色的用户可以设置策略，分配了此角色的任何人都可以访问 Microsoft 365 合规中心中的通信合规性页面和全局设置。  如果需要，可以将策略修改历史记录导出到 .csv (逗号分隔值) 文件，该文件还包括挂起审阅的警报、升级的项目和已解决项目的状态。 策略无法重命名，并且可在不再需要时删除。

>[!NOTE]
>在 Office 365 订阅&安全与合规中心创建的监督策略无法迁移到 Microsoft 365。 如果要从 Office 365 订阅迁移到 Microsoft 365 订阅，需要创建新的通信合规性策略来替换现有监督策略。
...
新的角色组结构取代了用于通信合规性的初始角色组结构。 对于已使用通信合规性的组织，需要分配有监管审核管理员角色，才能在 Microsoft 365 合规中心开始使用通信合规性。 此外，您必须为具有监管审核管理员、案例管理、合规性管理员和审阅角色的审阅者创建新角色组，以调查和修正具有策略匹配项的邮件。 实质上，所有管理员和审阅者都在同一个角色组中，并且每个人都具有相同的访问和管理权限。 随着通信合规性的最新更新，您应计划从以前的角色组结构迁移到新的角色组结构。 对以前的角色组结构的支持将逐步淘汰。
...
创建通信合规性策略时，必须确定审阅受监督用户消息的用户。 在策略中，用户电子邮件地址标识要审阅监督通信的个人或人员组。 所有审阅者都必须在 Exchange Online 上托管邮箱，并且必须分配到 *通信* 合规性分析或 *通信合规性调查* 角色。 审阅 (分析员或研究人员) 还必须分配 *通信合规性案例管理* 角色。 审阅者添加到策略时，他们会自动收到一封电子邮件，通知他们分配到此策略，并提供有关审阅过程的信息的链接。
...
在策略中分配通讯组时，该策略监视来自通讯组中每个用户的所有电子邮件。 在策略中分配 Microsoft 365 组时，该策略将监视发送到该组的所有电子邮件，而不是每个组的成员收到的单个电子邮件。
...
- **Exchange 电子邮件**：作为 Microsoft 365 或 Office 365 订阅的一部分托管在 Exchange Online 上的邮箱均有资格进行邮件扫描。 与通信合规性策略条件匹配的 Exchange 电子邮件和附件最多可能需要 24 小时处理。 支持的通信合规性附件类型与 Exchange 邮件流规则内容检查支持的 [文件类型相同](/exchange/security-and-compliance/mail-flow-rules/inspect-message-attachments#supported-file-types-for-mail-flow-rule-content-inspection)。

- **Yammer：** 可以扫描 Yammer 社区中的私人消息和公共对话以及关联的附件。 将用户添加到包括 Yammer 作为已定义频道的通信合规性策略时，该用户是其中成员的所有 Yammer 社区之间的通信将包含在扫描过程中。 与通信合规性策略条件匹配的 Yammer 聊天和附件最多可能需要 24 小时处理。 Yammer 必须进入 [本机模式](/yammer/configure-your-yammer-network/overview-native-mode) ，通信合规性策略可以监视 Yammer 通信和附件。 在本机模式下，所有 Yammer 用户都使用 Azure Active Directory (AAD) ，所有组都是 Office 365 组，所有文件都存储在 SharePoint Online 中。

- **Skype for Business Online：** 可以监督 Skype for Business Online 中的聊天通信和相关附件。 匹配通信合规性策略条件的 Skype for Business Online 聊天最多可能需要 24 小时处理。 监督聊天对话来自之前保存在 [Skype for Business Online 中的对话](https://support.office.com/article/Find-a-previous-Skype-for-Business-conversation-18892eba-5f18-4281-8c87-fd48bd72e6a2)。  使用以下组管理配置监督 Skype for Business Online 中的用户聊天通信：

    - **对于 Skype for Business Online 聊天通信**：向 [](https://support.office.com/article/Distribution-groups-E8BA58A8-FAB2-4AAF-8AA1-2A304052D2DE)通信合规性策略分配单个用户或分配通讯组。 此设置适用于一对一或一对多用户/聊天关系。

- **第三方** 源：你可以扫描通信，以搜索从 [Instant Bloomberg、Slack、Zoom、SMS](archive-instant-bloomberg-data.md)等第三方源导入到 Microsoft [](archive-slack-data.md)365 组织中邮箱的数据。 [](archive-zoommeetings-data.md) 有关通信合规性支持的连接器的完整列表，请参阅存档 [第三方数据](archiving-third-party-data.md)。
...
可以选择"所有用户 **"或在** 通信合规性策略中定义特定用户。 选择 **"所有用户** "将策略应用于所有用户以及任何用户作为成员包含在内的所有组。 定义特定用户将策略应用于已定义用户以及已定义用户作为成员包含在内的任何组。

### <a name="direction"></a>Direction

默认情况下，将显示 **Direction 为** 条件，并且无法删除。 单独或一起选择策略中的通信方向设置：

- **入站**：你可以选择" **入站** " **查看发送给你** 选择监督人员的通信。
- **出站**：如果要查看从选择 **监督** 的人发送的通信，可以选择"出站"。
- **内部**：**可以选择"内部**"查看在策略中标识的人之间发送的通信。

### <a name="sensitive-information-types"></a>敏感信息类型

您可以选择将敏感信息类型作为通信合规性策略的一部分。 敏感信息类型是预定义的或自定义的数据类型，可帮助识别和保护信用卡号、银行帐号、护照号等。 [作为 DLP (数据丢失](data-loss-prevention-policies.md)防护的一) ，敏感信息配置可以使用模式、字符邻近度、可信度，甚至是自定义数据类型来帮助标识和标记可能敏感的内容。 默认敏感信息类型为：
...
内置的可训练和全局分类器扫描组织中所有通信渠道发送或接收的邮件，以发现不同类型的合规性问题。 分类器使用人工智能和关键字的组合来识别邮件中可能违反反威胁策略的语言。 内置分类器当前支持多种语言的邮件关键字标识：
...
如果输入多个条件，Microsoft 365 会使用所有条件来确定何时将通信合规性策略应用于通信项目。 设置多个条件时，必须满足所有条件才能应用策略，除非输入例外。 例如，您需要一个在邮件包含单词"trade"且大于 2 MB 时应用的策略。 但是，如果邮件还包含单词"Contoso financial approved by Contoso financial"，则不应应用该策略。 本示例中，将按如下方式定义三个条件：
...
如果要减少要审阅的内容量，可以指定由通信合规性策略控制的所有通信的百分比。 从与所选策略条件匹配的内容的总百分比中，选择内容实时随机样本。 如果希望审阅者审阅所有项目，可以在通信合规性策略中配置 **100%。**
...
- **策略设置和状态** 详细报告：详细查看策略配置和设置，以及邮件上每个策略 (和操作) 的常规状态。 包括策略信息以及策略如何与用户和组、位置、审阅百分比、审阅者、状态以及策略的上次修改时间相关联。 使用 *"导出* "选项创建包含报告详细信息的 .csv 文件。
...
- Office 365 中的监督解决方案已完全替换为 Microsoft 365 中的通信合规性解决方案。 我们建议在通信合规性中创建新策略，这些策略的设置与现有监督策略相同，以使用新的调查和修正改进。
- 无法在 Microsoft 365 中移动或共享在 Office 365 策略匹配中监督保存的邮件。
- 对于在转换过程中并行使用这两个解决方案的组织，每个解决方案中使用的策略必须具有唯一的策略名称。 可以在转换期间在解决方案之间共享组和自定义关键字词典。
