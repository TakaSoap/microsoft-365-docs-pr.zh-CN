...
- **Microsoft 团队**：可以扫描公共和私有 Microsoft 团队频道和个人聊天中的聊天通信。 如果将用户分配给选择 Microsoft 团队覆盖范围的通信合规性策略，则会在用户所属的所有 Microsoft 团队中自动监视用户的聊天通信。 Microsoft 团队覆盖范围将自动包含在预定义的策略模板中，并且默认情况下将在自定义策略模板中选中。 团队聊天匹配通信合规性策略条件可能需要长达48小时才能处理。 使用以下组管理配置监督团队中的个人用户聊天和通道通信：
...
>[!NOTE]
>使用分类器的策略将检查和评估字数为6或更高的邮件。 包含不到六个单词的邮件在使用分类器的策略中不会得到评估。 若要确定并对包含不适当内容的较短邮件执行操作，建议将自定义关键字词典包含到针对此类型内容的通信合规性策略监视中。
...
|**Condition**|**如何使用此条件**|
|:-----|:-----|
| **内容与这些分类器中的任何类元匹配** | 在邮件中包含或排除任何分类器时，应用于该策略。 某些分类器在租户中预定义，必须单独配置自定义分类器，然后才可用于此条件。 只能将一个分类器定义为策略中的条件。 有关配置分类器的详细信息，请参阅 [了解 trainable 类元 (preview) ](classifier-learn-about.md)。 |
...
- **显示匿名版本的用户名**：用户名是匿名，以防止 *通信合规性分析员* 角色组中的用户看到与策略通知关联的用户。 *通信合规性调查* 人员角色组中的用户将始终看到用户名，而不是匿名版本。 例如，在通信合规性体验的所有方面，都会有一个随机的 pseudonym （如 "AnonIS8-988"）显示用户的宽限期 Taylor。 选择此设置将 anonymizes 所有具有当前策略和过去策略匹配并适用于所有策略的用户。 如果选择此选项，则通信合规性警报详细信息中的用户配置文件信息将不可用。 但是，在将新用户添加到现有策略或将用户分配到新策略时，将显示用户名。 如果您选择关闭此设置，则会为具有当前或过去策略匹配的所有用户显示用户名。
- **不显示匿名版本的用户名**：将为通信合规性警报的所有当前和过去的策略匹配显示用户名。 用户配置文件信息 (显示所有通信合规性警报的用户的姓名、职务、别名和组织或部门) 。
...
为客户提供以下电源自动化模板以支持通信合规性警报的流程自动化：

- **当用户具有通信合规性警报时通知管理器**：当用户具有通信合规性警报时，某些组织可能需要立即进行管理通知。 当配置和选择此流时，将向事例用户的经理发送一封包含有关所有通知的以下信息的电子邮件：
    - 适用于警报的策略
    - 警报的日期/时间
    - 警报的严重性级别

### <a name="create-a-power-automate-flow"></a>创建电源自动化流

若要从推荐的默认模板创建电源自动流，在直接处理警报时，您将使用 "**自动化** 控制" 中的 "**管理电源自动流**" 选项。 若要创建具有 **管理电源自动化** 流的电源自动化流，您必须是至少一个通信合规性角色组的成员。

完成以下步骤以从默认模板创建 Power 自动化流：

1. 在 Microsoft 365 合规性中心中，转到 "**通信合规性**  >  **策略**"，然后选择包含要查看的警报的策略。
2. 从策略中，选择 " **挂起** " 选项卡并选择一个待处理的警报。
3. 从 "警告操作" 菜单中选择 " **电源自动** "。
4. 在 " **电源自动** 运行" 页面上，从页面上的 " **您可能喜欢的通信合规性模板** " 部分中选择一个默认模板。
5. 流将列出流所需的嵌入连接，并将显示连接状态是否可用。 如果需要，请更新不显示为可用的任何连接。 选择 " **继续**"。
6. 默认情况下，将使用建议的通信合规性和 Microsoft 365 服务数据字段预配置建议的流，以完成流所分配的任务。 如果需要，请使用 " **显示高级选项** " 控件并配置流组件的可用属性，从而自定义流组件。
7. 如果需要，通过选择 " **新建步骤** " 按钮，向流中添加任何其他步骤。 在大多数情况下，建议的默认模板不需要进行此更改。
8. 选择 " **保存草稿** " 以稍后保存该流，或选择 " **保存** " 以完成流的配置。
9. 选择 " **关闭** " 以返回 "电源自动流" 页面。 新模板将作为流显示在 " **我的流** " 选项卡上，并在使用通信合规性警报时创建流的用户的 "电源自动控制" 中自动提供。

### <a name="share-a-power-automate-flow"></a>共享电源自动化流

默认情况下，用户创建的电源自动流程仅对该用户可用。 若要使其他通信合规性用户具有访问权限并使用流，流的创建者必须共享流。 若要共享流，您将在直接处理警报时使用 **Power 自动** 控制。

若要共享电源自动流，您必须是至少一个通信合规性角色组的成员。
完成以下步骤以共享电源自动流：

1. 在 Microsoft 365 合规性中心中，转到 "**通信合规性**  >  **策略**"，然后选择包含要查看的警报的策略。
2. 从策略中，选择 " **挂起** " 选项卡并选择一个待处理的警报。
3. 从 "警告操作" 菜单中选择 " **电源自动** "。
4. 在 " **电源自动流** " 页上，选择 " **我的流** " 或 " **团队流** " 选项卡。
5. 选择要共享的流，然后从 "流量选项" 菜单中选择 " **共享** "。
6. 在 "流共享" 页面上，输入要作为流的所有者添加的用户或组的名称。
7. 在 " **使用的连接** " 对话框中，选择 **"确定"** 以确认添加的用户或组将拥有对流的完全访问权限。

### <a name="edit-a-power-automate-flow"></a>编辑电源自动化流

如果需要编辑流，则在直接处理警报时，将使用 **Power 自动** 控制。 若要编辑电源自动流，您必须是至少一个通信合规性角色组的成员。
...
2. 从策略中，选择 " **挂起** " 选项卡并选择一个待处理的警报。
3. 从 "警告操作" 菜单中选择 " **电源自动** "。
4. 在 " **电源自动流** " 页上，选择 "流到编辑"。 从 "流控制" 菜单中选择 " **编辑** "。
5. 选择 **ellipsis**  >  **Settings** 用于更改流组件设置或通过 **省略号**  >  **删除** 以删除流组件的省略号设置。
6. 依次选择 " **保存** " 和 " **关闭** " 以完成流的编辑。

### <a name="delete-a-power-automate-flow"></a>删除电源自动流

如果您需要删除流，则直接在警报中工作时，将使用 **Power 自动** 控制。 若要删除电源自动流，您必须是至少一个通信合规性角色组的成员。
...
2. 从策略中，选择 " **挂起** " 选项卡并选择一个待处理的警报。
3. 从 "警告操作" 菜单中选择 " **电源自动** "。
4. 在 " **电源自动流** " 页上，选择 "要删除的流"。 从 "流控制" 菜单中选择 " **删除** "。
5. 在 "删除确认" 对话框中，选择 " **删除** " 以删除流，或选择 " **取消** " 退出删除操作。

## <a name="reports-preview"></a>报告 (预览) 

新的 " **报告** " 仪表板是查看所有通信合规性报告的中心位置。 报告构件提供了对通信合规性活动状态整体评估时最常用的见解的快速视图。 包含在报告小组件中的信息是不可导出的。 详细报告提供了与特定的通信合规性领域有关的详细信息，并提供了在审阅时筛选、分组、排序和导出信息的功能。

" **报告" 仪表板** 包含以下报告小组件和详细报告链接：

- **最近策略匹配** 小部件：按活动策略显示一段时间内的匹配项数。
- **按策略构件解析的项目** ：显示按策略解决的策略匹配警报的数量。
- **具有大多数策略匹配** 小部件的用户：显示用户 (或匿名用户名) 和给定时间段内的策略匹配项数。
- **具有最多匹配** 小组件的策略：显示给定时间段内的策略和匹配项数，以最高为最低级别的匹配项。
- **按策略小组件进行升级** ：显示在给定时间内每个策略的升级次数。
- **策略设置和状态** 详细报告：提供策略配置和设置的详细信息，以及每个策略 (匹配和邮件上的操作) 的常规状态。 使用 " *导出* " 选项来创建。包含报告详细信息的 CSV 文件。
- **每个策略的项目和操作** 详细报告：查看和导出匹配的项目以及每个策略的修正操作。 使用 " *导出* " 选项来创建。包含报告详细信息的 CSV 文件。
- **每个位置的项和操作** 详细报告：查看和导出每个 Microsoft 365 位置的匹配项和修正操作。 使用 " *导出* " 选项来创建。包含报告详细信息的 CSV 文件。
