...
- 无需管理审核的邮箱操作。 默认情况下，会针对管理员、代理和所有者管理员 (类型的每个预定义邮箱操作) 。
...
值 **False** 表示默认情况下为组织启用邮箱审核。 默认情况下，此启用的组织值会覆盖特定邮箱上的邮箱审核设置。 例如，如果对邮箱禁用邮箱审核 (则邮箱) 的 *AuditEnabled* 属性为 **False，** 则仍将对邮箱审核默认邮箱操作，因为默认情况下会为组织启用邮箱审核。
...
- 在选中 () 默认情况下，会记录登录类型的邮箱操作后，使用星号 <sup>\*</sup> 。
...
> [!NOTE]
> 尽管可以将开关添加到组邮箱的以下 `-GroupMailbox` **Get-Mailbox** Microsoft 365，但不要认为返回的值。 为组邮箱审核的默认和Microsoft 365操作在本文前面对组邮箱Microsoft 365[邮箱](#mailbox-actions-for-microsoft-365-group-mailboxes)的邮箱操作部分中进行了介绍。
...
> [!NOTE]
> 以下过程不适用于组Microsoft 365， (仅限于此处所述的[默认) 。](#mailbox-actions-for-microsoft-365-group-mailboxes)
...
- 尽管默认情况下会为所有组织启用邮箱审核日志记录，但只有具有 E5 许可证的用户才能在 审核日志 搜索中或通过 [Office 365](search-the-audit-log-in-security-and-compliance.md)管理活动 [API](/office/office-365-management-api/office-365-management-activity-api-reference)返回 Microsoft 365 合规中心 中的邮箱 审核日志 **事件**。
  ...
  - 手动启用单个邮箱的邮箱审核 (运行命令 `Set-Mailbox -Identity <MailboxIdentity> -AuditEnabled $true` ，) 。 完成此操作后，可以使用 审核日志 或 Microsoft 365 合规中心 管理活动 API Office 365搜索。
  ...
  - 在Exchange EAC (管理) Exchange Online管理中心执行以下操作：
...
- 邮箱审核日志记录存储在名为"审核 (文件夹的子文件夹中) 每个用户邮箱的"可恢复的项目"文件夹中。 对于邮箱审核记录和"可恢复的项目"文件夹，请牢记以下事项：
  ...
  - 邮箱审核记录还计入 ["可恢复的项目"文件夹的文件夹限制](/office365/servicedescriptions/exchange-online-service-description/exchange-online-limits#mailbox-folder-limits)。 审核记录中最多可以 (300 万) 审核记录存储在"审核"子文件夹。
...
- 如果邮箱处于保留状态或分配到合规中心的保留策略，审核日志 记录在默认情况下由邮箱 *的 AuditLogAgeLimit* 属性定义的保留期 (90 天) 。 若要将审核日志保留的邮箱保留更长时间，需要增加邮箱的 *AuditLogAgeLimit* 值。
