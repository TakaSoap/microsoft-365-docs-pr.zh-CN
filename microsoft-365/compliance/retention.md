...
与保留策略不同，如果内容移动到 Microsoft 365 租户内的不同位置，则保留标签的保留设置将随内容移动。 另外，保留标签具有保留策略不支持的以下功能：
...
- 将内容标记为[记录](records-management.md#records)，以使其成为标签设置的一部分，并对在保留期结束时删除的内容始终具有[处置证明](disposition.md#disposition-of-records)。
