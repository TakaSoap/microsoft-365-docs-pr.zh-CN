...
电子邮件中的高级Microsoft 365可帮助您履行要求对外部收件人对加密电子邮件的访问进行更灵活的控制的合规性义务。 通过 Office 365 中的高级邮件加密，作为管理员，您可以使用自动策略控制在组织外部共享的敏感电子邮件，这些策略可检测敏感信息类型 (例如 PII、财务或运行状况) 或关键字，以通过使通过安全 Web 门户对加密电子邮件的访问过期来增强保护。 作为管理员，你可以随时撤销对电子邮件的访问权限，以进一步控制通过 Microsoft 365 Web 门户访问的加密电子邮件。
...
|[如何打开受保护的邮件？](https://support.office.com/article/1157a286-8ecc-4b1e-ac43-2a608fbf3098.aspx)|想要阅读发送给您的受保护邮件的最终用户。 本文包含有关从不同电子邮件帐户（包括 gmail Outlook Yahoo！ 等 Microsoft 365帐户）中读取邮件的信息。 帐户。|
