...
内容搜索和与电子数据展示相关的活动 (针对在安全 & 合规性中心或通过运行相应的 PowerShell cmdlet 在审核日志中执行的核心电子数据展示和高级电子数据展示) 。 当管理员或电子数据展示管理员 (或任何用户分配的电子数据展示权限) 在安全 & 合规性中心中执行以下内容搜索和核心电子数据展示任务时，将记录事件：
...
|已下载内容搜索预览项  <br/> |PreviewItemDownloaded  <br/> |不适用  <br/> |用户通过单击预览搜索结果时) 的 " **下载原始项目** " 链接 (将项目下载到本地计算机。  <br/> |
...
|删除了内容搜索的分析  <br/> |RemovedSearchResultsSentToZoom  <br/> |New-compliancesearchaction  <br/> |已删除内容搜索准备操作 (为高级电子数据展示) 准备搜索结果。 如果准备操作的时间不是两周，则为高级电子数据展示准备的搜索结果将从 Microsoft Azure 存储区域中删除。 如果准备操作的时间已超过2周，则此事件指示仅删除了相应的准备操作。  <br/> |
...
|(无)|SearchViewed|Get-ComplianceSearch|用户查看了 " **搜索** " 选项卡上的 "内容搜索" 列表 (在 "安全与合规中心" 中的 ") " 或 "运行 cmdlet" 中列出。 当用户通过以下方式查看与电子数据展示事例 (相关联的内容搜索列表时，也会记录此活动：单击事例中的 " **搜索** " 选项卡) 或运行 **new-compliancesearch-case** 命令。|
|(无)|ViewedSearchExported|New-compliancesearchaction-导出|用户查看了 "安全与合规中心" 中的 " **导出** " 选项卡) 上的内容搜索导出作业列表，或通过运行 cmdlet (列出。 当用户在) 的 " **导出** " 选项卡上列出的电子数据展示事例 (中列出导出作业列表时，或通过运行 **New-compliancesearchaction-case 导出** 命令，也会记录此活动。|
...
下表介绍了审核日志中记录的高级电子数据展示活动。 除了相关的电子数据展示活动之外，这些活动 (还可用于帮助您跟踪高级电子数据展示事例中活动的进展情况。
...
|从电子数据展示案例中删除了保留  <br/> |[CaseHoldPolicy](https://go.microsoft.com/fwlink/p/?LinkId=823814) <br/> |删除了与电子数据展示事例相关联的保留。 删除保留将释放保留中的所有内容位置。 删除保留也会导致删除与保留相关的事例保留规则 (请参阅下面的 New-caseholdrule) 中的 **Remove** 。  <br/> |
...
|CreationTime  <br/> |在电子数据展示活动完成时 (UTC) 的日期和时间（采用协调通用时间）。  <br/> |
...
|NonPIIParameters  <br/> | (的参数列表，其中不包含任何值与 Operation 属性中标识的 cmdlet 一起使用) 。 此属性中列出的参数与 Parameters 属性中列出的参数相同。  <br/> |
|ObjectId  <br/> |对象的 GUID 或名称 (例如，内容搜索或电子数据展示事例) 由 Operation 属性中列出的活动创建、更改或删除。 此外，还会在审核日志搜索结果的项列中标识此对象。  <br/> |
...
|StartTime  <br/> |启动电子数据展示活动 (UTC) 的日期和时间（采用协调通用时间）。  <br/> |
|UserID  <br/> |执行活动的用户 (在操作属性) 中指定，从而导致记录记录。 系统帐户执行的电子数据展示活动的记录 (如 NT AUTHORITY\SYSTEM) 也包含在审核日志中。  <br/> |
...
|版本  <br/> |指示由已记录的 Operation 属性) 标识的活动 (的版本号。  <br/> |
