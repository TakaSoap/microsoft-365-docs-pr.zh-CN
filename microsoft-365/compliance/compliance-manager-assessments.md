...
若要准备此过程，首先需要组合一个特殊格式的电子表格Excel导入必要的模板数据。 扩展过程中使用的[格式化Excel有](compliance-manager-templates.md#formatting-your-template-data-with-excel)特殊要求。 请参阅以下其他要点，以帮助防止导入过程中出现错误：
...
在合规性管理器中创建自定义评估需要您创建自己的模板。 若要创建自己的模板，首先将格式化的Excel电子表格，以导入必要的模板数据。 它还有助于提前确定创建评估时将评估分配到的组， (组) 。 [](#what-are-groups)

**按照以下步骤创建自定义评估：**

1. **设置Excel格式。** 首先，使用这些说明将模板数据格式化为Excel电子表格[。](compliance-manager-templates.md#formatting-your-template-data-with-excel)
