...
为了开始操作，Microsoft 在合规性管理器 **中** 提供对 Microsoft 365 **数据保护基线的默认评估**。 此基线评估具有一组针对数据保护关键法规和标准以及一般数据管理的控制措施。 此基线主要来自 NIST CSF (国家标准和技术网络安全协会) 和 ISO (国际标准化组织) ，以及 FedRAMP (联邦风险和授权管理计划) 以及欧盟) 的 GDPR (一般数据保护条例。
...
3. 在飞出窗格中，选择评估所基于的法规或认证模板。 分为包含和高级类别的模板列表 ([详细信息) ](compliance-manager-templates.md#template-availability-and-licensing) 。 在飞出窗格顶部的"已激活 **/** 许可模板"计数器显示你使用的总可用模板数或你的组织如何使用 (了解详细信息。) 选择所选模板旁边的单选按钮，然后选择"保存"[](compliance-manager-templates.md#active-and-inactive-templates)。 你将返回到基本 **模板屏幕，** 可在其中查看模板详细信息，然后继续选择"下一步 **"**。
...
当存在影响评分的基础模板更改时，将发生评估更新。 更改可能涉及根据法规更改或产品更改调整控制映射或其他指南。 评估更新可能源自 (，例如，修改自定义模板时以及 Microsoft。[](compliance-manager-templates-modify.md)
