...
ms.locfileid: "50597192"
---
# <a name="message-encryption-faq"></a>邮件加密常见问题解答

对新的邮件保护功能如何工作有疑问？ 请在此处查看答案。 此外，查看 [有关 Azure](https://docs.microsoft.com/information-protection/get-started/faqs-rms) 信息保护中数据保护的常见问题解答，了解 Azure 信息保护中有关数据保护服务（Azure 权限管理）的问题。
...
若要使用新的 OME 功能，需要以下计划之一：
  
- Office 365 邮件加密作为 Office 365 企业版 E3 和 E5、Microsoft Enterprise E3 和 E5、Microsoft 365 商业高级版、Office 365 A1、A3 和 A5 以及 Office 365 政府版 G3 和 G5 的一部分提供。 客户不需要其他许可证来接收由 Azure 信息保护支持的新保护功能。
...
## <a name="can-i-use-exchange-online-with-bring-your-own-key-byok-in-azure-information-protection"></a>能否将 Exchange Online 与 Azure 信息保护中的 BYOK (一) 使用？
...
不正确。 OME 以及从 Azure 信息保护提供和控制你自己的加密密钥（称为 BYOK）的选项并非旨在响应执法机构的子请求。 OME 与用于 Azure 信息保护的 BYOK 一起专为以合规性为中心的客户设计。 Microsoft 非常重视第三方客户数据请求。 作为云服务提供商，我们始终宣传客户数据的隐私。 如果我们收到请求，我们始终尝试将第三方重定向到客户以获取信息。  (请阅读 Brad Smith 的 [博客：保护](https://blogs.microsoft.com/blog/2013/12/04/protecting-customer-data-from-government-snooping/) 客户数据免受政府) 。 我们定期发布收到的请求的详细信息。 有关第三方数据请求详细信息，请参阅"响应政府请求[](https://www.microsoft.com/trustcenter/privacy/govt-requests-for-data)"和"强制执行请求"，以访问 Microsoft 信任中心上的客户数据。 另请参阅联机服务条款中的"披露客户数据" ([OST) 。 ](https://www.microsoft.com/Licensing/product-licensing/products.aspx)
  
## <a name="how-is-this-feature-related-to-legacy-office-365-message-encryption-ome-and-information-rights-management-irm-features"></a>此功能与旧版 Office 365 邮件加密 (OME) 和信息权限 (IRM) 功能相关？

Office 365 邮件加密的新功能是现有 IRM 和旧 OME 解决方案的一个发展过程。 下表提供了更多详细信息。
  
**旧版 OME、IRM 和新 OME 功能的比较**

| 功能 | OME 的早期版本 | IRM | 新的 OME 功能 |
|:-----|:-----|:-----|:-----|
|**发送加密电子邮件**|仅通过 Exchange 邮件流规则|从 Outlook for Windows、Outlook for Mac 或 Outlook 网页 Outlook 启动的最终用户;或通过 Exchange 邮件流规则|从 Outlook for Windows、Outlook for Mac 或 Outlook 网页 Outlook 启动的最终用户;或者通过邮件流规则|
|**版权管理**|-|"不要转发"选项和自定义模板|"不转发"选项、仅加密选项、默认模板和自定义模板|
|**支持的收件人类型**|仅外部收件人|仅内部收件人|内部和外部收件人|
|**收件人体验**|外部收件人收到一条 HTML 邮件，邮件已下载，在浏览器或已下载的移动应用中打开。|内部收件人仅在 Outlook for Windows、Outlook for Mac 和 Web 上的 Outlook 中收到加密电子邮件。|内部和外部收件人在 Outlook for Windows、Outlook for Mac、Outlook 网页版、Outlook for Android 和 Outlook for iOS 中，或者通过 Web 门户接收电子邮件，无论他们是否在同一组织或任何组织中。 OME 门户无需单独下载。|
...
## <a name="will-the-previous-version-of-ome-be-deprecated"></a>早期版本的 OME 将被弃用吗？

您仍可以使用以前版本的 OME，目前不会弃用。 但是，我们强烈建议组织使用新的和改进的 OME 解决方案。 尚未部署 OME 的客户无法设置以前版本的 OME 的新部署。
...
不正确。 如果将 Exchange Online 与 Active Directory 权限管理服务 (AD RMS) ，则不能马上启用这些新功能。 相反，你需要首先 [将 AD RMS 迁移到 Azure 信息](https://docs.microsoft.com/information-protection/plan-design/migrate-from-ad-rms-to-azure-rms) 保护。
...
## <a name="what-email-client-do-i-need-to-use-in-order-to-create-an-ome-encrypted-message-what-applications-are-supported-for-sending-protected-messages"></a>创建 OME 加密邮件需要使用哪些电子邮件客户端？ 哪些应用程序支持发送受保护的邮件？

可以从 Outlook 2016、Outlook 2013 for Windows 和 Mac 以及 Outlook 网页版创建受保护的邮件。 有关发送加密邮件的信息，请参阅 Outlook for PC 中的"发送、查看和答复加密[邮件"。](https://support.microsoft.com/office/send-view-and-reply-to-encrypted-messages-in-outlook-for-pc-eaa43495-9bbb-4fca-922a-df90dee51980?ui=en-us&rs=en-us&ad=us)
  
## <a name="what-email-clients-are-supported-to-read-and-reply-to-protected-emails"></a>支持哪些电子邮件客户端读取和回复受保护的电子邮件？

Microsoft 365 用户可以从 Outlook for Windows 和 Mac (2013 和 2016) 、Outlook 网页版以及 Outlook mobile (Android 和 iOS) 进行读取和响应。 如果组织允许，您也可以使用 iOS 本机邮件客户端。 如果你不是 Microsoft 365 用户，可以通过 Web 浏览器读取和回复 Web 上的加密邮件。
...
是。 可以使用 OME 发送的最大邮件大小（包括附件）为 25 MB。 有关详细信息，请参阅"[邮件限制"。](https://docs.microsoft.com/office365/servicedescriptions/exchange-online-service-description/exchange-online-limits#message-limits-1)
...
您可以将任何文件类型附加到受保护的邮件。 有一个例外，保护策略仅应用于 Azure 信息保护客户端支持的文件类型中提到的 [文件格式](https://docs.microsoft.com/information-protection/rms-client/client-admin-guide-file-types)。 OME 不支持以下 Office 程序 97-2003 版本：Word (.doc) 、Excel (.xls) 和 PowerPoint (.ppt) 。

如果支持文件格式（如 Word、Excel 或 PowerPoint 文件），则文件将始终受到保护，即使附件已由收件人下载。 例如，假设附件受"请勿转发"保护。 原始收件人下载文件，创建一封邮件给新收件人并附加到该文件。 当新收件人收到文件时，收件人将无法打开受保护的文件。
  
## <a name="are-pdf-file-attachments-supported"></a>PDF 文件附件是否受支持？

简短回答是"是"！ PDF 加密允许你通过安全通信或安全协作保护敏感的 PDF 文档。 发送电子邮件时，Office 365 服务会加密 PDF 文件附件，而不是 Outlook 客户端。

对于 Outlook 网页版、Outlook for iOS 和 Outlook for Android，可以加密发送的 PDF，而无需执行任何其他步骤。 这些客户端本机支持 PDF 加密。

Outlook 桌面版在本机不支持加密 PDF 文件附件。 相反，您需要设置 Exchange 邮件流规则或 DLP，以首先将加密应用于 PDF 附件。 当您通过 PDF 附件从 Outlook 桌面发送邮件时，客户端会首先将包含附件的邮件发送到服务。 当服务接收文件时，该服务在 Exchange Online 中应用数据丢失防护的 OME 保护 (DLP) 策略或邮件流规则。 接下来，Exchange Online 发送包含受保护 PDF 文件附件的邮件。

若要为 PDF 附件启用加密，请运行 [Exchange Online PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-powershell)中的以下命令：

```powershell
Set-IRMConfiguration -EnablePdfEncryption $true
```

PDF 加密允许你通过安全通信或安全协作保护敏感的 PDF 文档。 对于所有 Outlook 客户端，邮件和未受保护的 PDF 附件继承 Exchange Online 中数据丢失防护的 OME 保护 (DLP) 策略或邮件流规则。 此外，如果 Outlook 网页版用户附加到未受保护的 PDF 文档，并且对邮件应用保护，则邮件将继承对邮件的保护。 用户只能在支持受保护 PDF 的应用程序中打开加密附件 (例如 OME 门户和 Azure 信息保护查看器) 。
...
## <a name="can-i-automatically-encrypt-messages-by-setting-up-policies-in-data-loss-prevention-dlp-through-the-security-amp-compliance-center"></a>我能否通过安全合规中心在"数据丢失防护" (DLP) 设置策略来自动 &amp; 加密邮件？

是的。 可以在 Exchange Online 中设置邮件流规则，或在安全合规中心使用 DLP &amp; 设置邮件流规则。
  
## <a name="can-i-customize-encrypted-messages-with-my-company-branding"></a>我可以使用我的公司品牌自定义加密邮件吗？

是的。 有关自定义电子邮件和 OME 门户的信息，请参阅"将组织品牌添加到加密邮件"。 请参阅["将组织品牌添加到加密邮件"。](add-your-organization-brand-to-encrypted-messages.md)
  
## <a name="are-there-any-reporting-capabilities-or-insights-for-encrypted-emails"></a>是否有加密电子邮件的报告功能或见解？

安全与合规中心有一份加密报告。 请参阅 [安全与合规中心&电子邮件安全报告](../security/office-365-security/view-email-security-reports.md)。
...
管理员可以设置邮件流规则以删除加密。 不能从另一个组织应用的邮件中删除使用邮件流规则的加密，除非邮件是使用仅加密保护进行接收的。
...
- 当用户使用 Outlook for Windows、Outlook for Mac 和 Web 上的 Outlook 时，可以查看从电子邮件继承保护的附件。
...
- 我们不支持通过启用电子邮件的安全组进行分配。 我们仅支持通过直接用户分配提供对共享邮箱的访问，并且为 Exchange Online 启用自动映射。 默认情况下为 Exchange Online 启用自动映射。

**将用户分配给共享邮箱**

1. [使用远程 PowerShell 连接到 Exchange Online。](https://technet.microsoft.com/library/jj984289?v=exchg.150%29.aspx)

2. 使用Add-MailboxPermission映射参数运行该 cmdlet。 此示例向 Ayla 授予对支持邮箱的完全访问权限。
