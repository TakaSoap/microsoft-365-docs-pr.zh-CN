...
大多数条件和例外都有一个支持一个或多个值的属性。 例如，如果 DLP 策略应用于 Exchange 电子邮件，则"发件人 **为"** 条件需要邮件的发件人。 一些条件有两个属性。 例如，“**邮件头包含以下任何词语**”条件需要一个属性来指定邮件头字段，需要第二个属性来指定要在头字段中查找的文本。 某些条件或例外没有任何属性。 例如 **，"附件受密码保护"** 条件只是查找邮件中受密码保护的附件。
...
|---------|---------|---------|---------|
|发件人为 |condition： *From* <br/> exception： *ExceptIfFrom*      |Addresses |     由组织中指定的邮箱、邮件用户、邮件联系人或 Microsoft 365 组发送的邮件。|
...
|主题匹配模式|condition： *SubjectMatchesPatterns* <br/> exception： *ExceptIf SubjectMatchesPatterns*|模式   |Subject 字段包含匹配指定正则表达式的文本模式的邮件。|
...
|**DLP 中的条件或例外**| **Microsoft 365 PowerShell 中的条件/例外参数**| **属性类型**   |**说明**|
|---------|---------|---------|---------|
|附件受密码保护|condition： *DocumentIsPasswordProtected* <br/> exception： *ExceptIfDocumentIsPasswordProtected*|无| 附件受密码保护的邮件（因而无法扫描）。 密码检测仅适用于 Office 文档、.zip 文件和 .7z 文件。|
|附件的文件扩展名为|condition： *ContentExtensionMatchesWords* <br/> 例外 *：ExceptIfContentExtensionMatchesWords*|  Words   |附件的文件扩展名匹配任意指定词语的邮件。|
|无法扫描任何电子邮件附件的内容|condition： *DocumentIsUnsupported* <br/>exception： *ExceptIf DocumentIsUnsupported*|   无|    Exchange Online 无法本地识别附件的邮件。|
|任何电子邮件附件的内容未完成扫描|   condition： *ProcessingLimitExceeded* <br/> exception： *ExceptIfProcessingLimitExceeded*|    无 |规则引擎无法完成附件扫描的邮件。可以使用此条件创建规则，以协同工作来标识并处理无法完全扫描内容的邮件。|
...
|标头与模式匹配|   condition： *HeaderMatchesPatterns* <br/> exception： *ExceptIfHeaderMatchesPatterns*|    哈希表  |包含指定头字段以及该头字段的值的邮件包含指定的正则表达式。|
