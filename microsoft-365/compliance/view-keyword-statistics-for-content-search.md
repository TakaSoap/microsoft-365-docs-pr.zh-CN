...
ms.locfileid: "59196838"
---
# <a name="view-statistics-for-ediscovery-search-results"></a>查看电子数据展示搜索结果的统计信息

创建并运行内容搜索或与核心电子数据展示案例关联的搜索后，可以查看有关估计搜索结果的统计信息。 这包括搜索结果 (类似于在搜索飞出页) 上显示的预计搜索结果摘要、查询统计信息（如包含与搜索查询匹配的项目的内容位置数量）和具有最匹配项目的内容位置的标识。
...
- **位置** 类型：查询统计信息适用于的内容位置的类型。 值 **Exchange指示** 邮箱位置;一个 **SharePoint** 值，指示站点位置。
...
- **Size (MB)**：在"条件"列中 (搜索查询匹配的指定内容) 找到的所有 **项目的总大小** 。
...
6. 在"**搜索统计信息**"选项卡上，单击"条件"报告以显示搜索的关键字统计信息。
    ...
    - 完整的搜索查询 (**在**"部件"列) 标识为"主"，并且每个位置类型的完整查询的统计信息。 请注意，这些统计信息与"摘要"选项卡 **上显示的** 统计信息相同。
