...
从  **yourassessment templatespage**  中，选择要修改的模板，这将显示其详细信息页面。 然后选择" **导入"以Excel**。 将Excel模板数据的文件。 将文件保存到本地计算机。
...
您可以更改除标题之外的任何 *改进操作的信息*。 您可以编辑 B 列前向的任何单元格，当您将文件导入回模板时，该模板中的改进操作现在将包含更新的数据。
...
> [!NOTE]
> 如果使用英语语言而非英语的合规性管理器，你会注意到在导出模板以使用安全模板时，某些文本Excel。 操作标题 (改进操作，如果适用，Microsoft) 必须为英语，控件必须识别。 如果对操作标题进行更改，请确保以英语编写它，以便正确导入文件。
