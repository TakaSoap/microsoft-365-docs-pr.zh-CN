...
---
# <a name="create-a-draft-collection-in-advanced-ediscovery"></a>在高级电子数据展示中创建草稿集合
...
- 您可以搜索其他 (尚未作为当前位置或非中央位置添加到案例的数据源) 在草稿集合中搜索可能与案例相关的内容。 这些数据源可能包括邮箱、SharePoint 网站和 Teams。 如果这种情况适用于您的情况，请编译这些数据源的列表，以便您可以将它们添加到集合中。

## <a name="create-a-draft-collection"></a>创建草稿集合

1. 在 Microsoft 365 合规中心，打开"高级电子数据展示"案例，然后选择"集合 **"** 选项卡。
...
6. 在 **"其他数据源"** 页上，可以选择其他邮箱和网站作为集合的一部分进行搜索。 在这种情况下，不会将这些类型的数据源添加为主要或非安全数据位置。 搜索其他数据源时，还有两个选项：
   ...
   - 若要在 Exchange 邮箱、SharePoint 和 OneDrive (或 Exchange 公用文件夹) 中搜索特定服务的所有内容位置，请单击"状态"列中相应的"全选 **"开关。** 此选项将搜索所选服务中所有内容位置。

   - 若要搜索服务的特定内容位置，请单击"状态"列中对应的"全选"切换，然后单击"Exchange 邮箱的用户、组或团队 **(")** 或"为 (SharePoint 和 OneDrive 网站选择网站) "以搜索特定内容位置。 
...
在草稿收集过程中，高级电子数据展示使用在集合中指定的搜索标准和数据源执行搜索估计。 高级电子数据展示还会准备可以预览的项目的采样。 集合完成后，将更新"集合"页上的以下 **列** 和相应值：
...
- 选择" **搜索统计信息** "选项卡以查看有关集合的统计信息。 这包括每个服务 (（例如，Exchange 邮箱或 SharePoint 网站) ）中发现的项目总数和大小，以及显示集合所使用的搜索查询的不同组件返回的项目数量的统计信息的条件报告。 有关详细信息，请参阅集合 [统计信息和报告](collection-statistics-reports.md)。
...
- **导出为报告**：将有关草稿集合的信息导出到可下载到本地计算机的 CSV 文件。 导出报告包含以下信息：
  ...
  - 服务 (，例如) 位置所在的 Exchange 或 SharePoint 网站。
