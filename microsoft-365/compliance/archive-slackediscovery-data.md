...
使用 Microsoft 365 合规性中心中的 Globanet 连接器将第三方数据从社交媒体、即时消息和文档协作平台导入并存档到 Microsoft 365 组织中的邮箱。 Globanet 提供了一个可宽延时间的电子数据展示连接器，该连接器配置为定期从第三方数据源捕获项目 () 然后将这些项目导入到 Microsoft 365。 可宽延时间电子数据展示从可宽延时间 API 中提取邮件和文件并将其转换为电子邮件格式，然后将项目导入到用户邮箱。

在可宽延时间的电子数据展示数据存储在用户邮箱中之后，可以应用 Microsoft 365 合规性功能，如诉讼保留、电子数据展示、保留策略和保留标签，以及通信合规性。 使用可宽延时间的电子数据展示连接器在 Microsoft 365 中导入和存档数据可帮助您的组织遵守政府和法规策略。

## <a name="overview-of-archiving-slack-ediscovery-data"></a>存档可宽延的电子数据展示数据概述

以下概述说明使用连接器在 Microsoft 365 中存档可宽延时间的电子数据展示信息的过程。

![可宽延电子数据展示存档工作流](../media/SlackeDiscoveryConnectorWorkflow.png)

1. 您的组织使用可宽延时间电子数据展示设置和配置时差电子数据展示网站。
...
第一步是访问 Microsoft 365 合规性中心中的 " **数据连接器** " 页，并为可宽延性电子数据展示数据创建连接器。
