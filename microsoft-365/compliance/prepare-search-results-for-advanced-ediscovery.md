...
- **光学字符识别** -当你准备高级电子数据展示的搜索结果时，光学字符识别 (OCR) 功能将自动从图像中提取文本，并将其包含在加载到高级电子数据展示的搜索结果中进行分析。 稀疏文件、电子邮件附件和嵌入图像支持 OCR。 这样，您就可以将高级电子数据展示 (的文本分析功能应用于近重复、电子邮件线程、主题和预测编码) 到图像文件中的文本内容。 高级电子数据展示 OCR 支持图像文件的以下格式：
...
- **预测编码** -使您可以根据所需的内容对系统进行培训，方法是让您做出决定， (一小组的文档中是否有相关或不) 的内容。 然后，高级电子数据展示根据您在分析数据集中的所有文档时的指导) 应用该学习 (。 根据该学习，高级电子数据展示会提供相关性排名，以便您可以根据最可能与案例相关的文档来决定要查看哪些文档。 
...
- 若要使用高级电子数据展示分析用户的数据，必须为用户 (必须为其分配 Office 365 E5 许可证的数据) 的管理员。 或者，可以为具有 Office 365 E1 或 E3 许可证的用户分配高级电子数据展示独立许可证。 分配给案例并使用高级电子数据展示分析数据的管理员和合规性监察官不需要 E5 许可证。 
...
5. 在“准备进行分析的结果”**** 页面上，请执行以下操作：  
    ...
    - 指定在准备过程完成且数据准备在高级电子数据展示中进行处理时，是将通知邮件发送 (还是将) 复制到个人。
...
准备完成后，下一步是转到 "高级电子数据展示"，并加载已上载到 Microsoft 云中的 Azure 存储区域的搜索结果数据 () 在高级电子数据展示中的情况。 如前所述，若要访问高级电子数据展示，您必须是安全 & 合规性中心中的电子数据展示管理员或高级电子数据展示中的管理员。
...
- 如果在准备高级电子数据展示中的搜索结果进行分析时需要防止某人对受 RMS 加密的邮件进行解密，则必须通过复制内置的电子数据展示管理器角色组来创建自定义角色组 () 然后从自定义角色组中删除 RMS 解密管理角色。 然后，将您不想将邮件解密的人员添加为自定义角色组的成员。
