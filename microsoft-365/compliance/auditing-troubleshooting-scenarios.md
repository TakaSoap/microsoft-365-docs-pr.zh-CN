...
您必须在 Exchange Online 中向您分配 "View-Only 审核日志" 或 "审核日志" 角色，才能搜索审核日志。 默认情况下，在 Exchange 管理中心中的“ **权限** ”页上将这些角色分配给“合规性管理”和“组织管理”角色组。 Office 365 和 Microsoft 365 中的全局管理员将自动添加为 Exchange Online 中的 "组织管理" 角色组的成员。 有关详细信息，请参阅[在 Exchange Online 中管理角色组](https://go.microsoft.com/fwlink/p/?LinkID=730688)。
...
有关与电子邮件转发相关的参数的详细信息，请参阅 [设置邮箱](https://docs.microsoft.com/powershell/module/exchange/set-mailbox) 文章。
...
**用户：** 如果您在此字段中选择一个用户，则审核日志搜索工具将返回您指定的用户)  (SoftDeleted 或 HardDeleted 删除的电子邮件项目的审核记录。 有时，删除电子邮件的用户可能不是邮箱所有者。
...
b. 在 " **参数** " 字段中，显示收件箱规则的条件。 在此示例中，条件由 *From* 参数指定。 为 *From* 参数定义的值表示收件箱规则作用于由 admin@alpinehouse.onmicrosoft.com 发送的电子邮件。 有关可用于定义收件箱规则条件的参数的完整列表，请参阅 [new-inboxrule](https://docs.microsoft.com/powershell/module/exchange/new-inboxrule) 文章。

c. *MoveToFolder* 参数指定收件箱规则的操作。 在此示例中，从 admin@alpinehouse.onmicrosoft.com 接收的邮件将移至名为 *AdminSearch* 的文件夹中。 此外，有关可用于定义收件箱规则操作的完整参数列表，请参阅 [new-inboxrule](https://docs.microsoft.com/powershell/module/exchange/new-inboxrule) 文章。
...
在审核日志中查看审核记录时，您可能会看到指示外部用户通过 Azure Active Directory 进行身份验证并成功登录到组织的记录。 例如，contoso.onmicrosoft.com 中的管理员可能会看到一条审核记录，其中显示来自不同组织的用户 (例如，fabrikam.onmicrosoft.com) 成功登录到 contoso.onmicrosoft.com。 同样，您可能会看到审核记录，这些记录表明用户使用 Microsoft 帐户 (MSA) （如 Outlook.com 或 Live.com）已成功登录您的组织。 在这些情况下，审核的活动是 **用户登录** 的。 

此行为是设计使然。 如果外部用户尝试访问您组织中的 SharePoint 网站或 OneDrive 位置，则 azure Active Directory (Azure AD) （目录服务）将允许出现一种称为 " *传递身份验证* " 的情况。 当外部用户尝试执行此操作时，系统会提示他们输入自己的凭据。 Azure AD 使用凭据对用户进行身份验证，即仅 Azure AD 验证用户是否为他们所声称的用户。 审核记录中成功登录的指示是对用户进行身份验证的 Azure AD 的结果。 成功的登录并不意味着用户能够访问任何资源或在您的组织中执行任何其他操作。 它仅指示用户由 Azure AD 进行身份验证。 为了使传递用户能够访问 SharePoint 或 OneDrive 资源，组织中的用户必须通过向外部用户发送共享邀请或匿名共享链接来明确地与外部用户共享资源。 
...
![成功传递身份验证的审核记录示例](../media/PassThroughAuth1.png)
   ...
   e. **RecordType** 的值为 **15** 表示审核的活动 (UserLoggedIn) 是在 AZURE AD 中 (STS) 登录事件的安全令牌服务。
...
以下两个示例方案将导致成功的 **用户在审核活动中登录** ，这是因为传递身份验证： 
  ...
  - 拥有 Microsoft 帐户 (（如 SaraD@outlook.com) ）的用户尝试访问 fourthcoffee.onmicrosoft.com 中的 OneDrive for Business 帐户中的文档，并且在 fourthcoffee.onmicrosoft.com 中没有与 SaraD@outlook.com 相对应的来宾用户帐户。
...
## <a name="search-for-mailbox-activities-performed-by-users-with-non-e5-licenses"></a>搜索由具有非 E5 许可证的用户执行的邮箱活动

即使为您的组织启用了 [默认情况下的邮箱审核](enable-mailbox-auditing.md) ，您也可能会注意到，使用合规性中心、 **UnifiedAuditLog** Cmdlet 或 OFFICE 365 管理活动 API 在审核日志搜索中找不到某些用户的邮箱审核事件。 原因是，只有当您使用上述方法之一搜索统一审核日志时，才会为拥有 E5 许可证的用户返回邮箱审核事件。

若要检索非 E5 用户的邮箱审核日志记录，可以执行以下解决方法之一：

- 手动启用单个邮箱的邮箱审核 (`Set-Mailbox -Identity <MailboxIdentity> -AuditEnabled $true` 在 Exchange Online PowerShell) 中运行命令。 执行此操作后，使用合规性中心、 **UnifiedAuditLog** Cmdlet 或 Office 365 管理活动 API 搜索邮箱审核活动。
...
## <a name="search-for-mailbox-activities-performed-in-a-specific-mailbox-including-shared-mailboxes"></a>搜索在特定邮箱中执行的邮箱活动 (包括共享邮箱) 

当您使用合规中心中的 " **用户** " 下拉列表或 Exchange Online PowerShell 中的 **UnifiedAuditLog-UserIds** 命令时，您可以搜索特定用户执行的活动。 对于邮箱审核活动，此类型的搜索将搜索由指定用户执行的活动。 它并不保证在搜索结果中返回在同一邮箱中执行的所有活动。 例如，审核日志搜索不会返回代理用户执行的活动的审核记录，因为搜索特定用户执行的邮箱活动不会返回委派用户执行的、被分配访问其他用户的邮箱的权限的用户所执行的活动。  (代理用户是指向其他用户的邮箱分配了 SendAs、SendOnBehalf 或 FullAccess 邮箱权限的人员。 ) 

此外，使用审核日志搜索工具或 **UnifiedAuditLog-UserIds** 中的 " **用户** " 下拉列表不会返回在共享邮箱中执行的活动的结果。

若要搜索在特定邮箱中执行的活动，或搜索在共享邮箱中执行的活动，请在运行 **UnifiedAuditLog** cmdlet 时使用以下语法：

```powershell
Search-UnifiedAuditLog  -StartDate <date> -EndDate <date> -FreeText (Get-Mailbox <mailbox identity).ExchangeGuid
```

例如，以下命令返回在八月：2020到10月2020之间的 Contoso 合规性团队共享邮箱中执行的活动的审核记录：

```powershell
Search-UnifiedAuditLog  -StartDate 08/01/2020 -EndDate 10/31/2020 -FreeText (Get-Mailbox complianceteam@contoso.onmicrosoft.com).ExchangeGuid
```

或者，您可以使用 **search-mailboxauditlog** cmdlet 来搜索在特定邮箱中执行的活动的审核记录。 这包括搜索在共享邮箱中执行的活动。

下面的示例返回在 Contoso 合规性团队共享邮箱中执行的活动的邮箱审核日志记录：

```powershell
Search-MailboxAuditLog -Identity complianceteam@contoso.onmicrosoft.com -StartDate 08/01/2020 -EndDate 10/31/2020 -ShowDetails
```

下面的示例返回代理用户在指定邮箱中执行的活动的邮箱审核日志记录：

```powershell
Search-MailboxAuditLog -Identity <mailbox identity> -StartDate <date> -EndDate <date> -LogonTypes Delegate -ShowDetails
```

您还可以使用 **new-mailboxauditlogsearch** cmdlet 搜索特定邮箱的审核日志，并将结果通过电子邮件发送给指定的收件人。