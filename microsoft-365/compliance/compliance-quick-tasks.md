...
如果你是第一次了解Microsoft 365并想知道从何处开始，本文将提供有关基本要求的指导，并优先执行重要的合规性任务。 本文将帮助你快速开始管理和监视数据、保护信息并最大限度地减少内部风险。
...
保留 [策略](retention.md) 允许你主动决定是保留内容还是删除内容，还是同时保留内容，然后在指定的保留期结束时删除内容。 可能需要这些操作来遵守行业法规和内部策略，并降低诉讼或安全漏洞的风险。
