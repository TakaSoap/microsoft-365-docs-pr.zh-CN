...
此深入了解驱动的建议可帮助你的组织在 Microsoft 365 中存储和共享敏感内容，并在你的 DLP 策略覆盖范围内存在可能的差距时通知你。 **Home** &amp; 如果您的文档包含任何最常见类型的敏感信息，但不受数据丢失防护（DLP）策略保护，则您将在安全合规中心的主页上看到此建议。 
...
使用小部件创建 DLP 策略后，该策略将显示在安全合规中心的 "**策略**" 页上的 "**数据丢失防护**" 下 &amp; 。 
...
名为 "未**受保护的敏感信息**" 的小部件将显示在安全合规中心**主页**的 "**建议的用户**" 部分中 &amp; 。 
