...
用户活动报告允许你检查指定时段内特定用户的活动，而无需将其临时或显式分配给内部风险管理策略。 在大多数内部风险管理方案中，用户在策略中明确定义，并且他们可能具有策略警报 (具体取决于触发事件) 活动的风险评分。 但在某些情况下，你可能想要检查未在策略中显式定义的用户的活动。 这些活动可能适用于你已收到有关用户和可能存在风险的活动的提示的用户，或者通常不需要分配给内部风险管理策略的用户。
...
3. 在 **"警报详细信息** "页上，你可以查看有关警报的信息，并可以确认警报并创建新案例、确认警报并添加到现有案例，或者消除警报。 此页面还包括警报的当前状态和警报风险严重性级别，列为"高、中"或"低"。 如果警报未分类，严重性级别可能会随着时间的推移而增加或减小。
    ...
    - **摘要：** 此选项卡包含有关警报的常规信息。
        ...
        - **检测到的内容**：包括与警报的风险活动相关的内容，并按关键区域汇总活动事件。 选择活动链接将打开活动资源管理器并显示有关活动的其他详细信息。
...
从活动时间线选择活动的事件时，资源管理器中显示的活动数可能与时间线中列出的活动事件数不匹配。 出现此情况的原因的示例：

- **累积排查** 检测：累积筛选检测可分析事件日志，但应用包括取消复制类似活动的模型来计算累积的筛选风险。
...
因内部风险管理策略产生的警报数量而不知所措可能会让人感到沮丧。 这可以通过简单步骤快速解决，具体取决于你接收的警报卷的类型。 您可能会收到太多有效警报，或者收到太多过时的低风险警报。 请考虑执行以下操作：
