...
用户活动报告允许你检查指定时段内特定用户的活动，而无需将其临时或显式分配给内部风险管理策略。 在大多数内部风险管理方案中，用户在策略中明确定义，并且他们可能具有策略警报 (具体取决于触发事件) 活动的风险评分。 但在某些情况下，你可能想要检查未在策略中显式定义的用户的活动。 这些用户可能是你收到有关用户和可能存在风险的活动的提示的用户，或者通常不需要分配给内部风险管理策略的用户。
...
3. 在 **警报详细信息窗格中**，你可以查看以下选项卡并会审警报：
    - **摘要：** 此选项卡包含有关警报的常规信息，并允许你确认警报并创建新案例或允许你消除警报。 它包括警报的当前状态和警报风险严重性级别，列为 *"高*、中"或"*低"。*  如果警报未分类，严重性级别可能会随着时间的推移增加或减小。
        - **预览版** (发生了什么) ：显示活动评估期间前三个风险活动和策略匹配项，包括与活动关联的违反类型和发生次数。
    ...
    - **操作**：每个警报都提供以下操作：
        ...
        - **消除警报**：使用此操作可消除警报。 此操作将警报状态更改为"已 *解决"。*
