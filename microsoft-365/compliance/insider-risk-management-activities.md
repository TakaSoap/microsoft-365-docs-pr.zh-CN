...
ms.locfileid: "63314673"
---
# <a name="investigate-insider-risk-management-activities"></a>调查内部风险管理活动

调查有风险的用户活动是最大程度地降低组织内部风险的重要第一步。 这些风险可能是从内部风险管理策略生成警报的活动，或者是从策略检测到的活动（但不立即为用户创建内部风险管理警报）产生的风险。 可以使用"用户活动报表"或"警报"仪表板 (或) **调查这些** **类型的活动**。

## <a name="user-activity-reports-preview"></a>用户活动报表 (预览) 

用户活动报告允许你检查指定时段内特定用户的活动，而无需将其临时或显式分配给内部风险管理策略。 在大多数内部风险管理方案中，用户在策略中明确定义，并且他们可能具有策略警报 (具体取决于触发事件) 活动的风险评分。 但在某些情况下，你可能想要检查未在策略中显式定义的用户的活动。 这些活动可能针对你已收到有关用户和可能存在风险的活动的提示的用户，或者通常不需要分配给内部风险管理策略的用户。

在内部风险管理中心页面上配置 **设置后，** 将检测到与所选指标关联的风险活动的用户活动。 不必为用户活动报告配置策略来检测和报告组织中用户的风险活动。 用户活动报告中包括的活动不需要触发事件来显示活动。 此配置意味着用户的所有检测到的活动都可供查看，无论其是否具有触发事件或是否创建警报。 报告基于每个用户创建，可以包含自定义 90 天期间的所有活动。 不支持同一用户的多个报告。
...
- **已** 确认：确认警报并将其分配给新案例或现有案例。
- **已消除**：在会审过程中消除的警报是无害的。 你可以提供警报解除的原因，并包括用户警报历史记录中提供的注释，以便提供其他上下文供将来参考或其他审阅者参考。 这些原因可能涵盖预期活动、无影响事件、仅减少用户的警报活动数或与警报注释相关的原因。 Reason classification choices include *Activity is expected for this user*， *Activity is impactful for me to investigate further*， and *Alerts for this user contain too much activity*.
- **需要审阅**：尚未执行会审操作的新警报。
- **已** 解决：属于已关闭和已解决案例的警报。

警报风险评分从多个风险活动指示器自动计算。 这些指标包括风险活动的类型、活动发生的数量和频率、用户风险活动的历史记录，以及增加可能提高活动严重程度的活动风险。 警报风险分数将推动针对每个警报的风险严重性级别的编程分配，并且无法进行自定义。 如果警报保持未重试状态，并且风险活动继续计入警报，则风险严重性级别可能会增加。 风险分析师和研究人员可以使用警报风险严重性来帮助根据组织的风险策略和标准对警报进行会审。

警报风险严重性级别为：

- **高严重性**：警报的活动和指示器会带来高风险。 相关的风险活动是严重的、重复的，并且对于其他重大风险因素影响很大。
- **中等严重性**：警报的活动和指示器会带来中等风险。 关联的风险活动适中、频繁，且与其他风险因素具有一些相关性。
- **低严重性**：警报的活动和指示器构成较小风险。 关联的风险活动较小、更不频繁，并且不会与其他重大风险因素相关。

## <a name="filter-alerts-on-the-alert-dashboard"></a>筛选警报仪表板上的警报

查看大型警报队列可能困难，具体取决于组织中活动的预览体验计划风险管理策略的数量和类型。 使用警报筛选器可帮助分析员和研究人员按多个属性对警报进行排序。 若要筛选警报仪表板 **上的警报，****请选择筛选器控件**。 可以按一个或多个属性筛选警报：
...
- **风险因素**：选择更多风险因素之一来筛选警报列表。 选项包括 *累积筛选活动*、 *活动包括优先级* 内容、 *序列* 活动，而 *活动包括不允许的域*。
...
1. In the [Microsoft 365 合规中心](https://compliance.microsoft.com)， go to **Insider risk management** and select the **Alerts** tab.
2. 在 **警报仪表板上**，选择要会审的警报。
3. 在 **警报详细信息** 页面上，你可以查看有关警报的信息。 可以确认警报并创建新案例、确认警报并将其添加到现有案例，或消除警报。 此页面还包括警报的当前状态和警报风险严重性级别，列为"高、中"或"低"。 如果警报未分类，严重性级别可能会随着时间的推移增加或减小。

有关警报的详细信息，请使用"警报详细信息"页上的以下部分和选项卡：

### <a name="headersummary-section"></a>"标题/摘要"部分

此部分包含有关用户和警报的常规信息。 此信息可用于上下文，同时查看有关用户警报中包含的检测到的活动的详细信息：

- **生成此警报的活动**：显示活动评估期间导致生成警报的顶级风险活动和策略匹配。
- **触发事件**：显示提示策略开始为用户活动分配风险评分的最新触发事件。
- **用户配置文件**：显示有关分配给警报的用户的常规信息。 如果启用匿名处理，则用户名、电子邮件地址、别名和组织字段将进行匿名处理。
- **用户警报历史记录**：显示用户过去 30 天的警报列表。 包含一个链接，用于查看用户的完整警报历史记录。

### <a name="all-risk-factors"></a>所有风险因素

此选项卡将打开用户提醒活动的风险因素摘要。 风险因素可以帮助您确定此用户活动在审查过程中的风险。 风险因素包括以下因素的摘要：

- **热门过滤活动**：显示具有最高警报数量或事件的 exfiltration 活动。
- **累计 exfiltration 活动**：显示与累积 exfiltration 活动关联的事件。
- **活动序列**：显示检测到与风险序列关联的活动。
- **此用户的异常活动**：为用户显示被视为异常活动的活动和与常规活动不一样的活动。
- **优先级内容**：显示与优先级内容关联的活动。
- **不允许的域**：显示与不允许的域关联的事件的活动。
- **运行状况记录** 访问：显示与访问运行状况记录关联的事件的活动。

使用这些筛选器，你将只看到具有这些风险因素的警报，但生成警报的活动可能不会属于这些类别中的任何类别。 例如，生成包含序列活动的警报可能仅仅是因为用户将文件复制到 USB 设备。

### <a name="content-detected"></a>检测到的内容

"所有 **风险因素"** 选项卡上的部分包括与警报的风险活动相关的内容，并按关键区域汇总活动事件。 选择活动链接将打开活动资源管理器并显示有关活动的更多详细信息。
...
用户 **活动** 图表是内部风险管理解决方案中用于内部风险分析和调查警报和案例的最强大工具之一。 此选项卡的结构用于快速查看用户的所有活动，包括所有警报的历史时间线、警报详细信息、用户的当前风险评分以及风险事件序列。  

![内部风险管理用户活动。](../media/insider-risk-user-activities.png)

1. **时间筛选器**：默认情况下，"用户活动"图表中显示的最近三个月的活动。 通过在气泡图上选择" *6* 个月、 *3* 个月"或 *"1* 个月"选项卡，可以轻松筛选图表视图。
2. **风险提醒活动和详细信息**：风险活动在用户活动图表中以彩色气泡视觉显示。 为不同的风险类别和创建气泡。 选择气泡以显示每个风险活动的详细信息。 详细信息包括：
    - 风险活动的 **日期**。
    - 风险 **活动类别**。 例如，*电子邮件 ()* 组织外部发送的附件，或从 *(Online*) 下载的文件SharePoint文件。
    - 警报的 **风险分数**。 此分数是警报风险严重性级别的数字分数。
    - 与警报相关联的事件数。 还会提供指向与风险活动关联的每个文件或电子邮件的链接。
3.  **筛选和排序 (预览)**：
    - **风险类别**：按以下风险类别筛选活动：风险分数为 *15 > 15* (，除非按序列和顺序) *活动*。
    - **活动类型**：按以下类型筛选活动 *：访问*、删除、集合、*Exfiltration*、*筛选、* 模糊 *处理**和安全性*。
    - **排序方式**：按发生日期或 *风险分数* 列出 *时间线活动*。
4. 风险 **(预览**) ：风险活动按时间顺序是风险调查的一个重要方面，识别这些相关活动是评估组织的总体风险的重要部分。 与相关的警报活动通过连接线显示，以突出显示这些活动与较大的风险区域相关联。 此活动视图可帮助调查人员逐字地"连接点"可能被视为独立或一次事件的风险活动。 选择序列中的任意气泡以显示所有关联风险活动的详细信息。 详细信息包括：
...
6. **案例操作**：用于解决案例的选项位于案例操作工具栏上。 在查看案例时，可以解决案例、向用户发送电子邮件通知，或上报案例以便进行数据或用户调查。
...
- **风险因素筛选器**：适用于分配风险分数的所有策略的风险因素活动的筛选器 这包括范围内用户的所有策略的所有活动。
    - 异常活动
    - 包含具有优先级内容的事件
    - 包含具有不允许域的事件
    - 序列活动
...
在活动资源管理器中查看活动时，研究人员和分析师可以选择特定活动并打开活动详细信息窗格。 该窗格显示有关调查人员和分析人员可在警报会审过程中使用的活动的详细信息。 详细信息可能会提供警报上下文，并有助于确定触发警报的风险活动的完整范围。

从活动时间线选择活动的事件时，资源管理器中显示的活动数可能与时间线中列出的活动事件数不匹配。 出现此差异的原因的示例：

- **累积筛选** 检测：累积筛选检测可分析事件日志，但应用包括取消复制类似活动的模型来计算累积的筛选风险。 此外，如果对现有策略或设置进行更改，活动资源管理器中显示的活动数也可能有所不同。 例如，如果在创建策略并发生活动匹配后修改允许/不允许的域或添加新的文件类型排除项，则累积的 exfiltration 检测活动将不同于策略或设置更改之前的结果。 累积 exfiltration 检测活动总数基于计算时的策略和设置配置，不包括策略和设置更改之前的活动
...
3. 在"**警报详细信息"窗格中，** 选择 **"ActionsConfirm** >  **alerts &创建案例"**。
...
## <a name="retention-and-item-limits"></a>保留和项目限制

随着内部风险管理警报的不断消失，对于大多数组织来说，其用于最大限度地降低风险活动的价值。 相反，活动案例和相关项目 (警报、见解) 活动对组织始终有价值，不应具有自动到期日期。 这包括与活动案例关联的任何用户的所有未来处于活动状态的警报和项目。

为了帮助最大程度地减少提供有限当前值的较旧项目的数量，以下保留和限制适用于内部风险管理警报、案例和用户活动报告：

|**项目**|**保留/限制**|
|:-------|:------------------|
| 具有需求审阅状态的警报 | 自警报创建起 120 天，然后自动删除 |
| 活动用例 (关联项目)  | 无限期保留，永不过期 |
| 已解决 (关联项目的情况)  | 从案例解决起 120 天，然后自动删除 |
| 最大活动案例数 | 100 |
| 用户活动报告 | 从活动检测起 120 天，然后自动删除 |
...
- **调整内部风险策略**：选择和配置正确的内部风险策略是解决警报类型和数量的最基本方法。 从相应的 [策略模板开始](insider-risk-management-policies.md#policy-templates) 有助于关注你将看到的风险活动和警报的类型。 影响警报量的其他因素包括范围内用户和组的大小以及已设定优先级 [的内容和频道](insider-risk-management-policies.md#prioritize-content-in-policies)。 请考虑调整策略，以将这些方面细化到对组织最重要的方面。
- **修改内部风险设置**：内部风险设置包括各种配置选项，这些选项会影响将收到的警报数量和类型。 其中包括策略 [指示器、](insider-risk-management-settings.md#indicators)指示器 [阈值和](insider-risk-management-settings.md#indicator-level-settings-preview)策略 [时间范围的设置](insider-risk-management-settings.md#policy-timeframes)。 请考虑配置 [智能检测](insider-risk-management-settings.md#intelligent-detections) 选项以排除特定文件类型，在策略报告活动警报之前定义最小阈值，以及将警报量配置更改为较低设置。
- **批量删除警报（如果适用**）：这有助于为分析员和调查人员节省会审时间，以立即立即 [消除多个](insider-risk-management-activities.md#dismiss-multiple-alerts-preview) 警报。 可以选择一次最多消除 400 个警报。
...
4. **对警报采取行动**。 你可以确认并 [创建警报](insider-risk-management-activities.md#create-a-case-for-an-alert) 案例，也可以消除并解决警报。
...
- **使用自动内部风险功能帮助发现高风险活动**。 内部风险管理[序列检测和](insider-risk-management-policies.md#sequence-detection-preview)[累积的筛选检测](insider-risk-management-policies.md#cumulative-exfiltration-detection-preview)功能可帮助你快速发现更难在组织中查找风险。 请考虑微调风险[评分风险](insider-risk-management-settings.md#indicators)、文件类型排除项、[](insider-risk-management-settings.md#file-type-exclusions)域[和策略的最低](insider-risk-management-settings.md#indicator-level-settings-preview)指示器[](insider-risk-management-settings.md#domains)阈值设置。
