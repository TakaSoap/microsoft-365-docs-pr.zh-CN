...
为了帮助降低超出此限制的可能性，当邮箱中的邮箱置于保留时，"可恢复的项目"文件夹的存储配额会自动从 30 GB 增加到 100 GB Exchange Online。 如果存档邮箱已启用，则存档邮箱中"可恢复的项目"文件夹的存储配额也将从 30 GB 增加到 100 GB。 如果启用 Exchange Online中的自动扩展存档功能，则用户的存档邮箱（包括"可恢复的项目"文件夹）的总存储配额为 1.5 TB。
...
> [!NOTE]
> <sup>\*</sup>对于拥有存档邮箱许可证的用户，存档邮箱的初始存储配额Exchange Online（计划 2） GB。 但是，当为保留邮箱启用自动扩展存档时，存档邮箱和"可恢复的项目"文件夹的存储配额将增加到 110 GB。 额外存档存储空间 (包括"可恢复的项目"文件夹) 将在必要时预配高达 1.5 TB 的空间。 有关自动扩展存档的信息，请参阅 [了解自动扩展存档](autoexpanding-archiving.md)。
...
2. 运行以下命令，为"可恢复的项目"文件夹创建新的 RPT：
    ...
    > [!TIP]
    > 我们建议由  _AgeLimitForRetention_ 参数) 为"可恢复的项目"RPT 定义的保留期 (与将应用 RPT 的邮箱的已删除项目保留期相同。 这允许用户在将已删除项目移至存档邮箱之前，在已删除邮件的整个保留期内恢复这些项目。 在上一示例中，根据邮箱的已删除项目保留期也为 30 天的假设，将保留期设置为 30 天。 默认情况下Exchange Online邮箱配置为将已删除项目保留 14 天。 但你可以将此设置更改为最多 30 天。 有关详细信息，请参阅 Change [the deleted item retention period for a mailbox in Exchange Online](https://www.microsoft.com/?ref=go)。
...
可以使用 PowerShell Exchange Online单个邮箱应用新的保留策略。 但是 PowerShell 的真正功能是，您可以使用它快速标识组织中置于诉讼保留或 In-Place 保留状态的所有邮箱，然后在单个命令中将新的保留策略应用于所有保留邮箱。 下面是一些使用 Exchange PowerShell 将保留策略应用于一个或多个邮箱的示例。 所有示例都应用在步骤 2 中创建的保留策略。
...
- 启用用户的存档邮箱后，您还可以告诉用户他们可以恢复其存档邮箱中"可恢复的项目"文件夹中的已删除项目。 他们可以通过选择存档邮箱Outlook"已删除邮件"文件夹，然后单击"开始"选项卡上的"从服务器恢复已删除邮件"，在邮件 **中可以执行** 这一操作。有关恢复已删除项目的信息，请参阅 Recover [deleted items in Outlook for Windows](https://go.microsoft.com/fwlink/p/?LinkId=624829)。
