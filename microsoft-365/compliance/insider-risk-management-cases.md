...
- " **风险级别历史记录** "图表显示过去 30 天内的用户风险级别。 此线图使分析员和研究人员可以快速查看一段时间的总体用户风险趋势。 
- " **风险活动内容** "小部件汇总了添加到案例的警报中包含的数据和内容类型。 此小组件提供完整视图，以查看在情况下处于风险中的整个数据和内容集。
...
- **相关内容**：按内容源和类型排序的内容列表。 例如，对于 SharePoint Online 中的案例警报内容，你可能会看到列出的文件夹或文件名与案例警报的风险活动相关联。
...
- **状态**：选择一个或多个状态值以筛选警报列表。 选项为 *Confirmed、Dismissed、Needs* *review* 和 Resolved 。
- **严重性：** 选择一个或多个警报风险严重性级别以筛选警报列表。 选项有 *"高"、**中"* 和"*低"。*
- **检测到的时间**：选择创建警报的开始日期和结束日期。
- **策略**：选择一个或多个策略以筛选所选策略生成的警报。

### <a name="user-activity"></a>用户活动

" **用户活动** "选项卡是内部风险管理解决方案中用于内部风险分析和调查案例的最强大工具之一。 此选项卡的结构用于快速查看案例，包括所有警报的历史时间线、警报详细信息、用户在这种情况下的当前风险评分、风险事件序列，以及采取有效措施以包含案例风险的控制。
...
" **内容资源管理器** "选项卡允许风险分析师和研究人员查看与风险警报相关联的所有单个文件和电子邮件的副本。 例如，如果用户从 SharePoint Online 下载数百个文件时创建了警报，并且活动触发了策略警报，则警报的所有下载文件将被捕获并复制到原始存储源中的内部风险管理案例。
...
在 **案例的"** 案例备注"选项卡中，风险分析员和研究人员可以分享有关其案例工作的评论、反馈和见解。 备注是案例的永久添加内容，在保存便笺后无法编辑或删除。 从警报创建案例时，在"确认警报和创建内部风险案例"对话框中输入的注释会自动添加为案例注释。
...
在 **案例** 的"参与者"选项卡中，风险分析师和研究人员可以将其他审阅者添加到案例。 默认情况下，分配有 **"内部** 风险管理分析师"和"内部风险管理研究人员"角色的所有用户将列为每个活动案例和已结束案例的参与者。 只有分配了 **"内部风险管理** 调查人员"角色的用户才有权在内容资源管理器中查看文件和邮件。
...
必须记住，向用户 ***** 发送电子邮件通知不会将大小写解析为 _Closed*。 在某些情况下，在向用户发送通知以查找更多风险活动后，您可能需要保持案例打开状态，而无需打开新案例。 如果要在发送通知后解决案例，则必须在发送通知后选择"解决案例"作为后续步骤。
...
### <a name="escalate-for-investigation"></a>上报进行调查

在需要针对用户的风险活动进行其他法律审查的情况下，升级此案例以用于用户调查。 此升级将打开 Microsoft 365 组织中新的高级电子数据展示案例。 高级电子数据展示提供端到端工作流，以保留、收集、审阅、分析和导出对组织内部和外部法律调查做出响应的内容。 它还允许您的法律团队管理整个法定保留通知工作流，以与案例所涉及的保管人进行通信。 在从内部风险管理案例创建的高级电子数据展示案例中将审阅者分配为保管人可帮助法律团队采取适当操作和管理内容保留。 若要详细了解高级电子数据展示事例，请参阅 Microsoft [365](overview-ediscovery-20.md)中的高级电子数据展示概述。
...
风险分析师和调查人员完成审阅和调查后，可以解决案例，以对当前包含在该案例内的所有警报采取行动。 解决案例会添加解决方案分类，将案例状态更改为"已关闭"，解决方案操作原因会自动添加到"案例笔记"仪表板上的案例 **注释队列中。** 案例解析为：
