...
|UserID|执行操作（在**Operation**属性中指定）导致记录记录的用户。 审核日志中还包含由系统帐户（如 SHAREPOINT\system 或 NT AUTHORITY\SYSTEM）执行的活动的审核记录。 UserId 属性的另一个常见值为 app@sharepoint。 这表示执行活动的 "用户" 是在 SharePoint 中具有必要权限的应用程序，该应用程序代表用户、管理员或服务执行组织范围内的操作（如搜索 SharePoint 网站或 OneDrive 帐户）。 有关详细信息，请参阅[audit\@记录中的应用程序 sharepoint 用户](search-the-audit-log-in-security-and-compliance.md#the-appsharepoint-user-in-audit-records)。 |全部|
