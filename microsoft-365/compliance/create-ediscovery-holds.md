...
- 指定一个日期范围，以仅保留在此日期范围内发送、接收或创建的内容。 或者，您可以将所有内容保留于指定位置，而不管内容何时发送、接收或创建。
...
- 如果电子数据展示保留的状态为 On **(Pending) ，** 则仍可在启用保留时搜索保留位置。
...
- **SharePoint和OneDrive网站：** 从SharePoint电子数据展示保留后，OneDrive保留库中保留的任何内容不会在 30 天延迟保留期内删除。 这类似于从保留策略中释放网站时发生的情况。 此外，在 30 天延迟保留期间，无法手动删除保留库中的此内容。 
...
下表列出了电子数据展示事例和事例保留的限制。
  ...
  |单个电子数据展示保留中的最大邮箱数。 此限制包括用户邮箱总数，以及与组、组Microsoft 365组、Microsoft Teams组Yammer邮箱。  <br/> |1,000  <br/> |
