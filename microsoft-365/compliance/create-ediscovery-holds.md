...
- 您可以创建无限保留，其中指定位置的所有内容置于保留状态。 或者，您可以创建基于查询的保留，其中只有与搜索查询匹配的指定位置的内容置于保留状态。

- 您可以指定一个日期范围，以仅保留在该日期范围内发送、接收或创建的内容。 或者，您可以保留指定位置中的所有内容，无论它是在何时发送、接收或创建的。
...
在位于 SharePoint 网站中的文档上放置基于查询的电子数据展示保留时，请记住以下几点：

- 基于查询的保留最初会在删除网站中的所有文档后保持较短的一段时间。 这意味着当文档被删除后，它将移至保存保留库，即使它与基于查询的保留的条件不匹配也是如此。 但是，与基于查询的保留不匹配的已删除文档将由处理保留保留库的计时器作业删除。 计时器作业将定期运行，并将保留保留库中的所有文档与基于查询的电子数据展示保留 (以及其他类型的保留和保留策略) 进行比较。 计时器作业将删除不符合基于查询的保留的文档，并保留执行的文档。

- 不应使用基于查询的保留执行目标保留，如在特定文件夹或网站中保留文档或使用其他基于位置的保留条件。 这样做可能会产生意外的结果。 我们建议使用不基于位置的保留条件，如关键字、日期范围或其他文档属性，以保留网站文档。
...
- 如果内容位置是同一大小写中的多个保留的一部分，则当您使用 "全部事例内容" 选项搜索该内容位置时，保留查询将由 **OR** 运算符组合。 同样，如果内容位置是两个不同保留的一部分，其中一个是基于查询的，另一个是无限保留 (将所有内容置于保留状态中) ，则所有内容都是由于无限保留而进行搜索的。

- 如果将搜索配置为在保留范围内搜索位置，然后通过添加或删除位置或更改保留查询来更改事例中的电子数据展示保留 () ，则会使用这些更改更新搜索配置。 但是，在更改保留后，必须重新运行搜索以更新搜索结果。
...
> [!IMPORTANT]
> 在基于云的组织中，参与加入团队对话的用户必须具有 Exchange Online 邮箱，才能在将邮箱放在电子数据展示保留中时保留聊天对话。 这是因为聊天列表中的对话存储在聊天参与者的基于云的邮箱中。 如果聊天参与者没有 Exchange Online 邮箱，将无法保留这些聊天对话。 例如，在 Exchange 混合部署中，具有本地邮箱的用户可以参与属于团队中的聊天列表的对话。 但在这种情况下，无法保留这些对话中的内容，因为这些用户没有可置于保留状态的基于云的邮箱。
...
> [!NOTE]
> 将团队的 SharePoint 网站置于保留状态时，为团队或团队频道 (保留 Wiki 内容的功能是在2017年6月22日发布的) 发布的。 如果工作组网站处于保留状态，则会在该日期开始保留 Wiki 内容。 但是，如果工作组网站处于保留状态，并且在6月 22 2017 日之前删除了 Wiki 内容，则不会保留 Wiki 内容。
...
> [!IMPORTANT]
> 用户的 OneDrive 帐户的 URL 包含其用户主体名称 (UPN)  (例如， `https://alpinehouse-my.sharepoint.com/personal/sarad_alpinehouse_onmicrosoft_com`) 。 在极少数情况下，某人的 UPN 发生更改时，其 OneDrive URL 也将更改为包含新的 UPN。 如果用户的 OneDrive 帐户是电子数据展示保留的一部分（旧的和其 UPN 的更改），则需要更新保留，并且必须更新保留，然后添加用户的新 OneDrive URL 并删除旧的旧 URL。 有关详细信息，请参阅 [UPN 更改如何影响 OneDrive URL](https://docs.microsoft.com/onedrive/upn-changes)。
