...
- Strat_O365_IP
description: 了解 Exchange Online 和 Microsoft 365 如何使用传输层安全性（TLS）和转发保密（FS）来保护电子邮件通信。 此外，还可获取 Microsoft 为 Exchange Online 颁发的证书的相关信息。
...
ms.locfileid: "44170961"
---
# <a name="how-exchange-online-uses-tls-to-secure-email-connections"></a>Exchange Online 如何使用 TLS 保护电子邮件连接

了解 Exchange Online 和 Microsoft 365 如何使用传输层安全性（TLS）和转发保密（FS）来保护电子邮件通信。 此外，还提供了 Microsoft 为 Exchange Online 颁发的证书的相关信息。
  
## <a name="tls-basics-for-microsoft-365-and-exchange-online"></a>适用于 Microsoft 365 和 Exchange Online 的 TLS 基础知识
...
如果您想要在 Microsoft 与您的内部部署组织或其他组织（如合作伙伴）之间设置安全通道的通信，我们建议使用 TLS。 Exchange Online 始终尝试首先使用 TLS 保护您的电子邮件，但如果另一方没有提供 TLS 安全，则无法始终这么做。 请继续阅读，了解如何使用*连接器*保护对本地服务器或重要合作伙伴的所有邮件。 
  
## <a name="how-exchange-online-uses-tls-between-exchange-online-customers"></a>Exchange Online 如何在 Exchange Online 客户之间使用 TLS

Exchange Online 服务器始终通过 TLS 1.2 加密到我们数据中心其他 Exchange Online 服务器的连接。 当您将邮件发送到组织内的某个收件人时，将通过使用 TLS 加密的连接自动发送该电子邮件。 此外，发送给其他客户的所有电子邮件都通过使用 TLS 加密的连接进行发送，并使用 "向前保密" 进行保护。
  
## <a name="how-microsoft-365-uses-tls-between-microsoft-365-and-external-trusted-partners"></a>Microsoft 365 如何在 Microsoft 365 与外部的受信任合作伙伴之间使用 TLS
...
如果您正在管理混合 Exchange 部署，则内部部署 Exchange 服务器需要使用安全证书向 Microsoft 365 进行身份验证，以便将邮件发送到其邮箱仅位于 Office 365 中的收件人。 因此，您需要为本地 Exchange 服务器管理自己的安全证书。 您还必须以安全的方式存储并维护这些服务器证书。 有关在混合部署中管理证书的详细信息，请参阅[混合部署的证书要求](https://technet.microsoft.com/library/hh563848%28v=exchg.150%29.aspx)。
...
## <a name="get-more-information-about-tls-and-microsoft-365"></a>获取有关 TLS 和 Microsoft 365 的详细信息

有关受支持的密码套件的列表，请参阅[有关加密的技术参考详细信息](technical-reference-details-about-encryption.md)。
