...
导入第三方数据后，可以将 Microsoft 365 合规性功能&mdash;（如诉讼保留、电子数据展示、就地存档、审核、通信合规性和保留策略&mdash;）应用于此类数据。 例如，当将邮箱置于诉讼保留状态时，将保留第三方数据。 您可以使用 Microsoft 电子数据展示工具搜索第三方数据。 或者，您可以将存档和保留策略应用于第三方数据，就像 Microsoft 数据一样。 简言之，在 Microsoft 365 中存档第三方数据可帮助您的组织遵守政府和法规策略。
