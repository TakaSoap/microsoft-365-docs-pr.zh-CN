...
1. 若要查看特定用户帐户的详细信息（如属性值和分配的段 (s) ），请使用带有 Identity 参数的 **InformationBarrierRecipientStatus** cmdlet。 
...
2. 确定要为用户帐户配置文件 (s) 编辑的属性。 有关详细信息，请参阅 [信息障碍策略的属性](information-barriers-attributes.md) 。 
...
1. 若要查看所有现有段，请使用 **OrganizationSegment** cmdlet。
    ...
    > [!TIP]
    > 打印或保存段列表以便日后参考。 例如，如果您想要编辑某一段，则需要知道它的名称或标识值 (与 Identity 参数一起使用) 。
...
3. 完成策略的编辑后，请确保应用所做的更改。  (请参阅 [Apply information 屏障策略](information-barriers-policies.md#part-3-apply-information-barrier-policies)。 ) 
...
3. 若要应用所做的更改，请使用 **InformationBarrierPoliciesApplication** cmdlet。
    ...
    为您的组织应用更改的用户。 如果您的组织规模较大，则可能需要24小时 (或更多) 才能完成此过程。  (一般准则是一小时的时间来处理5000用户帐户。 ) 

在这种情况下，一个或多个信息障碍策略将设置为非活动状态。 在这里，您可以执行以下任一操作：
- 将其保持为 " (策略设置为非活动状态将对用户不起作用) 
- [编辑策略](#edit-a-policy) 
