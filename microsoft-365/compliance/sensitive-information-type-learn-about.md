...
- **低可信度**：匹配的项将包含最小的漏报，但包含最多的误报。 低可信度返回所有低、中和高可信度匹配。 低可信度的值为 65。
- **中等可信度**：匹配的项将包含误报和漏报的平均数量。 中置信度返回所有中高可信度匹配。 中等可信度的值为 75。
- **高可信度**：匹配的项将包含最小的误报，但包含最多的漏报。 高可信度仅返回高可信度匹配，值为 85。  
...
- **使用 EDM** - 可以使用精确数据匹配和基于 EDM 分类 (EDM) 敏感信息类型。 通过此方法，你可以使用可定期刷新的安全数据库创建动态敏感信息类型。 请参阅 [了解基于精确数据匹配的敏感信息类型](sit-learn-about-exact-data-match-based-sits.md#learn-about-exact-data-match-based-sensitive-information-types)。
...
> Microsoft 365信息保护现在支持使用双字节字符集语言：
> - 简体中文
> - 繁体中文
> - 韩语
> - 日语
