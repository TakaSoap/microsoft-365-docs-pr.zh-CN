...
- 模式：模式定义敏感信息类型检测到的信息。 它由以下组件组成
    - 主元素 – 敏感信息类型正在查找的主元素。 它可以 **是包含或** 不带校验和验证、关键字列表、关键字词典或函数的 **正则表达式**。 
    - 支持元素 – 充当支持证据的元素，可帮助提高匹配可信度。 例如，邻近 SSN 号码的关键字"SSN"。 它可以是包含或不带校验和验证、关键字列表、关键字词典的正则表达式。
    - 可信度 - 置信 (高、中、低) 反映与主要元素一起检测到的支持证据量。 项目包含的支持性证据越高，匹配项包含的敏感信息的置信度越高。
    - 邻近度 – 主要元素和支持元素之间的字符数
...
在敏感信息类型实体定义中，可信度反映除了主要元素之外检测到的支持证据。 项目包含的支持性证据越高，匹配项包含的敏感信息的置信度越高。 例如，高可信度的匹配将包含与主要元素接近的更多支持证据，而置信度较低的匹配在接近度方面几乎不会包含支持证据。 

高可信度返回最小的误报，但可能会导致更多的误报。 低或中等可信度返回更多的误报，但很少返回到零的误报。

- **低可信度**：值 65，匹配项将包含最小的漏报，但误报最多。 低可信度返回所有低、中和高可信度匹配。
- **中置** 信度：值 75，匹配项将包含误报和漏报的平均数量。 中置信度返回所有中高可信度匹配项。  
- **高可信度**：值 85，匹配项将包含最小的误报，但包含最多误报。 高可信度仅返回高可信度匹配。  

你应该将高可信度模式与较低的计数（如 5 到 10）和低可信度模式（例如 20 个或多个）一同使用。
...
> [!NOTE]
> 改进的可信度可用于 Microsoft 365 服务、Microsoft 365 服务的 Microsoft 信息保护、通信合规性、信息管理和记录管理中的数据丢失防护。
