...
SIT (的) 类型是基于模式的分类器。 它们检测敏感信息（如社会保险、信用卡或银行帐号）以标识敏感项目，请参阅敏感信息类型 [实体定义](sensitive-information-type-entity-definitions.md) ，了解所有 SIT 的完整列表。
...
默认情况下，Microsoft 创建的这些 SI 显示在合规性控制台中。 这些 SIT 无法编辑，但它们可以用作模板并复制以创建自定义敏感信息类型。 有关 [所有 SI 的完整列表，](sensitive-information-type-entity-definitions.md) 请参阅敏感信息类型实体定义。
...
> [!NOTE]
> 如果你已有策略或自定义敏感信息类型 (SIT) 使用基于数字的可信度 (也知道其准确性) ，它们将自动映射到三个离散可信度;整个安全 @ 合规中心 UI 中的低可信度、中等可信度和高可信度。
...
> 
> 除了中文/日语/双字节字符外，如果关键字/短语列表还包含非中文/日语单词 (如仅英语) ，应创建两个词典/关键字列表。 一个针对包含中文/日语/双字节字符的关键字，另一个针对仅含英语的关键字。 
> - 例如，如果要创建一个关键字字典/列表，其短语为"Highly confidential"、"がが高い"和"机密"，您应创建两个关键字列表。 
