...
Microsoft 已培训并测试了使用非常大的示例数据集的分类器数，可以帮助确定特定的内容类别。 请参阅[trainable 分类器的入门（预览）](classifier-getting-started-with.md)。 默认情况下， `Ready to use`这些分类器显示在组中。
...
1. 收集您认为已准备好使用分类器的类别（正匹配）的可释放测试内容项，以及要测试的类别中不应包含的（负匹配项）。

> [!IMPORTANT]
> 示例项目不能加密且必须为英语。

2. 创建一个专用的 SharePoint Online 文件夹，至少等待一个小时，将该文件夹添加到搜索索引中。 请记下文件夹 URL。

3. 使用合规性管理员或安全管理员角色访问权限登录 microsoft 365 合规性中心，并打开**microsoft 365 合规性中心**或**microsoft 365 安全中心** > **记录管理（预览）** > "**标签策略**" 选项卡。

4. 选择`Auto-apply a label`。

5. 选择`Choose a label to auto-apply`。

6. 选择`Create new labels`并创建一个标签，以便在此测试中直接使用。 执行此操作时，请`Retention`将设置为 "关闭"。 您不希望启用任何保留或其他操作。 在这种情况下，您将使用保留标签，只是在不强制执行任何操作的情况下使用 "文本标签"。 例如，您可以创建一个名为 "SourceCode 分类程序测试" 的保留标签，其中不包含任何操作，然后将该保留标签自动应用于具有源代码分类器作为条件的内容。 若要了解有关创建保留标签的详细信息，请参阅[保留标签概述](labels.md)。
