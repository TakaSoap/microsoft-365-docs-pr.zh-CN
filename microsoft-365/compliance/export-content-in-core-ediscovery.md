...
ms.locfileid: "58567020"
---
# <a name="export-content-from-a-core-ediscovery-case"></a>从核心电子数据展示事例导出内容

成功运行与核心电子数据展示案例关联的搜索后，可以导出搜索结果。 导出搜索结果时，邮箱项目以 PST 文件或单个邮件方式下载。 当您从网站和SharePoint网站OneDrive for Business内容时，将Office本机文档和其他文档的副本。 一Results.csv导出的每个项目的信息，以及包含有关每个搜索结果的信息的清单 (XML 格式) 包含每个搜索结果的信息的清单文件也会导出。
...
7. 单击创建的导出作业，在飞出页上显示状态信息。 此信息包括已转移到其他位置的项目的Azure 存储百分比。
...
- 此外，如果重新启动导出，复制到导出位置的Azure 存储会覆盖以前的结果。 无法下载以前复制的结果。
