...
description: 了解如何通过 SHAREPOINT RmS Microsoft Azure Active Directory Rights Management Services (Online IRM) 保护SharePoint列表和文档库。
...
5. 在"**信息权限管理 (IRM**) "部分，选择"使用配置中指定的 **IRM** 服务"，然后选择"刷新 **IRM 设置"。** 刷新 IRM 设置后，组织成员可以开始在列表和文档库中SharePoint IRM。 但是，要这样做的选项可能需要一个小时，才显示在"库""列表设置列表设置。
...
不能在启用 IRM 的库中使用浏览器Office或编辑文档。 相反，一个人一次可以下载和编辑 IRM 加密的文件。 使用签入和签出管理多个  *用户*  之间的共同创作或创作。 
...
Windows 的新 OneDrive 同步 客户端现在支持同步受 IRM 保护的 SharePoint 文档库和 OneDrive 位置 (只要库的 IRM 设置未设置为过期文档访问权限) 。 有关详细信息，或者要开始部署新的同步客户端，请参阅部署适用于 OneDrive 同步[客户端的新Windows。](/onedrive/deploy-on-windows)
