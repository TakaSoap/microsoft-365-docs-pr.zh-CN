...
警报策略包含一组规则和条件，用于定义生成警报的用户或管理员活动、触发警报的用户列表（如果他们执行活动）以及一个定义触发通知之前必须发生活动次数的阈值。 还可以对策略进行分类，并为其分配严重性级别。 这两个设置有助于管理警报策略 (策略条件与) 匹配时触发的警报，因为可以在管理策略和在合规中心查看警报时筛选这些设置。 例如，可以查看与同一类别的条件匹配的警报，或查看严重性级别相同的警报。
...
- **警报正在跟踪的活动**。 您可以创建一个策略来跟踪活动，或在某些情况下创建一些相关活动，例如通过共享文件、分配访问权限或创建匿名链接与外部用户共享文件。 当用户执行策略定义的活动时，将基于警报阈值设置触发警报。
    ...
    > [!NOTE]
    > 你可以跟踪的活动取决于你组织的活动Office 365 企业版或Office 365美国政府计划。 通常，与恶意软件市场活动和网络钓鱼攻击相关的活动需要 E5/G5 订阅或 E1/F1/G1 或 E3/F3/G3 订阅以及 Defender for Office 365 计划[2](../security/office-365-security/defender-for-office-365.md)附加订阅。
...
该表还指明了Office 365 企业版美国政府Office 365计划所需的计划。 如果您的组织具有除 E1/F1/G1 或 E3/F3/G3 订阅之外的适当附加订阅，则某些默认警报策略可用。
...
|**由于 ZAP 已禁用，因此未删除恶意软件**| 当 Microsoft 检测到向邮箱传递恶意软件邮件时，Zero-Hour"网络钓鱼邮件的自动清除"功能。 此策略具有 **信息** 严重性设置。 |威胁管理|适用于 P2 附加Office 365订阅的 E5/G5 或 Defender|
...
|**由于 ETR 覆盖而传递的网络钓鱼**|当 Microsoft 检测到允许将高可信度网络钓鱼Exchange邮件 (ETR) ，将生成警报。 此策略具有 **信息** 严重性设置。 有关传输规则Exchange邮件 (规则) ，请参阅邮件流规则 ([中的) 规则](/exchange/security-and-compliance/mail-flow-rules/mail-flow-rules)Exchange Online。|威胁管理|E1/F1/G1、E3/F3/G3 或 E5/G5|
...
|**由于 ZAP 已禁用，因此未进行钓鱼**| 当 Microsoft 检测到向邮箱传递高可信度网络钓鱼邮件时，Zero-Hour网络钓鱼邮件的自动清除功能。 此策略具有 **信息** 严重性设置。|威胁管理|适用于 P2 附加Office 365订阅的 E5/G5 或 Defender|
...
|**报告为钓鱼的电子邮件异常增加**|当使用 Outlook 中的"报告邮件"外接程序将邮件报告为网络钓鱼邮件时，将生成警报。 此 **策略具有中等** 严重性设置。 有关此外接程序详细信息，请参阅 [使用报告邮件外接程序](https://support.office.com/article/b5caa9f1-cdf3-4443-af8c-ff724ea719d2)。|威胁管理|适用于 P2 附加Office 365订阅的 E5/G5 或 Defender|
...
此 (基于 RBAC 权限) ，您可以确定哪些警报 (组织中特定工作角色的用户可以查看和管理) 。
