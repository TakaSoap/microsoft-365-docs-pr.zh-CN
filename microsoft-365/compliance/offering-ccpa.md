...
---
# <a name="compliance-offering-california-consumer-privacy-act-ccpa"></a>合规性服务：加利福尼亚消费者隐私法案（CCPA）
...
CCPA 将在2020年1月1日生效。 但是，由加利福尼亚律师常规（AG）强制执行将于2020年7月1日开始。

加利福尼亚州的 AG 将强制实施 CCPA，并且将有能力发出不合规性罚款。 CCPA 还提供了仅限于数据泄露的私有操作权限。 在 "操作" 的私有权限下，每个使用者每个事件的损坏可能会介于 $100 和 $750 之间。 加利福尼亚州的 AG 还可以强制实施完整的 CCPA，从而能够 levy 的民事处罚不超过 $2500，每个故意的冲突或 $7500。
...
- 在合规性管理器中开始利用 GDPR 评估作为你的 CCPA 隐私计划的一部分。
- 建立一个过程，以使用数据主体请求工具有效响应数据主体访问请求（DSARs）。
- 设置标签和策略以发现、使用 Microsoft 信息保护对敏感数据进行分类 & 标签和保护敏感数据。
- 使用电子邮件加密功能进一步控制敏感信息。
...
CCPA 的最大区别是，允许从数据销售到第三方（带有 "销售" 广泛定义以包括共享数据以进行重要考虑）的核心要求。

**在 CCPA 下，公司必须启用哪些权限？**

CCPA 需要管控企业来收集、转移和销售个人信息，以及其他一些内容：

- 提供针对集合的类别和用途的对消费者的披露（在收集之前）。
...
- 确保不区分使用者，以在 CCPA 下行使其任何权利。

**CCPA 如何应用于儿童？**

- CCPA 引入了家长同意义务，与孩子的年龄为13的儿童的孩子的在线隐私保护法案（COPPA）保持一致。
...
- [加利福尼亚州消费者隐私法案（CCPA）常见问题解答](ccpa-faq.md)
