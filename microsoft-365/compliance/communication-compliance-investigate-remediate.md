...
- MET150
- MOE150
ms.openlocfilehash: bccd0ae5ce6ecfadd3ecc46569252c2d0004cf80e36a20e827232813d47e8b8d
...
配置通信合规性策略后，你将开始在 Microsoft 365 合规中心 收到符合策略条件的邮件问题的警报。 请按照此处的工作流说明调查和修正警报问题。
...
- **源**：此视图是大多数基于 Web 的邮件平台中通常看到的标准邮件视图。 邮件头信息采用正常样式格式，邮件正文支持带符号的图形文件和换行文本。 如果为策略启用了光学字符识别 ([OCR ](communication-compliance-feature-reference.md#optical-character-recognition-ocr)) ，则包含匹配策略条件的打印或手写文本的图像将在此视图中视为关联邮件的子项。
...
- **翻译**：此视图自动将通知邮件文本转换为每个审阅者在 Microsoft 365订阅中的"显示语言"设置中配置的语言。 " *翻译* "视图有助于扩大对具有多语言用户的组织的调查支持，并且无需在通信合规性审阅过程之外提供其他翻译服务。 使用 Microsoft 翻译服务 *，可以* 根据需要打开和关闭翻译视图，并支持多种语言。 有关支持的语言的完整列表，请参阅Microsoft 翻译工具[语言。](https://www.microsoft.com/translator/business/languages/) 翻译视图中翻译工具语言 *列表**中列出的语言*。
...
- **报告为错误分类 (预览**) ：在邮件审阅工作流期间，您始终可以在任何时间点将邮件解析为错误分类。 错误分类表示警报不可操作，或者警报进程和任何可训练分类器错误生成警报。 将项目解析为错误分类会向 Microsoft 发送邮件内容、附件和邮件 (包括元数据) Microsoft，以帮助改进可训练分类器。 发送给 Microsoft 的数据不包含可标识或用于标识组织中任何用户的信息。 无法对邮件执行进一步操作，所有错误分类的邮件都显示在"已解决 **"选项卡** 中。
