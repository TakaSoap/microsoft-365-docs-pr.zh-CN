...
3. **确定修正操作**：现在您已经查看了警报消息的详细信息，您可以选择多个修正操作：
    ...
    - **删除工作组中的邮件 (预览) **：使用 "**在团队中删除邮件**" 控件中，可以阻止来自 Microsoft 团队频道和1:1 和组聊天的警报中标识的不适当的邮件和内容。 删除的邮件和内容将被替换为策略提示，表明已阻止该邮件和应用于从视图中删除的策略。 收件人在策略提示中提供了一个链接，以了解有关适用策略和审阅过程的详细信息。 发件人接收被阻止的邮件和内容的策略提示，但可以查看阻止的邮件的详细信息和有关删除上下文的内容。
